...
def extract_coverage_data(
    ...
    for filename in measured_files:
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not process file {filename}: {e}[/yellow]"
            )
            continue
        ...
        try:
            for line_num, contexts in contexts_map.items():
                for context_name in contexts:
                    ...
                    clean_context = (
                        context_name.split("|")[0] if context_name else "global"
                    )
...
def calculate_redundancy_analysis(
    ...
    if has_arc_columns and has_arcs:
        ...
        coverage_data = con.execute(f"""
            ...
            WHERE context IN ({context_placeholders})
            GROUP BY context
        """).fetchall()  # Process aggregated results into test_coverage and test_source_coverage dictionaries
    test_coverage = {}
    ...
    for row in coverage_data:
        ...
        all_lines = (
            {(item["file"], item["line"]) for item in lines_array}
            if lines_array
            else set()
        )
        all_arcs = (
        ...
        source_lines = {
            (file, line) for file, line in all_lines if file in source_files
        }
        source_arcs = {
            (file, fr, to) for file, fr, to in all_arcs if file in source_files
        }
    ...
    # Prefix unions: prefix[i] = union of test_source_coverage[sorted_contexts[0]] to sorted_contexts[i-1]
    prefix_lines = [set()] * (n + 1)
    prefix_arcs = [set()] * (n + 1) if with_branches else None

    for i in range(1, n + 1):
        prev_union_lines = prefix_lines[i - 1]
        current_lines = test_source_coverage[sorted_contexts[i - 1]]["lines"]
        prefix_lines[i] = prev_union_lines | current_lines
        if with_branches:
    ...
    # Suffix unions: suffix[i] = union of test_source_coverage[sorted_contexts[i]] to sorted_contexts[n-1]
    suffix_lines = [set()] * (n + 1)
    ...
    for i in range(n - 1, -1, -1):
        next_union_lines = suffix_lines[i + 1]
        current_lines = test_source_coverage[sorted_contexts[i]]["lines"]
        suffix_lines[i] = current_lines | next_union_lines
        if with_branches:
    ...
    for idx, test_context in enumerate(sorted_contexts):
        # Union of all except this one: prefix[idx] | suffix[idx + 1]
        union_lines = prefix_lines[idx] | suffix_lines[idx + 1]
        union_arcs = (
...
def analyze_coverage_for_redundancy(
    ...
    test_results = calculate_redundancy_analysis(
        coverage_list, source_files, with_branches
    )
    if not test_results:
        return
