...
def test_select_paragraph_at_no_match():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        paragraph = pdf.page(0).select_paragraph_at(10000, 10000)
...
def test_select_paragraph_starting_with():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        paragraph = pdf.page(0).select_paragraph_starting_with(
...
def test_select_paragraph_starting_with_no_match():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        paragraph = pdf.page(0).select_paragraph_starting_with("Nonexistent text")
        assert paragraph is None
...
def test_select_paragraph_matching_page_level():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        # Test singular version
        paragraph = pdf.page(0).select_paragraph_matching(r"Invoice #[0-9]+")
        assert paragraph is not None
...
def test_select_paragraph_matching_no_match():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        paragraph = pdf.page(0).select_paragraph_matching(r"[0-9]{5}")
        assert paragraph is None
...
def test_select_text_line_starting_with():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        text_line = pdf.page(0).select_text_line_starting_with("Second Line")
        assert text_line is not None
...
def test_select_text_line_matching():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        text_line = pdf.page(0).select_text_line_matching(r"\$[0-9]+\.[0-9]+")
        assert text_line is not None
...
def test_select_image_at():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        if len(images) > 0:
            ...
            # Test singular version
            image = pdf.page(0).select_image_at(x, y)
...
def test_select_image_at_no_match():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        image = pdf.page(0).select_image_at(10000, 10000)
...
def test_select_form_field_at():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        if len(fields) > 0:
            ...
            # Test singular version
            field = pdf.page(0).select_form_field_at(x, y)
            assert field is not None
...
def test_select_path_at():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        # Add a path (line)
        pdf.page(0).new_line().from_point(100, 100).to_point(200, 200).add()

        # Test singular version
        path = pdf.page(0).select_path_at(100, 100)
...
def test_select_path_at_no_match():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        path = pdf.page(0).select_path_at(10000, 10000)
        assert path is None
...
def test_select_form_at():
    ...
    with PDFDancer.open(pdf_path, token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        if len(forms) > 0:
            ...
            # Test singular version
            form = pdf.page(0).select_form_at(x, y)
...
def test_singular_methods_return_type():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        paragraph = pdf.page(0).select_paragraph_at(100, 100)
        assert paragraph is not None
        assert not isinstance(paragraph, list)

        # Test that None is returned when no match (not empty list)
        no_match = pdf.page(0).select_paragraph_at(10000, 10000)
...
def test_singular_methods_consistency():
    ...
    with PDFDancer.new(token=token, base_url=base_url, timeout=30.0) as pdf:
        ...
        paragraphs = pdf.page(0).select_paragraphs_at(100, 100)
        paragraph = pdf.page(0).select_paragraph_at(100, 100)
