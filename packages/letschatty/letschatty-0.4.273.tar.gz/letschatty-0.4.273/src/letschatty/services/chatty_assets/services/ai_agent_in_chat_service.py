...
class AIAgentInChatService(AssetService[ChattyAIAgentInChat, ChattyAIAgentInChat]):
    ...
    async def get_by_chat_id(self, chat_id: StrObjectId) -> Optional[ChattyAIAgentInChat]:
        ...
        doc = await self.collection.async_collection.find_one({"chat_id": chat_id, "deleted_at": None}, projection={"chain_of_thoughts": 0})
        if not doc:
            return None
        return self.collection.create_instance(doc)
