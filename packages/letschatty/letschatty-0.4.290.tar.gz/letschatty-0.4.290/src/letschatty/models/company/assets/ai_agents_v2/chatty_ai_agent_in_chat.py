...
class ChattyAIAgentInChat(CompanyAssetModel):
    ...
    def get_chain_of_thought(self, cot_id: StrObjectId) -> Optional[ChainOfThoughtInChat]:
        ...
        logger.info(f"Getting chain of thought by id {cot_id} for chat {self.chat_id} of {len(self.chain_of_thoughts)} chain of thoughts")
        for cot in self.chain_of_thoughts:
            if cot.id == cot_id:
                logger.info(f"Chain of thought {cot_id} found for chat {self.chat_id}")
                return cot
        logger.info(f"Chain of thought {cot_id} not found for chat {self.chat_id}")
        return None
