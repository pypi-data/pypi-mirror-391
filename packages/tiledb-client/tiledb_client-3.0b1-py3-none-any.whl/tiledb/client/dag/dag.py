        ...
        *,
        ...
        try:
            api_client.register_registered_task_graph(
                ...
                teamspace_id,
                path_id,
                graph=json_safe.Value(self._tdb_to_json(path_id)),
            )
        except ApiException as exc:
            # Is there a folder at path? If so, try again.
            ast: assets.Asset = assets.get_asset(path_id, teamspace=teamspace_id)
            if ast and ast.type == assets.AssetType.FOLDER:
                logger.info(
                    "Registration targeting a folder: self.id=%r, self.name=%r, teamspace_id=%r, path_id=%r",
                    self.id,
                    self.name,
                    teamspace_id,
                    path_id,
                ...
                target_path = os.path.join(ast.path, dag_name)
                try:
                    api_client.register_registered_task_graph(
                        client.get_workspace_id(),
                        teamspace_id,
                        target_path,
                        graph=json_safe.Value(self._tdb_to_json(dag_name)),
                    )
                except ApiException as exc2:
                    raise DAGError(
                        "Registration of DAG to a folder failed."
                    ) from tiledb_cloud_error.maybe_wrap(exc2)
            else:
                raise DAGError(
