...
class ValueVisualization:
    ...
    def forward_with_params(op_name, operands, **params):
        ...
        ops_map = {
            "pow_scalar": lambda: (
                operands[0].data ** params["exponent"]
            ),
            "transpose": lambda: (
                np.transpose(operands[0].data, params["axes"])
            ),
            "sum": lambda: np.sum(
                operands[0].data,
                axis=params["axis"],
            ...
            "reshape": lambda: (
                operands[0].data.reshape(params["shape"])
            ),
            "softmax": lambda: ValueVisualization._softmax(
                operands[0].data, params["axis"]
            ),
            "log_softmax": lambda: ValueVisualization._log_softmax(
                operands[0].data, params["axis"]
            ),
            "getitem": lambda: operands[0].data[params["key"]],
            "stack": lambda: np.stack(
                [v.data for v in operands], axis=params["axis"]
            ),
            "concatenate": lambda: np.concatenate(
                [v.data for v in operands], axis=params["axis"]
            ),
        }
...
class Value:
    ...
    def _unbroadcast(self, grad, target_shape):
        ...
        for i, (grad_dim, target_dim) in enumerate(
    ...
    def _create_binary_op(
        self, other, forward_fn, self_grad_fn, other_grad_fn, op_name
    ):
        ...
        def _backward():
            self.grad += self._unbroadcast(
                self_grad_fn(out.grad, other.data), self.data.shape
            )
            other.grad += other._unbroadcast(
                other_grad_fn(out.grad, self.data), other.data.shape
            )

        out._backward = _backward
        return out

    def _reduce_op(self, op_fn, op_name, axis=None, keepdims=False,
                   scale_factor=None):
        ...
        def _backward():
            if axis is None:
                ...
                axes = [
                    ax if ax >= 0 else ax + self.data.ndim for ax in axes
                ]

                n = (
                    scale_factor(
                        np.prod([self.data.shape[ax] for ax in axes])
                    )
                    if scale_factor
                    else 1
                )
                ...
                if not keepdims:
                    ...
                    grad = np.broadcast_to(
                        reshaped_grad / n, self.data.shape
                    )
                else:
    ...
    def __add__(self, other):
        return self._create_binary_op(
            other, lambda a, b: a + b, lambda g, b: g,
            lambda g, a: g, "add"
        )

    def __mul__(self, other):
        return self._create_binary_op(
            other, lambda a, b: a * b, lambda g, b: g * b,
            lambda g, a: g * a, "mul"
        )
    ...
    def __pow__(self, other):
        if isinstance(other, (int, float)):
            ...
            if np.any(self.data <= 0):
                raise ValueError(
                    "Base must be positive for Value^Value operations"
                )
            ...
            def _backward():
                ...
                other.grad += other._unbroadcast(
                    (result * np.log(self.data)) * out.grad,
                    other.data.shape
                )
    ...
    def tanh(self):
        return self._create_unary_op(
            np.tanh, lambda _, r: 1 - r**2, "tanh"
        )
    ...
    def sigmoid(self):
        ...
        return self._create_unary_op(
            forward, lambda _, r: r * (1 - r), "sigmoid"
        )

    def relu(self):
        return self._create_unary_op(
            lambda x: np.maximum(0, x),
            lambda x, _: (x > 0).astype(np.float64),
            "relu"
        )

    def abs(self):
        return self._create_unary_op(
            np.abs, lambda x, _: np.sign(x), "abs"
        )

    def sin(self):
        return self._create_unary_op(
            np.sin, lambda x, _: np.cos(x), "sin"
        )

    def cos(self):
        return self._create_unary_op(
            np.cos, lambda x, _: -np.sin(x), "cos"
        )

    def tan(self):
        return self._create_unary_op(
            np.tan, lambda x, _: 1 / np.cos(x) ** 2, "tan"
        )
    ...
    def transpose(self, axes=None):
        out = Value(
            np.transpose(self.data, axes), (self,), _op="transpose"
        )
        out._op_params = {"axes": axes}

        def _backward():
            inv_axes = (
                np.argsort(axes) if axes else None
            )
            grad = (
                np.transpose(out.grad, inv_axes) if axes
                else np.transpose(out.grad)
            )
            self.grad += grad
    ...
    def __rmatmul__(self, other):
        return self._to_value(other).matmul(self)
    ...
    def mean(self, axis=None, keepdims=False):
        return self._reduce_op(
            np.mean, "mean", axis, keepdims, scale_factor=lambda n: n
        )
    ...
    def softmax(self, axis=-1):
        ...
        result = exp_shifted / np.sum(
            exp_shifted, axis=axis, keepdims=True
        )
        ...
        def _backward():
            s_dot_grad = np.sum(
                result * out.grad, axis=axis, keepdims=True
            )
    ...
    def log_softmax(self, axis=-1):
        ...
        def _backward():
            ...
            grad = out.grad - softmax_vals * np.sum(
                out.grad, axis=axis, keepdims=True
            )
            self.grad += self._unbroadcast(grad, self.data.shape)
    ...
    def concatenate(values, axis=0):
        ...
        def _backward():
            split_indices = np.cumsum(
                [v.shape[axis] for v in values[:-1]]
            )
    ...
    def mean_values(values):
        values = list(values)
        return (
            Value.sum_values(values) / len(values) if values
            else Value(0.0)
        )
    ...
    def backward(self, gradient=None):
        ...
        if gradient is not None:
            ...
            raise ValueError(
                f"For non-scalar outputs with shape {self.data.shape}, "
                f"gradient must be provided. Expected shape: "
                f"{self.data.shape}"
            )
    ...
    def _analyze_dependencies(self, variable):
        """
        Single-pass dependency analysis returning dependencies
        ...
        def build_topo(node):
            ...
            for parent in node._prev:
                if parent in dependencies:
                    build_topo(parent)
            if node is not variable:  # variable itself not recomputed
    ...
    def recompute_with_value(self, variable, new_value) -> np.ndarray:
        ...
        dependencies, recompute_order = self._analyze_dependencies(
            variable
        )
        ...
        for node in recompute_order:
            ...
            if hasattr(node, "_op_params"):
                new_data = ValueVisualization.forward_with_params(
                    node._operation, operands, **node._op_params
                )
            elif node._operation in ValueVisualization.FORWARD_OPS:
                new_data = ValueVisualization.FORWARD_OPS[
                    node._operation
                ](operands)
            else:
                raise ValueError(
                    f"Unknown operation: {node._operation}"
                )
    ...
    def visualize(
        ...
        (single frame, dark mode).
        """
        # Validation
        if (not hasattr(variable, "_operation")
                or variable._operation != "leaf"):
            raise ValueError(
                "Can only visualize with respect to leaf variables"
            )

        if not self.depends_on(variable):
            raise ValueError(
                "Output doesn't depend on the given variable"
            )

        if variable.data.size != 1 or self.data.size != 1:
            raise ValueError(
                "Can only visualize scalar variables and outputs"
            )
        ...
        var_range = np.linspace(
            current_var - range_scale,
            current_var + range_scale,
            num_points
        )
        output_values = np.array(
            [float(self.recompute_with_value(variable, v))
             for v in var_range]
        )
        ...
        ax.plot(
            ...
            markersize=12,
            label=(
                f"Current ({current_var:.3f}, {current_output:.3f})"
            ),
            zorder=5,
        )
        ...
        if show_tangent:
            tangent = (
                current_output + current_grad * (var_range - current_var)
            )
            ax.plot(
                var_range,
                tangent,
                ...
                color="#8338ec",
                linewidth=2,
                label=f"Tangent (slope={current_grad:.3f})",
                alpha=0.8,
            )
        ...
        ax.set_title(
            title or f"{output_name} as a function of {var_name}",
            fontsize=15,
            color="#ffffff",
            pad=20,
        )
        ...
        ax.legend(
            fontsize=11, facecolor="#1a1a1a", edgecolor="#404040"
        )
    ...
    def visualize_sequence(
        ...
        """
        Interactive visualization showing optimization trajectory
        with slider.

        Args:
            variable: The leaf Value to vary
            history: List of variable values over time
                     (e.g., from optimization steps)
            range_scale: How far to extend x-axis beyond
                         history min/max
        ...
        if len(history) < 2:
            raise ValueError(
                "History must contain at least 2 points"
            )
        ...
        # Pre-compute function values and gradients for entire
        # trajectory
        trajectory_data = []
        for var_val in history:
            output_val = float(
                self.recompute_with_value(variable, var_val)
            )
        ...
        y_margin = (
            (np.max(all_outputs) - np.min(all_outputs)) * 0.1
        )
        y_min = (
            min(np.min(output_values), np.min(all_outputs)) - y_margin
        )
        y_max = (
            max(np.max(output_values), np.max(all_outputs)) + y_margin
        )
        ...
        (point_plot,) = ax.plot(
            [], [], "o", color="#ff006e", markersize=14, zorder=5
        )
        (tangent_plot,) = ax.plot(
            [], [], "--", color="#8338ec", linewidth=2, alpha=0.8,
            zorder=3
        )
        ...
        ax.set_ylabel(output_name, fontsize=13, color="#e0e0e0")
        ax.set_title(
            title or f"{output_name} optimization trajectory",
            fontsize=15,
            color="#ffffff",
            pad=20,
        )
        ...
        # Slider
        ax_slider = plt.axes(
            [0.15, 0.05, 0.7, 0.03], facecolor="#2a2a2a"
        )
        slider = Slider(
            ax_slider,
            "Step",
        ...
        def update(val):
            ...
            # Update tangent
            if show_tangent:
                tangent_y = data["output_val"] + data["grad"] * (
                    var_range - data["var_val"]
                )
