"""
frpc 命令集合：负责部署目录、配置文件以及 Windows 计划任务的统一入口。
"""
...
from .file_ops import frpc_edit, frpc_show
from .listing import frpc_list
from .pwd import frpc_pwd
from .tasks import frpc_addtask, frpc_rmtask


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="frpc",
        description="Windows 上的 frpc 管理工具，简化部署、配置与计划任务导入。",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    deploy = subparsers.add_parser("deploy", help="部署或刷新 frpc 安装目录")
    deploy.add_argument(
        "install_dir",
        nargs="?",
    ...
    cleanup = subparsers.add_parser("cleanup", help="清理 frpc 部署记录")
...
def frpc_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "cleanup":
        return frpc_cleanup(keep_dir=args.keep_dir, keep_service=args.keep_service)
    ...
    if args.command in {"edit", "vim"}:
        name = resolve_config_name("frpc edit", args.name, default=DEFAULT_CONFIG_NAME)
        return frpc_edit(name)

    if args.command in {"show", "cat"}:
        name = resolve_config_name("frpc show", args.name, default=DEFAULT_CONFIG_NAME)
        return frpc_show(name)

    if args.command == "addtask":
        name = resolve_config_name("frpc addtask", args.name, default=DEFAULT_CONFIG_NAME)
        return frpc_addtask(name)

    if args.command == "rmtask":
        name = resolve_config_name("frpc rmtask", args.name, default=DEFAULT_CONFIG_NAME)
        return frpc_rmtask(name)
...
register_command(
    name="frpc",
    handler=frpc_main,
    description="frpc 命令入口（Windows 计划任务部署）",
    usage="frpc <命令> [选项]",
    examples=(
        "frpc deploy",
        "frpc addtask main",
