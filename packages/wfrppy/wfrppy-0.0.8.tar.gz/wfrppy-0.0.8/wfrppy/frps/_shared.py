...
FRP_VERSION = "0.65.0"
DEFAULT_INSTALL_DIR = str(Path.home() / "frp" / "frps")
STATE_FILE = Path.home() / ".wfrppy" / "frps_state.json"
TASK_PREFIX = "frps"
...
BATCH_NAME = "frps.bat"
BIN_NAME = "frps.exe"
TASK_TEMPLATE_FILE = BIN_DIR / "frps_task_template.xml"
...
def _load_state() -> dict:
    if not STATE_FILE.exists():
        raise FileNotFoundError("未检测到 frps 部署记录，请先运行 frps deploy。")
...
def _binary_path() -> Path:
    ...
    if not candidate.exists():
        raise FileNotFoundError(f"未找到可执行程序：{candidate}")
    return candidate
...
def _write_task(config_name: str, *, install_dir: Path, auto_start: bool) -> bool:
    config = _config_path(install_dir, config_name)
    if not config.exists():
        print(f"找不到配置文件：{config}，无法导入计划任务。")
        return False

    task = _task_name(config_name)
    try:
...
def _register_task(name: str, *, install_dir: Path, auto_start: bool) -> bool:
    return _write_task(name, install_dir=install_dir, auto_start=auto_start)
...
def _remove_task(name: str) -> bool:
    ...
    if not task_exists(task):
        return True
    success, detail = _stop_task(task)
