"""
Core thermal DICOM functionality with thermal-specific private tags and metadata handling.
"""
...
class ThermalDicom:
    ...
    # Private tag group for thermal parameters (must be an odd group)
    #THERMAL_GROUP = 0x7FE1
    THERMAL_GROUP = 0x0019

    # Private Creator ID (max length 16). RadiAnt will display this as the owner of the
    # private block; individual element names are not shown by viewers, but the values
    # will no longer appear as "Private Creator" entries.
    PRIVATE_CREATOR_ID = "THERMAL_DICOM"

    # Offsets within the private block (mapped to elements 0x10xx in the group)
    # Keep these <= 0xFF; pydicom will place them starting at element 0x1000
    PRIVATE_OFFSETS = {
        'emissivity': 0x10,
        ...
        'camera_serial': 0x21,
        'lens_model': 0x22,
        'calibration_date': 0x30,
        'temperature_range_min': 0x31,
        'temperature_range_max': 0x32,
        'temperature_unit': 0x33,
        'thermal_sensitivity': 0x34,
        'spectral_range': 0x35,
        'acquisition_mode': 0x40,
        'measurement_conditions': 0x41,
        'roi_temperature_stats': 0x50,
        'temperature_colormap': 0x51,
    }
    ...
    PRIVATE_TAG_DESCRIPTIONS = {
        ...
        'camera_serial': 'Thermal Camera Serial Number',
        'lens_model': 'Thermal Lens Model',
        'calibration_date': 'Thermal Camera Calibration Date',
        ...
        'acquisition_mode': 'Thermal Acquisition Mode',
        'measurement_conditions': 'Measurement Conditions',
        'roi_temperature_stats': 'ROI Temperature Statistics',
        'temperature_colormap': 'Temperature Color Map',
    }
    ...
    def _get_private_block(self, create: bool = True):
        ...
        creator row and place our values under that block instead of listing
    ...
    def _initialize_dicom_structure(self):
        ...
        file_meta.ImplementationVersionName = "THERMAL_DCM_1"
        
        self.dataset.file_meta = file_meta
        
        # Patient Information
        self.dataset.PatientName = "THERMAL^PATIENT"
        self.dataset.PatientID = "THERMAL001"
    ...
    @classmethod
    def load_dicom(cls, filepath: str) -> 'ThermalDicom':
        """
    ...
    def create_standard_thermal_dicom(self, 
                                    ...
                                    patient_sex: Optional[str] = None,
                                    patient_birth_date: Optional[str] = None,
                                    study_date: Optional[str] = None) -> 'ThermalDicom':
        """
        ...
        Returns:
            Configured ThermalDicom instance
        """
        ...
        # Set standard thermal parameters if not provided
        if thermal_params is None:
            thermal_params = {
                'emissivity': 0.98,  # Human skin emissivity
                'distance_from_camera': 1.0,  # meters
                'ambient_temperature': 22.0,  # Celsius
                'reflected_temperature': 22.0,  # Celsius
                'relative_humidity': 50.0,  # percentage
                'temperature_unit': 'Celsius',
