...
class PerformanceMonitorMiddleware(AgentMiddleware):
    ...
    def wrap_model_call(
        ...
        # 估算token数量
        request_tokens = self._estimate_tokens(request.content) if request.content else 0
        ...
        try:
            ...
            # 估算响应token数量
            response_tokens = self._estimate_tokens(response.content) if response.content else 0
