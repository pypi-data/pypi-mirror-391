...
def save_prompt_version(
	...
	*,
	...
	# Check if the last version is identical
	existing_versions = list_prompt_versions(name, root=root)
	if existing_versions:
		last_version_path = existing_versions[-1]
		try:
			last_data = json.loads(last_version_path.read_text(encoding="utf-8"))
			# Compare the critical fields (system, template, variables)
			if (last_data.get("system") == system and 
			    last_data.get("template") == template and 
			    sorted(last_data.get("variables", [])) == var_list):
				# Identical - return existing version without creating a new one
				return last_version_path, False
		except Exception:
			pass  # If we can't read/compare, create a new version
