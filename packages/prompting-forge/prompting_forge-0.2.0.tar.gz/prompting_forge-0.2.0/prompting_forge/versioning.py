...
def save_refined_prompt(
	...
	*,
	...
	template: str,
	variables: List[str],
	filename: str = "refined_prompt.json",
	root: Path | None = None,
) -> Path:
	"""
	Save a refined prompt (result of iterative LLM-based refinement).
	
	Args:
		instance_name: Name of the prompt instance
		system: System message (optional)
		template: Refined template string
		variables: List of variable names
		filename: Name for the refined prompt file (default: "refined_prompt.json")
		root: Root directory for .prompt/ folder
	
	Returns:
		Path to the saved refined prompt file
	"""
	name = instance_name.strip()
	target_dir = _instance_dir(name, root=root)
	target_dir.mkdir(parents=True, exist_ok=True)
	
	# Ensure filename doesn't conflict with final_prompt.json
	if filename == "final_prompt.json":
		raise ValueError("Refined prompt filename cannot be 'final_prompt.json'. Use a different name.")
	...
	record = {
		"ts": _ts(),
		"instance": name,
		"version": -2,  # Special marker for refined prompt
		"system": system,
		"template": template,
		"variables": sorted(variables),
	}
...
def read_refined_prompt(
	...
	*,
	...
	"""
	Read a refined prompt if it exists.
	
	Args:
		instance_name: Name of the prompt instance
		filename: Name of the refined prompt file (default: "refined_prompt.json")
		root: Root directory for .prompt/ folder
	
	Returns:
		Tuple of (system, template) if found, None otherwise
	"""
	name = instance_name.strip()
	path = _instance_dir(name, root=root) / filename
	if not path.exists():
		return None
	try:
		data = json.loads(path.read_text(encoding="utf-8"))
