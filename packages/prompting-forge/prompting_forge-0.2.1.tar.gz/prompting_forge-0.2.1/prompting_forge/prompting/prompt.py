...
class BasePromptTemplate:
	...
	def __or__(self, other: Any) -> Any:
		...
		if callable(other):
			...
			# It's an LLM - return a ChainCallable class
			class ChainCallable:
				def __init__(self, template, llm):
					self._template = template
					self._llm = llm
				
				def __call__(self, variables: Any) -> Any:
					# Format the template
					chat_prompt = self._template.format(variables)
					# Call the LLM
					return self._llm(chat_prompt)
				
				def __or__(self, next_callable: Any) -> Any:
					if hasattr(next_callable, "parse"):
						...
						class ParseChain:
							...
							def __call__(self, variables: Any) -> Any:
								...
								instructions = None
								if hasattr(self._parser, "get_format_instructions"):
									instructions = self._parser.get_format_instructions()
								
								# Format with instructions
								chat_prompt = self._template.format(variables, instructions=instructions)
								# Call LLM
								llm_output = self._llm(chat_prompt)
								# Parse output
								return self._parser.parse(str(llm_output))
						
						return ParseChain(self._template, self._llm, next_callable)
					else:
						# Continue chaining
						raise TypeError(f"Cannot pipe to {type(next_callable)}")
			
			return ChainCallable(self, other)
