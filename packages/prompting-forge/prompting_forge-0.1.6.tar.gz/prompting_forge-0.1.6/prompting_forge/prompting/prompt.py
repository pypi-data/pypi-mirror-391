...
class BasePromptTemplate:
	...
	def __ror__(self, other: Any) -> Any:
		...
		"query" | template -> returns a callable that formats with query
		"""
		if isinstance(other, str):
			# Return a function that takes variables and includes the query
			def with_query(variables: Any = None) -> ChatPrompt:
			...
			# Allow further piping: "query" | template | llm
			def query_or(next_item: Any) -> Any:
				if callable(next_item):
					# It's an LLM
					def query_chain(variables: Any = None) -> Any:
						if variables is None:
							variables = {}
						if isinstance(variables, dict):
							ctx = dict(variables)
							...
							ctx = {"query": other}
						chat_prompt = self.format(ctx)
						return next_item(chat_prompt)
					
					# Allow further piping for parser
					def query_chain_or(parser: Any) -> Any:
						if hasattr(parser, "parse"):
							def full_chain(variables: Any = None) -> Any:
								...
								chat_prompt = self.format(ctx, instructions=instructions)
								llm_output = next_item(chat_prompt)
								return parser.parse(str(llm_output))
							return full_chain
						raise TypeError(f"Cannot pipe to {type(parser)}")
					
					query_chain.__or__ = query_chain_or
					return query_chain
				raise TypeError(f"Cannot pipe to {type(next_item)}")
			
			with_query.__or__ = query_or
			return with_query
