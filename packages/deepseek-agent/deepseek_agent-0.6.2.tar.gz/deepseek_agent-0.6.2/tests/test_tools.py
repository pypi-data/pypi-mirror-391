...
def test_todo_write_appends_entry(tmp_path: Path) -> None:
    executor = ToolExecutor(root=tmp_path)
    result = executor.todo_write("Investigate flaky test", path="notes/TODO.md")
    assert "Added todo entry" in result
    todo_path = tmp_path / "notes" / "TODO.md"
    assert todo_path.exists()
    assert "Investigate flaky test" in todo_path.read_text(encoding="utf-8")
