...
from .agent import AgentOptions, ExecutionMode, ToolExecutor, agent_loop
...
def build_parser() -> argparse.ArgumentParser:
    ...
    if WORKFLOW_CHOICES:
        parser.add_argument(
            ...
            help="Apply a Claude Code-inspired workflow overlay to the system prompt.",
        )
    if PERSONA_CHOICES:
        parser.add_argument(
            "--persona",
            choices=PERSONA_CHOICES,
            help="Adopt a Claude Code-inspired specialist persona.",
        )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    parser.add_argument(
        "--unrestricted",
        action="store_true",
        help="Enable unrestricted execution mode (no permission prompts, direct execution of all commands)",
    )
    parser.add_argument(
        "--no-stream",
        action="store_false",
        dest="stream_output",
        help="Disable streaming output from commands",
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
        help="Enable parallel tool execution for better performance",
    )
    parser.add_argument(
        "--no-reasoning",
        action="store_false",
        dest="enable_reasoning",
        help="Disable reasoning/thinking display",
    )
    ...
    tavily_parser = subparsers.add_parser(
        "tavily",
        help="Invoke Tavily search/extract utilities directly from the CLI.",
    )
    tavily_sub = tavily_parser.add_subparsers(dest="tavily_command")
    tavily_search = tavily_sub.add_parser("search", help="Run a Tavily web search")
    tavily_search.add_argument("query", help="Search query text")
    tavily_search.add_argument(
        "--search-depth",
        choices=["basic", "advanced"],
        default="basic",
        help="Search depth requested from Tavily (default: basic).",
    )
    tavily_search.add_argument(
        "--max-results",
        type=int,
    ...
    tavily_extract = tavily_sub.add_parser("extract", help="Extract a document via Tavily")
    tavily_extract.add_argument("url", help="URL to extract structured content from")
    tavily_extract.add_argument(
        "--extract-depth",
        choices=["basic", "advanced"],
        default="basic",
        help="Extraction depth requested from Tavily (default: basic).",
    )
    tavily_extract.add_argument(
        "--max-pages",
        type=int,
        default=1,
        help="Maximum pages Tavily should crawl when extracting (default: 1, max 5).",
    )
    tavily_extract.add_argument(
        "--no-summary",
        action="store_true",
        help="Disable Tavily's automatic summarization when extracting.",
    )
...
class InteractiveSessionState:
    ...
    default_workflow: Optional[str] = field(init=False)
    default_persona: Optional[str] = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.default_workflow = self.workflow
        self.default_persona = self.persona
...
def _command_reference_table() -> Table:
    ...
    rows = [
        ...
        ("@workflow [NAME|list|none]", "Apply Claude Code-style workflow overlays"),
        ("@persona [NAME|list|none]", "Adopt Claude Code-style specialist personas"),
...
def _workflow_table() -> Table:
    ...
    for slug in WORKFLOW_CHOICES:
        preset = WORKFLOW_PRESETS[slug]
        phases = ", ".join(preset.phases) if preset.phases else "—"
        table.add_row(slug, preset.title, phases)
    return table
...
def _persona_table() -> Table:
    ...
    for slug in PERSONA_CHOICES:
        preset = PERSONA_PRESETS[slug]
        table.add_row(slug, preset.title, preset.description)
    return table
...
def _session_status_panel(state: InteractiveSessionState) -> Panel:
    ...
    grid.add_row("Workflow", state.workflow or "none")
    grid.add_row("Persona", state.persona or "none")
...
def handle_tavily_cli(args: argparse.Namespace) -> int:
    if args.tavily_command is None:
        print("Select a Tavily subcommand (search, extract).", file=sys.stderr)
        return 1
    stored = load_config()
    api_key = (
        getattr(args, "tavily_api_key", None)
    ...
    if args.tavily_command == "search":
        output = executor.tavily_search(
            args.query,
            search_depth=args.search_depth,
            max_results=args.max_results,
            api_key=getattr(args, "tavily_api_key", None),
        )
    elif args.tavily_command == "extract":
        output = executor.tavily_extract(
            args.url,
        ...
        print(f"Unknown Tavily command '{args.tavily_command}'.", file=sys.stderr)
        return 1
    print(output)
    return 0
    ...
    *,
    ...
    if name == "workflow":
        if not WORKFLOW_CHOICES:
            MAIN_CONSOLE.print("[yellow]No bundled workflows are available.[/]")
            return True
        if not args:
            MAIN_CONSOLE.print(f"[cyan]Workflow:[/] {state.workflow or 'none'}")
            MAIN_CONSOLE.print(_workflow_table())
            return True
        keyword = args[0].lower()
        if keyword in {"list", "ls"}:
            MAIN_CONSOLE.print(_workflow_table())
            return True
        if keyword in {"none", "clear", "reset"}:
            state.workflow = None
            MAIN_CONSOLE.print("[yellow]Workflow overlay cleared.[/]")
            return True
        preset = WORKFLOW_PRESETS.get(keyword)
        if not preset:
            choices = ", ".join(WORKFLOW_CHOICES)
            MAIN_CONSOLE.print(f"[red]Unknown workflow '{keyword}'.[/] Available: {choices}")
            return True
        state.workflow = keyword
        MAIN_CONSOLE.print(
            Panel(
                Text(f"{preset.title}\n{preset.description}", style="bright_white"),
                title=f"Workflow · {keyword}",
                border_style="green",
            )
        )
        return True
    if name == "persona":
        if not PERSONA_CHOICES:
            MAIN_CONSOLE.print("[yellow]No personas are bundled in this build.[/]")
            return True
        if not args:
            MAIN_CONSOLE.print(f"[cyan]Persona:[/] {state.persona or 'none'}")
            MAIN_CONSOLE.print(_persona_table())
            return True
        keyword = args[0].lower()
        if keyword in {"list", "ls"}:
            MAIN_CONSOLE.print(_persona_table())
        ...
        if keyword in {"none", "clear", "reset"}:
            state.persona = None
            MAIN_CONSOLE.print("[yellow]Persona overlay cleared.[/]")
            return True
        preset = PERSONA_PRESETS.get(keyword)
        if not preset:
            choices = ", ".join(PERSONA_CHOICES)
            MAIN_CONSOLE.print(f"[red]Unknown persona '{keyword}'.[/] Available: {choices}")
            return True
        state.persona = keyword
        MAIN_CONSOLE.print(
            Panel(
                Text(f"{preset.title}\n{preset.description}", style="bright_white"),
                title=f"Persona · {keyword}",
                border_style="green",
            )
        )
        return True
    ...
    if name == "reset":
        ...
        state.workflow = state.default_workflow
        state.persona = state.default_persona
...
def _run_interactive_agent_prompt(
    ...
    options = AgentOptions(
        ...
        execution_mode=ExecutionMode.UNRESTRICTED,  # Always unrestricted in CLI
        parallel_tools=True,
        auto_approve=True,
        stream_output=True,
...
def handle_agent(args: argparse.Namespace, resolved: ResolvedConfig) -> int:
    ...
    execution_mode = ExecutionMode.UNRESTRICTED if getattr(args, 'unrestricted', True) else (
        ExecutionMode.READ_ONLY if args.read_only else ExecutionMode.SANDBOXED
    )

    system_prompt = compose_system_prompt(
        args.system or resolved.system_prompt,
    ...
    options = AgentOptions(
        ...
        parallel_tools=getattr(args, 'parallel', True),
        auto_approve=True,  # Always auto-approve in CLI mode
        stream_output=getattr(args, 'stream_output', True),
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "tavily":
        return handle_tavily_cli(args)
    ...
    return run_interactive_agent_shell(
        ...
        persona_override=getattr(args, "persona", None),
