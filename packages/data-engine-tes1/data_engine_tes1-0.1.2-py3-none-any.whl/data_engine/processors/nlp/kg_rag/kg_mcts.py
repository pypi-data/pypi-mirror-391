...
class MCTSNode:
    ...
    def select_child(self):
        ...
        return max(
            self.children,
            key=lambda c: c.value / (c.visits + 1)
            + c_puct * c.prior * (self.visits**0.5) / (c.visits + 1),
        )
...
class MCTS:
    def __init__(
        self,
        llm_model,
        kg_query: KGQuery = None,
        alg_se: StructEntropy = None,
    ...
    def collect_paths_above_threshold(self, node, m=0):
    ...
    def hdpo_loss(self, logits, values):
        ...
        value_loss = torch.max(
            torch.zeros_like(preferred_values),
            0.1 - (preferred_values - dispreferred_values),
        )
    ...
    def fine_tune_llm(self):
        ...
        for preference in self.preferences:
            ...
            inputs = self.llm_model.tokenizer(
                input_text, return_tensors="pt", padding=True, truncation=True
            )

            policy_output, value_output = self.llm_model(
