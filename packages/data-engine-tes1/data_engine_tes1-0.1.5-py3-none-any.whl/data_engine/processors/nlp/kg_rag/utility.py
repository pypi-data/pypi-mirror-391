...
def get_context_using_spoke_api(node_value):
    ...
    merge_1 = pd.merge(
        nbr_edges_df, nbr_nodes_df, left_on="source", right_on="node_id"
    ).drop("node_id", axis=1)
    ...
    merge_2 = pd.merge(
        merge_1, nbr_nodes_df, left_on="target", right_on="node_id"
    ).drop("node_id", axis=1)
...
def llama_model(
    ...
    if method == "method-1":
        ...
        tokenizer = transformers.LlamaTokenizer.from_pretrained(
            model_name, revision=branch_name, cache_dir=cache_dir, legacy=False
        )
    ...
    llm = HuggingFacePipeline(
        pipeline=pipe, model_kwargs={"temperature": temperature, "top_p": top_p}
    )
    return llm
...
def fetch_GPT_response(
    instruction, system_prompt, chat_model_id, chat_deployment_id, temperature=0
):
...
def get_GPT_response(
    ...
    return fetch_GPT_response(
        instruction, system_prompt, chat_model_id, chat_deployment_id, temperature
    )
        ...
        for node_name in node_hits:
            ...
            if edge_evidence:
                ...
                context_table = context_table[
                    context_table.context.isin(high_similarity_context)
                ]
...
def interactive(
    ...
    for node_name in node_hits:
        ...
        node_context_embeddings = (
            embedding_function_for_context_retrieval.embed_documents(node_context_list)
        )
        similarities = [
            cosine_similarity(
                np.array(question_embedding).reshape(1, -1),
        ...
        high_similarity_indices = [
            s[1]
            for s in similarities
            if s[0] > percentile_threshold
            and s[0] > config_data["QUESTION_VS_CONTEXT_MINIMUM_SIMILARITY"]
        ]
        if (
            len(high_similarity_indices)
            > max_number_of_high_similarity_context_per_node
...
class StructEntropy:
    def __init__(self, graph: nx.Graph):
        ...
        self.division = (
            {}
        )  # {comm1: [node11, node12, ...], comm2: [node21, node22, ...], ...}
        self.struc_data = (
            {}
        )  # {comm1: [vol1, cut1, community_node_SE, leaf_nodes_SE], comm2:[vol2, cut2, community_node_SE, leaf_nodes_SE]，... }
        self.struc_data_2d = (
            {}
        )  # {comm1: {comm2: [vol_after_merge, cut_after_merge, comm_node_SE_after_merge, leaf_nodes_SE_after_merge], comm3: [], ...}, ...}
    ...
    def update_struc_data_2d(self):
        ...
        self.struc_data_2d = (
            {}
        )  # {(comm1, comm2): [vol_after_merge, cut_after_merge, comm_node_SE_after_merge, leaf_nodes_SE_after_merge], (comm1, comm3): [], ...}
    ...
    def update_division_MinSE(self):
        ...
        while True:
            ...
            if delta_SE < 0:
                ...
                vmnodeSE = (
                    self.struc_data[vm1][3]
                    - (self.struc_data[vm1][0] / self.vol)
                    * math.log2(self.struc_data[vm1][0] / volume)
                    + self.struc_data[vm2][3]
                    - (self.struc_data[vm2][0] / self.vol)
                    * math.log2(self.struc_data[vm2][0] / volume)
                )
