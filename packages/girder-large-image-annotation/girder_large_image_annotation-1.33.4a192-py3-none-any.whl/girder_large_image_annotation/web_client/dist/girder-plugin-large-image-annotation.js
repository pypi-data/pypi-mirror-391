...
const st = Ve.extend({
  ...
  handleElementChanged: function(a, t, n) {
    ...
    if (a.get("center"))
      ...
      for (const [L, R] of h)
        p = Math.min(p, L), b = Math.min(b, R), f = Math.max(f, L), w = Math.max(w, R);
      ...
      if (l && (k.each(this._annotations[t.id].features, (h, p) => {
        ...
        this.getUnclampBoundsForOverlay() && this._annotations[t.id].overlays.length > 0 && (this.viewer.clampBoundsY(!1), this.viewer.clampBoundsX(!1)), k.each(this._annotations[t.id].overlays, (h) => {
          ...
          Pn({
            ...
            const w = this._generateOverlayLayerParams(f, p, h), L = h.type === "pixelmap" ? "pixelmap" : "osm", R = this._getOverlayTransformProjString(h), S = this.viewer.createLayer(L, Object.assign({}, w, { id: h.id, gcs: R }));
