...
def build_pteredactyl_recogniser(
    ...
    r"""Build a custom regex ptererecogniser for pteredactyl.
    ...
    Example:
        ...
        >>> recogniser = build_pteredactyl_recogniser(
        ...     entity_type="SOUTHAMPTON_LANDLINE",
        ...     regex=r"(?:\\d[\\s-]?){11}",
...
def fetch_pteredactyl_recogniser(entity_type: str) -> PteredactylRecogniser:
    if entity_type not in REGEX_ENTITIES.keys():
        raise MissingRegexRecogniserError(f"""Entity '{entity_type}' not found. Consider adding to the 'REGEX_ENTITIES' dictionary, or feeding a custom recogniser to regex_entities with:
...
def build_regex_entity_recogniser_list(
    ...
    r"""Build a list of custom regex PteredactylRecognisers.
    ...
    Example:
        >>> recognisers = build_regex_entity_recogniser_list([
        ...     "NHS_NUMBER",
        ...     "ENTITY_2",
        ...     PteredactylRecogniser(
        ...         entity_type="SOUTHAMPTON_LANDLINE",
...
def rebuild_analyser_regex_recognisers(
    ...
    """Rebuilds the analyser's regex recognisers with the supplied list of regex
    entities.
