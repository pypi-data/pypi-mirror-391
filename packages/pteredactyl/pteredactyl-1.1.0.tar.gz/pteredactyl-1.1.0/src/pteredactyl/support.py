...
def find_substring_positions(s: str, sep: str = " ") -> list[tuple[int, int]]:
    """Finds the starting and ending indexes of substrings in the input string `s`. The
    substrings are determined by splitting `s` at separator.
...
def split_results_into_individual_words(
    ...
    """Splits identified RecognizerResults into individual words. For example, Jane
    Smith becomes <PERSON> <PERSON>, rather than <PERSON>.
...
def return_allowed_results(
    ...
    """Checks list of RecognizerResults for allowed entities returns a list of allowed
    results.
...
def load_registry(
    ...
    """Creates an AnalyzerEngine.registry by combining a TransformersRecogniser with a
    list of custom PteredactylRecognisers.
