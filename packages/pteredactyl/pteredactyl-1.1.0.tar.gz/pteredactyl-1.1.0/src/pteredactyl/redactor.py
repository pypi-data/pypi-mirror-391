...
def create_analyser(
    ...
    """Create an analyser engine with a Transformers NER model and spaCy model. It is
    recommended to use this function to create an analyser and pass it to
    analyse/anonymise functions downstream to be reused, rather than allowing downstream
    functions to spin up new analyser engines automatically.
...
def analyse(
    ...
    **kwargs,
    ...
    Example:
        ...
        >>> text = "My name is John Doe and my NHS number is 1234567890"
...
def anonymise(
    ...
    **kwargs,
    ...
    """Anonymises the given text by replacing specified entities by NER, and and regex
    entities by REGEX. Regex entities take priority and are analysed first. It is
    recommended to first create an analyser and feed this in to be reused.

    Args:
    text (str): The text to be anonymised.
    analyser (AnalyzerEngine, optional): An instance of AnalyzerEngine. If not provided, a new analyser will be created.
    entities (list, optional): A list of entity types to anonymise. If not provided, a default list will be used.
    regex_entities (list, optional): A list of regex entities or PteredactylRecognisers to analyse. If not provided, a default list will be used.
    highlight (bool): If True, highlights the anonymised parts in the text.
    ...
    **kwargs: Additional keyword arguments for analyse.
    ...
    if not mask_individual_words:
        anonymised_result = anonymiser.anonymize(
        ...
        anonymised_result = anonymiser._operate(
    ...
    anonymised_text = anonymised_result.text.replace("PHONE_NUMBER", "NUMBER")
...
def anonymise_df(
    ...
    **kwargs,
    ...
    entities (list, optional): A list of entity types to anonymise. If not provided, a default list will be used.
    regex_entities (list, optional): A list of regex entities or PteredactylRecognisers to analyse. If not provided, a default list will be used.
    highlight (bool): If True, highlights the anonymisedd parts in the text.
    replacement_lists: (dict, optional): A dictionary with entity types as keys and lists of replacement values for hide-in-plain-sight redaction.
    model_path (str): The path to the model used for analysis (e.g. 'StanfordAIMI/stanford-deidentifier-base'). Used only if analyser not provided.
    spacy_model (str): The spaCy model to use (e.g. 'en_core_web_sm'). Used only if analyser not provided.
    language (str): The language of the text to be analyzed. Defaults to "en". Used only if analyser not provided.
    ...
    col_inplace (bool): If True, replaces the original column with the anonymisedd column. If False, returns anonymised text in a new column.
    col_header_append (str): String to append to the header of the anonymised column.
    rebuild_regex_recognisers (bool): If True, and an existing analyser is provided, the analyser's regex recognisers will be rebuilt before execution.
    ignore_nan (bool): If True, ignores NaN values in the column(s) to anonymise.
    **kwargs: Additional keyword arguments for analyse.
    ...
    def anonymise_row(text: str, ignore_nan: bool = True) -> str:
    ...
    for col in tqdm(columns):
        ...
        df[new_col] = df[col].progress_apply(anonymise_row, ignore_nan=ignore_nan)
