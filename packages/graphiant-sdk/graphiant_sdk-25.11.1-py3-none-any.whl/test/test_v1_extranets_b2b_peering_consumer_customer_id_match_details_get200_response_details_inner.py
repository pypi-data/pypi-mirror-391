# coding: utf-8

"""
    Graphiant APIs

    Graphiant API documentation.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.v1_extranets_b2b_peering_consumer_customer_id_match_details_get200_response_details_inner import V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner

class TestV1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner(unittest.TestCase):
    """V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner:
        """Test V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner`
        """
        model = V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner()
        if include_optional:
            return V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner(
                consumer_prefixes = [
                    'example string'
                    ],
                customer = graphiant_sdk.models._v1_extranets_b2b_peering_consumer__customer_id__match_details_get_200_response_details_inner_customer._v1_extranets_b2b_peering_consumer__customerId__match_details_get_200_response_details_inner_customer(
                    company_name = 'example string', 
                    emails = [
                        'example string'
                        ], 
                    id = 1234567891011, 
                    num_sites = 123, ),
                service = graphiant_sdk.models._v1_extranets_b2b_peering_consumer__customer_id__match_details_get_200_response_details_inner_service._v1_extranets_b2b_peering_consumer__customerId__match_details_get_200_response_details_inner_service(
                    company_name = 'example string', 
                    contact_email = [
                        'example string'
                        ], 
                    description = 'example string', 
                    id = 1234567891011, ),
                service_prefixes = [
                    graphiant_sdk.models._v1_extranets_b2b_peering_match_service_to_customer_put_request_service_service_prefixes_inner._v1_extranets_b2b_peering_match_service_to_customer_put_request_service_servicePrefixes_inner(
                        prefix = 'example string', 
                        tag = 'example string', )
                    ]
            )
        else:
            return V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner(
        )
        """

    def testV1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner(self):
        """Test V1ExtranetsB2bPeeringConsumerCustomerIdMatchDetailsGet200ResponseDetailsInner"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
