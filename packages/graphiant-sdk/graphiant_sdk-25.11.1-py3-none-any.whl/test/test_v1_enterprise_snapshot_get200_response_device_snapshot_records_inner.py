# coding: utf-8

"""
    Graphiant APIs

    **To use the APIs:**  1) Login using `/api/v1/auth/login`   2) Copy the value of \"token\" in the response   3) Click the \"Authorize\" button   4) In the \"Value\" text field enter: `Bearer <your token>`   5) Click \"Authorize\"   6) All requests are now authorized.  **Token valid for 2 hours. If expired:**   - Login again, click \"Authorize\", paste new token.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.v1_enterprise_snapshot_get200_response_device_snapshot_records_inner import V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner

class TestV1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner(unittest.TestCase):
    """V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner:
        """Test V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner`
        """
        model = V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner()
        if include_optional:
            return V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner(
                device_id = 1234567891011,
                hostname = 'example string',
                region = 'example string',
                site = 'example string',
                snapshot_count = 123,
                snapshots = [
                    graphiant_sdk.models._v1_device_snapshot_get_200_response_first_snapshot._v1_device_snapshot_get_200_response_firstSnapshot(
                        author = graphiant_sdk.models._v1_device_snapshot_get_200_response_first_snapshot_author._v1_device_snapshot_get_200_response_firstSnapshot_author(
                            enterprise_id = 1234567891011, 
                            exp = 1234567891011, 
                            original_enterprise_id = 1234567891011, 
                            permissions = graphiant_sdk.models._v1_groups_get_200_response_groups_inner_permissions._v1_groups_get_200_response_groups_inner_permissions(
                                asset_manager = 'ENUM_VALUE', 
                                b2b = 'ENUM_VALUE', 
                                billing_and_invoicing = 'ENUM_VALUE', 
                                compliance = 'ENUM_VALUE', 
                                developer_tools = 'ENUM_VALUE', 
                                gateway = 'ENUM_VALUE', 
                                global_services = 'ENUM_VALUE', 
                                insights = 'ENUM_VALUE', 
                                licensing = 'ENUM_VALUE', 
                                logs = 'ENUM_VALUE', 
                                monitoring_and_troubleshooting = 'ENUM_VALUE', 
                                network_configuration = 'ENUM_VALUE', 
                                order_status = 'ENUM_VALUE', 
                                reports = 'ENUM_VALUE', 
                                safety_and_security = 'ENUM_VALUE', 
                                service_policies = 'ENUM_VALUE', 
                                support = 'ENUM_VALUE', 
                                user_and_tenant_management = 'ENUM_VALUE', ), 
                            time_zone = 'example string', 
                            user_id = 'example string', ), 
                        category = 'ENUM_VALUE', 
                        created_on = graphiant_sdk.models._v1_alarm_history_get_200_response_history_inner_time._v1_alarm_history_get_200_response_history_inner_time(
                            nanos = 123, 
                            seconds = 1234567891011, ), 
                        data = graphiant_sdk.models._v1_device_snapshot_get_200_response_first_snapshot_data._v1_device_snapshot_get_200_response_firstSnapshot_data(
                            ospf_installed_route_count = 123, 
                            t2_session_count = 123, 
                            twamp_session_count = 123, 
                            bfd_session_count = 123, 
                            bgp_neighbor_ip_list = [
                                'example string'
                                ], 
                            bgp_session_count = 123, 
                            device_role = 'ENUM_VALUE', 
                            failed_services_count = 123, 
                            graphnos_version = 'example string', 
                            installed_labels = 123, 
                            ipsec_session_count = 123, 
                            ongoing_alerts = 123, 
                            ospf_neighbor_ip_list = [
                                'example string'
                                ], 
                            ospf_session_count = 123, ), 
                        golden = True, 
                        id = 1234567891011, 
                        name = '6-1-24-status-check', )
                    ],
                uptime = graphiant_sdk.models._v1_alarm_history_get_200_response_history_inner_time._v1_alarm_history_get_200_response_history_inner_time(
                    nanos = 123, 
                    seconds = 1234567891011, )
            )
        else:
            return V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner(
        )
        """

    def testV1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner(self):
        """Test V1EnterpriseSnapshotGet200ResponseDeviceSnapshotRecordsInner"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
