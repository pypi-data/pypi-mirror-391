# coding: utf-8

"""
    Graphiant APIs

    **To use the APIs:**  1) Login using `/api/v1/auth/login`   2) Copy the value of \"token\" in the response   3) Click the \"Authorize\" button   4) In the \"Value\" text field enter: `Bearer <your token>`   5) Click \"Authorize\"   6) All requests are now authorized.  **Token valid for 2 hours. If expired:**   - Login again, click \"Authorize\", paste new token.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.v1_devices_device_id_config_put_request_edge_nat_policy_nat_rulesets_value import V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue

class TestV1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue(unittest.TestCase):
    """V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue:
        """Test V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue`
        """
        model = V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue()
        if include_optional:
            return V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue(
                ruleset = graphiant_sdk.models._v1_devices__device_id__config_put_request_edge_nat_policy_nat_rulesets_value_ruleset._v1_devices__deviceId__config_put_request_edge_natPolicy_natRulesets_value_ruleset(
                    description = 'example string', 
                    name = 'example string', 
                    rules = {
                        'key' : graphiant_sdk.models._v1_devices__device_id__config_put_request_edge_nat_policy_nat_rulesets_value_ruleset_rules_value._v1_devices__deviceId__config_put_request_edge_natPolicy_natRulesets_value_ruleset_rules_value(
                            rule = graphiant_sdk.models._v1_devices__device_id__config_put_request_edge_nat_policy_nat_rulesets_value_ruleset_rules_value_rule._v1_devices__deviceId__config_put_request_edge_natPolicy_natRulesets_value_ruleset_rules_value_rule(
                                advertise_pre_nat_prefixes = True, 
                                description = 'example string', 
                                name = 'example string', 
                                original_dst_ip_prefix = 'example string', 
                                original_src_ip_prefix = 'example string', 
                                seq = 1234567891011, 
                                translated_dst_ip_prefix = 'example string', 
                                translated_src_ip_prefix = 'example string', 
                                type = 'ENUM_VALUE', ), )
                        }, )
            )
        else:
            return V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue(
        )
        """

    def testV1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue(self):
        """Test V1DevicesDeviceIdConfigPutRequestEdgeNatPolicyNatRulesetsValue"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
