...
When comparing two fields (for example, simulations vs observations, different model runs...) we often need the fields to have matching spatial and temporal coordinates. We can select entries with matching coordinates with thWe can select entries with matching coordinates with thWe can select entries with matching coordinates with the :func:`overlap` function, as in the following example:
