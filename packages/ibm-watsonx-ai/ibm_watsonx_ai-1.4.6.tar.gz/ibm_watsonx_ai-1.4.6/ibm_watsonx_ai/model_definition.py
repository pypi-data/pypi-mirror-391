...
class ModelDefinition(WMLResource):
    ...
    def store(
        ...
        **Example:**
        ...
        if isinstance(model_definition, Path):
        ...
        try:
            ...
            with open(model_definition, "rb") as file:
                if self._ICP_PLATFORM_SPACES:
                    put_response = requests.put(
                        model_definition_attachment_put_url,
                        files={
                            "file": (model_definition, file, "application/octet-stream")
                        },
                    )
                else:
                    put_response = requests.put(
    ...
    async def adownload(
        ...
        **Example:**
        ...
        .. code-block:: python
            ...
            await client.model_definitions.adownload(model_definition_id, "model_definition_file")
        """
        if isinstance(filename, Path):
            filename = str(filename)
    ...
    async def _aget_attachment_id(
        ...
        response_get = await self._client.async_httpx_client.get(
            url, params=paramvalue, headers=await self._client._aget_headers()
        )
        details = self._handle_response(200, op_name, response_get)
        try:
            attachment_id = details["attachments"][0]["id"]
    ...
    async def acreate_revision(self, model_definition_id: str) -> dict:
        ...
        **Example:**
        ...
        .. code-block:: python
            ...
            model_definition_revision = await client.model_definitions.acreate_revision(model_definition_id)
        """
        self._client._check_if_either_is_set()
