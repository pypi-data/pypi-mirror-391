...
class RAGEngine(WMLResource):
    ...
    def summary(self, scoring: str | list[str] | None = None) -> "DataFrame":
            ...
            for key, sub_keys in setting_keys.items():
                if key in settings:
                    for sub_key in sub_keys:
                        ...
                        value = get_from_json(settings, [key, sub_key], "")
        ...
        # Normalize in case of missing values
        for rows in data_sorted_by_optimization_metrics.values():
            rows += [""] * (len(rag_pattern_names) - len(rows))
