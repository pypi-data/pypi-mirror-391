...
from ibm_watsonx_ai.utils.utils import _async_httpx_retry, _requests_retry_session
...
class Connections(WMLResource):
    ...
    def _handle_asset_search_response(
        self,
        response: _requests.Response | httpx.Response,
        limit: int | None,
        assets: list[dict],
    ...
    def get_details(
        ...
        **Example:**
        ...
        if connection_id:
            with _requests_retry_session() as session:
        ...
        with _requests_retry_session() as session:
            while payload is not None and (limit is None or limit > 0):
                ...
                payload, limit = self._handle_asset_search_response(
                    response, limit, assets
                )
    ...
    @_async_httpx_retry()
    async def _aget_connection_by_id(
        self, connection_id: str, headers: dict
    ) -> httpx.Response:
        return await self._client.async_httpx_client.get(
            self._client._href_definitions.get_connection_by_id_href(connection_id),
            params=self._client._params(),
            headers=headers,
        )

    @_async_httpx_retry()
    async def _apost_connection_details(
        self, payload: dict, headers: dict
    ) -> httpx.Response:
        return await self._client.async_httpx_client.post(
            self._client._href_definitions.get_asset_search_href("connection"),
            json=payload,
            params=self._client._params(),
            headers=headers,
    ...
    async def aget_details(
        ...
        **Example:**
        ...
        if connection_id:
            response = await self._aget_connection_by_id(connection_id, headers)
        ...
        while payload is not None and (limit is None or limit > 0):
            ...
            response = await self._apost_connection_details(payload, headers)
    ...
    def _get_datasource_details(self) -> list:
        ...
        def get_res(url):
            with _requests_retry_session() as sess:
                response = sess.get(
                    url,
                    headers=self._client._get_headers(),
                )
    ...
    async def _aget_datasource_type_details(
        ...
        @_async_httpx_retry()
        async def _aget_connection_data_type(self) -> httpx.Response:
            return await self._client.async_httpx_client.get(
                self._client._href_definitions.get_connection_data_type_href(
                    datasource_type
                ),
