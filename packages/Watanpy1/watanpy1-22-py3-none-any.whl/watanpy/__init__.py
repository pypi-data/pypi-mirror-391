...
class GMAIL:
    ...
    def CheckEmail(email):
        ...
        if '..' in email or '_' in email or len(email) < 5 or len(email) > 30:
            return {"data":{"status":False,"email":email,"error_code":1,"programmer":"@Q_e_9"}}
        try:
            ...
            if 'steps/signup/password'in r:
                return {"data":{"status":True,"email":email,"error_code":0,"programmer":"@Q_e_9"}}
            else:
                return {"data":{"status":False,"email":email,"error_code":1,"programmer":"@Q_e_9"}}
        except:""
class instagram:
    @staticmethod
    def CheckEmail(email):
        if ("@")not in email:return {"data":{"status":"fail","message":"Error Email.","programmer":"@Q_e_9"}}
            ...
            'Content-Length':'364',
                ...
                try:
                    ...
                    if ('"can_recover_with_code"')in re.text:
                        return {"data":{"status":True,"programmer":"@Q_e_9"}}
                    elif "ip"in re.text:
                        return  {"data":{"status":"ip_block run vpn or use proxy","programmer":"@Q_e_9"}}            	             
                    else:
                        return {"data":{"status":False,"programmer":"@Q_e_9"}}
                except Exception as e:""    
        elif choice_ == "3":
...
#"spam":true
    ...
    def generateUsername2011() -> str:
        ...
        try:
            ...
            return {"data":{"message":"ok","username":username,"follower_count":fol,"programmer":"@Q_e_9"}}
        except:
...
'abcdefghijklmnopqrstuvwxyzéèêëàâäôùûüîïç',                'abcdefghijklmnopqrstuvwxyzñ',  
        ...
        try:
            ...
            for i in response:
                us=i['user']['username']
                return {"data":{"message":"ok","username":us,"programmer":"@Q_e_9"}}
        except:
            return{"data":{"Errur":"try agin"}}
    @staticmethod
    def Reset(email):
        ...
        if ('"can_recover_with_code"')in res.text:
            sdd=res.json()["email"]
            return {"data":{"status":True,"email_or_user":email,"reset":sdd,"programmer":"@Q_e_9"}}
        elif "user_not_found"in res.text:
            return {"data":{"status":True,"email_or_user":email,"reset":"user_not_found","programmer":"@Q_e_9"}} 	       
        elif "ip" or "Please wait a few minutes before you try again"in  res.text:  
            return {"data":{"status":False,"result":"ip block run vpn or use proxy","programmer":"@Q_e_9"}} 	 
        else:
            return {"data":{"status":None,"email_or_user":email,"reset":None,"programmer":"@Q_e_9"}}
    @staticmethod
    def GetData(Id):
        try:
            if int(Id) >1 and int(Id)<1279000:
                ...
                d= "2020-2025"
            return {"message":{"status":True,"data":d,"programmer":"@Q_e_9"}}
        except:
            return {"message":{"status":False,"data":False,"programmer":"@Q_e_9"}}
    @staticmethod
    def loginweb(username,password):
        ...
        if "userId" and '"authenticated":true' in response.text:
            ...
            return {"data":{"status":True,"username":username,"paswerd":password,"login":True,"sessionId":ses,"programmer":"@Q_e_9"}}
        elif '{"user":true,"authenticated":false,"error_type":"UserInvalidCredentials","status":"ok"}'in response.text:
            return {"data":{"status":True,"username":username,"paswerd":password,"login":"bad_password","programmer":"@Q_e_9"}}
        elif "two_factor_required"in response.text:
            return {"data":{"status":True,"username":username,"paswerd":password,"login":"two_factor_required","programmer":"@Q_e_9"}}
        elif "challenge_required" or "checkpoint_required"in response.text:
            return {"data":{"status":True,"username":username,"paswerd":password,"login":"sceure","programmer":"@Q_e_9"}}
        elif '"spam":true'or "ip" or "Please wait a few minutes before you try again"in response.text:
            return {"data":{"status":False,"username":username,"paswerd":password,"login":"ip block run vpn or use proxy","programmer":"@Q_e_9"}}
        else:
...
class HOTMAIL:
    ...
    def CheckEmail(email):
        if not "@"in email:
            return {"data":{"status":None,"message":"use @hotmail.com or @outlook.com in email","error_code":3,"programmer":"@Q_e_9"}}
        ...
        try:
            ...
            if res.json()['isAvailable']==False:
                return {"data":{"status":False,"email":email,"error_code":0,"programmer":"@Q_e_9"}}
                ...
                return {"data":{"status":True,"email":email,"error_code":1,"programmer":"@Q_e_9"}}
        except:""
class AOL:
    @staticmethod
    #by l7n - asyncio
    ...
    def CheckEmail(email,st):
        ...
        try:
            ...
            if int(errur_code)==1023:
                return{"data":{"status":True,"description":description,"error_code":errur_code,"programmer":"@Q_e_9"}}
            elif int(errur_code)==1:
                return{"data":{"status":description,"error_code":errur_code,"programmer":"@Q_e_9"}}
            else:
                return{"data":{"status":False,"description":description,"error_code":errur_code,"programmer":"@Q_e_9"}}
        except Exception as e:
            return{"data":{"status":"Error {}".format(e),"programmer":"@Q_e_9"}}
    @staticmethod
    def info(user):
        he={
        ...
        try:
            ...
            return{"data":{"status":True,"username":user,"id":ids,"name":name,"following":folon,"follower":folos,"like":lik,"video":vid,"'private":priv,"programmer":"@Q_e_9"}}
        except:
            return{"data":{"status":"user not found ","programmer":"@Q_e_9"}}
