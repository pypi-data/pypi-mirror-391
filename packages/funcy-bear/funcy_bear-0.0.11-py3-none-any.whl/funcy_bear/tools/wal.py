...
class WALConfig:
    ...
    def __init__(
        ...
        """Initialize WALConfig with given parameters.

        Args:
            flush_mode: When to flush WAL writes to disk
            auto_checkpoint: Automatically checkpoint when WAL reaches threshold
            checkpoint_threshold: Number of WAL operations before auto-checkpoint
            flush_interval: Seconds between automatic flushes in BUFFERED mode
            flush_batch_size: Number of operations before forcing flush in BUFFERED mode
            on_flush_error: Optional callback for flush errors in background thread
        """
        self.flush_mode: WALFlushMode = self.validate_flush_mode(flush_mode)
        self.auto_checkpoint: bool = auto_checkpoint
        self.checkpoint_threshold: int = self.validate_checkpoint_threshold(checkpoint_threshold)
        self.flush_interval: float = self.validate_flush_interval(flush_interval)
        self.flush_batch_size: int = self.validate_flush_batch_size(flush_batch_size)
        self.on_flush_error: Callable[[Exception], None] | None = on_flush_error
