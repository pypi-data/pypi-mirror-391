...
def test_issues_create_success(runner: CliRunner, mock_api_create_issue: mock.MagicMock) -> None:
    ...
    assert "ID:" in result.stdout and "issue-id-" in result.stdout
    assert "URL:" in result.stdout and "https://linear.app/team/issue/issue-3" in result.stdout
    assert "Branch name:" in result.stdout and "linear/issue-" in result.stdout and "new-issue" in result.stdout
