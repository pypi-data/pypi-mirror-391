...
async def create_issue_async(
    *,
    ...
    """
    try:
        team_states_response = await _execute_query_async(
            team_states_query,
        ...
        states = team_states_response["data"]["team"]["states"]["nodes"]
        
        todo_variations = ["todo", "to do", "to-do"]
        for state in states:
            state_name_lower = state["name"].lower()
            if state_name_lower in todo_variations:
    ...
    if todo_state_id:
        variables["input"]["stateId"] = todo_state_id
