...
class AGClient:
        ...
        **kwargs,
        ...
        if external_token_header_val and not api_key:
            try:
                ...
                access_token = f"Bearer {exchanged_token['access_token']}"
    ...
    def __get_refresh_token(self) -> None:
        try:
            ...
            if access_token:
                self.__headers[self.oblv_auth_header] = f"Bearer {access_token}"
            if refresh_token:
                self.__headers['refresh_token'] = refresh_token
        except Exception:
    ...
    def __process_message(self, data: Union[Dict[str, Any], requests.Response]) -> None:
        ...
        if approval_status == 'approved':
            ...
            if token:
                self.__headers[self.oblv_auth_header] = f"Bearer {token}"
