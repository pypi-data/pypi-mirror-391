...
class AsyncMailer:
    """
    Asynchronous SMTP mailer for sending emails in async context.
    """
    ...
    async def send(self, msg, debug=False):
        """
        Send one message or a sequence of messages asynchronously.
        """
        ...
        if self.use_ssl:
            server = await loop.run_in_executor(
                None,
                lambda: smtplib.SMTP_SSL(self.host, self.port, timeout=self.timeout)
            )
        else:
            server = await loop.run_in_executor(
                None,
        ...
        if self._usr and self._pwd:
            ...
            await loop.run_in_executor(None, lambda: server.login(self._usr, self._pwd))

        if isinstance(msg, Message):
            msg = [msg]

        for m in msg:
            await self._send(server, m, loop)

        await loop.run_in_executor(None, server.quit)

    async def _send(self, server, msg, loop):
        """
        Sends a single message asynchronously using the server
        """
        me = msg.From
        if isinstance(msg.To, str):
            to = [msg.To]
        ...
        await loop.run_in_executor(
            None,
            lambda: server.sendmail(me, you, message_str)
        )
