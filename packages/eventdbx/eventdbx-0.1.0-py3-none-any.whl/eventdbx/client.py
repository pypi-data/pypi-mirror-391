...
class EventDBXClient:
    ...
    def send_event(
        ...
        *,
        ...
        response = self._send_request(build_payload)
    ...
    def _handshake(self) -> None:
        ...
        hello_response = self._schema.ControlHelloResponse.from_bytes(response_bytes)
        if not hello_response.accepted:
            raise EventDBXHandshakeError(hello_response.message or "Handshake rejected")

    def _send_request(self, payload_builder: Callable[[Any], None]) -> Any:
        if self._transport is None:
            raise EventDBXConnectionError("Transport is not available")
        ...
        response = self._schema.ControlResponse.from_bytes(response_bytes)
