...
class NoiseSession:
    ...
    def __init__(
        ...
        *,
        is_initiator: bool,
        protocol_name: str = DEFAULT_NOISE_PROTOCOL,
        prologue: bytes = DEFAULT_NOISE_PROLOGUE,
        local_static: bytes | None = None,
        remote_static: bytes | None = None,
    ) -> None:
        self._connection = NoiseConnection.from_name(protocol_name)
        if is_initiator:
        ...
        if local_static is not None:
            self._connection.set_keypair_from_private_bytes(Keypair.STATIC, local_static)
    ...
    def encrypt(self, plaintext: bytes, *, aad: bytes | None = None) -> bytes:
        ...
        return self._connection.encrypt(aad or b"", plaintext)
    ...
    def decrypt(self, ciphertext: bytes, *, aad: bytes | None = None) -> bytes:
        ...
        return self._connection.decrypt(aad or b"", ciphertext)
