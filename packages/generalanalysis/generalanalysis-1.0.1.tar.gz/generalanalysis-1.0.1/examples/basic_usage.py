...
def sync_example() -> None:
    ...
    if guards:
        guard = guards[0]
        result = client.guards.invoke(text="Hello world", guard=guard.name)
        print(f"Invoked {guard.name}, block={result.block}")
...
async def async_example() -> None:
    ...
    async with generalanalysis.AsyncClient() as client:
        ...
        if guards:
            guard = guards[0]
            result = await client.guards.invoke(text="Hello world", guard=guard.name)
