...
### Changed
- Removed support for invoking guards by numeric `guard_id`; callers must now supply a `guard` handle, `configuration_id`, or inline `configuration`.
- Renamed the `guard_name` parameter on `Client.guards.invoke()` (sync + async) to `guard`, matching the new selector semantics and updated all docs/examples accordingly.
