...
class TestSyncGuards:
    ...
    def test_invoke_with_guard_selector(self):
        mock_client = Mock()
        mock_client.post.return_value = _sample_invoke_response()
        guards_resource = Guards(mock_client)

        result = guards_resource.invoke(text="hello world", guard="@pii_guard")
    ...
    def test_invoke_requires_single_selector(self):
        ...
        with pytest.raises(ValueError, match="guard, configuration_id, or configuration"):
            guards_resource.invoke(text="hi", guard="@a", configuration_id=1)

    def test_invoke_requires_selector(self):
        guards_resource = Guards(Mock())
        with pytest.raises(ValueError, match="guard, configuration_id, or configuration"):
            guards_resource.invoke(text="hi")
    ...
    def test_invoke_guard_not_found_maps_exception(self):
        ...
        with pytest.raises(GuardNotFoundError):
            guards_resource.invoke(text="hi", guard="@missing")
...
class TestAsyncGuards:
    ...
    @pytest.mark.asyncio
    async def test_async_invoke_guard_selector(self):
        mock_client = AsyncMock()
        mock_client.post.return_value = _sample_invoke_response()
        guards_resource = AsyncGuards(mock_client)

        await guards_resource.invoke(text="hello", guard="@pii_guard")
    ...
    @pytest.mark.asyncio
    async def test_async_invoke_guard_not_found_maps_exception(self):
        mock_client = AsyncMock()
        mock_client.post.side_effect = GeneralAnalysisError("missing", status_code=404)
        guards_resource = AsyncGuards(mock_client)

        with pytest.raises(GuardNotFoundError):
            await guards_resource.invoke(text="oops", guard="@missing")

    @pytest.mark.asyncio
    async def test_async_invoke_requires_selector(self):
        guards_resource = AsyncGuards(AsyncMock())

        with pytest.raises(ValueError, match="guard, configuration_id, or configuration"):
            await guards_resource.invoke(text="oops")
