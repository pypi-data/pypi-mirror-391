...
class TestSyncGuards:
    def test_list_guards_with_project_scope(self):
        mock_client = Mock()
        mock_client.get.return_value = _sample_guard_response()
        guards_resource = Guards(mock_client, default_project_id=123)

        guards = guards_resource.list()

        assert len(guards) == 2
        mock_client.get.assert_called_once_with("/guards", params={"project_id": 123})

    def test_invoke_with_guard_name(self):
        mock_client = Mock()
        mock_client.post.return_value = _sample_invoke_response()
        guards_resource = Guards(mock_client, default_project_id=456)
    ...
    def test_invoke_with_configuration_id(self):
        ...
        guards_resource.invoke(text="hi", configuration_id=77, project_id=10)
    ...
    def test_invoke_with_guard_id(self):
        ...
        guards_resource.invoke(text="hello", guard_id=21, project_id=7)

        mock_client.get.assert_called_once_with("/guards", params={"project_id": 7})
        mock_client.post.assert_called_once_with(
            "/guards/invoke",
            data={
                "text": "hello",
    ...
    def test_list_logs_includes_guard_name_filter(self):
        ...
        guards_resource = Guards(mock_client, default_project_id=5)
        ...
        mock_client.get.assert_called_once_with(
            "/guards/logs",
            params={"page": 2, "page_size": 10, "project_id": 5, "guard_name": "@pii_guard"},
        )

    def test_list_policies(self):
        mock_client = Mock()
        mock_client.get.return_value = _sample_policies_response()
        guards_resource = Guards(mock_client, default_project_id=12)

        policies = guards_resource.list_policies()

        assert policies[0].name == "EMAIL_ADDRESS"
        mock_client.get.assert_called_once_with("/policies", params={"project_id": 12})
...
class TestAsyncGuards:
    ...
    async def test_async_list_guards(self):
        ...
        mock_client.get.return_value = _sample_guard_response()
        guards_resource = AsyncGuards(mock_client, default_project_id=99)

        guards = await guards_resource.list()

        assert guards[0].name == "@pii_guard"
        mock_client.get.assert_awaited_once_with("/guards", params={"project_id": 99})
    ...
    async def test_async_invoke_guard_name(self):
        ...
        await guards_resource.invoke(text="hello", guard_name="@pii_guard", project_id=12)

        mock_client.post.assert_awaited_once_with(
            "/guards/invoke",
            data={"text": "hello", "project_id": 12, "guard_name": "@pii_guard"},
        )
    ...
    async def test_async_list_policies(self):
        ...
        policies = await guards_resource.list_policies(project_id=44)

        assert policies[0].guard_count == 2
        mock_client.get.assert_awaited_once_with("/policies", params={"project_id": 44})
...
class TestGuardConfigurations:
    def test_list_guard_configurations(self):
        ...
        configs = configs_resource.list(project_id=9)

        assert configs[0].id == 7
        mock_client.get.assert_called_once_with("/guard-configs", params={"project_id": 9})
...
class TestAsyncGuardConfigurations:
    ...
    async def test_async_list_guard_configurations(self):
        ...
        configs_resource = AsyncGuardConfigurations(mock_client, default_project_id=10)

        configs = await configs_resource.list()

        assert configs[0].name == "override"
        mock_client.get.assert_awaited_once_with("/guard-configs", params={"project_id": 10})
