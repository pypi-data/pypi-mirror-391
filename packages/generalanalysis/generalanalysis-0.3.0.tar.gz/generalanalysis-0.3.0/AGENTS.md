...
## API Contract Notes
- Base URL defaults to the **dev** Guardrails API (`https://guardrails-api-dev.generalanalysis.com`). Allow overrides via constructor args or `GA_BASE_URL` when needed.
- Guards:
  - Listing: `GET /guards`
  - Invocation: `POST /guards/invoke` (supports `guard_name`, `configuration_id`, or inline `configuration`; enforce “exactly one selector” rule in resources and types).
...
- Always surface server errors via `GeneralAnalysisError`, `AuthenticationError`, or `GuardNotFoundError`. Wrap 404s from guard invocations as `GuardNotFoundError` when a selector was provided.
