...
class ReadinessValidator:
    ...
    def validate(self, max_level=3):
        ...
        try:
            ...
            if not level1_passed:
                print("\n[ERROR] Level 1 FAILED - Cannot proceed to higher levels")
                self._save_results()
                self._print_final_summary()
                return self.results
            
            print("[OK] Level 1 PASSED\n")
            ...
            if max_level >= 2:
                ...
                print(f"{'[OK] Level 2 PASSED' if self.results['levels']['level2']['passed'] else '[ERROR] Level 2 FAILED'}\n")
            ...
            if max_level >= 3:
                ...
                print(f"{'[OK] Level 3 PASSED' if self.results['levels']['level3']['passed'] else '[ERROR] Level 3 FAILED'}\n")
    ...
    def _level1_basic_validation(self):
        ...
        if not os.path.exists(self.node_file):
            ...
            print("    [ERROR] node.csv - NOT FOUND")
        else:
            print(f"    [OK] node.csv - Found")
            
        if not os.path.exists(self.link_file):
            self._add_issue('ERROR', 'link.csv not found', 'level1_file_existence')
            level_result['passed'] = False
            print("    [ERROR] link.csv - NOT FOUND")
        else:
            print(f"    [OK] link.csv - Found")
        ...
        try:
            ...
            if missing_node:
                ...
                level_result['passed'] = False
                print(f"    [ERROR] node.csv - Missing fields: {', '.join(missing_node)}")
            else:
                print(f"    [OK] node.csv - All required fields present")
                ...
                if not pd.api.types.is_integer_dtype(self.nodes_df['node_id']):
                    ...
                    print(f"    [WARNING] node.csv - node_id is not integer type")
                
        except Exception as e:
            self._add_issue('ERROR', f'Cannot read node.csv: {str(e)}', 'level1_file_read')
            level_result['passed'] = False
            print(f"    [ERROR] node.csv - Read error: {str(e)}")
        ...
        try:
            ...
            if missing_link:
                ...
                level_result['passed'] = False
                print(f"    [ERROR] link.csv - Missing fields: {', '.join(missing_link)}")
            else:
                print(f"    [OK] link.csv - All required fields present")
                ...
                for field in required_link_fields:
                    if not pd.api.types.is_integer_dtype(self.links_df[field]):
                        ...
                        print(f"    [WARNING] link.csv - {field} is not integer type")
                
        except Exception as e:
            self._add_issue('ERROR', f'Cannot read link.csv: {str(e)}', 'level1_file_read')
            level_result['passed'] = False
            print(f"    [ERROR] link.csv - Read error: {str(e)}")
        ...
        if not self.nodes_df['node_id'].is_monotonic_increasing:
            ...
            print("    [WARNING] node.csv - Not sorted by node_id")
        else:
            print("    [OK] node.csv - Sorted by node_id")
        ...
        if 'from_node_id' in self.links_df.columns and 'to_node_id' in self.links_df.columns:
            ...
            if not links_sorted:
                ...
                print("    [WARNING] link.csv - Not sorted by from_node_id")
            else:
                print("    [OK] link.csv - Sorted by from_node_id")
        ...
        try:
            ...
            level_result['passed'] = False
            print(f"    [ERROR] Node ID type conversion failed: {str(e)}")
            self.results['levels']['level1'] = level_result
            return False
        ...
        if invalid_from:
            ...
            level_result['passed'] = False
            print(f"    [ERROR] {len(invalid_from)} invalid from_node_id values")
        else:
            print("    [OK] All from_node_id values valid")
        ...
        if invalid_to:
            ...
            self._add_issue('ERROR', 
                          ...
                          'level1_link_endpoints')
            level_result['passed'] = False
            print(f"    [ERROR] {len(invalid_to)} invalid to_node_id values")
        else:
            print("    [OK] All to_node_id values valid")
    ...
    def _level2_zone_validation(self):
        ...
        if 'zone_id' not in self.nodes_df.columns:
            ...
            level_result['passed'] = False
            print("    [ERROR] node.csv - No zone_id column found")
        else:
            ...
            if len(centroids) == 0:
                ...
                level_result['passed'] = False
                print("    [ERROR] node.csv - No centroid nodes found")
            else:
                print(f"    [OK] node.csv - Found {len(centroids)} centroid nodes")
                ...
                if len(zone_nodes) > 0:
                    ...
                    if centroids_at_top < len(centroids):
                        ...
                        print(f"    [WARNING] node.csv - Centroids not grouped at top")
                    else:
                        print(f"    [OK] node.csv - Centroids properly grouped at top")
        
        # Check 2.2: Connector Links
        print("  Check 2.2: Connector link validation...")
        
        if 'zone_id' not in self.nodes_df.columns:
            print("    [ERROR] Skipping connector check - no zone_id in node.csv")
        else:
            centroids = self.nodes_df[self.nodes_df['node_id'] == self.nodes_df['zone_id']]
            centroid_ids = set(centroids['node_id'].values)
            
            if len(centroid_ids) == 0:
                print("    [ERROR] Skipping connector check - no centroids found")
            else:
                ...
                if len(connector_links) == 0:
                    ...
                    level_result['passed'] = False
                    print(f"    [ERROR] link.csv - No connector links found")
                else:
                    print(f"    [OK] link.csv - Found {len(connector_links)} connector links")
    ...
    def _level3_attribute_validation(self):
        ...
        if has_metric:
            print("    [OK] Metric system: length (meters), free_speed (km/h)")
        else:
            missing = []
            ...
            level_result['passed'] = False
            print(f"    [ERROR] Metric system incomplete. Missing: {', '.join(missing)}")
        
        if has_imperial:
            print("    [OK] Imperial system: vdf_length_mi (miles), vdf_free_speed_mph (mph)")
        else:
            missing = []
            ...
            level_result['passed'] = False
            print(f"    [ERROR] Imperial system incomplete. Missing: {', '.join(missing)}")
        ...
        if has_metric:
            ...
            if len(lengths) == 0:
                ...
                level_result['passed'] = False
                print("    [ERROR] length has no values")
            else:
                min_len = lengths.min()
                max_len = lengths.max()
                ...
                if min_len < 0:
                    ...
                    level_result['passed'] = False
                    print(f"    [ERROR] length has negative values")
                elif max_len > 50000:
                    self._add_issue('ERROR', f'length in link.csv unrealistic (max={max_len:.1f}m)', 'level3_metric')
                    level_result['passed'] = False
                    print(f"    [ERROR] length unrealistic (max={max_len:.1f}m)")
                else:
                    print(f"    [OK] length (meters): {min_len:.1f} - {max_len:.1f}")
            
            if len(speeds) == 0:
                self._add_issue('ERROR', 'free_speed column in link.csv has no valid values', 'level3_metric')
                level_result['passed'] = False
                print("    [ERROR] free_speed has no values")
            else:
                ...
                if max_speed > 200:
                    ...
                    level_result['passed'] = False
                    print(f"    [ERROR] free_speed unrealistic (max={max_speed:.1f} km/h)")
                elif max_speed < 5:
                    self._add_issue('ERROR', f'free_speed in link.csv too low (max={max_speed:.1f} km/h)', 'level3_metric')
                    level_result['passed'] = False
                    print(f"    [ERROR] free_speed too low (max={max_speed:.1f} km/h)")
                else:
                    print(f"    [OK] free_speed (km/h): {min_speed:.1f} - {max_speed:.1f}")
        ...
        if has_imperial:
            ...
            if len(lengths_mi) == 0:
                ...
                level_result['passed'] = False
                print("    [ERROR] vdf_length_mi has no values")
            else:
                ...
                if min_len < 0:
                    ...
                    level_result['passed'] = False
                    print(f"    [ERROR] vdf_length_mi has negative values")
                elif max_len > 50:
                    self._add_issue('ERROR', f'vdf_length_mi in link.csv unrealistic (max={max_len:.1f} miles)', 'level3_imperial')
                    level_result['passed'] = False
                    print(f"    [ERROR] vdf_length_mi unrealistic (max={max_len:.1f} miles)")
                else:
                    print(f"    [OK] vdf_length_mi (miles): {min_len:.3f} - {max_len:.1f}")
            
            if len(speeds_mph) == 0:
                self._add_issue('ERROR', 'vdf_free_speed_mph column in link.csv has no valid values', 'level3_imperial')
                level_result['passed'] = False
                print("    [ERROR] vdf_free_speed_mph has no values")
            else:
                ...
                if max_speed > 120:
                    ...
                    level_result['passed'] = False
                    print(f"    [ERROR] vdf_free_speed_mph unrealistic (max={max_speed:.1f} mph)")
                elif max_speed < 5:
                    self._add_issue('ERROR', f'vdf_free_speed_mph in link.csv too low (max={max_speed:.1f} mph)', 'level3_imperial')
                    level_result['passed'] = False
                    print(f"    [ERROR] vdf_free_speed_mph too low (max={max_speed:.1f} mph)")
                else:
                    print(f"    [OK] vdf_free_speed_mph (mph): {min_speed:.1f} - {max_speed:.1f}")
            ...
            if len(valid_data) > 0:
                ...
                if ratio_error > 0.05:  # More than 5% error
                    self._add_issue('WARNING', 
                                  ...
                                  'level3_conversion')
                    print(f"    [WARNING] Length conversion: ratio={avg_ratio:.1f} m/mi (expected ~1609.3)")
                else:
                    print(f"    [OK] Length conversion correct: {avg_ratio:.1f} m/mi")
            ...
            if len(valid_speed) > 0:
                ...
                if ratio_error > 0.05:
                    self._add_issue('WARNING', 
                                  ...
                                  'level3_conversion')
                    print(f"    [WARNING] Speed conversion: ratio={avg_ratio:.2f} (expected ~1.609)")
                else:
                    print(f"    [OK] Speed conversion correct: {avg_ratio:.2f}")
        ...
        if 'capacity' not in self.links_df.columns:
            ...
            print("    [WARNING] capacity column not found")
        else:
            ...
            if len(capacities) == 0:
                ...
                print("    [WARNING] capacity has no values")
            else:
                ...
                if max_cap > 10000:
                    self._add_issue('WARNING', 
                                  ...
                                  'level3_capacity')
                    print(f"    [WARNING] capacity very high (max={max_cap:.0f})")
                elif min_cap < 0:
                    self._add_issue('ERROR', 'capacity in link.csv contains negative values', 'level3_capacity')
                    level_result['passed'] = False
                    print(f"    [ERROR] capacity has negative values")
                elif min_cap < 100:
                    self._add_issue('WARNING', f'capacity in link.csv low (min={min_cap:.0f})', 'level3_capacity')
                    print(f"    [WARNING] capacity low (min={min_cap:.0f})")
                else:
                    print(f"    [OK] capacity: {min_cap:.0f} - {max_cap:.0f}")
                    ...
                    if len(links_with_lanes) > 0:
                        ...
                        if avg_cap < 800 or avg_cap > 3000:
                            self._add_issue('WARNING', 
                                          ...
                                          'level3_capacity')
                            print(f"    [WARNING] capacity/lane unusual ({avg_cap:.0f} veh/hr)")
                        else:
                            print(f"    [OK] capacity/lane: {avg_cap:.0f} veh/hr")
    ...
    def _print_final_summary(self):
        ...
        if self.results['summary']['errors'] == 0 and self.results['summary']['warnings'] == 0:
            print("[OK] NETWORK VALIDATION CHECK PASSED")
            ...
            print("[WARNING] NETWORK VALIDATION CHECK PASSED WITH WARNINGS")
            ...
            print("[ERROR] NETWORK VALIDATION CHECK FAILED")
