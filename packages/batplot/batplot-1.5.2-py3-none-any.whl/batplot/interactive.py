...
def _display_color_cube(color_code: str) -> str:
    """Display a colored cube (█) with ANSI color codes.
    
    Args:
        color_code: Color name or hex code
        
    Returns:
        String with colored cube or empty if color invalid
    """
    try:
...
def _display_palette_preview(palette_name: str, n_colors: int = 8) -> str:
    """Display a palette preview with colored cubes.
    
    Args:
        palette_name: Name of the colormap
        n_colors: Number of color samples to show
        
    Returns:
        String with colored cubes representing the palette
    """
    try:
        ...
        for color in colors:
            ...
            cubes.append(f"\033[38;2;{r};{g};{b}m█\033[0m")
        return "".join(cubes)
    except (ValueError, AttributeError):
        # Try cmcrameri for batlow palettes
        if palette_name.lower().startswith("batlow"):
            try:
                import importlib
                cmc = importlib.import_module('cmcrameri.cm')
                cmap = getattr(cmc, palette_name.replace('_r', ''))
        ...
        if key == 'q':
            ...
            try:
                ...
                while True:
                    ...
                    print(f"  {colorize_menu('u : user-defined color list (manage/use your custom colors)')}")
                    ...
                    sub = input(colorize_prompt("Choose (m/p/s/u/t/q): ")).strip().lower()
                    if sub == 'q':
                        break
                    if sub == '':
                        continue
                    if sub == 'm':
                        from .config import get_user_colors
                        ...
                        if user_colors:
                            ...
                            for idx, color in enumerate(user_colors):
                                cube = _display_color_cube(color)
                                print(f"  {cube} {idx+1}: {color}")
                            print(colorize_inline_commands("\nUse numbers (e.g., 1:4) or codes (e.g., 1:red 2:#00B006)"))
                        
                        color_input = input("Enter colors (e.g., 1:red 2:4 3:#00B006) or q: ").strip()
                        if not color_input or color_input.lower() == 'q':
                            ...
                            for entry in entries:
                                ...
                                try:
                                    i = int(idx_str) - 1
                                    if 0 <= i < len(ax.lines):
                                        # Check if color is a number (user color index)
                                        try:
                                            color_idx = int(color) - 1
                                            if 0 <= color_idx < len(user_colors):
                                                ax.lines[i].set_color(user_colors[color_idx])
                                            else:
                                                print(f"User color index out of range: {color}")
                                        except ValueError:
                                            # Not a number, treat as color code
                        ...
                        if user_colors:
                            ...
                            for idx, color in enumerate(user_colors):
                                cube = _display_color_cube(color)
                                print(f"  {cube} {idx+1}: {color}")
                            print(colorize_inline_commands("\nUse numbers (e.g., w:2 a:1) or codes (e.g., w:red a:#4561F7)"))
                        else:
                            print(colorize_inline_commands("Example: w:red a:#4561F7 s:blue d:green"))
                        
                        line = input("Enter mappings (e.g., w:red or w:2) or q: ").strip()
                            ...
                            for token in tokens:
                                ...
                                try:
                                    ...
                                    try:
                                        color_idx = int(color) - 1
                                        if 0 <= color_idx < len(user_colors):
                                            actual_color = user_colors[color_idx]
                                        else:
                                            print(f"User color index out of range: {color}")
                                            continue
                                    except ValueError:
                                    ...
                                    # Set spine color
                                    ax.spines[spine_name].set_edgecolor(actual_color)
                                    # Set tick colors and axis label color for this axis
                                    if spine_name in ('top', 'bottom'):
                                        ax.tick_params(axis='x', which='both', colors=actual_color)
                                        ax.xaxis.label.set_color(actual_color)
                                    else:  # left or right
                                        ax.tick_params(axis='y', which='both', colors=actual_color)
                                        ax.yaxis.label.set_color(actual_color)
                                    print(f"Set {spine_name} spine to {actual_color}")
                                except Exception as e:
                                    print(f"Error setting {spine_name} color: {e}")
                            fig.canvas.draw()
                    elif sub == 'u':
                        # User-defined color list management
                        from .config import get_user_colors, save_user_colors
                        
                        while True:
                            user_colors = get_user_colors()
                            print("\033[1mUser Color List:\033[0m")
                            if user_colors:
                                for idx, color in enumerate(user_colors):
                                    cube = _display_color_cube(color)
                                    print(f"  {cube} {idx+1}: {color}")
                            else:
                                print("  (empty - use 'a' to add colors)")
                            print(f"\n{colorize_menu('a : add colors to list (e.g., red #00B006 blue)')}")
                            print(f"{colorize_menu('d : delete color by number (e.g., 1 or 1,3,5)')}")
                            print(f"{colorize_menu('c : clear all colors')}")
                            print(f"{colorize_menu('q : return to color menu')}")
                            ucmd = input(colorize_prompt("Choose (a/d/c/q): ")).strip().lower()
                            ...
                            if ucmd == 'q':
                                ...
                                line = input("Enter colors (space-separated, e.g., red #00B006 blue) or q: ").strip()
                                if line and line.lower() != 'q':
                                    new_colors = line.split()
                                    user_colors.extend(new_colors)
                                    save_user_colors(user_colors)
                                    print(f"✓ Added {len(new_colors)} color(s). Total: {len(user_colors)}")
                            elif ucmd == 'd':
                                if not user_colors:
                                    print("No colors to delete.")
                                    continue
                                line = input("Enter number(s) to delete (e.g., 1 or 1,3,5) or q: ").strip()
                                if line and line.lower() != 'q':
                                    ...
                                    for tok in tokens:
                                        try:
                                            idx = int(tok) - 1
                                            if 0 <= idx < len(user_colors):
                                                indices_to_delete.add(idx)
                                            else:
                                                print(f"Index out of range: {tok}")
                                        except ValueError:
                                            print(f"Invalid number: {tok}")
                                    if indices_to_delete:
                                        new_list = [c for i, c in enumerate(user_colors) if i not in indices_to_delete]
                                        save_user_colors(new_list)
                                        print(f"✓ Deleted {len(indices_to_delete)} color(s). Remaining: {len(new_list)}")
                            elif ucmd == 'c':
                                confirm = input("Clear all user colors? (y/n): ").strip().lower()
                                if confirm == 'y':
                                    save_user_colors([])
                                    print("✓ Cleared all user colors.")
                        ...
                        preview_viridis = _display_palette_preview('viridis')
                        preview_plasma = _display_palette_preview('plasma')
                        preview_inferno = _display_palette_preview('inferno')
                        preview_magma = _display_palette_preview('magma')
                        preview_batlow = _display_palette_preview('batlow')
                        print(f"  {preview_viridis} 1. viridis    - Blue→yellow, colorblind-friendly")
                        print(f"  {preview_plasma} 2. plasma     - Purple→yellow, high contrast")
                        print(f"  {preview_inferno} 3. inferno    - Black→yellow, good for dark backgrounds")
                        print(f"  {preview_magma} 4. magma      - Black→white, excellent for grayscale")
                        print(f"  {preview_batlow} 5. batlow     - Scientific multi-hue palette")
                        if extras:
                            extra_previews = []
                            for pal in extras:
                                preview = _display_palette_preview(pal)
                                extra_previews.append(f"{preview} {pal}")
                            print("\nOther available: " + ", ".join(extra_previews))
