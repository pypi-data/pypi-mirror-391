...
def read_ec_csv_file(fname: str, prefer_specific: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    # --- Cycle numbering ---
    # If CSV has a 'Cycle Index' column, use those values directly (they are already read above).
    # Otherwise infer cycles by pairing alternating charge/discharge runs from voltage trend.
    if cyc_idx is None:
    ...
    # else: cycles array already contains values read from 'Cycle Index' column above (line 1026)
