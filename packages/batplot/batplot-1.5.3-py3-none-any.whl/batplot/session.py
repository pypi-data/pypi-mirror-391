...
def _atomic_pickle_dump(obj, filepath: str) -> None:
    """Atomically write a pickle file to prevent data loss on save failure.
    ...
    fd, temp_path = tempfile.mkstemp(suffix='.tmp', prefix='.batplot_', dir=target_dir)
    
    try:
        # Write pickle to temporary file
        with os.fdopen(fd, 'wb') as f:
            pickle.dump(obj, f)
        
        # Atomic rename (on POSIX) or best-effort on Windows
        # On POSIX: os.rename is atomic if source and dest are on same filesystem
        # On Windows: may fail if target exists, but we handle it
        try:
            # Try atomic rename first (works on POSIX, may fail on Windows)
            os.rename(temp_path, filepath)
        except OSError:
            # Windows fallback: remove target, then rename
            # Small race condition window, but better than non-atomic write
...
def dump_session(
    ...
    *,
    ...
    try:
        ...
        # Use atomic write to prevent data loss on save failure
        _atomic_pickle_dump(sess, target)
...
def dump_cpc_session(
    ...
    *,
    ...
    try:
        ...
        # Use atomic write to prevent data loss on save failure
        _atomic_pickle_dump(meta, target)
