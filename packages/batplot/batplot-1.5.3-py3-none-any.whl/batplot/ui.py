...
def resize_plot_frame(fig, ax, y_data_list: List, label_text_objects: List, args, update_labels_func):
    while True:
        try:
            ...
            # Check if labels would clip, but DON'T auto-adjust (respect user's explicit size choice)
            labels_will_clip = ensure_text_visibility(fig, ax, label_text_objects, check_only=True)
            ...
            # Warn if labels are clipping
            if labels_will_clip:
                print("Warning: Some axis labels may be clipped at current size. Consider using a smaller plot frame or larger canvas.")
        except KeyboardInterrupt:
            print("Canceled.")
            return
