...
def batplot_main() -> int:
    ...
    if len(args.files) == 1:
        ...
        if sole.lower() == 'all':
            ...
            for f in sorted(os.listdir(target_dir), key=_natural_sort_key):
            ...
            exit()
    elif len(args.files) == 2 and args.files[0].lower() == 'allfiles':
        # Handle: batplot allfiles /path/to/directory
        target_dir = args.files[1]
        ...
        print(f"Found {len(all_xy_files)} files in {target_dir}")
        args.files = all_xy_files
        # Continue to normal plotting mode with all files
