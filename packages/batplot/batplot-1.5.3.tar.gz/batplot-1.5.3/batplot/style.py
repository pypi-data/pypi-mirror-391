...
def _color_to_hex(color):
    """Convert matplotlib color (RGBA tuple, array, or string) to hex string for JSON serialization."""
    try:
        if color is None:
            return None
...
def print_style_info(
    ...
    for name, spn in ax.spines.items():
        spine_color = _color_to_hex(spn.get_edgecolor())
        print(
            f"  {name:<5} lw={spn.get_linewidth()} color={spine_color} visible={spn.get_visible()}"
        )
    ...
    for i, ln in enumerate(ax.lines):
        ...
        # Convert color to hex for cleaner output
        col_hex = _color_to_hex(col) if col is not None else col
        print(f"  {i+1:02d}: label='{base_label}' color={col_hex} lw={lw} ls={ls} marker={mk} ms={ms} alpha={a}{offset_str}")
        ...
        cfg = {
            ...
            "spines": {
                name: {
                    ...
                    "color": _color_to_hex(spn.get_edgecolor()),
                    "visible": spine_vis.get(name, True),
                }
                for name, spn in ax.spines.items()
            },
            "tick_colors": {
                "x": _color_to_hex(ax.xaxis.get_tick_params().get('color', 'black')) if ax.xaxis.get_tick_params() else 'black',
                "y": _color_to_hex(ax.yaxis.get_tick_params().get('color', 'black')) if ax.yaxis.get_tick_params() else 'black',
            },
            "axis_label_colors": {
...
def apply_style_config(
    ...
    try:
        ...
        try:
            ...
            # Always use frame_size if available, as it's the absolute inches value
            # axes_fraction is dependent on the figure size at save time and may not match
            if frame_size and isinstance(frame_size, (list, tuple)) and len(frame_size) == 2:
                ...
                # Fallback to axes_fraction only if frame_size not available
