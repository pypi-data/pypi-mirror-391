...
def electrochem_interactive_menu(fig, ax, cycle_lines: Dict[int, Dict[str, Optional[object]]], file_path=None):
    ...
    while True:
        ...
        if key == 'q':
            ...
            # Show user colors if available
            if user_colors:
                print("\nYour saved colors:")
                for idx, color in enumerate(user_colors):
                    cube = _display_color_cube(color)
                    print(f"  {cube} {idx+1}: {color}")
                print(_colorize_inline_commands("Use numbers to apply: e.g. 1:2 5:3 (cycle 1→color 2, cycle 5→color 3)"))
            
            print("\nEnter one of:")
            print(_colorize_inline_commands("  - numbers: e.g. 1 5 10"))
            print(_colorize_inline_commands("  - mappings: e.g. 1:red 5:#00B006 10:blue 2:4 (4=user color #4)"))
            ...
            print(_colorize_inline_commands("  - u : manage user color list"))
            
            print("\nRecommended palettes for scientific publications:")
            preview_tab10 = _display_palette_preview('tab10')
            preview_set2 = _display_palette_preview('Set2')
            preview_dark2 = _display_palette_preview('Dark2')
            preview_viridis = _display_palette_preview('viridis')
            preview_plasma = _display_palette_preview('plasma')
            print(f"  {preview_tab10} 1. tab10      - Distinct, colorblind-friendly (default matplotlib)")
            print(f"  {preview_set2} 2. Set2       - Soft, pastel colors for presentations")
            print(f"  {preview_dark2} 3. Dark2      - Bold, saturated colors for print")
            print(f"  {preview_viridis} 4. viridis    - Perceptually uniform (blue→yellow)")
            print(f"  {preview_plasma} 5. plasma     - Perceptually uniform (purple→yellow)")
            ...
            # Check if user wants to manage color list
            if line.lower() == 'u':
                while True:
                    user_colors = get_user_colors()
                    ...
                    print(f"\n{_colorize_menu('a : add colors to list (e.g., red #00B006 blue)')}")
                    print(f"{_colorize_menu('d : delete color by number (e.g., 1 or 1,3,5)')}")
                    print(f"{_colorize_menu('c : clear all colors')}")
                    print(f"{_colorize_menu('q : return to cycles/colors menu')}")
                    ucmd = input(_colorize_prompt("Choose (a/d/c/q): ")).strip().lower()
                    
                    if ucmd == 'q':
                        break
                    elif ucmd == 'a':
                        uline = input("Enter colors (space-separated, e.g., red #00B006 blue) or q: ").strip()
                        if uline and uline.lower() != 'q':
                            new_colors = uline.split()
                        ...
                        if not user_colors:
                            ...
                            continue
                        uline = input("Enter number(s) to delete (e.g., 1 or 1,3,5) or q: ").strip()
                        if uline and uline.lower() != 'q':
                            indices_to_delete = set()
                            tokens = uline.replace(',', ' ').split()
                            for tok in tokens:
                                try:
                                    idx = int(tok) - 1
            ...
            # Resolve user color indices in mapping
            if mode == 'map' and mapping:
                resolved_mapping = {}
                for cyc, color in mapping.items():
                    # Check if color is a number (user color index)
                    try:
                        color_idx = int(color) - 1
                        if 0 <= color_idx < len(user_colors):
                            resolved_mapping[cyc] = user_colors[color_idx]
                        else:
                            print(f"User color index {color} out of range, using as-is")
                            resolved_mapping[cyc] = color
                    except ValueError:
                        # Not a number, use as color code
                        resolved_mapping[cyc] = color
        ...
        for cyc, role, ln in _iter_cycle_lines(cycle_lines):
            try:
                curve_marker_props = {
                    ...
                    'markerfacecolor': to_hex(ln.get_markerfacecolor()) if ln.get_markerfacecolor() else None,
                    'markeredgecolor': to_hex(ln.get_markeredgecolor()) if ln.get_markeredgecolor() else None
                }
                break
            except Exception:
                pass
