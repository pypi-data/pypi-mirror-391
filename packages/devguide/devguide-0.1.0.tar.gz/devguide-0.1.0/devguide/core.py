...
def handle_recommendation(repo: Repository, desc: str):
...
def handle_start(repo: Repository, desc: str = None):
    ...
    if not desc:
        try:
            ...
            print("\nOperação cancelada.")
            sys.exit(0)
    handle_recommendation(repo, desc)

def handle_list(repo: Repository):
