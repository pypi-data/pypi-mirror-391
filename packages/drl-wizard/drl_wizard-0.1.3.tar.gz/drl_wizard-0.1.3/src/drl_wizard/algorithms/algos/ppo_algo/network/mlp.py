...
class MLPLayer(nn.Module):
    """Multi-Layer Perceptron (MLP) neural network layer implementation.

    A configurable MLP layer that can process input features through multiple fully connected
    layers with customizable activation functions and normalization options.

    Args:
        input_shape (tuple): Shape of the input tensor (input_dim,)
        output_dim (int): Dimension of the output
        cfg (PPOConfig): Configuration object containing network parameters
        norm_last_layer (bool): Whether to apply layer normalization to the output layer

    Features:
        - Configurable number of hidden layers
        - Choice between ReLU and Tanh activation functions
        - Optional feature normalization
        - Orthogonal or Xavier weight initialization
        - Layer normalization after each hidden layer
    """

    def __init__(self, input_shape, output_dim, cfg: PPOConfig, norm_last_layer):
