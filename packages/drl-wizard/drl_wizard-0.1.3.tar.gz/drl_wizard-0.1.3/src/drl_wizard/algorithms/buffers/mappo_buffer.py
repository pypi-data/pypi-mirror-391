import torch
import numpy as np
from drl_wizard.configs import AppConfig, PPOConfig
...
class PPOBuffer:
    def __init__(self, cfg: AppConfig, obs_space, act_space, shared_obs_space=None, action_masking=False):
        if cfg.algo_cfg.algo_id != "ppo":
            raise ValueError("PPOBuffer only supports PPO")
        self.cfg: PPOConfig = cfg.algo_cfg
    ...
    def feed_forward_generator(self):
        ...
        for indices in sampler:
            obs=obs[indices]
            shared_obs=shared_obs[indices] if shared_obs is not None else None
            actions=actions[indices]
            masked_actions=masked_actions[indices] if masked_actions is not None else None
            returns=returns[indices]
            value_predicts=value_predicts[indices]
            advantages=advantages[indices]
            log_probs=log_probs[indices]
            yield obs,shared_obs,actions,masked_actions,returns,value_predicts,advantages,log_probs
