...
PLANNER_SYSTEM = SystemMessage(
    content=(
        ...
        "- tool_library(...) to create, update, run, or list persistent custom tools\n"
        "- research_vault(...) to persist, retrieve, or append research notes across agent runs\n"
        "- self_improve(...) to analyze results, iterate on solutions, store learnings, and improve over multiple attempts.\n"
...
EXECUTOR_SYSTEM = SystemMessage(
    content=(
        ...
        "Available tools:\n"
        "- tavily_search/query for research, tavily_extract(url) for page content once you have a URL\n"
        "- get_weather(location, units) for conditions\n"
        "- run_python(code, timeout) to execute multi-step Python code\n"
        ...
        "- save_shell_automation(...) to persist and replay scripts\n"
        "- headless_browse(...) for Playwright browser automation (can create accounts, fill forms, click buttons, etc.)\n"
        "- tool_library(...) to create/update/run custom reusable tools\n"
        "- research_vault(...) to store/retrieve persistent notes across runs\n"
        "- self_improve(...) to analyze results, iterate on failed attempts, store learnings, and improve over multiple tries\n"
