...
def setup(parser: argparse.ArgumentParser) -> DistributedObject:
    ...
    if config["type"] is State.PREDICTION_HF:
        ...
        model_HF = Model_HF(config["config"])
...
def get_available_models_on_hf_repo(repo_id: str) -> dict[str, str]:
    api = HfApi()
    model_names = []
    try:
...
class Model_HF():
    ...
    def _read_metadata(self) -> dict[str, str]:
        metadata_file_path = hf_hub_download(
                ...
                filename=f"{self.model_name}/metadata.json",
                repo_type="model",
                revision="main"
            )
    ...
    def set_number_of_augmentation(self, inference_file_path: str, new_value: int) -> None:
        new_value = np.clip(new_value, 0, self._maximum_tta)
        if new_value > 0:
            yaml = YAML()
    ...
    def download(self, number_of_model: int) -> tuple[list[str], str, str]:
        api = HfApi()
        models_path = []
        codes_path = []
    ...
    def is_mc_dropout(self):
        return self._is_mc_dropout
