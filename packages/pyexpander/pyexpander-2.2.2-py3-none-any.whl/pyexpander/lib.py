...
def parseString(st):
    ...
    >>> pprint(parseString("ab $(1+2)"))
    [ParsedLiteral(IndexedString('ab $(1+2)'), 0, 2, 0, 2),
     ParsedEval(IndexedString('ab $(1+2)'), 5, 7, 3, 8)]
    """
...
class Block:
    ...
    def _strlist(self):
        ...
        self._append(lst, "permissive_mode")
    ...
    def __init__(self,
        ...
        Properties of a Block:
            ...
            safe_mode     -- if True, allow only "safe" commands that have no
                             ...
                             $extend() are disabled.
            permissive_mode --
                             Ignore when an expression in $(EXPRESSION) has an
        ...
        if previous is None:
            ...
            self.permissive_mode= previous.permissive_mode
    ...
    def eval_(self, st):
        ...
        try:
            ...
            raise e.__class__("%s at %s" % (str(e), self.posmsg())) from None
    ...
    def set_permissivemode(self, value):
        """sets the permissive_mode."""
        self.permissive_mode= value
...
def processToList(parse_list, filename=None,
    ...
    args:
        ...
        auto_continuation (bool): If True, remove newline at the end of lines
            ...
            line with a command.
        safemode: Disable the execution of arbitrary python commands and
            commands that extend the pyexpander language.
        permissive: Ignore when an expression in $(EXPRESSION) has an undefined
            variable error (NameError). Such expressions are not replaced.
        preserve_backslashes: Preserve all backslashes except the ones used for
            line continuation.
        ...
        if isinstance(tp, EP.ParsedLiteral):
            ...
            if not block.skip:
                ...
                if st_=="\\\\":
                    ...
                    if not preserve_backslashes:
                        st_="$"
                # if current column is 1, we must do an initial indent, needed
                # for macro blocks:
        ...
        if isinstance(tp, EP.ParsedVar):
            ...
            if not block.skip:
                try:
                    ...
                    if block.permissive_mode:
                        result.append(tp.complete_string())
                    else:
                        raise
            continue
...
def processToPrint(parse_list, filename=None,
                   ...
                   print_mode="full"):
    r"""Gets a parse list, expand the text in it and print it.
    ...
    - auto_continuation (bool): If True, remove newline at the end of lines
      with a command. This works like having an '\' at the end of each
      line with a command.
    - safemode: Disable the execution of arbitrary python commands and commands
      that extend the pyexpander language.
    - permissive: Ignore when an expression in $(EXPRESSION) has an undefined
      variable error (NameError). Such expressions are not replaced.
    - preserve_backslashes: Preserve all backslashes except the ones used for
      line continuation.
...
def expandToStr(st, filename=None,
                ...
                auto_indent= False,
                include_paths=None):
    r"""Get a string, expand the text in it and return it as a string.
    ...
    # pylint: disable=too-many-arguments
    # pylint: disable=too-many-positional-arguments
    (result,exp_globals,file_deps)= processToList(\
            parse_list= parseString(st),
            filename= filename,
            external_definitions= external_definitions,
            allow_nobracket_vars= allow_nobracket_vars,
...
def expandFile(filename,
               ...
               output_encoding=SYS_DEFAULT_ENCODING,
               print_mode="full"):
    f"""Get a filename, expand the text in it and print it.
    ...
    >>> (gd, deps)= expandFile(None, no_stdin_warning=True\
    )
