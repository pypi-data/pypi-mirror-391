...
rx_StringLiteralStart= re.compile(
        r'''(t|r|u|R|U|f|F|fr|Fr|fR|FR|rf|rF|Rf|RF|b|B|br|Br|bR|BR|rb|rB|Rb|RB|)("""|''' + \
        """'''""" + \
        r'''|'|")''')
...
class ParsedItem:
    ...
    def __init__(self, idxst, start, end, c_start, c_end):
        """Create the object.

        The parsed item is not stored as a string in the object, instead the
        object stores the start and end positions in idxst.

        parameters: 
        - idxst   : the string, type IndexedString
        - start   : start position in the string
        - end     : end position in the string
        - c_start : start position of the complete item in the string
        - c_end   : end position of the complete item in the string
    ...
    def complete_string(self):
        """return the string that represents the complete ParsedItem."""
        return self._idxst.st()[self._c_start:self._c_end+1]
    def start(self):
    ...
    def complete_positions(self):
        """return c_start and c_end of ParsedItem in the source string."""
        return (self._c_start, self._c_end)
    def __str__(self):
        return "(%s, %s, %s, %s, %s)" % (repr(self.__class__.__name__), \
               repr(self.positions()),
               repr(self.string()),
               repr(self.complete_positions()),
               repr(self.complete_string()))
    def __repr__(self):
        return "%s(%s, %s, %s, %s, %s)" % (self.__class__.__name__, \
                repr(self._idxst),
                repr(self._start), repr(self._end),
                repr(self._c_start), repr(self._c_end))
...
class ParsedLiteral(ParsedItem):
    ...
    def __init__(self, idxst, start, end, c_start, c_end):
        # pylint: disable=too-many-arguments
        # pylint: disable=too-many-positional-arguments
        ParsedItem.__init__(self, idxst, start, end, c_start, c_end)
...
class ParsedCommand(ParsedItem):
    ...
    """
    def __init__(self, idxst, start, end, c_start, c_end, ident):
        # pylint: disable=too-many-arguments
        # pylint: disable=too-many-positional-arguments
        ParsedItem.__init__(self, idxst, start, end, c_start, c_end)
        self.ident= ident
    def args(self):
        ...
        return self.string()
    def __str__(self):
        return "(%s, %s, %s, %s, %s, %s)" % (repr(self.__class__.__name__), \
               repr(self.positions()),
               repr(self.ident),
               repr(self.string()),
               repr(self.complete_positions()),
...
def parseDollar(idxst, pos):
    ...
    ...
    >>> test("$abc",0)
    Parsed: ('ParsedPureCommand', (1, 3), 'abc', (0, 3), '$abc')
    rest of string:
    >>> test("$abc%&/",0)
    Parsed: ('ParsedPureCommand', (1, 3), 'abc', (0, 3), '$abc')
    rest of string:%&/
    >>> test("$abc(2*3)",0)
    Parsed: ('ParsedCommand', (5, 7), 'abc', '2*3', (0, 8), '$abc(2*3)')
    rest of string:
    >>> test(" $abc(2*sin(x))",1)
    Parsed: ('ParsedCommand', (6, 13), 'abc', '2*sin(x)', (1, 14), '$abc(2*sin(x))')
    rest of string:
    ...
    >>> test("$(abc)",0)
    Parsed: ('ParsedVar', (2, 4), 'abc', (0, 5), '$(abc)')
    rest of string:
    >>> test("$(abc*2)",0)
    Parsed: ('ParsedEval', (2, 6), 'abc*2', (0, 7), '$(abc*2)')
    rest of string:
    >>> test(" $(2*x(y))abc",1)
    Parsed: ('ParsedEval', (3, 8), '2*x(y)', (1, 9), '$(2*x(y))')
    rest of string:abc
    >>> test(" $   (2*x(y))abc",1)
    Parsed: ('ParsedEval', (6, 11), '2*x(y)', (1, 12), '$   (2*x(y))')
    rest of string:abc
    """
    if not isinstance(idxst, IndexedString):
        raise TypeError("idxst par wrong: %s" % repr(idxst))
    st= idxst.st()
    dpos= pos # position of the '$' character
    if st[pos]!="$":
    ...
    if matched=="#":
        # an expander comment
        (a, b)= parseComment(idxst, pos)
        elm= ParsedComment(idxst, a+1, b-1, dpos, b-1)
        return (b, elm)
    if matched=="(":
        (a, b)= parseCode(idxst, pos)
        m_ident= rx_pyIdent.match(st, a+1, b-1)
        if m_ident is not None:
            elm= ParsedVar(idxst, a+1, b-2, dpos, b-1)
        else:
            elm= ParsedEval(idxst, a+1, b-2, dpos, b-1)
        return (b, elm)
    if matched=="{":
        # a purecommand enclosed in "{}" brackets
        (a, b)= parseBracketed(idxst, pos)
        elm= ParsedPureCommand(idxst, a+1, b-2, dpos, b-1)
        return (b, elm)
    ...
    if mb is not None:
        ...
        elm= ParsedCommand(idxst, a+1, b-2, dpos, b-1, matched)
        return (b, elm)
    elm= ParsedPureCommand(idxst, pos, m.end()-1, dpos, m.end()-1)
    return (m.end(), elm)
...
def parseBackslash(idxst, pos):
    ...
    >>> test(r"\abc",0)
    Parsed: ('ParsedLiteral', (0, 0), '\\', (0, 0), '\\')
    rest of string:'abc'
    >>> test("\\",0)
    Parsed: ('ParsedLiteral', (0, 0), '\\', (0, 0), '\\')
    rest of string:''
    >>> test("\\$py()",0)
    Parsed: ('ParsedLiteral', (0, 1), '\\$', (0, 1), '\\$')
    rest of string:'py()'
    >>> test("\\\\",0)
    Parsed: ('ParsedLiteral', (0, 1), '\\\\', (0, 1), '\\\\')
    rest of string:''
    ...
    if pos+1>=l_:
        # no more characters
        elm= ParsedLiteral(idxst, pos, pos, pos, pos)
        return (pos+1, elm)
    else:
        # at least one more character
        nextchar= st[pos+1]
        if nextchar=="\\":
            elm= ParsedLiteral(idxst, pos, pos+1, pos, pos+1)
            return (pos+2, elm)
        if nextchar=="$":
            elm= ParsedLiteral(idxst, pos, pos+1, pos, pos+1)
            return (pos+2, elm)
...
def parseAll(idxst, pos):
    ...
    >>> test("abc",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    >>> test("abc$xyz",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    ('ParsedPureCommand', (4, 6), 'xyz', (3, 6), '$xyz')
    >>> test("abc${xyz}efg",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    ('ParsedPureCommand', (5, 7), 'xyz', (3, 8), '${xyz}')
    ('ParsedLiteral', (9, 11), 'efg', (9, 11), 'efg')
    >>> test("abc$xyz(2*4)",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    ('ParsedCommand', (8, 10), 'xyz', '2*4', (3, 11), '$xyz(2*4)')
    >>> test("abc$(2*4)ab",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    ('ParsedEval', (5, 7), '2*4', (3, 8), '$(2*4)')
    ('ParsedLiteral', (9, 10), 'ab', (9, 10), 'ab')
    >>> test("abc\\$(2*4)ab",0)
    ('ParsedLiteral', (0, 2), 'abc', (0, 2), 'abc')
    ('ParsedLiteral', (3, 4), '\\$', (3, 4), '\\$')
    ('ParsedLiteral', (5, 11), '(2*4)ab', (5, 11), '(2*4)ab')
    >>> test("ab$func(1+2)\\\nnew line",0)
    ('ParsedLiteral', (0, 1), 'ab', (0, 1), 'ab')
    ('ParsedCommand', (8, 10), 'func', '1+2', (2, 11), '$func(1+2)')
    ('ParsedLiteral', (14, 21), 'new line', (14, 21), 'new line')
    >>> test("ab$func(1+2)\nnew line",0)
    ('ParsedLiteral', (0, 1), 'ab', (0, 1), 'ab')
    ('ParsedCommand', (8, 10), 'func', '1+2', (2, 11), '$func(1+2)')
    ('ParsedLiteral', (12, 20), '\nnew line', (12, 20), '\nnew line')
    >>> test("ab$(xyz)(56)",0)
    ('ParsedLiteral', (0, 1), 'ab', (0, 1), 'ab')
    ('ParsedVar', (4, 6), 'xyz', (2, 7), '$(xyz)')
    ('ParsedLiteral', (8, 11), '(56)', (8, 11), '(56)')
    >>> test(r'''
    ...
    ('ParsedVar', (27, 28), 'xy', (25, 29), '$(xy)')
    ('ParsedLiteral', (30, 49), '\nan escaped dollar: ', (30, 49), '\nan escaped dollar: ')
    ('ParsedLiteral', (50, 51), '\\$', (50, 51), '\\$')
    ('ParsedLiteral', (52, 83), '(xy)\na macro within letters: abc', (52, 83), '(xy)\na macro within letters: abc')
    ('ParsedPureCommand', (86, 87), 'xy', (84, 88), '${xy}')
    ('ParsedLiteral', (89, 124), 'def\na pyexpander command structure:\n', (89, 124), 'def\na pyexpander command structure:\n')
    ('ParsedCommand', (129, 131), 'if', 'a=1', (125, 132), '$if(a=1)')
    ('ParsedLiteral', (133, 138), '\nhere\n', (133, 138), '\nhere\n')
    ('ParsedPureCommand', (140, 143), 'else', (139, 143), '$else')
    ('ParsedLiteral', (144, 150), '\nthere\n', (144, 150), '\nthere\n')
    ('ParsedPureCommand', (152, 156), 'endif', (151, 156), '$endif')
    ...
    ('ParsedLiteral', (215, 238), 'now an escaped continued', (215, 238), 'now an escaped continued')
    ('ParsedLiteral', (239, 240), '\\\\', (239, 240), '\\\\')
    ...
    while True:
        ...
        if m is None:
            parselist.append(ParsedLiteral(idxst, pos, len(st)-1, pos, len(st)-1))
            return parselist
        if m.start()>pos:
            parselist.append(ParsedLiteral(idxst, pos, m.start()-1, pos, m.start()-1))
        if m.group(1)=="\\":
