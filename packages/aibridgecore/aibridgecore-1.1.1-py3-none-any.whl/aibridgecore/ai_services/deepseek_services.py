...
import time
import uuid
from aibridgecore.exceptions import DeepseekException, AIBridgeException, ValidationException
        ...
        try:
            ...
            if output_format:
                if len(output_format) != len(prompts_list):
                    raise ValidationException("length of output_format must be equal to length of the prompts")
            
            if format_strcture:
                if len(format_strcture) != len(prompts_list):
                    raise ValidationException("length of format_structure must be equal to length of the prompts")
        ...
        try:
            ...
            for index, prompt in enumerate(prompts):
                ...
                for res in response.choices:
                    content = (
                        res.message.content if res.message.content else res.message.function_call.arguments
