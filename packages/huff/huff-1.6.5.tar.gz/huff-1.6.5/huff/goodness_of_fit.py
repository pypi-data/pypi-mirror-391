...
def modelfit(
    ...
    if remove_nan:
        ...
        obs_exp = pd.DataFrame(
            {
                helper.DEFAULT_OBSERVED_COL: observed, 
                helper.DEFAULT_EXPECTED_COL: expected
                }
            )
        
        obs_exp_clean = obs_exp.dropna(subset=[helper.DEFAULT_OBSERVED_COL, helper.DEFAULT_EXPECTED_COL])
        ...
        observed = obs_exp_clean[helper.DEFAULT_OBSERVED_COL].to_numpy()
        expected = obs_exp_clean[helper.DEFAULT_EXPECTED_COL].to_numpy()
    ...
    data_residuals = pd.DataFrame({
        ...
        helper.APE_PREFIX: APE,
        f"s{helper.APE_PREFIX}": sAPE
        })
    ...
    for APE_value in range(helper.APE_MIN, helper.APE_MAX + 1):
        i = i+1
        APEs[f"{helper.APE_PREFIX}{APE_value}"] = float(len(data_residuals[data_residuals[helper.APE_PREFIX] < APE_value])/expected_no*perc_factor)

    data_lossfunctions = {
        helper.GOODNESS_OF_FIT["Sum of squared residuals"]: SQR,
        "SAR": SAR,
        "SQT": SQT,
        helper.GOODNESS_OF_FIT["R-squared"]: Rsq,
        helper.GOODNESS_OF_FIT["Mean squared error"]: MSE,
        helper.GOODNESS_OF_FIT["Root mean squared error"]: RMSE,
        helper.GOODNESS_OF_FIT["Mean absolute error"]: MAE,
        helper.GOODNESS_OF_FIT["Mean absolute percentage error"]: MAPE,
        helper.GOODNESS_OF_FIT["Symmetric MAPE"]: sMAPE,
        "LL": -LL,
        **APEs          
    }    
...
def modelfit_plot(
    ...
    all_values = np.concatenate([entry[0][helper.DEFAULT_OBSERVED_COL] for entry in modelfit_list] + [entry[0][helper.DEFAULT_EXPECTED_COL] for entry in modelfit_list])
    ...
    for i, entry in enumerate(modelfit_list):
        ...
        plt.scatter(
            entry[0][helper.DEFAULT_OBSERVED_COL],
            entry[0][helper.DEFAULT_EXPECTED_COL],
            color=color,
            alpha=points_alpha,
            label=label
        )
...
def modelfit_print(modelfit_results):
    ...
    maxlen = max(len(str(key)) for key in helper.GOODNESS_OF_FIT.keys())

    for gof_key, gof_value in helper.GOODNESS_OF_FIT.items():
                    
        if gof_key in helper.GOODNESS_OF_FIT.keys():                        
...
def modelfit_cat(
    ...
    data_residuals.loc[data_residuals[helper.DEFAULT_OBSERVED_COL] == data_residuals[helper.DEFAULT_EXPECTED_COL], "fit"] = 1
