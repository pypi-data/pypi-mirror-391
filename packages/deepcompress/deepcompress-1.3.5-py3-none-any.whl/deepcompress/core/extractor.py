...
class OCRExtractor:
    ...
    async def _extract_page(self, image: Any, page_number: int) -> Page:
        ...
        try:
            ...
            # Explicit generation parameters to ensure proper stopping
            generation_kwargs = {
                "max_new_tokens": self.config.ocr_max_new_tokens,
                "max_length": self.config.ocr_max_new_tokens + 100,  # Explicit max length
                ...
                "early_stopping": True,  # Force stop when EOS is reached
            ...
            while retry_count <= max_retries and result_text is None:
                try:
                    # Suppress warnings and ALL model outputs during inference
                    with warnings.catch_warnings():
                        warnings.filterwarnings("ignore")
                        
                        # Completely silence the model's outputs (stdout + stderr)
                        import sys
                        import os
                        ...
                        # Disable tqdm via environment variable
                        old_tqdm_disable = os.environ.get('TQDM_DISABLE')
                        os.environ['TQDM_DISABLE'] = '1'
                        
                        # Redirect both stdout and stderr to devnull with no buffering
                        devnull_stdout = open(os.devnull, 'w', buffering=1)
                        devnull_stderr = open(os.devnull, 'w', buffering=1)
                        sys.stdout = devnull_stdout
                        sys.stderr = devnull_stderr
                        
                        # Also suppress at the file descriptor level for C extensions
                        ...
                        try:
                            # Run inference with timeout to prevent infinite generation
                            timeout_seconds = self.config.ocr_inference_timeout
                            logger.debug(f"Page {page_number}: Running inference with {timeout_seconds}s timeout")
                                ...
                                result = await asyncio.wait_for(
                                    asyncio.get_event_loop().run_in_executor(
                                        ...
                                        lambda: self._model.infer(
                                            ...
                                            crop_mode=True,
                                            save_results=False,  # Disable file saving to reduce I/O
                                            test_compress=False,
                                            **generation_kwargs,
                                        )
                                    ),
                            ...
                            except (TypeError, asyncio.TimeoutError) as te:
                                if isinstance(te, asyncio.TimeoutError):
                                    logger.warning(f"Page {page_number}: Inference timed out after {timeout_seconds}s, trying fallback")
                                else:
                            ...
                            # Close the devnull streams
                            try:
                                if sys.stdout == devnull_stdout:
                                    devnull_stdout.close()
                                if sys.stderr == devnull_stderr:
                    ...
                    logger.error(f"Page {page_number} inference timed out after {timeout_seconds}s (attempt {retry_count + 1}/{max_retries + 1})")
                    retry_count += 1
                    
                    if retry_count <= max_retries:
                        logger.info(f"Retrying page {page_number} after timeout...")
                        await asyncio.sleep(1.0 * retry_count)
                    else:
                        raise OCRError(
                            f"OCR inference timed out for page {page_number} after {max_retries + 1} attempts",
                            details={
                                "page_number": page_number,
                                "timeout_seconds": timeout_seconds,
                                "attempts": retry_count,
                                "suggestion": "Try reducing ocr_max_new_tokens or increasing ocr_inference_timeout"
                            }
                        )
