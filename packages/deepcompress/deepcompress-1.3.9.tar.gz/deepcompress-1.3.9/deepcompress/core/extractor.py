...
class OCRExtractor:
    ...
    async def _extract_pages_in_batches(self, images: list[Any]) -> list[Page]:
        ...
        logger.info(f"Starting OCR extraction: {total_pages} pages, {total_batch_count} batches")
        ...
        try:
            ...
            for batch_idx, batch_start in enumerate(range(0, total_pages, batch_size)):
                ...
                logger.info(f"Processing batch {batch_num}/{total_batch_count} "
        ...
        try:
            ...
            while retry_count <= max_retries and result_text is None:
                try:
                    ...
                    # Suppress model's debug prints during inference (including C-level prints)
                    import sys
                    import os
