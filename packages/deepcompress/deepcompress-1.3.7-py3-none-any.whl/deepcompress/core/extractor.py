...
class OCRExtractor:
    ...
    async def _extract_page(self, image: Any, page_number: int) -> Page:
        ...
        try:
            ...
            # Simplified: Just call the model directly without complex redirection
            # The model's infer method should handle stopping internally
            ...
            while retry_count <= max_retries and result_text is None:
                try:
                    ...
                    # Call model.infer() directly in the event loop
                    loop = asyncio.get_event_loop()
