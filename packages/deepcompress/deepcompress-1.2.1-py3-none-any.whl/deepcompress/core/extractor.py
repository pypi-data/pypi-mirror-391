...
class OCRExtractor:
    ...
    async def _extract_page(self, image: Any, page_number: int) -> Page:
        ...
        try:
            ...
            if not result_text:
                logger.debug(f"Model returned None/empty, checking output files in: {output_dir}")
                        ...
                        try:
                            with open(output_file, 'r', encoding='utf-8', errors='ignore') as f:
                                ...
                                if content and len(content) > 10 and content not in ["None", "null", ""]:
                                    logger.info(f"Found OCR output in file: {output_file} ({len(content)} chars)")
                                    result_text = content
                                    break
            ...
            if not result_text or result_text.strip() in ["", "None", "null"]:
                raise OCRError(
                    f"OCR extraction returned empty result for page {page_number}",
