...
from src.lindorm_memobase_client import LindormMemobaseADD, LindormMemobaseSearch
import argparse
...
async def main():
    ...
    elif args.technique_type == "lindorm":
        if args.method == "add":
            memobase_manager = LindormMemobaseADD(data_path="datasets/locomo10.json")
            await memobase_manager.process_all_conversations()
        elif args.method == "search":
            memobase_manager = LindormMemobaseSearch()
            memobase_manager.process_data_file("dataset/locomo10.json")
    else:
