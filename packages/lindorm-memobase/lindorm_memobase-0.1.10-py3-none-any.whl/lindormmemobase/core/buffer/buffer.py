...
async def flush_buffer(user_id: str, blob_type: BlobType, config: Config, profile_config=None) -> Promise[
    ...
    # 并发获取 idle 和 failed 状态的 buffer ids
    idle_task = storage.get_ids_by_status(user_id, blob_type, BufferStatus.idle)
    failed_task = storage.get_ids_by_status(user_id, blob_type, BufferStatus.failed)

    results = await asyncio.gather(idle_task, failed_task, return_exceptions=True)

    buffer_ids = set()

    if isinstance(results[0], Exception) or not results[0].ok():
        return results[0] if not isinstance(results[0], Exception) else Promise.fail(str(results[0]))
    buffer_ids.update(results[0].data())

    # 处理 failed 结果
    if isinstance(results[1], Exception) or not results[1].ok():
        return results[1] if not isinstance(results[1], Exception) else Promise.fail(str(results[1]))
    buffer_ids.update(results[1].data())
