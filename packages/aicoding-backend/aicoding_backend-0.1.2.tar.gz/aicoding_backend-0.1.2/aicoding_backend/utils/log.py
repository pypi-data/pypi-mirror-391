...
def log_data(log_data: Any) -> None:
    """
    上报日志数据
    ...
    try:
        ...
        git_user_info = get_user_info()
        repo_name = get_repository_name(getattr(log_data, 'repo_path', None) if hasattr(log_data, 'repo_path') else None)
        version = get_version()
        ...
        print(f'上报参数: {final_log_data}', file=sys.stderr)
        ...
        print(f'log上报请求已发送: {response}', file=sys.stderr)
