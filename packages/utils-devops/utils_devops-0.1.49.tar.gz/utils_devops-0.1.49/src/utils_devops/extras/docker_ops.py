...
def _parse_compose_yaml(path: str, logger: Optional[logging.Logger] = None) -> Dict[str, Any]:
    ...
    if not text.strip():
        logger.warning(f"Compose file {path} is empty")
        return {}
    ...
    # ✅ FIX: Handle None result from YAML parser
    if data is None:
        logger.warning(f"Compose file {path} contains only comments or is invalid YAML")
        return {}
...
def read_compose(file_path: str, project_name: Optional[str] = None, logger: Optional[logging.Logger] = None) -> DockerCompose:
    ...
    # ✅ FIX: Parse networks with null checks
    networks = {}
    for name, net_data in networks_data.items():
        # ✅ ADD THIS NULL CHECK
        if net_data is None:
            logger.warning(f"Network '{name}' has null configuration, using defaults")
            net_data = {}
        
        networks[name] = NetworkSpec(
            name=name,
            driver=net_data.get("driver", "bridge"),  # Now safe
            external=net_data.get("external", False),
            labels=net_data.get("labels", {})
        )
    
    # ✅ FIX: Parse volumes with null checks
    volumes = {}
    for name, vol_data in volumes_data.items():
        # ✅ ADD THIS NULL CHECK
        if vol_data is None:
            logger.warning(f"Volume '{name}' has null configuration, using defaults")
            vol_data = {}
