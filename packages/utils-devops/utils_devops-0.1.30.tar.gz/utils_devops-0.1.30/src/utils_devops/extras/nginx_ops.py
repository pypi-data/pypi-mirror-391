...
and service control with dry-run capability. Supports both text and YAML configuration.
...
Location: Dataclass representing nested location configuration.
...
parse_sites_yaml(yaml_path) -> List[Site]
    Parse YAML configuration into Site objects.
generate_sites_yaml(sites) -> str
...
sync_from_yaml(yaml_path, force_all) -> None
...
build_location_blocks_extended(locations, location_tpl) -> List[str]
    Generate location blocks with full YAML support.
...
--------------------------
YAML format supports nested locations with individual upstreams, flags, and upload limits:
- host: site.domain.com
  ...
  locations:
    - path: /api
      upstream: "https://api-backend:3000"
      flags:
        cache: false
      upload_limit: "100M"
...
----------------------
Text format: site upstream [flags] [/path=upstream]
YAML format: See YAML Configuration Support section above.
...
upload_limit: Set client_max_body_size from YAML configuration
...
Common Patterns
---------------
# Basic setup with YAML format
mgr = NginxManager(dry_run=True)
sites = mgr.parse_sites_yaml("sites.yaml")
...
# Migration from text to YAML
mgr.migrate_text_to_yaml("sites.txt", "sites.yaml")

Backup & Restore Operations
---------------------------
backup_site_config(site_name, suffix) -> Path
    Backup site configuration with timestamp.
restore_site_config(site_name, backup_path) -> bool
    Restore site configuration from backup.
list_site_backups(site_name) -> List[Path]
    List available backups for sites.
backup_all_sites() -> Dict[str, Path]
    Backup all site configurations.
create_site_with_backup(site) -> Path
...
safe_apply_flag(site, flag, value) -> bool
    Apply flag with backup and validation.

Backup Features
---------------
- Automatic timestamped backups
- Pre-operation backups with rollback
- Config validation before/after operations
- Batch backup operations
- Safe flag application with validation
...
class NginxManager:
    ...
    def parse_sites_yaml(self, yaml_path: Union[str, Path]) -> List[Site]:
        ...
        if not isinstance(data, list):
            raise NginxOpsError(f"YAML file should contain a list of sites, got {type(data)}")
    ...
    def generate_sites_yaml(self, sites: List[Site]) -> str:
        ...
        yaml_data = []
        for site in sites:
            yaml_data.append(site.to_yaml_dict())
...
__all__ = [
    ...
    "parse_sites_yaml",
    "generate_sites_yaml",
    # Add new backup-related functions
    "backup_site_config",
    "restore_site_config", 
    "list_site_backups",
    "backup_all_sites",
