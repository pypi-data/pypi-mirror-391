...
### 0.5. Check for Claude GitHub App (Optional)

```bash
# Check if Claude Code GitHub App is installed on this repository
claude_app_installed=false

if gh api "repos/$REPO/installation" 2>/dev/null | grep -q "app_slug"; then
  claude_app_installed=true
  echo "✅ Claude Code GitHub App detected - @claude mentions will work"
else
  echo "💡 Tip: Install Claude Code GitHub App for AI assistance on issues"
  echo "   Visit: https://github.com/apps/claude-code"
  echo "   This enables @claude mentions in issues for AI-assisted analysis"
fi

# Store detection for future use
mkdir -p .claude/learned
cat > .claude/learned/github-config.json << EOF
{
  "claude_app_installed": $claude_app_installed,
  "repo": "$REPO",
...
**If Claude App is installed**, issues will include @claude mentions for AI assistance.
