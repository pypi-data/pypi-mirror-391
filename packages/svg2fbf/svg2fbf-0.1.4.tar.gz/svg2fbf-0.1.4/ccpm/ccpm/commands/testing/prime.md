...
**Python:**
- Check for pytest: `find . -name "pytest.ini" -o -name "conftest.py" -o -name "setup.cfg" -o -name "pyproject.toml" 2>/dev/null | head -5`
- Check for pyproject.toml pytest config: `test -f pyproject.toml && grep -q "\[tool\.pytest" pyproject.toml && echo "pytest config in pyproject.toml"`
- Check for unittest: `find . -path "*/test*.py" -o -path "*/test_*.py" 2>/dev/null | head -5`
- Check requirements: `grep -E "pytest|unittest|nose" requirements.txt pyproject.toml 2>/dev/null`
...
try:
    ...
    if pytest_opts:
        # Print each option on a new line for YAML
    ...
    print('  - -v')
    print('  - --tb=short')
    print('  - --strict-markers')
" 2>/dev/null
```

**Generic Example** (when no pyproject.toml):
```yaml
framework: pytest
test_command: pytest
...
**svg2fbf-style Example** (with pyproject.toml):
```yaml
framework: pytest
test_command: pytest
test_directory: tests
