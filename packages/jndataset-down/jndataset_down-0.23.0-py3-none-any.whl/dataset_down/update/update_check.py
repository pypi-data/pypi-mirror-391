...
def update_check():
    try:
        ...
        if  not isLatestVersion and needUpdate:
            print(f'\nsdk latest version is {latestVersion} and need update, now will try "pip install --upgrade {NAME}"\n')
            SDKVersion.update_sdk_to_latest()
            print(f'\nupdate sdk success,please restart your command!\n')
            sys.exit(0)
    except Exception as e:
        raise Exception(f"\nupdate sdk version error: {e}\n")
