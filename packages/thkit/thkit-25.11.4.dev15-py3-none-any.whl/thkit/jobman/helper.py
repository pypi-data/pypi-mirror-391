...
from thkit.pkg import create_logger, write_to_logfile
...
def log_machine_info(num_jobs: int, mdict: dict, machine_index: int, logger: object):
    """Log remote machine information."""
    color = _index2color(machine_index)
    remote_host = mdict["machine"]["remote_profile"]["hostname"]
    remote_root = mdict["machine"]["remote_root"]
    submit_size = mdict.get("submit_size", 5)
    ### Header
    margin = 2 * " "
    h = ["Index", "Jobs", "Sub", "Hostname", "Remote_root"]
    hh = ["-" * len(x) for x in h]
    w = [5, 6, 4, 14, None]
    ...
    ### Text
    text = f"{margin}{machine_index:^{w[0]}} {num_jobs:^{w[1]}} {submit_size:^{w[2]}} {remote_host:{w[3]}} {remote_root}"
    if machine_index == 0:
...
class ConfigRemoteMachines(Config):
    ...
    def check_connection(self, mdict_prefix: str = ""):
        ...
        def _connect_one_machine(mdict: dict) -> dict | None:
            ...
            if mdict["context_type"] == "SSHContext":
