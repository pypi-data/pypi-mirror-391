...
class EnumExTests(unittest.TestCase):
    ...
    def test_intflagex_operators_abstract(self):
        ...
        and_result = B.F1 & 0b11
        xor_result = B.F1 ^ 0b10
        lshift_result = B.F1 << 3
        rshift_result = B.F4 >> 3

        self.assertIsInstance(or_result,  B,        msg="OR is B")
        self.assertIsInstance(and_result, B,        msg="AND is B")
        self.assertIsInstance(xor_result, B,        msg="XOR is B")
        self.assertIsInstance(lshift_result, B,     msg="<< is B")
        self.assertIsInstance(rshift_result, B,     msg=">> is B")
        self.assertEqual(0b111,     or_result,      msg="B | A equal")
        self.assertEqual(1,         and_result,     msg="B & A equal")
        self.assertEqual(0b11,      xor_result,     msg="B ^ A equal")
        self.assertEqual(0b1000,    lshift_result,  msg="B << A equal")
        self.assertEqual(0b1,       rshift_result,  msg="B >> A equal")

        or_result  = B.F3 | A.F1
        and_result = B.F1 & A.F1
        xor_result = B.F1 ^ A.F2
        lshift_result = B.F1 << A.F2
        rshift_result = B.F4 >> A.F2
        ...
        self.assertEqual(1,         and_result,     msg="B & A equal")
        self.assertEqual(0b11,      xor_result,     msg="B ^ A equal")
        self.assertEqual(0b100,     lshift_result,  msg="B << A equal")
        self.assertEqual(0b10,      rshift_result,  msg="B >> A equal")
        ...
        _assert_invalidabstract(self, A, lambda: A.F1 << 3,    'foo')
        _assert_invalidabstract(self, A, lambda: A.F1 >> 3,    'foo')
        _assert_invalidabstract(self, A, lambda: A.F1 << A.F2, 'foo')
        _assert_invalidabstract(self, A, lambda: A.F1 >> A.F2, 'foo')
    ...
    def test_intflagex_lshift_operator(self):
        ...
        lshift_result = A.F1 << 3
        self.assertIsInstance(lshift_result, A,     msg="<< is A")
        self.assertEqual(0b1000, lshift_result,     msg="A << equal")

        lshift_result = B.F1 << 4
        self.assertIsInstance(lshift_result, B,     msg="<< is B")
        self.assertEqual(0b10000, lshift_result,    msg="B << equal")
    ...
    def test_intflagex_rshift_operator(self):
        ...
        shift_result = A.F4 >> 3
        self.assertIsInstance(shift_result, A,      msg=">> is A")
        self.assertEqual(0b1, shift_result,         msg="A >> equal")

        shift_result = B.F6 >> 2
        self.assertIsInstance(shift_result, B,      msg=">> is B")
