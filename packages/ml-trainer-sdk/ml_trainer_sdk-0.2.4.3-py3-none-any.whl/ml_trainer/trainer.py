...
class BaseTrainer(abc.ABC):
    ...
    #         self.logger.info(f"Model saved via .save() to {self.save_model_dir}")
    ...
    def save_model(self):
        ...
        if hasattr(self.model, "save"):
            ...
            self.logger.info(f"Model saved via .save() to {model_file}")
        else:
            self.logger.warning("Model has no `.save()` method.")
    ...
    def load_model(self):
        if hasattr(self.model, "load"):
            ...
            self.logger.info(f"Model loaded via .load() from {self.load_model_dir}")
        else:
