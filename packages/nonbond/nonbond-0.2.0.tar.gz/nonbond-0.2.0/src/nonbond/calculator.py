...
class Nonbond(Calculator):
    ...
    def _update_neighbor_list(self):
        ...
        if torch.any(self.parameters.cutoff > 0.5 * box_lengths):
            combined = torch.cat((self._neighbor_indices, self._neighbor_distances.reshape(-1, 1)),dim=1)
            _, unique_idx = torch.unique(combined[:, :2], dim=0, return_inverse=True)
            
            min_idx = torch.zeros(unique_idx.max() + 1, dtype=torch.long)
            for i in range(unique_idx.max() + 1):
                mask = unique_idx == i
                distances = combined[mask][:, 2]
                min_dist_idx = torch.argmin(distances)
                min_idx[i] = torch.nonzero(mask)[min_dist_idx]
            filtered_pairs = combined[min_idx]
            self._neighbor_indices = filtered_pairs[:, :2].to(torch.long)
            self._neighbor_distances = filtered_pairs[:, 2]
