    ...
    if not env_file.exists():
        ...
        while True:
            ...
            if choice == "1":
                ...
                api_key = input(
                    "Enter your Anthropic API key (or 'skip' to configure later): "
                ).strip()
...
"""
                ...
                if not ollama_running:
                    ...
                    if not ollama_running:
                        print(
                            "❌ Still cannot connect to Ollama. Please check your installation."
                        )
                        return False
                ...
                try:
                    ...
                    if model_names:
                        ...
                        model_choice = (
                            input(f"Enter model name (default: llama3.1): ").strip()
                            or "llama3.1"
                        )
                        ...
                        if model_choice not in model_names:
                            ...
                            download = (
                                input(f"Download {model_choice}? (yes/no): ")
                                .strip()
                                .lower()
                            )
    ...
    try:
        context = (
            subprocess.check_output(
                ["kubectl", "config", "current-context"], stderr=subprocess.DEVNULL
            )
            .decode()
            .strip()
        )
...
def translate_with_ollama(
    user_input, context_info, ollama_url, model, history_context=None
):
...
def translate_to_kubectl(
    user_input, context_info, provider_config, history_context=None
):
    """Route translation to the appropriate provider"""
    if provider_config["type"] == "claude":
        return translate_with_claude(
            user_input, context_info, provider_config["api_key"], history_context
        )
...
def analyze_resource_usage(params=None):
    ...
    try:
        ...
        output.append("\n" + "=" * 140)
        output.append(
            f"{'POD NAME':<45} {'CPU REQ':<12} {'CPU USE':<12} {'CPU %':<10} {'MEM REQ':<12} {'MEM USE':<12} {'MEM %':<10} {'STATUS':<15}"
        )
        output.append("=" * 140)
        ...
        for pod in pods_data["items"]:
            ...
            # Parse actual usage
            actual_cpu_val = (
                parse_resource(actual_cpu_str) if actual_cpu_str != "N/A" else 0
            )
            actual_mem_val = (
                parse_resource(actual_mem_str) if actual_mem_str != "N/A" else 0
            )
            ...
            cpu_use_display = (
                format_cpu(actual_cpu_str) if actual_cpu_str != "N/A" else "No Metrics"
            )
            mem_req_display = (
                format_memory(f"{int(pod_mem_req)}Mi") if pod_mem_req > 0 else "Not Set"
            )
            mem_use_display = (
                format_memory(actual_mem_str)
                if actual_mem_str != "N/A"
                else "No Metrics"
            )
            ...
            output.append(
                f"{pod_name_str:<45} {cpu_req_display:<12} {cpu_use_display:<12} {cpu_pct:<10} {mem_req_display:<12} {mem_use_display:<12} {mem_pct:<10} {status:<15}"
            )
        ...
        total_cpu_pct = (
            f"{(total_cpu_use / total_cpu_req * 100):.1f}%"
            if total_cpu_req > 0
            else "N/A"
        )
        total_mem_pct = (
            f"{(total_mem_use / total_mem_req * 100):.1f}%"
            if total_mem_req > 0
            else "N/A"
        )

        output.append(
            f"{'TOTAL':<45} {total_cpu_req_display:<12} {total_cpu_use_display:<12} {total_cpu_pct:<10} {total_mem_req_display:<12} {total_mem_use_display:<12} {total_mem_pct:<10}"
        )
        output.append("=" * 140)

        output.append("\n📊 SUMMARY:")
        output.append(
            f"  • Total CPU Requested: {total_cpu_req_display} | Total CPU Used: {total_cpu_use_display} | Usage: {total_cpu_pct}"
        )
        output.append(
            f"  • Total Memory Requested: {total_mem_req_display} | Total Memory Used: {total_mem_use_display} | Usage: {total_mem_pct}"
        )
        output.append(
            f"  • Pods with metrics: {pods_with_metrics}/{len(pods_data['items'])}"
        )

        output.append("\n💡 LEGEND:")
        output.append(
            "  • REQ = Resources Requested (what you asked Kubernetes to reserve)"
        )
        output.append("  • USE = Actual Usage (what the pod is currently using)")
        output.append("  • % = Percentage of requested resources being used")
        output.append(
            "  • 'Not Set' = No resource requests configured (not recommended)"
        )
        output.append(
            "  • 'No Metrics' = Pod exists but metrics-server has no data yet"
        )
        output.append("  • ⚠ HIGH = Using >90% of requested (may need more resources)")
        output.append(
            "  • 💡 Low = Using <30% of requested (potentially over-provisioned)"
        )
    ...
    try:
        ...
        if ("-p " in command or "--patch " in command) and (
            ...
            if patch_json:
                ...
                try:
                    result = subprocess.run(
                        new_parts, capture_output=True, text=True, timeout=30
                    )
                finally:
                    # Clean up temp file
                    try:
...
"""
    ...
    response_text = translate_to_kubectl(
        diagnosis_prompt, context_info, provider_config
    )
    ...
    try:
        ...
        if potential_fixes:
            ...
            for i, fix in enumerate(potential_fixes, 1):
                ...
                # Color code by risk
                risk_indicator = {"LOW": "✅", "MEDIUM": "⚠️ ", "HIGH": "🚨"}.get(
                    risk, "❓"
                )
...
"""
                        ...
                        analysis_response = translate_to_kubectl(
                            analysis_prompt, context_info, provider_config
                        )
                        ...
                        try:
                            ...
                            if is_resolved:
                                ...
                                print(
                                    f"   {confidence_icon} Issue appears RESOLVED (confidence: {confidence})"
                                )
                                ...
                                if confidence == "HIGH":
                                    ...
                                    cont = input(
                                        "   Continue with more fixes? (yes/no): "
                                    ).lower()
...
class SessionLogger:
    ...
    def log_interaction(
        self, user_input, command, explanation, safe, output, error=None
    ):
        """Log a complete interaction"""
        self.interactions.append(
            {"user_input": user_input, "command": command, "explanation": explanation}
        )
...
class SessionHistory:
                ...
                if "User:" in block and "Command:" in block:
                    ...
                    for line in lines:
                        if line.startswith("[") and "User:" in line:
                            interaction["user_input"] = line.split("User:", 1)[
                                1
                            ].strip()
                        elif line.startswith("Command:"):
                            interaction["command"] = line.split("Command:", 1)[
                                1
                            ].strip()
                        elif line.startswith("Info:"):
                            interaction["explanation"] = line.split("Info:", 1)[
                                1
                            ].strip()
    ...
    def get_context_summary(self, user_input):
        ...
        for item in similar:
            ...
            summary += (
                f"  • \"{interaction['user_input']}\" → {interaction['command']}\n"
            )
        ...
        while True:
            try:
                ...
                is_troubleshooting = any(
                    keyword in user_input.lower() for keyword in troubleshoot_keywords
                )

                if is_troubleshooting:
                    result = troubleshoot_interactive(
                        user_input, context_info, provider_config
                    )
                ...
                response_text = translate_to_kubectl(
                    user_input, context_info, provider_config, history_context
                )
                ...
                if not safe:
                    confirm = input(
                        "⚠️  This is a destructive command. Execute? (yes/no): "
                    )
                ...
                try:
                    if needs_analysis or command.startswith("ANALYZE_"):
                        output = analyze_resource_usage(
                            command.replace("ANALYZE_RESOURCES", "").strip()
                            if command.startswith("ANALYZE_RESOURCES")
                            else None
                        )
                    else:
                    ...
                    logger.log_interaction(
                        user_input, command, explanation, safe, output
                    )
                except Exception as e:
