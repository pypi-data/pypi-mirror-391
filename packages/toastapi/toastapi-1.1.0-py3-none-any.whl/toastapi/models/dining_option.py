...
class DiningOption(BaseModel):
    ...
    curbside: Optional[StrictBool] = Field(default=None, description="Indicates whether the dining option has curbside behavior, and allows the guest to provide identifying information such as a description of their vehicle. ")
    __properties: ClassVar[List[str]] = ["guid", "entityType", "externalId", "name", "behavior", "curbside"]
    ...
    def behavior_validate_enum(cls, value):
        ...
        if value not in set(['DINE_IN', 'TAKE_OUT', 'DELIVERY']):
            raise ValueError("must be one of enum values ('DINE_IN', 'TAKE_OUT', 'DELIVERY')")
        return value
