...
class Schedules(BaseModel):
    ...
    day_schedules: Optional[Dict[str, DaySchedule]] = Field(default=None, description="A map of `DaySchedule` objects that define the services and hours for different types of days. Each `DaySchedule` object is presented as a key/value pair. A pair's key matches the `scheduleName` of the object contained in the pair's value. ", alias="daySchedules")
    __properties: ClassVar[List[str]] = ["weekSchedule", "daySchedules"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in day_schedules (dict)
        _field_dict = {}
        if self.day_schedules:
            for _key_day_schedules in self.day_schedules:
                if self.day_schedules[_key_day_schedules]:
                    _field_dict[_key_day_schedules] = self.day_schedules[_key_day_schedules].to_dict()
            _dict['daySchedules'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "daySchedules": dict(
                (_k, DaySchedule.from_dict(_v))
                for _k, _v in obj["daySchedules"].items()
            )
            if obj.get("daySchedules") is not None
            else None
        })
        return _obj
