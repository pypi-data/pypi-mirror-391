...
class TempAccountValidator:
    ...
    def restart_ssh_and_retry(self):
        """重启SSH服务并重试连接"""
        ...
        try:
            ...
            result = subprocess.run(['systemctl', 'restart', 'sshd'], 
                                  capture_output=True, text=True)
            if result.returncode != 0:
                logger.error(f"重启SSH服务失败: {result.stderr}")
                return False
...
def validate_ssh_connection_after_upgrade():
    """
    验证SSH升级后的连接
    ...
    try:
        ...
        return validator.validate_ssh_upgrade()
    except Exception as e:
