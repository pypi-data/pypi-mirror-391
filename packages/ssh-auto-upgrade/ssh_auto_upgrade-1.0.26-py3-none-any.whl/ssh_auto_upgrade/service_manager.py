...
""".format(description, self.user, self.group, working_dir, env_vars, 
    ...
    def stop_ssh_service_with_port_check(self, check_port=True, ssh_port=22):
        """
        停止SSH服务并检查端口占用情况
        
        Args:
            check_port: 是否检查端口占用，默认为True
            ssh_port: SSH端口号，默认为22
            
        Returns:
            tuple: (success, message)
        """
        logger.info(f"开始停止服务: {self.service_name}")
        
        # 首先停止服务
        success, message = self.stop_service()
        
        if not success:
            logger.error(f"停止服务失败: {message}")
            return False, f"停止服务失败: {message}"
        
        logger.info("服务已停止，等待服务完全停止...")
        
        # 如果是SSH服务且需要检查端口
        if check_port and ('ssh' in self.service_name.lower() or self.service_name == 'ssh'):
            logger.info(f"检测到SSH服务，添加{ssh_port}端口占用检查...")
            ...
            # 检查指定端口是否被占用
            if port_checker.check_port_occupied(ssh_port):
                logger.info(f"检测到{ssh_port}端口仍被占用，开始清理...")
                cleanup_success, cleanup_message = port_checker.terminate_processes_using_port(ssh_port)
                
                if cleanup_success:
                    logger.info(f"SSH服务已停止，{ssh_port}端口已清理")
                    return True, f"SSH服务已停止，{ssh_port}端口已清理"
                else:
                    logger.warning(f"SSH服务已停止，但{ssh_port}端口清理遇到问题: {cleanup_message}")
                    return True, f"SSH服务已停止，但{ssh_port}端口清理遇到问题: {cleanup_message}"
            else:
                logger.info(f"SSH服务已停止，{ssh_port}端口当前未被占用")
                return True, f"SSH服务已停止，{ssh_port}端口未被占用"
        else:
            logger.info("服务已停止（无端口检查）")
            return True, "服务已停止"
    ...
    def remove_service(self, ssh_port=22):
        """
        移除服务
        
        Args:
            ssh_port: SSH端口号，默认为22，用于SSH服务的端口检查
        ...
        try:
            # 停止服务（对于SSH服务包含端口检查）
            if 'ssh' in self.service_name.lower() or self.service_name == 'ssh':
                success, message = self.stop_ssh_service_with_port_check(check_port=True, ssh_port=ssh_port)
            else:
                success, message = self.stop_service()
            ...
            if service_exists:
                ...
                try:
                    ...
                    if active_state == "active":
                        ...
                        # 对于SSH服务，使用包含端口检查的停止方法
                        if 'ssh' in self.service_name.lower() or self.service_name == 'ssh':
                            success, message = self.stop_ssh_service_with_port_check()
                        else:
                            success, message = self.stop_service()
                        if not success:
