...
class ParsedScenario(BaseModel):
    ...
    # Mapping from custom field human-readable names to technical names (for multi-input mappings)
    name_to_technical: Optional[Dict[str, str]] = None
    name_to_pythonic: Optional[Dict[str, str]] = None
    ...
    def from_api_dict(cls, scenario: Dict[str, Any], source_sdk: Any, sdk_mapping_config: Any) -> "ParsedScenario":
        ...
        # Build name_to_technical mapping for custom fields (needed for multi-input mappings)
        name_to_technical: Dict[str, str] = {}

        for detail in details:
            detail_model = ScenarioDetail.model_validate(detail)

            # Extract name-to-technical mappings from CUSTOM source fields
            if isinstance(detail_model.source, CustomSourceTarget):
                for item in detail_model.source.data:
                    if item.name and item.technical_name:
                        name_to_technical[item.name] = item.technical_name
        ...
        if source_sdk and sdk_mapping_config:
            ...
            # Convert name_to_technical to name_to_pythonic using alias_to_pythonic mapping
            name_to_pythonic: Dict[str, str] = {}
            for name, technical in name_to_technical.items():
                pythonic_name = alias_to_pythonic.get(technical, technical)
                name_to_pythonic[name] = pythonic_name
        else:
            name_to_pythonic = None
        ...
        instance = cls(
            ...
            name_to_technical=name_to_technical,
            name_to_pythonic=name_to_pythonic,
        )
        return instance
