...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        _, L, _ = x.size()
        result = x + self.pos_table[:, :L, :] # 添加位置信息
        result = result.clone().detach() # 不参与反向传播
        return result
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        '''
        当L2=L_in时, any=1 (自动广播到L_in或L_out)
        当L2=L_out时, any=L_out (因为有mask编码)
        对于encoder的self-attention的mask: [B, n_head, 1, L_in]
            mask用于padding(防止encoder关注到<PAD>)
        对于encoder的self-attention的mask: [B, n_head, L_out, L_out]
            mask用于padding以及防止观看未来信息
        对于decoder的enc-dec-attention的mask: [B, n_head, 1, L_in]
        ...
        attn = torch.matmul(q / self.scale_factor, k.transpose(2, 3)) # [B, n_head, L1, L2]
        if mask is not None:
