...
from ..utils.sendme_utils import ensure_sendme_installed, get_sendme_command
...
def restore_send_mode(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        file_args = [str(temp_path / f.split("/")[-1]) for f in files_to_send]
        ...
        try:
            ...
            cmd = [sendme_cmd] + file_args

            if verbose:
                stderr_console.print(f"[dim]$ {' '.join(cmd)}[/dim]")

            # Run sendme interactively so user can see the ticket
            result = subprocess.run(cmd, cwd=temp_dir)

            if result.returncode != 0:
                stderr_console.print("[bold red]Error:[/bold red] Failed to create sendme ticket")
                raise typer.Exit(code=1)
...
def restore_receive_mode(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if files_archive:
            cmd += f" --with-public-files {shlex.quote(files_archive.name)}"
        if private_files_archive:
            cmd += f" --with-private-files {shlex.quote(private_files_archive.name)}"
