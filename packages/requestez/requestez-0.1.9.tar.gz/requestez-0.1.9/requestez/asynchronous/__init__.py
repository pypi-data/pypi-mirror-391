...
class Session:
        ...
        async with self.client.request(method, url, **kwargs) as response:
            ...
            try:
                if read_as == "json":
                    ...
                    response.release()  # <-- FIX 1: Removed 'await'
    ...
    def _create_cookie_from_dict(
            self, d: Dict[str, Any]
    ) -> Morsel:  # <-- CHANGED return type
        """Helper to create a Morsel object from a dictionary."""
        name = d.get("name")
        value = d.get("value")

        m = Morsel()
        # Morsel.set() takes (key, real_value, coded_value)
        ...
        if epoch_seconds:
            try:
                # Format for Morsel: Wdy, DD Mon YYYY HH:MM:SS GMT
    ...
    async def save_data(self) -> Dict[str, Any]:
        ...
        for cookie in self.client.cookie_jar:
            ...
            # FIX: Check if the cookie object has the 'rest' attribute.
            # 'Morsel' objects do not, 'Cookie' objects do.
            httponly = False
    ...
    async def load_data(self, data: Dict[str, Any]):
        ...
        """
        # <-- FIX 2: Add type check for cookie jar
        jar = self.client.cookie_jar

        # The isinstance check was causing a premature return.
        # We'll remove it, as we need jar.clear() and setting the current_url
        # to run regardless. aiohttp.CookieJar has the methods we need.
        #
        ...
        for cookie_dict in data.get("cookies", []):
            ...
            morsel = self._create_cookie_from_dict(cookie_dict)  # <-- Renamed var
            if morsel.key:
                jar.update_cookies({morsel.key: morsel})  # <-- CHANGED from set_cookie
            else:
