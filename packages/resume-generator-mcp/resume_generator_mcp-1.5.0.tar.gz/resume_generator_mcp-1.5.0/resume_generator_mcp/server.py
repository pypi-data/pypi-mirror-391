...
def pdf_to_images(pdf_bytes: bytes, dpi: int = 150) -> List[ImageContent]:
    """
    Convert PDF bytes to a list of ImageContent objects (one per page).
...
def generate_resume(
    ...
    try:
        ...
        if success and pdf_bytes:
            ...
            # Return PDF as embedded resource with proper file URI
            pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')

            # Create response with text and embedded PDF resource
            response = [
                TextContent(
                    type="text",
                    text=f"""✅ Resume PDF generated successfully!

📄 File path: {output_path}
📊 Size: {file_size_kb:.1f} KB

NEXT STEP: Please use the Read tool to open and display this PDF file to the user."""
                ),
                EmbeddedResource(
                    type="resource",
...
@mcp.tool()
def create_resume(yaml_content: str, output_filename: str = "resume.pdf") -> List[TextContent | EmbeddedResource]:
    """
