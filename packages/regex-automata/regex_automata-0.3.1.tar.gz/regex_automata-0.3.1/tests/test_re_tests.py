# The following code is adapted from CPython "re_tests.py" module,
# available at https://github.com/python/cpython/blob/0ac890bea79d3e0162c8909b0999f626f1141d89/Lib/test/re_tests.py
...
@pytest.mark.xfail  # <-- remove this if you wish to see which tests fail for other reason than UnsupportedSyntaxError
@pytest.mark.parametrize("spec", map(json.dumps, tests))
def test_re_tests(spec):
    args = json.loads(spec)
    if len(args) == 3:
        pattern, text, expected_result = args
        test_string, test_string_expected_result = "", ""
    elif len(args) == 5:
        pattern, text, expected_result, test_string, test_string_expected_result = args
    else:
        assert False

    try:
        m = regex_automata.search(pattern, text)
        if m is None:
            assert expected_result == FAIL
        else:
            assert expected_result == SUCCEED
            if test_string is not None:
                env = {"found": m.group(), "groups": m.groups()}
                for i in range(1, 100):
                    try:
                        tmp = m.group(i)
                        value = tmp if tmp is not None else "None"
                    except IndexError:
                        value = "Error"
                    env[f"g{i}"] = value
    ...
    except regex_automata.errors.PatternError:
        assert expected_result == SYNTAX_ERROR
    except regex_automata.errors.UnsupportedSyntaxError as e:
        pytest.xfail(f"unsupported ({e})")
