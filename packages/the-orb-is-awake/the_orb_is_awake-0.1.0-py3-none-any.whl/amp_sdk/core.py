...
async def _build_settings_file(
    ...
    """
    Build a temporary settings file if permissions are provided.
    ...
    Returns:
        ...
        Both are None if no permissions are provided
    """
...
def _build_cli_args(options: Optional[AmpOptions] = None) -> list[str]:
    ...
    if options.mcp_config:
        if isinstance(options.mcp_config, str):
        ...
        elif isinstance(options.mcp_config, dict):
            ...
            config_str = str(options.mcp_config)
...
async def _handle_streaming_input(
    ...
    prompt: AsyncIterator[UserInputMessage]
) -> None:
    """
...
async def _handle_string_input(
    proc: asyncio.subprocess.Process,
    prompt: str
) -> None:
    """
    ...
    while True:
        ...
        try:
            ...
            if msg_type == "result":
                ...
                message_class = ErrorResultMessage if is_error else ResultMessage
