...
class YoloTrackStatistics:
    ...
    def run(self) -> None:
        ...
        for video_cnt, (video_name, data_path) in enumerate(self.data_paths.items()):
            ...
            self.track_lengths[video_name] = track_lengths
            self.all_track_lengths.extend(track_lengths.values())
            
            # Convert to seconds
            track_lengths_s = [length * self.seconds_per_frame for length in track_lengths.values()]
            self.all_track_lengths_s.extend(track_lengths_s)
    ...
    def plot_distribution(self, bin_width_s: int = 1, save_plot: bool = True) -> None:
        """
        Plot histogram of track length distribution.
        
        :param int bin_width_s: Width of each bin in seconds. Default: 1 second.
        ...
        # Add statistics text
        stats_text = (
            f"Mean: {self.statistics['mean_length']:.1f} s\n"
            f"Median: {self.statistics['median_length']:.1f} s\n"
