...
from cognite_typed_functions import FunctionApp, create_function_service, create_introspection_app
from cognite_typed_functions.client_generation import (
...
class TestFunctionClient:
    ...
    def test_function_client_requires_httpx(self):
        ...
        with patch.dict(sys.modules, {"httpx": None}):
            ...
            if "cognite_typed_functions.client" in sys.modules:
                del sys.modules["cognite_typed_functions.client"]

            # Now try to import - this might not work perfectly in tests
            # but documents the intent
            try:
                from cognite_typed_functions.client import FunctionClient
...
class TestFunctionClientDiscovery:
    ...
    def test_automatic_response_parsing_after_discover(self):
        ...
        # Mock response data
        response_data = {"success": True, "data": {"id": 42, "total": 100.0}}
        ...
        with patch("httpx.get") as mock_get:
            ...
            def side_effect(url: str, *args: Any, **kwargs: Any) -> Mock:
                ...
                if "__schema__" in url:
                    ...
                    response.json = Mock(return_value={"success": True, "data": openapi_schema})
    ...
    def test_no_parsing_for_dict_return_type(self):
        ...
        # Mock response data
        response_data = {"success": True, "data": {"key": "value"}}
