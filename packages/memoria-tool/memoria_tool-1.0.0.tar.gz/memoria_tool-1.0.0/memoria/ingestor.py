...
class Ingestor:
    ...
    def ingest_gmail(self, credentials_file: str, labels: list[str] = ['INBOX'], max_results: int = 10):
        ...
        results = service.users().messages().list(userId='me', labelIds=labels, max_results=max_results).execute()
...
class WhatsAppIngestor(Ingestor):
    ...
    def ingest_whatsapp(self, token: str, phone_id: str, limit: int = 10):
        ...
        # This is pseudo-code; adapt to actual library when implemented
        try:
            from whatsapp import WhatsApp  # Placeholder import
        except ImportError:
            raise ImportError("WhatsApp library is not installed.")
