...
class ColumnNameMap:
    ...
    def _get_snowpark_column_names_from_spark_column_names(
        ...
        for original_name in spark_column_names:
            name = original_name
            if not global_config.spark_sql_caseSensitive:
                name = name.upper()
            ...
            is_qualified_lookup = "." in original_name
    ...
    def get_qualifiers_for_spark_column(
        ...
        # Filter out hidden columns if there are multiple matches
        if len(col) > 1:
            visible_cols = [c for c in col if not c.is_hidden]
            if visible_cols:
                return visible_cols[0].qualifiers
