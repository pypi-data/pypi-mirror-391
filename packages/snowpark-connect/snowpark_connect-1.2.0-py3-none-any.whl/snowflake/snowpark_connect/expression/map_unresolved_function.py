...
def map_unresolved_function(
    ...
    match function_name:
        ...
        case "format_number":
            ...
            # Optimized path for literal scale values
            if exp.unresolved_function.arguments[1].HasField("literal"):
                scale_value = unwrap_literal(exp.unresolved_function.arguments[1])
                ...
                if scale_value < 0:
                    ...
                    if scale_value > 0:
                        format_pattern = "999,999,999,999,990." + "0" * scale_value
                    else:
                        format_pattern = "999,999,999,999,990"

                    result_exp = snowpark_fn.ltrim(
                        snowpark_fn.to_varchar(
                            rounded_col, snowpark_fn.lit(format_pattern)
                        )
                    )
                ...
                format_pattern_base = snowpark_fn.lit("999,999,999,999,990")
                format_pattern = snowpark_fn.when(
                    scale > 0,
                    snowpark_fn.concat(
                        format_pattern_base,
                        snowpark_fn.lit("."),
                        snowpark_fn.repeat(snowpark_fn.lit("0"), scale),
                    ),
                ).otherwise(format_pattern_base)
