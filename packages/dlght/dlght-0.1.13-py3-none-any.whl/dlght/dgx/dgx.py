...
class DGX:
    ...
    def __init__(self, url: str, output: str = None, resume: bool = True, unzip: bool = False, proxy: str = None) -> None:
        """
        ...
        Args:
            ...
            proxy: GitHub代理服务 (可选: 'ghfast', 'ghproxy', 默认: None)
        ...
        Examples:
            ...
            dlght x https://github.com/user/repo/blob/main/file.txt --proxy=ghfast
            dlght x https://github.com/user/repo/blob/main/file.txt --proxy=ghproxy
        """
        try:
            ...
            # 如果启用代理，应用代理
            if proxy:
                raw_url = self._apply_proxy(raw_url, proxy)
    ...
    def _apply_proxy(self, url: str, proxy: str) -> str:
        """
        为URL应用GitHub代理
        
        Args:
            url: 原始URL
            proxy: 代理类型 ('ghfast' 或 'ghproxy')
        ...
        proxy_url = proxy_map.get(proxy.lower())
        if proxy_url:
            return f"{proxy_url}/{url}"
        else:
            print(f"⚠ 未知的代理类型: {proxy}，将使用原始URL")
            return url
