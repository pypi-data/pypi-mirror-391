...
class TimeInterval:
    ...
    def _validate_start(
        ...
        *For internal use only*
        ...
        This method will validate if the `start` is lower than the
        `end` parameter provided (or the current `self.end` if None)
        and also lower than the `start_limit` provided (or the 
        current `self.start_limit` if None).
        ...
        if start >= end:
            raise Exception(f'The `start` value ({str(float(start))}) is greater or equal to the current `end` value ({str(float(end))}).')
        
        if start < start_limit:
            raise Exception(f'The `start` value ({str(float(start))}) is lower than the `start_limit` ({str(float(start_limit))}).')
    ...
    def _validate_end(
        ...
        *For internal use only*
        ...
        This method will validate if the `end` is greater than the
        `start` parameter provided (or the current `self.start` if
        None) and also lower than the `end_limit` provided (or the 
        current `self.end_limit` if None).
        ...
        if end <= start:
            raise Exception(f'The `end` value ({str(float(end))}) is lower or equal to the current `start` value ({str(float(start))}).')
        
        if end > end_limit:
            raise Exception(f'The `end` value ({str(float(end))}) is greater than the `end_limit` ({str(float(end_limit))}).')
    ...
    def shift_by(
        ...
        Move the `start` the `delta` amount of time provided (if
        possible and valid), and also the `end` in the same
    ...
    def shift_to(
        ...
        Move the `start` to the given `t` global time moment and
        the `end` the same amount in the same direction, so the
    ...
    def cut(
        ...
        return self.with_start_and_end(cut.start, cut.end)
    ...
    def split(
        ...
        return (
            self.copy.with_start_and_end(splits[0].start, splits[0].end),
            self.copy.with_start_and_end(splits[1].start, splits[1].end)
        )
    ...
    def with_start_and_end(
        ...
        Get this instance but with the `start` and the `end`
        ...
        self._validate_duration(end - start)
    ...
    def shift_start_to(
        ...
        the `start` time moment has been modified to the `t`
    ...
    def shift_end_to(
        ...
        Transform the time interval into a new one in which
        the `end` time moment has been modified to the `t`
        provided (if possible and valid).
