...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QLineEdit, QComboBox, QDialog, QToolButton, QWidgetAction, QCheckBox, QTableWidget, QHeaderView, QApplication, QStyle, QStyledItemDelegate, QScrollArea, QSizePolicy
from PyQt6.QtGui import QIcon, QTextDocument
import re
import requests
import importlib.resources

main_directory = importlib.resources.files("gui_utilities")
icons_directory = main_directory / "icons"
tlds_path = main_directory / "tlds"
...
def create_text_box(
    ...
    disabled_border_width = 2,
    disabled_border_color = "#4a4a4a",
    show_text_icon_url = str(icons_directory / "show_text_icon.png"),
    hide_text_icon_url = str(icons_directory / "hide_text_icon.png"),
    focused_show_text_icon_url = str(icons_directory / "focused_show_text_icon.png"),
    focused_hide_text_icon_url = str(icons_directory / "focused_hide_text_icon.png"),
    hide_text = False,
    math_expression = False,
    parent = None
):
...
def create_information_message_box(
    ...
    label_padding_left_value = label_padding_left if label_padding_left is not None else label_padding
    label_padding_right_value = label_padding_right if label_padding_right is not None else label_padding
    ...
    text_label = create_label(
        ...
        word_wrap = True,
        math_expression = math_expression,
        parent = message_box  # importante: parent del dialog
    )

    # --- Forzar comportamiento que permite wrap ---
    text_label.setTextFormat(Qt.TextFormat.RichText if math_expression else Qt.TextFormat.PlainText)
    text_label.setWordWrap(True)
    text_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)

    # Si el diálogo tiene fixed_width calculá el ancho real disponible y asignalo al label
    if fixed_width is not None:
        # quitar márgenes del layout y paddings del label y un pequeño margen extra
        available_width = fixed_width - (left_margin + right_margin) - (label_padding_left_value + label_padding_right_value) - 8
        if available_width > 50:
            text_label.setMaximumWidth(available_width)
...
tlds_list_path = main_directory / tlds_path / "tlds_list.txt"
