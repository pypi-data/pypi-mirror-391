...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QLineEdit, QComboBox, QDialog, QToolButton, QWidgetAction, QCheckBox, QTableWidget, QHeaderView, QApplication, QStyle, QStyledItemDelegate, QScrollArea, QSizePolicy, QTextEdit, QFrame
...
def create_information_message_box(
    ...
    text_widget = QTextEdit(message_box)
    ...
    text_widget.setText(format_html(text) if math_expression else text)
    text_widget.setWordWrapMode(QTextOption.wrapMode(QTextOption.WrapMode.WordWrap))
    text_widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
    text_widget.setStyleSheet("background-color: transparent;")  # para que combine con dialog

    # si tenés fixed_width, limitá el ancho para que el widget haga wrap interior
    if fixed_width is not None:
        available_width = fixed_width - (left_margin + right_margin) - (label_padding_left + label_padding_right) - 8
