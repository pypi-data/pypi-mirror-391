...
class CommittedQuoteType(BaseModel):
    ...
    CommittedQuoteType
    """ # noqa: E501
    type: StrictStr = Field(description="Indicates this is a committed quote")
    expires_at: datetime = Field(description="ISO 8601 timestamp of the expiration time of the quote.", alias="expiresAt")
    __properties: ClassVar[List[str]] = ["type", "expiresAt"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['COMMITTED']):
            raise ValueError("must be one of enum values ('COMMITTED')")
        return value
