...
class Manifest(BaseModel):
    ...
    Manifest
    """ # noqa: E501
    asset_types: List[StrictStr] = Field(alias="assetTypes")
    capabilities: List[Capability]
    __properties: ClassVar[List[str]] = ["assetTypes", "capabilities"]

    @field_validator('asset_types')
    def asset_types_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['DIGITAL', 'FIAT']):
                raise ValueError("each list item must be one of ('DIGITAL', 'FIAT')")
        return value
