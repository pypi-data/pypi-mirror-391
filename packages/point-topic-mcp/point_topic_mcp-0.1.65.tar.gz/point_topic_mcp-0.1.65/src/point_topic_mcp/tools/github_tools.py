...
if check_env_vars('github_tools', ['GITHUB_TOKEN']):
    ...
    def propose_code_change(
        ...
        """Propose code change using search/replace pattern. Commits to agent-dev and creates PR.
        
        SAFE: Can only write to agent-dev branch. Creates PR for human review.
        Uses search/replace - only specify the lines that need changing (saves tokens).
