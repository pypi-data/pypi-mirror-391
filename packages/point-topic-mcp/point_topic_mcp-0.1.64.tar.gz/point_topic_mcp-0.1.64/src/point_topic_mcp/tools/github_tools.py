...
if check_env_vars('github_tools', ['GITHUB_TOKEN']):
    ...
    def search_issues(
        ...
        """Search issues/PRs in Point-Topic org. Use this to find issues by keyword, label, assignee, etc.
        
        Args:
            query: Search text (e.g. "bug in API", "performance")
            repo: Limit to repo name (e.g. "point-topic-mcp", "UPC_Core")
            state: "open" (default), "closed", or "all"
            labels: Comma-separated (e.g. "bug,urgent")
            assignee: GitHub username
            type: "issue" (default), "pr", or "both"
            max_results: Max results (default 30)
        
        Key repos: UPC_Core, UPC_Client, upc_query_agent, point-topic-mcp
        """
        try:
            g = _gh()
    ...
    def create_pr(
        ...
        """Create PR from existing branch. SAFE: branch must exist, cannot modify files.
        
        Args:
            repo: Repo name (e.g. "point-topic-mcp", "UPC_Core")
            title: PR title
            head: Source branch name (must already exist with commits)
    ...
    def propose_code_change(
        ...
        """Propose code change by committing to agent-dev branch and auto-creating PR.
        
        SAFE: Can only write to agent-dev branch. Creates PR for human review.
        Branch protection on main prevents direct changes or merges.
        
        Args:
            repo: Repo name (e.g. "point-topic-mcp", "UPC_Core")
            file_path: Path to file (e.g. "src/point_topic_mcp/tools/github_tools.py")
            new_content: Complete new file contents
        ...
        Example:
            propose_code_change(
                ...
                new_content="<full updated file>",
                change_description="Fix search_issues docstring to clarify type parameter"
            )
        """
        try:
            import base64
            ...
            file_sha = None
            try:
                contents = r.get_contents(file_path, ref=AGENT_BRANCH)
