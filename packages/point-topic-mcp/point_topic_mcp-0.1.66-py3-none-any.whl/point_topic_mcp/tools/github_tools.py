...
if check_env_vars('github_tools', ['GITHUB_TOKEN']):
    ...
    def create_pr(
        ...
        """Create PR from existing branch that already has commits. SAFE: cannot modify files.
        
        NOTE: This does NOT make code changes. Branch with commits must already exist.
        To propose code changes, use propose_code_change instead.
        
        Args:
            repo: Repo name (e.g. "point-topic-mcp", "UPC_Core")
            title: PR title
            head: Source branch name (must already exist with commits on it)
            base: Target branch (default: repo default branch)
            body: PR body (markdown supported)
            draft: Create as draft (default False)
        
        When to use: Only when a human or external process has already created a branch with commits.
        """
        try:
            g = _gh()
            repo_name = f"{ORG_NAME}/{repo}" if "/" not in repo else repo
