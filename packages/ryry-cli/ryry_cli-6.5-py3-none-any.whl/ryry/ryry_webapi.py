...
g_user_token, g_author = store.get_token_and_authorization()
def _domain_post(func, params, files={}, timeout=10, domain="api.dalipen.com"):
    headers = {
        'Connection': 'close',
        'Usertoken': g_user_token,
        'Authorization': f"Bearer {g_author}"
    }
...
def checkLicense():
    req = { }
    r1, r2, r3 = _aigc_post("aigc/task/checkLicense", req)
    if r1 != 0:
        raise Exception(f"check license fail!, reason={r2}")
    return r3

def set_authorization(author):
    store.save_authorization(author)
    global g_user_token, g_author
    g_user_token, g_author = store.get_token_and_authorization()
    if checkLicense():
