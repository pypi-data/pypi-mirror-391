...
def updateWidgetInfo(widget_id, local_path):
    try:
        sp = Store()
        read_data = sp.read()
        
        with open(os.path.join(local_path, 'config.json'), 'r', encoding='UTF-8') as f:
            data = json.load(f)
        version = data["version"]
        if "max_task_number" in data:
        ...
        widgetsMap = read_data["widgets"]
        if widget_id in widgetsMap:
            widgetsMap[widget_id]["version"] = version
            widgetsMap[widget_id]["max_task_number"] = max_task_number
        sp.write(read_data)
    except:
        pass
#============================== device id ================================
    ...
    if len(GLOBAL_EXT_JSON) < 10:
        ...
        if "extend" in read_data:
            ...
            if len(GLOBAL_EXT_JSON) < 10:
                ...
                _genExtend()
            elif read_data["extend"].get("app", "") == "ryry-cli 6.12":
                _genExtend()
        else:
            _genExtend()
