...
def _size():
    return Quartz.CGDisplayPixelsWide(
...
def _mouseDown(x, y, button):
    if button == LEFT:
        ...
        _sendMouseEvent(
            Quartz.kCGEventOtherMouseDown, x, y, Quartz.kCGMouseButtonCenter
        )
    elif button == RIGHT:
...
def _mouse_is_swapped():
    ...
    if _mouse_is_swapped_setting is None:
        _mouse_is_swapped_setting = (
            False  # TODO - for now, we can't detect this setting
        )
...
def _dragTo(x, y, button):
    if button == LEFT:
        _sendMouseEvent(
            Quartz.kCGEventLeftMouseDragged, x, y, Quartz.kCGMouseButtonLeft
        )
    elif button == MIDDLE:
        _sendMouseEvent(
            Quartz.kCGEventOtherMouseDragged, x, y, Quartz.kCGMouseButtonCenter
        )
    elif button == RIGHT:
        _sendMouseEvent(
