...
def is_frame_filter(filter: FilterLike) -> TypeGuard[FrameFilterPredicate]:
    # A filter is a frame filter if it has type "frame" or "any",
    # and it can be called with FrameInfo
    return filter.type in ("frame", "any")


def is_signal_filter(filter: FilterLike) -> TypeGuard[SignalFilterPredicate]:
    # A filter is a signal filter if it has type "signal" or "any",
    # and it can be called with SignalInfo
    return filter.type in ("signal", "any")
...
def _someip_match(sep: str, service: str | None, name: str | None, is_signal: bool, full_name: str) -> bool:
    if sep not in full_name:
        return False
    service_part, rest = full_name.split(sep, 1)
    if not rest or (service and service_part != service):
        return False
    if is_signal and name:
        return rest.startswith(f"{name}.")
    return name == rest if name else True
...
def _apply_frame_filters(frame_info: FrameInfo, frame_filters: Sequence[FrameFilterPredicate]) -> FrameInfo | None:
