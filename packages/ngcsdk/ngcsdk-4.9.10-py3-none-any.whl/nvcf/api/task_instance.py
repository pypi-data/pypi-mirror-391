...
class TaskInstanceAPI:  # noqa: D101
    ...
    def _construct_instances_ep(
        ...
        return TaskInstanceAPI._construct_skyway_ep("instances", org_name, task_id, team_name)
    ...
    def _construct_execute_commands_ep(
        ...
        ep = TaskInstanceAPI._construct_skyway_ep("commands", org_name, task_id, team_name)
    ...
    def _construct_logs_ep(
        ...
        ep: str = TaskInstanceAPI._construct_skyway_ep("logs", org_name, task_id, team_name)
    ...
    def _construct_error_message(
        ...
        error_message = f"Failed to {action} task '{task_id}'"
        if error and error.explanation:
            error_message += f"\nError code {error.status_code}: {json.loads(error.explanation).get('detail')}"
        return error_message

    @extra_args
    def list(self, task_id: str) -> DotDict:
        """List instances of a given task."""
        self.config.validate_configuration()
        org_name: str = self.config.org_name
        team_name: Optional[str] = self.config.team_name
        url = self._construct_instances_ep(org_name, task_id, team_name)
        response = self.connection.make_api_request(
            "GET", url, auth_org=org_name, auth_team=team_name, operation_name="get task instances"
        )
    ...
    def query_logs(
        ...
        Args:
            task_id: Id of task logs are pulled from.
            pod_name: Name of the task instance pod
            container_name: Name of the task instance container in the target pod
        ...
        Returns:
            Generator[bytes, None, None]: Streaming response of task logs.
        """  # noqa: E501
        ...
        with ServerSentEventsHandler(
            ...
            return sse_handler.make_sse_request(
                ...
                operation_name="query task logs in real-time",
    ...
    def execute_commands(
        ...
        """Execute command on selected container for specific task deployments.
