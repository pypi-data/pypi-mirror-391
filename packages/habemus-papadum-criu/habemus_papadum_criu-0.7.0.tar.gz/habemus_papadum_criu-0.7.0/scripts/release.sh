...
"""Release automation script for criu.
...
INIT_PY = REPO_ROOT / "src" / "pdum" / "criu" / "__init__.py"
...
def main() -> None:
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Automate the release process for criu")
    parser.add_argument(
        "bump_level",
    ...
    # Show title
    title = Panel.fit(
        "[bold cyan]criu Release Script[/bold cyan]\n\n"
