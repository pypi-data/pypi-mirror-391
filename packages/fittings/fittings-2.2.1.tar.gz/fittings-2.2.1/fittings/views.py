...
from .app_settings import is_installed
...
def _get_accessible_categories(user):
    ...
    cats = cats.union(
        Category.objects.filter(
            ...
            doctrines_count=Count('doctrines', distinct=True)
        ).annotate(
            fittings_count=Count('fittings', distinct=True)
        ).annotate(
            d_fittings_count=Count('doctrines__fittings', distinct=True)
        ).annotate(
            total_fits=F('fittings_count')+F('d_fittings_count')
        )
    )
...
def view_fit(request, fit_id):
    ...
    # If the CorpTools app is installed, we want to render the skillcheck template and pass it to the context.
    ctx['skillcheck'] = render_to_string(
        template_name='fittings/corptools/skillcheck.html', request=request
    ) if is_installed('corptools') else ''
