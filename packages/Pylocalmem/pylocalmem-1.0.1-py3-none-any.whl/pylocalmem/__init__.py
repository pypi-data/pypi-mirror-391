...
class Process():
    ...
    def read_bytes(self, address:int, length:int):
        ...
        try:
            ...
            return None
        # return ctypes.cast(address, ctypes.POINTER(ctypes.c_char * length)).contents.raw # Very very very bad and i have no idea why so i guess ill stick to from_address instead (using this cast method causes tons of null pointer errors and or length errors)
    ...
    def read_ctype(self, address:int, ctype:ctypes.Structure):
        ...
        # raw = (ctypes.c_char * size).from_address(address).raw
        # return ctype.from_buffer_copy(raw)
        try:
    ...
    def read_string(self, address:int, length:int=50, encoding:str='UTF-8'):
        """
        Reads a string but cannot read a Utf-16 string so use read_bytes with a hard coded length or instead of Utf-16
    ...
    def read_unicode_string(self, address:int, length:int):
        ...
        If the function succeeds the return value is a decoded unicode string
    ...
    def read_char(self, address:int):
        ...
        """
        return self.read_ctype(address, ctypes.c_char).value
    ...
    def __parse_module(self, module:str):
        ...
        # Get the rva needed for OptionalHeader
        ...
        for i in range(len(name_address_array)):
            ...
            exports[export_name] = {'ordinal': export_ordinal, 'rva': hex(export_rva)}
    ...
    def get_modules(self):
        ...
        if snapshot:
            ...
            while True:
                ...
                # module_name = module.szExePath.decode().split('\\')[-1]
    ...
    def get_modules_from_peb(self):
        ...
        while True:
            ...
            # Read the fields to get the name and full path of the dll
            # module_name = self.read_bytes(module_info.BaseDllName.Buffer, module_info.BaseDllName.Length).decode('utf-16le') # Replaced with a dedicated read_unicode_string instead
            # module_path = self.read_bytes(module_info.FullDllName.Buffer, module_info.FullDllName.Length).decode('utf-16le') # Replaced with a dedicated read_unicode_string instead
    ...
    def get_module_exports(self, module:str):
        ...
        """
        self.__parse_module(module)
        return self.__exports.get(module)
    ...
    def get_proc_address(self, module:str, function:str):
        ...
        if self.__exports.get(module.lower()):
            return module_base + int(self.__exports.get(module.lower())[function]['rva'], 16)


        return module_base + int(self.__parse_module(module_base, module).get(function)['rva'], 16)
    ...
    def get_peb(self):
        ...
        """
        peb_return = self.allocate(8) # i for some reason had this labeled as "addr" with 8 bytes and had it slapped in mov rbx, {hex(peb_return)} as mov rbx, {hex(addr)} this meant when this ran it overrided the original asm at offset 0x0 idk how i was so stupid to not realize this lmao
    ...
    def allocate(self, amount:int):
        ...
        self.buffer = _VirtualAlloc(None, ctypes.c_void_p(amount), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)
        return self.buffer
    ...
    def execute_code_async(self, code:bytes|str):
        ...
        assembly_address = self.allocate(len(code))
        self.write_bytes(assembly_address, code, len(code))
    ...
    def hide_module(self, module:str):
        ...
        """
        #TODO: maybe replace all these "read_longlong" to read_ctype and just read the whole structure or something (most likely not gonna ever do that though)


        # This is put in a seperate function for nicer looking code
        def replace_flink_blink(_address):
            # Get current module's blink and flink
            flink = self.read_longlong(_address + 0x0)
            blink = self.read_longlong(_address + 0x8)


            # Go to next module and replace its blink
            previous_module = self.read_longlong(_address)
            self.write_longlong(previous_module + 0x8, blink)


            # Go to next module and replace its flink
            next_module = self.read_longlong(_address + 0x8)
            self.write_longlong(next_module + 0x0, flink)
            
        # Helper function for replacing multiple module list pointers
        def process_module_list(module_list, module, offset):
            modules = set()
            
            while True:
                unicode_string = module_list + offset

                
                length = self.read_short(unicode_string)
                max_length = self.read_short(unicode_string + 0x2)
                buffer_addr = self.read_longlong(unicode_string + 0x8)
                if buffer_addr and length: # Apparently all of a sudden in this windows install sometimes we get a invalid buffer or invalid length so this wasnt needed before
                    module_name = self.read_unicode_string(buffer_addr, length)
                ...
                # Compare ignoring case (Windows module names are case-insensitive)
        ...
        process_module_list(InLoadOrderModuleList, module, 0x58)
        process_module_list(InMemoryOrderModuleList, module, 0x48)
        process_module_list(InInitializationOrderModuleList, module, 0x58 - 0x020)
        return True
    ...
    def hide_python(self):
        ...
        """
        return self.hide_module('python{0}{1}.dll'.format(sys.version_info.major, sys.version_info.minor))
