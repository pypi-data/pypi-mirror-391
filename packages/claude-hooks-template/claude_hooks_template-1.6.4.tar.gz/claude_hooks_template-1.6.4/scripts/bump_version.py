...
Manual override:
- --force-bump patch|minor|major -> force version bump regardless of commits
...
def main() -> int:
    """Main entry point."""
    # Parse arguments
    parser = argparse.ArgumentParser(description="Create version tag based on commits")
    parser.add_argument(
        "--force-bump",
        choices=["patch", "minor", "major"],
        help="Force version bump type (overrides commit analysis)",
    )
    ...
    try:
        ...
        # Check for forced bump first
        if args.force_bump:
            bump_type = args.force_bump
            print(f"\nBump type: {bump_type} (forced)")
            if commits:
                print(f"\nIgnoring {len(commits)} commits due to forced bump")
        else:
            # Only check commits if not forcing bump
            if not commits:
