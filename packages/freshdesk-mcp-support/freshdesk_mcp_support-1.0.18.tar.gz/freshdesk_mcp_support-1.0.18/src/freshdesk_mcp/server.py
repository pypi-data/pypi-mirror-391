...
async def my_unresolved_tickets_v2() -> Dict[str, Any]:
    ...
    # Manually construct HTTP request to avoid URL encoding
    import ssl
    import asyncio
    import base64
    import json
    
    host = FRESHDESK_DOMAIN
    # Construct path with query parameter - no encoding
    path = f"/api/v2/search/tickets?query={query}"
    auth = _get_auth()
    ...
    try:
        # Manually construct HTTP request without encoding
        reader, writer = await asyncio.open_connection(host, 443, ssl=ssl_context)
        
        try:
            # Construct HTTP request manually - query parameter is not encoded
            request = (
                f"GET {path} HTTP/1.1\r\n"
                f"Host: {host}\r\n"
            ...
            if len(status_parts) < 2:
                return {"error": "Invalid HTTP status line"}
            ...
            if status_code >= 400:
                return {"error": f"HTTP {status_code}: {body_str[:200]}"}
            
            # Parse JSON response
            tickets = json.loads(body_str)
            
            # Parse pagination from Link header
            link_header = headers_dict.get('Link', '')
