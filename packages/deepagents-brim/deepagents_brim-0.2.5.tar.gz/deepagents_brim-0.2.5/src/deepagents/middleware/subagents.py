...
def _create_task_tool(
    *,
    ...
    Returns:
        A tool that can invoke subagents by type.
    """
    ...
    subagent_names = tuple(sorted(subagent_graphs.keys()))
    if not subagent_names:
        raise ValueError("No subagents available")
    enum_members = {enum_key(name): name for name in subagent_names}
    SubagentLiteral = Enum("SubagentLiteral", enum_members)
    ...
    async def task(
        ...
        subagent_type: Annotated[
            SubagentLiteral,
            Field(description="The type of subagent to invoke."),
        ],
        runtime: Annotated[ToolRuntime, Field(exclude=True)],
    ) -> Command:
        subagent_name = subagent_type.value if isinstance(subagent_type, Enum) else subagent_type
        subagent, subagent_state = _validate_and_prepare_state(subagent_name, description, runtime)
