...
- [login](#md-login)
- [logout](#md-logout)
- [Oauth_login](#md-oauth_login)
- [Oauth_login_using_code](#md-oauth_login_using_code)
...
**⚠️ DEPRECATED - This login method will be disabled**

The old login flow (`login()`) has limitations:
- Sessions expire after a few days and stop working
- This authentication method will be **disabled** in the future

**Please use [OAuth login](#md-oauth_login) instead** for reliable, long-term authentication.
...
#### <a name="md-oauth_login"></a> Oauth_login(client_id, client_secret)
Initiates the complete OAuth 2.0 login flow with automatic callback handling.

This method:
1. Displays your system's public IP address
2. Creates a localhost callback URL (`http://localhost:8189/callback`)
3. Shows the OAuth authorization URL you need to visit
4. Starts a local HTTP server to receive the OAuth callback
5. Waits for you to complete authorization in your browser
6. Automatically exchanges the authorization code for an access token
7. Sets up your session for subsequent API calls
8. Updates the API endpoint to the OAuth endpoint
...
# OAuth credentials (obtain from go.mynt.in)
client_id = 'your_client_id'
client_secret = 'your_client_secret'

# Perform OAuth login
result = mt.Oauth_login(client_id=client_id, client_secret=client_secret)

if result:
    print("Login successful!")
    # Now you can use all API methods
    positions = mt.get_positions()
    holdings = mt.get_holdings()
```
...
| --- | --- | --- | --- |
| `client_id` | `string` | Yes | Your OAuth client ID from go.mynt.in |
| `client_secret` | `string` | Yes | Your OAuth client secret from go.mynt.in |
...
When you call `Oauth_login()`, you'll see:

```
======================================================================
OAuth LOGIN - STEP BY STEP INSTRUCTIONS
======================================================================

📋 INFORMATION:
   • Your Public IP: 203.0.113.42
   • Callback URL: http://localhost:8189/callback
   (Note: Make sure this callback URL is registered in your OAuth app settings)
...
👉 Copy and open this URL in your web browser:
   ...
   https://go.mynt.in/OAuthlogin/authorize/oauth?client_id=ZP00XXX_U

   After opening, you will be asked to login and authorize the application.
...
----------------------------------------------------------------------
   ...
   Once you authorize in the browser, you will be redirected back automatically.
   You can press Ctrl+C at any time to cancel.

✅ Callback server is ready and waiting...
...
✅ Your session has been established successfully!
   • User ID: ZP00XXX
   • Session Token: 3b97f4c67762259a9ded6dbd7bfafe2787e662b3870422ddd343a59895f423a0...
   • API Endpoint: https://go.mynt.in/NorenWClientAPI/
...
1. **Callback URL Registration**: The callback URL `http://localhost:8189/callback` must be registered in your OAuth application settings on go.mynt.in
2. **Timeout**: The function waits up to 5 minutes for the OAuth callback
3. **Ctrl+C Support**: You can cancel the process at any time using Ctrl+C
4. **Automatic Endpoint Update**: After successful login, the API endpoint automatically changes to `https://go.mynt.in/NorenWClientAPI/`

#### <a name="md-oauth_login_using_code"></a> Oauth_login_using_code(client_id, client_secret, code)
Login using an existing authorization code without setting up a callback server.

This method allows you to login if you already have an authorization code from a previous OAuth flow or from handling the callback yourself.
...
code = "f199319d-a5df-444d-b962-27d2886cf252"
result = mt.Oauth_login_using_code(
    client_id='ZP00XXX_U',
    client_secret='your_client_secret',
    code=code
)
...
| `code` | `string` | Yes | The authorization code from OAuth callback |
...
- You're handling the OAuth callback in your own web application
- You've stored an authorization code from a previous session
- You want to login programmatically without browser interaction
...
### Recommended: OAuth Login (New Method)

**⚠️ The old login method is deprecated and will be disabled. Use OAuth login instead.**
...
if result:
    print("Login successful!")
    print(f"Logged in as: {result.get('uid')}")
    # Now you can use all API methods
    positions = mt.get_positions()
    holdings = mt.get_holdings()
```
...
**Note: This method will stop working after a few days and will be disabled. Use OAuth login above instead.**
