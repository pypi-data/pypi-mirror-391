#!/bin/bash
# Quick test script for MCP Cookie Cutter with Petstore using URL

set -e

echo "🚀 MCP Cookie Cutter - Petstore Quick Test (URL)"
echo "================================================"
...
echo -e "${BLUE}Using OpenAPI spec from URL: https://petstore.swagger.io/v2/swagger.json${NC}"
echo -e "${BLUE}Using package manager: ${PACKAGE_MANAGER}${NC}"
echo ""

# Remove existing project if it exists
if [ -d "petstore_mcp_server" ]; then
    echo "Removing existing petstore_mcp_server directory..."
    rm -rf petstore_mcp_server
fi

# Run cookiecutter with URL-based OpenAPI spec
