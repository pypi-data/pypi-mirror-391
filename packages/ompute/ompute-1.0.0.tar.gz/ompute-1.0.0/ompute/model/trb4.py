...
def normal_rvs(rng: np.random.RandomState, loc: float, scale: float, size: int = 1) -> np.ndarray:
    if not np.isfinite(scale) or scale <= 0:
        return np.array([float(loc)] * size)
...
def impute_cell_median(feature_vals: np.ndarray) -> Optional[float]:
...
def impute_cell_iterative_elasticnet(X_numeric: np.ndarray, target_r: int, target_c: int, seed: int = 0, en_alpha: float = 1.0, en_l1_ratio: float = 0.5) -> Optional[float]:
    try:
        from sklearn.experimental import enable_iterative_imputer  # noqa: F401
        from sklearn.impute import IterativeImputer
        from sklearn.linear_model import ElasticNet
    except Exception:
        raise ImportError("scikit-learn with IterativeImputer and ElasticNet required: pip install scikit-learn")
    tmp = X_numeric.copy()
    ...
    est = ElasticNet(alpha=float(en_alpha), l1_ratio=float(en_l1_ratio), random_state=seed, max_iter=5000)
    imp = IterativeImputer(estimator=est, max_iter=10, random_state=seed, imputation_order='ascending', sample_posterior=False)
...
def impute_cell_perseus_downshift(feature_vals: np.ndarray, rng: np.random.RandomState, downshift: float = 1.8, width: float = 0.3) -> Optional[float]:
    ...
    if pos.size == 0:
        return None
    logpos = np.log2(pos)
    mu = float(np.nanmean(logpos))
    sd = float(np.nanstd(logpos, ddof=0))
    if not np.isfinite(sd) or sd == 0:
        sd = max(abs(mu) * 0.1, 1e-6)
    samplog = rng.normal(loc=(mu - downshift * sd), scale=max(sd * width, 1e-12), size=1)[0]
    val_lin = 2.0 ** float(samplog)
    if not np.isfinite(val_lin) or val_lin <= 0:
        return float(np.min(pos) * 0.5)
    return float(val_lin)

def impute_cellwise_trb4(input_path: str,
                        expdes_path: str,
                        output_path: str,
                        seed: int = 0,
                        chi2_p: float = 0.05,
                        ...
                        perseus_downshift: float = 1.8,
                        perseus_width: float = 0.3):
    ...
    for feat_idx, missing_cols in feat_to_missing.items():
        ...
        for c in missing_cols:
            ...
            if cls == "MCAR":
                val = impute_cell_median(feature_vector)
                if val is None or not np.isfinite(val):
                    fb = float(global_med) if not math.isnan(global_med) else 0.0
                    X[feat_idx, c] = fb
                    log_entry.update({"imputed": fb, "method": "Median", "fallback": "global_median"})
                else:
                    X[feat_idx, c] = float(val)
                    log_entry.update({"imputed": float(val), "method": "Median"})
            elif cls == "MAR":
                try:
                    val = impute_cell_iterative_elasticnet(X, feat_idx, c, seed=seed, en_alpha=elasticnet_alpha, en_l1_ratio=elasticnet_l1_ratio)
                    if val is None or not np.isfinite(val):
                        raise ValueError("ElasticNet IterativeImputer produced invalid")
                    X[feat_idx, c] = float(val)
                    log_entry.update({"imputed": float(val), "method": "IterativeElasticNet"})
                except Exception:
                    if not math.isnan(stats["median"]):
                        fb = float(stats["median"])
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "IterativeElasticNet", "fallback": "feature_median"})
                    else:
                        col_vals = X[:, c]
                        ...
                        log_entry.update({"imputed": col_med, "method": "IterativeElasticNet", "fallback": "column_median"})
            else:  # MNAR -> Perseus downshifted normal
                try:
                    rng_cell = np.random.RandomState(seed + feat_idx * 1009 + c * 9176)
                    val = impute_cell_perseus_downshift(feature_vector, rng_cell, downshift=perseus_downshift, width=perseus_width)
                    if val is None or not np.isfinite(val):
                        raise ValueError("Perseus downshift invalid")
                    X[feat_idx, c] = float(val)
                    log_entry.update({"imputed": float(val), "method": "PerseusDownshift"})
                except Exception:
                    if not math.isnan(stats["min_pos"]):
                        fb = float(stats["min_pos"] * 0.5)
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "PerseusDownshift", "fallback": "min_pos*0.5"})
                    else:
                        fb = float(global_med) if not math.isnan(global_med) else 0.0
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "PerseusDownshift", "fallback": "global_median"})
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="trb4:(Median / ElasticNet / PerseusDownshift)")
    ...
    p.add_argument("--en-alpha", type=float, default=1.0, help="ElasticNet alpha")
    p.add_argument("--en-l1", type=float, default=0.5, help="ElasticNet l1_ratio")
    p.add_argument("--perseus-downshift", type=float, default=1.8, help="Perseus downshift multiplier (sd units)")
    p.add_argument("--perseus-width", type=float, default=0.3, help="Perseus downshift sampling width fraction")
    args = p.parse_args()

    impute_cellwise_trb4(args.i, args.d, args.o, seed=args.seed,
                        chi2_p=args.chi2_p, mnar_median_factor=args.mnar_median_factor,
                        elasticnet_alpha=args.en_alpha, elasticnet_l1_ratio=args.en_l1,
                        perseus_downshift=args.perseus_downshift, perseus_width=args.perseus_width)
