...
def safe_normal_rvs(rng: np.random.RandomState, loc: float, scale: float, size: int = 1) -> np.ndarray:
...
def impute_cell_mean(feature_vals: np.ndarray) -> Optional[float]:
...
def impute_cell_iterative_mice(X_numeric: np.ndarray, target_r: int, target_c: int, seed: int = 0) -> Optional[float]:
    ...
    imp = IterativeImputer(estimator=estimator, max_iter=10, random_state=seed, sample_posterior=False, imputation_order='ascending')
...
def impute_cell_minprob(feature_vals: np.ndarray, rng: np.random.RandomState, lower_q: float = 0.05, shift_frac: float = 0.5, sd_frac: float = 0.3) -> Optional[float]:
    ...
    cutoff = np.percentile(logpos, lower_q * 100.0)
    tail = logpos[logpos <= cutoff]
    if tail.size < 2:

        tail = np.sort(logpos)[:max(1, int(max(1, 0.1 * logpos.size)))]
    mu = float(np.nanmean(tail))
    sd = float(np.nanstd(tail, ddof=0))
    if not np.isfinite(sd) or sd == 0:
        sd = max(abs(mu) * 0.1, 1e-6)

    shift = shift_frac * sd
    draw = safe_normal_rvs(rng, loc=mu - shift, scale=max(sd * sd_frac, 1e-12), size=1)[0]
    val_lin = 2.0 ** float(draw)
    if not np.isfinite(val_lin) or val_lin <= 0:

        minpos = float(np.min(pos))
        return float(minpos * 0.5)
    return float(val_lin)

def impute_cellwise_trb3(input_path: str,
                        expdes_path: str,
                        output_path: str,
                        seed: int = 0,
                        chi2_p: float = 0.05,
                        ...
                        minprob_lower_q: float = 0.05,
                        minprob_shift_frac: float = 0.5,
                        minprob_sd_frac: float = 0.3):
    ...
    for feat_idx, missing_cols in feat_to_missing.items():
        ...
        for c in missing_cols:
            ...
            if cls == "MCAR":
                ...
                val = impute_cell_mean(feature_vector)
                if val is None or not np.isfinite(val):

                    val = float(global_med) if not math.isnan(global_med) else 0.0
                    log_entry.update({"imputed": val, "method": "Mean", "fallback": "global_median"})
                else:
                    log_entry.update({"imputed": float(val), "method": "Mean"})
                X[feat_idx, c] = float(val)
            elif cls == "MAR":
                try:
                    val = impute_cell_iterative_mice(X, feat_idx, c, seed=seed)
                    if val is None or not np.isfinite(val):
                        raise ValueError("MICE produced invalid")
                    X[feat_idx, c] = float(val)
                    log_entry.update({"imputed": float(val), "method": "MICE"})
                except Exception:

                    if not math.isnan(stats["median"]):
                        fb = float(stats["median"])
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "MICE", "fallback": "feature_median"})
                    else:
                        col_vals = X[:, c]
                        ...
                        log_entry.update({"imputed": col_med, "method": "MICE", "fallback": "column_median"})
            else:  # MNAR -> MinProb
                try:
                    rng_cell = np.random.RandomState(seed + feat_idx * 1009 + c * 9176)
                    val = impute_cell_minprob(feature_vector, rng_cell, lower_q=minprob_lower_q, shift_frac=minprob_shift_frac, sd_frac=minprob_sd_frac)
                    if val is None or not np.isfinite(val):
                        raise ValueError("MinProb invalid")
                    X[feat_idx, c] = float(val)
                    log_entry.update({"imputed": float(val), "method": "MinProb"})
                except Exception:
                    if not math.isnan(stats["min_pos"]):
                        fb = float(stats["min_pos"] * 0.5)
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "MinProb", "fallback": "min_pos*0.5"})
                    else:
                        fb = float(global_med) if not math.isnan(global_med) else 0.0
                        X[feat_idx, c] = fb
                        log_entry.update({"imputed": fb, "method": "MinProb", "fallback": "global_median"})
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="trb3: (Mean / MICE / MinProb)")
    ...
    p.add_argument("--minprob-lq", type=float, default=0.05, help="lower quantile fraction for MinProb (default 0.05)")
    p.add_argument("--minprob-shift", type=float, default=0.5, help="shift fraction for MinProb mean downshift")
    p.add_argument("--minprob-sdfrac", type=float, default=0.3, help="sd fraction for MinProb sampling")
    args = p.parse_args()

    impute_cellwise_trb3(args.i, args.d, args.o, seed=args.seed,
                        chi2_p=args.chi2_p, mnar_median_factor=args.mnar_median_factor,
                        minprob_lower_q=args.minprob_lq, minprob_shift_frac=args.minprob_shift,
                        minprob_sd_frac=args.minprob_sdfrac)
