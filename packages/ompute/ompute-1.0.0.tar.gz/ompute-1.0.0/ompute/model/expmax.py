...
def em_impute(X, max_iter=200, tol=1e-6, random_state=0):
    ...
    for j in range(p):
        X[mask[:, j], j] = col_means[j] if not np.isnan(col_means[j]) else 0.0
    ...
    for iteration in range(max_iter):
        ...
        cov += np.eye(p) * 1e-6
        for i in range(n):
            miss = np.isnan(X_old[i, :])
            obs = ~miss
            if np.any(miss):
                sigma_oo = cov[np.ix_(obs, obs)]
                sigma_mo = cov[np.ix_(miss, obs)]
                try:
                    sigma_oo_inv = np.linalg.inv(sigma_oo)
                except np.linalg.LinAlgError:
                    sigma_oo_inv = np.linalg.pinv(sigma_oo)
                X[i, miss] = mu[miss] + sigma_mo @ sigma_oo_inv @ (X[i, obs] - mu[obs])
        diff = np.nanmax(np.abs(X - X_old))
        if diff < tol:
            break
...
def impute(i: str, d: str, o: str):
    ...
    X_imputed = em_impute(X, max_iter=200, tol=1e-6, random_state=0)
