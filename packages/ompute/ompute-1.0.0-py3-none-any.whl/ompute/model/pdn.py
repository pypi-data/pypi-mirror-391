...
def impute(i: str, d: str, o: str):
    ...
    for col in sel_cols:
        ...
        use_mu = mu_col if not np.isnan(mu_col) else (mu_grp if not np.isnan(mu_grp) else global_mu)
        use_sd = sd_col if not np.isnan(sd_col) else (sd_grp if not np.isnan(sd_grp) else global_sd)

        for idx in work.index:
            if pd.notna(work.at[idx, col]):
                out.at[idx, col] = work.at[idx, col]
                continue
            if np.isnan(use_mu) or np.isnan(use_sd) or use_sd == 0:
                row_vals = work.loc[idx, sel_cols].dropna()
                row_pos = row_vals[row_vals > 0]
                if not row_pos.empty:
                    base = float(row_pos.min())
                    rep = base * 0.5
                    out.at[idx, col] = rep
                    continue
                if not np.isnan(global_mu) and not np.isnan(global_sd) and global_sd > 0:
                    ...
                    out.at[idx, col] = val if (np.isfinite(val) and val > 0) else max(FLOOR, loc if loc > 0 else FLOOR)
                    continue
