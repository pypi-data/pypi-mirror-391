...
def impute_cell_knn(X_numeric: np.ndarray, target_r: int, target_c: int, k: int = 3) -> Optional[float]:
...
def impute_cell_missforest(X_numeric: np.ndarray, target_r: int, target_c: int, seed: int = 0, rf_n_estimators: int = 100) -> Optional[float]:
...
def impute_cellwise_trb7(input_path: str, expdes_path: str, output_path: str, seed: int = 0,
                        chi2_p: float = 0.05, mnar_median_factor: float = 0.5,
                        knn_k: int = 3, rf_n_estimators: int = 100,
                        perseus_downshift: float = 1.8, perseus_width: float = 0.3):
    ...
    for feat_idx, missing_cols in feat_to_missing.items():
        ...
        for c in missing_cols:
            ...
            if cls == "MCAR":
                try:
                    val = impute_cell_knn(X, feat_idx, c, k=knn_k)
                    if val is None or not np.isfinite(val):
                        raise ValueError("KNN invalid")
                    ...
                    X[feat_idx, c] = fb
                    log_entry.update({"imputed": fb, "method": "KNN", "fallback": "feature_median_or_global"})
            elif cls == "MAR":
                try:
                    val = impute_cell_missforest(X, feat_idx, c, seed=seed, rf_n_estimators=rf_n_estimators)
                    if val is None or not np.isfinite(val):
                        raise ValueError("MissForest/IterativeRF invalid")
                    X[feat_idx, c] = float(val)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="trb7: (KNN / MissForest / PerseusDownshift)")
    ...
    p.add_argument("--rf-est", type=int, default=100, help="n_estimators for RF fallback")
    p.add_argument("--perseus-downshift", type=float, default=1.8, help="Perseus downshift (sd units)")
    p.add_argument("--perseus-width", type=float, default=0.3, help="Perseus width fraction")
    args = p.parse_args()

    impute_cellwise_trb7(args.i, args.d, args.o, seed=args.seed,
