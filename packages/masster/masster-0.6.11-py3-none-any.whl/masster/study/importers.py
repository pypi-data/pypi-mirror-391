...
def import_tima(self, folder, file="results_annotation"):
    ...
    id_columns = {
        "id_top_name": best_ids.select("consensus_uid", pl.col("label_compound_short").alias("label_compound")),
        "id_top_adduct": best_ids.select("consensus_uid", "adduct"),
        "id_top_score": best_ids.select("consensus_uid", pl.col("score").round(3).alias("score")),
        "id_source": best_ids.select("consensus_uid", "matcher"),
    }
    
    # Only add id_top_class if label_classyfire column exists
    if "label_classyfire" in best_ids.columns:
        id_columns["id_top_class"] = best_ids.select("consensus_uid", "label_classyfire")
