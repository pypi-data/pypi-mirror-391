...
def _original_insurance_type_selection_logic(parsed_data):
    ...
    # If API provided the code, treat it as source of truth (accept 1-3 alphanumeric)
    try:
        if str(parsed_data.get('insurance_type_source', '')) == 'API':
            if insurance_type_code and len(insurance_type_code) <= 3 and insurance_type_code.isalnum():
                MediLink_ConfigLoader.log("Insurance type (API) accepted: {}".format(insurance_type_code), level="DEBUG")
