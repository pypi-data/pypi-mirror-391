            ...
            if csv_dir:
                ...
                for idx, data in enumerate(detailed_patient_data):
                    try:
                        ...
                        lookup_count += 1
                        # Log lookup attempt (PATID is the 5-digit numeric ID)
                        MediLink_ConfigLoader.log("Cache lookup attempt {}: patid='{}'".format(
                            idx + 1, patid), level="INFO")
                        api_code = cache_lookup(patient_id=patid, csv_dir=csv_dir)
                        if api_code:
                            success_count += 1
                            MediLink_ConfigLoader.log("Cache lookup SUCCESS for patid='{}': code='{}'".format(patid, api_code), level="INFO")
                            data['insurance_type'] = api_code
                            data['insurance_type_source'] = 'API'
                        else:
                            MediLink_ConfigLoader.log("Cache lookup MISS for patid='{}' (not found in cache)".format(patid), level="INFO")
                    except Exception as e:
