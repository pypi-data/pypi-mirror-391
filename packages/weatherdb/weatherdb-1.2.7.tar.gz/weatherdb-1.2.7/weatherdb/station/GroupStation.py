...
class GroupStation(object):
    ...
    def get_df(self, period=(None, None), kinds="best", paras="all",
        ...
        if len(dfs) > 1:
            ...
            raise ValueError(f"No timeserie was found for {', '.join(paras)} and Station {self.id}")
    ...
    def create_ts(self,
        ...
        **kwargs:
        ...
        # prepare loop
        name_suffix = f"_{self.id:0>5}.txt"
        x, y = self.get_geom().coords.xy
        name = self.get_name() + f" (ID: {self.id})"
        ...
        if isinstance(dir, Path):
            if dir.is_dir():
                if len(list(dir.iterdir())) > 0:
                    raise ValueError(f"The given directory '{dir}' is not empty.")
            elif dir.suffix == "":
                dir.mkdir()
            else:
                raise ValueError(f"The given directory '{dir}' is not a directory.")
        elif not isinstance(dir, zipfile.ZipFile):
            raise ValueError(f"The given directory '{dir}' is not a directory or zipfile.")
        return dir
