...
class GroupStations(object):
        ...
        if isinstance(dir, zipfile.ZipFile):
            ...
            if dir.is_dir():
                if len(list(dir.iterdir())) > 0:
                    raise ValueError(
                        f"The given directory '{dir}' is not empty.")
            elif dir.suffix == "":
                dir.mkdir()
            elif dir.suffix == ".zip":
                if not dir.parent.is_dir():
                    raise ValueError(
                        f"The given parent directory '{dir.parent}' of the zipfile is not a directory.")
            else:
                raise ValueError(
                    f"The given directory '{dir}' is not a directory.")
        else:
            raise ValueError(
                f"The given directory '{dir}' is not a directory or zipfile.")
        return dir
    ...
    def create_ts(self, dir,
        ...
        **kwargs:
        ...
        # create log message
        log.debug(
            f"The timeseries tables for {len(stids)} stations got created in {dir}")
