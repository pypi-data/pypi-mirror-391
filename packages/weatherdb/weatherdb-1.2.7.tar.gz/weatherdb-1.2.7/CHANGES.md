...
- add cli command to unlock the keyring
- add keyring.cryptfile as an alternative keyring.backend. It is possible to store the cryptfile key in the user configuration or in environment variables to make the script work without user inputs
- fix error on creating a user config via command line without file handed
- reworked multiprocessing to also stream logs
- fix error in check_min_date
