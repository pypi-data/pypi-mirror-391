...
class StationBase:
    ...
    def _check_isin_meta(self):
            if self.isin_meta():
                ...
                raise NotImplementedError(f"""
                    The given {self._para_long} station with id {self.id}
                    is not in the corresponding meta table in the DB""")
    ...
    def _check_kind_tstp_meta(cls, kind):
        ...
        if kind not in cls._valid_kinds_tstp_meta:
            raise NotImplementedError(textwrap.dedent(f"""\
                The given kind "{kind}" is not a valid kind.
                Must be one of "{'", "'.join(cls._valid_kinds_tstp_meta)}"
                """))
    ...
    def _check_period(self, period, kinds, nas_allowed=False):
        ...
        if max_period.is_empty():
            raise ValueError(
                f"No maximum period was found for the {self._para_long} Station with ID {self.id} and kinds '{', '.join(kinds)}'.")
    ...
    def _check_agg_to(self, agg_to):
        ...
        if agg_to not in agg_to_valid:
            raise ValueError(
                f"The given agg_to Parameter \"{agg_to}\" is not a valid aggregating period. Please use one of:\n"+
                ", ".join([str(item) for item in agg_to_valid])
                )
    ...
    def _check_min_date(self, **kwargs):
        ...
        with db_engine.connect() as con:
            ...
            if sa.inspect(db_engine.engine).has_table(self._table.name, schema='timeseries'):
    ...
    def _update_db_timeserie(self, df, kinds):
        ...
        if len(df) == 0:
            log.debug("The _update_db_timeserie method got an empty df " +
                    f"for the {self._para_long} Station with ID {self.id}")
            return None
        else:
            self._create_timeseries_table()
        ...
        with db_engine.connect() as con:
            ...
            for group in groups:
                ...
                sql_insert = f'''
                    INSERT INTO timeseries."{self.id}_{self._para}" (timestamp, "{'", "'.join(kinds)}")
                    Values {values}
                    ON CONFLICT (timestamp) DO UPDATE SET
                '''

                for kind_i in kinds:
                    sql_insert += f'"{kind_i}" = EXCLUDED."{kind_i}",'
    ...
    def _execute_long_sql(self, sql, description="treated"):
        ...
        while not done:
            ...
            try:
                ...
                log_msg = f"There was an operational error for the {self._para_long} Station (ID:{self.id})" + \
        ...
        if done:
            log.info(
                f"The {self._para_long} Station ({self.id}) got successfully {description}.")
        else:
            raise Exception(
                f"The {self._para_long} Station ({self.id}) could not get {description}.")
    ...
    def isin_db(self):
        ...
        sql = f"""
            select '{self.id}_{self._para}' in (
                select table_name
    ...
    def update_raw(self, only_new=True, ftp_file_list=None, remove_nas=True, **kwargs):
        ...
        **kwargs : dict
        ...
        if len(selection_without_na) == 0:
            log_msg = f"raw_update of {self._para_long} Station {self.id}: " + \
    ...
    def fillup(self, period=(None, None), **kwargs):
        ...
        **kwargs : dict, optional
        ...
        if len(self._ma_terms) == 1:
            sql_format_dict.update(dict(
                is_winter_col="",
                coef_calc="ma_stat.{ma_term}{coef_sign}ma_other.{ma_term}::float AS coef"
                .format(
                    ma_term=self._ma_terms[0],
                    coef_sign=self._coef_sign[0]),
                coef_format="i.coef",
            ...
            sql_format_dict.update(dict(
                is_winter_col=""",
                    ...
                    END AS is_winter""",
                coef_calc=(
                    "ma_stat.{ma_terms[0]}{coef_sign}ma_other.{ma_terms[0]}::float AS coef_wi, \n" + " "*24 +
                    "ma_stat.{ma_terms[1]}{coef_sign}ma_other.{ma_terms[1]}::float AS coef_so"
                ).format(
    ...
    def get_df(self, kinds, period=(None, None), agg_to=None,
        ...
        if agg_to is not None:
            ...
            timestamp_col = f"date_trunc('{agg_to}', timestamp)"
    ...
    def plot(self, period=(None, None), kind="filled", agg_to=None, **kwargs):
        ...
        df.plot(
            ...
            title=f"{self._para_long} Station {self.id}",
            **kwargs
        )
