        ...
        sql_check_d = f"""
            SELECT EXISTS(
                ...
                WHERE table_schema = 'timeseries'
                    AND table_name = '{self.id}_{self._para}_d'
            );"""
    ...
    def richter_correct(self, period=(None, None), **kwargs):
        ...
        **kwargs : dict, optional
        ...
        if richter_class is None:
            raise Exception(f"No richter class was found for the precipitation station {self.id} and therefor no richter correction was possible.")
    ...
    def last_imp_richter_correct(self, _last_imp_period=None, **kwargs):
        ...
        **kwargs : dict, optional
        ...
        if not self.is_last_imp_done(kind="corr"):
            ...
            log.info(f"The last import of {self._para_long} Station {self.id} was already richter corrected and is therefor skiped")
