...
def setup_logging_handlers():
    ...
    for handler_type in config.get_list("logging", "handlers"):
        ...
        new_handler_names.append(handler_name)
        ...
        if handler_name not in handler_names:
            ...
            log.debug(f"Added logging handler '{handler_name}'")
            ...
            if handler_type == "file" and handler.baseFilename != str(log_file):
                ...
                handler.set_name(handler_name)
                log.addHandler(handler)
                log.debug(f"Updated logging file handler '{handler_name}'")
    ...
    # remove old handlers
    log.debug(f"Current logging handlers: " +
              ', '.join([h.get_name() if h.get_name() else 'noname' for h in log.handlers]))
    for handler in log.handlers:
        if handler.get_name()=="weatherDB_config:stream":
            log.removeHandler(handler)
            log.debug(f"Removed logging handler '{handler.get_name()}'")
            handler.close()
