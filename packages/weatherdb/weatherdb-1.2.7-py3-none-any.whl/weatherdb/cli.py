...
def create_user_config(file, on_exists):
    weatherdb.config.create_user_config(
        user_config_file="ask" if file is None else file,
        on_exists=on_exists)
...
@cli.command(short_help="unlock the keyring backend.")
def unlock_keyring():
    click.echo("unlocking the keyring backend")
    if weatherdb.config._check_keyring_backend(log_errors=True):
        click.echo("keyring backend seems to be working properly already --> nothing to do")
    else:
        import subprocess as sp
        import os
        pid = sp.check_output(
            "echo $(pgrep -u max.schmit bash | head -n 1)",
            shell=True
            ).decode().split()[0]
        os.environ["DBUS_SESSION_BUS_ADDRESS"] = sp.check_output(
            f"echo $(grep -z DBUS_SESSION_BUS_ADDRESS /proc/{pid}/environ | sed 's/DBUS_SESSION_BUS_ADDRESS=//' )",
            shell=True
            ).decode().strip()
        if weatherdb.config._check_keyring_backend(log_errors=True):
            click.echo("keyring backend seems to be working properly now")
        else:
            click.echo("keyring backend still not working properly", err=True)
