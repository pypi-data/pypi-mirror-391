...
def download(file: Path | str, output_dir: Path | str) -> Path:
    ...
    # Load STAC metadata from remote/local JSON
    item = load_stac_item(file)

    # Save the JSON metadata file locally
    ...
    for name, source in tqdm(to_download.items(), desc="Downloading files"):
        try:
            # ✅ FIX: Pass only the directory, not the full path
    ...
    # Return ModelLoader pointing to the downloaded JSON
...
class ModelLoader:
    ...
    def __init__(self, file: str | list):
        ...
        if self.scheme == "pt2_list":
            # Direct loading from .pt2 list
            self.item = self._create_minimal_stac_from_pt2s(file)
            self.source = str(Path(file[0]).parent)  # Use parent of first model
            print(f"⚠️  Loading ad-hoc ensemble from {len(file)} .pt2 files (no metadata)")
    ...
    def is_ensemble(self) -> bool:
        """Check if this is an ensemble model based on STAC metadata"""
        name = self.item.properties.get("mlm:name", "").lower()
        arch = self.item.properties.get("mlm:architecture", "").lower()
        
        # Check multiple indicators
        return (
            "ensemble" in name or 
            "ensemble" in arch or
            sum(1 for k in self.item.assets.keys() if k.startswith("model_")) > 1
        )
    ...
    def _create_minimal_stac_from_pt2s(self, model_paths: list) -> pystac.Item:
        ...
        # Create assets dict - MUST BE PLAIN DICTS, NOT pystac.Asset objects
        assets = {}
        for i, model_path in enumerate(sorted(model_paths), start=1):
            asset_key = f"model_{i}_{model_path.stem}"
            assets[asset_key] = {  # ✅ Plain dict, not pystac.Asset()
                "href": str(model_path.absolute()),
                "type": "application/octet-stream",  # ✅ "type", not "media_type"
                "title": f"Model {i}: {model_path.name}",
                "roles": ["mlm:model", "mlm:weights"]
            }
        
        # Minimal STAC item - ONLY what we know for sure
        stac_dict = {
            "type": "Feature",
            "stac_version": "1.1.0",
    ...
    def print_schema(self) -> None:
        ...
        description = self.item.properties.get("description", "No description available")
        ...
        deps_str = ", ".join([d.split(">=")[0] for d in dependencies[:3]]) if dependencies else "None"

        if in_notebook:
            from IPython.display import HTML, display
            
            # CAMBIO AQUÍ: Diseño "Ultra-Compacto"
            html_content = f"""
            <style>
                .mlstac-container {{
                    font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
                    /* CAMBIO DE COLOR AQUÍ: Tono azulado */
                ...
                .mlstac-card {{
                    ...
                    /* CAMBIO 3D: Añadida transición para suavizar el efecto */
                    transition: transform 0.2s ease-out, box-shadow 0.2s ease-out;
                }}
                /* CAMBIO 3D: Efecto hover */
                .mlstac-card:hover {{
                ...
                .mlstac-footer a {{
                    ...
                    transition: border-color 0.2s; /* Transición para link hover */
                }}
                .mlstac-footer a:hover {{
                    border-bottom-color: white; /* Hover más pronunciado */
                }}
                .icon {{
                    font-size: 16px;
                }}
            ...
            <div class="mlstac-container">
                ...
                <div class="mlstac-footer">
                    {'<strong>' + project + '</strong> | ' if project else ''}
                    <a href="{project_url}" target="_blank">Project Info</a>
                    {' | <a href="' + links.get('license', '#') + '" target="_blank">License</a>' if 'license' in links else ''}
                    | Source: <strong>{self.scheme.upper()}</strong>
                    
                    | Status: <strong>{self.status.capitalize()}</strong>
            ...
            print(f"   Framework:    {YELLOW}{framework} {framework_version[:10]}{RESET}")
        ...
        try:
            with fsspec.open(self.file, "r") as f:
                ...
                for key, value in mlm_data["assets"].items():
                    ...
                    mlm_data["assets"][key]["href"] = (Path(self.file).parent / filename).absolute().as_posix()
    ...
    def compiled_model(self, **kwargs):
        ...
        For ensembles: Accepts mode='mean'|'max'|'min' (default: 'max')
        ...
        # ✅ Only reload if scheme is "local" (not pt2_list, it's already loaded)
        ...
        # Set default mode for ensembles if not provided
    ...
    def predict_large(self, image: 'np.ndarray', model: 'torch.nn.Module' = None, **kwargs):
        """
        ...
        return self.module.predict_large(
            image=image,
            model=model,
            **kwargs
        )
    ...
    def display_results(self, *args, **kwargs) -> Any:
        ...
        # ✅ Only reload if scheme is "local"
        if self.scheme == "local":
            self.item = self._load()
        ...
        try:            
            return self.module.display_results(Path(self.source), stac_item=self.item, *args, **kwargs)
        except KeyError:
