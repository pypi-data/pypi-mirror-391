...
from ase_db_backends._sqlite import all_tables

dbtypes = ['postgresql', 'mysql', 'mariadb']


@pytest.mark.parametrize('dbtype', dbtypes)
def test_create_and_delete_ext_tab(get_db_name, dbtype):
...
@pytest.mark.parametrize('dbtype', dbtypes)
def test_insert_in_external_tables(get_db_name, dbtype):
    atoms = Atoms()
...
@pytest.mark.parametrize('dbtype', dbtypes)
def test_extract_from_table(get_db_name, dbtype):
    atoms = Atoms()
...
@pytest.mark.parametrize('dbtype', dbtypes)
def test_write_atoms_row(get_db_name, dbtype):
    atoms = Atoms()
...
@pytest.mark.parametrize('dbtype', dbtypes)
def test_external_table_upon_update(get_db_name, dbtype):
    name = get_db_name(dbtype)
    db = connect(name)
    no_features = 500
...
@pytest.mark.parametrize('dbtype', dbtypes)
def test_external_table_upon_update_with_float(get_db_name, dbtype):
    name = get_db_name(dbtype)
    db = connect(name)
