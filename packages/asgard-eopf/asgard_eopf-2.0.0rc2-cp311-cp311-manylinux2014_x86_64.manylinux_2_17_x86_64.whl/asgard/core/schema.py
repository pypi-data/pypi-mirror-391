...
DEM_DATASET_SCHEMA = {
    "oneOf": [
        ...
        {"type": "zarr.storage.FsspecStore", "description": "Store pointing to a remote to Zarr dataset"},
...
def _is_zarr_fsstore(checker, instance) -> bool:  # pylint: disable=unused-argument
    """
    Check if a given instance is a zarr.storage.FsspecStore
    """
    return isinstance(instance, FsspecStore)
    ...
    type_checker = _OldValidator.TYPE_CHECKER.redefine_many(
        {
            ...
            "zarr.storage.FsspecStore": _is_zarr_fsstore,
