...
def remote_store(path: str) -> FsspecStore:
    # https://zarr.readthedocs.io/en/latest/user-guide/v3_migration/
    # https://github.com/zarr-developers/zarr-python/issues/2988
    return FsspecStore.from_url(path, storage_options(path), read_only=True)


def setup_remote_dem(dem_filename: str) -> FsspecStore:
    """
    Create a FsspecStore pointing to a DEM stored remotely on S3
    """
...
def setup_remote_dem_geolib_input(dem_filename: str) -> FsspecStore:
    """
    Create a FsspecStore pointing to a DEM stored remotely on S3
    """
