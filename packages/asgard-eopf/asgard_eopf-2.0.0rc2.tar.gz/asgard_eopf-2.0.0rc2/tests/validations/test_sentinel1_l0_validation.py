...
def get_start_stop_time(path: str) -> tuple[str, str]:
    ...
    return find_text(root, path_start, namespace), find_text(root, path_stop, namespace)
...
def get_beam_nominal_angles(path: str) -> dict[str, dict[str, float]]:
    ...
    for node in root.iterfind("calibrationParamsList/calibrationParams"):
        ...
        swath_name = find_text(node, "swath")
        ...
        if swath_name != name:
            ...
            res.update(
                {
                    name: {
                        "beamNominalNearRange": float(find_text(node, "elevationAntennaPattern/beamNominalNearRange")),
                        "beamNominalFarRange": float(find_text(node, "elevationAntennaPattern/beamNominalFarRange")),
                    }
                }
            )
