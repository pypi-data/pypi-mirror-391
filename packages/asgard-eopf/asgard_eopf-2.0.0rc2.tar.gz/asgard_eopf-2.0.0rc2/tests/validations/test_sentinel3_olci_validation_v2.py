...
from asgard.core.toolbox import load_gzip_npy, save_gzip_json
...
@pytest.mark.slow
@pytest.mark.dem
def test_sun_angles_all_cam(olci: S3OLCIGeometry, dataset_name, breakpoint_gr, range_idx, img_coord):  # pylint: disable=too-many-locals
    """
...
def test_tie_point_grid(olci_config: dict, dataset_name: str):
    ...
    np.testing.assert_allclose(tp_geo_coords[0:rows, :, :], grid[:, :, :-1], rtol=0, atol=0.00243)  # legacy atol=3e-6

    ground_track_grid.compute_along_track_coordinates()
    # np.savez("378-ground_track_grid-track_points.npz", gnd_coords=ground_track_grid.track_points)
