...
def test_slstr_quasi_cartesian_grid(slstr):
    ...
    logging.info(f"QC grid direct_loc speed: {coordinates.size * 0.5 / (tac-tic):.1f}")
...
def test_slstr_direct_loc_204936_perf(slstr_204936, img_bloc_coords):
    ...
    tac = time.perf_counter()
    logging.info(f"SLSTR direct_loc on DEM speed: {img_bloc_coords.size / (tac-tic):.1f}")
    ...
    tac = time.perf_counter()
    logging.info(f"SLSTR direct_loc at constant height speed: {img_bloc_coords.size / (tac-tic):.1f}")
