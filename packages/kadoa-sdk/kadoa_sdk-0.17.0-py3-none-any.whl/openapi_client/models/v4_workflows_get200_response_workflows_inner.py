...
class V4WorkflowsGet200ResponseWorkflowsInner(BaseModel):
    ...
    def display_state_validate_enum(cls, value):
        ...
        if value not in set(['ACTIVE', 'ERROR', 'PAUSED', 'NOT_SUPPORTED', 'PREVIEW', 'COMPLIANCE_REVIEW', 'COMPLIANCE_REJECTED', 'QUEUED', 'SETUP', 'RUNNING', 'FAILED', 'DELETED']):
            raise ValueError("must be one of enum values ('ACTIVE', 'ERROR', 'PAUSED', 'NOT_SUPPORTED', 'PREVIEW', 'COMPLIANCE_REVIEW', 'COMPLIANCE_REJECTED', 'QUEUED', 'SETUP', 'RUNNING', 'FAILED', 'DELETED')")
        return value
