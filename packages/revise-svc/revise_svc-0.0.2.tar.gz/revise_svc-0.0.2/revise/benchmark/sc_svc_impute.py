...
class ScSvcImpute(BenchmarkSvc):
    """
    Single-cell SVC imputation for benchmark scenarios.
    ...
    def _adata_processing_dec(self):
        """
        Process data for deconvolution with transcript count filtering.
        """
        if "cell_id" in self.st_adata.obs.columns:
            self.st_adata.obs_names = self.st_adata.obs["cell_id"]
        adata_sp = self.st_adata[self.st_adata.obs['transcript_counts'] >= self.config.prep_min_counts, :]
        sc.pp.filter_genes(adata_sp, min_cells=self.config.prep_min_cells)
    ...
    def reconstruct(self, *args, **kwargs):
        ...
        if os.path.exists(in_panel_file) and os.path.exists(all_panel_file):
            ...
            adata_sc_in_panel.write(os.path.join(self.config.result_dir, "adata_sc_in_panel.h5ad"))
