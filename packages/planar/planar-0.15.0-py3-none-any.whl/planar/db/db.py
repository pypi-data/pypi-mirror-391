...
from sqlalchemy import Connection, MetaData, event, insert, make_url, text
...
class PlanarSession(AsyncSession):
    ...
    def upsert[T: SQLModel](
        ...
        """Create an upsert statement for inserting records with conflict handling.

        Creates a database-specific upsert statement that inserts records while
        handling conflicts based on the database backend. For PostgreSQL, uses
        ON CONFLICT DO NOTHING. For SQLite, uses OR IGNORE.
        ...
        Args:
            ...
            conflict_columns: Optional list of column names to check for conflicts.
                Only used in PostgreSQL to specify which columns trigger conflict
                detection. If None, conflicts are detected on any constraint.
        ...
        Example:
            ...
            stmt = session.upsert(User, users, conflict_columns=["email"])
