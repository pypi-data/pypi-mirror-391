...
class MapDlg(NodeDlg):
    ...
    def updateNode(self, index=QModelIndex()):
        ...
        selects = self.node.firstChildElement(str("selections"))
        if not self.selections and selects \
                and selects.nodeName() == "selections":
            self.removeElement(selects, mindex)
        elif self.selections:
            newDimens = self.root.createElement(str("selections"))
            newDimens.setAttribute(str("rank"),
                                   str(unicode(self.rank)))
            dimDefined = True
            ...
            if dimens and dimens.nodeName() == "selections":
                self.replaceElement(dimens, newDimens, mindex)
            else:
                self.appendElement(newDimens, mindex)
