...
from aria_mps_cli.cli_lib.common import Config, CustomAdapter
...
class SingleRecordingModel:
    ...
    async def on_enter_DEVICE_TYPE_CHECK(self, event: EventData) -> None:
        ...
        """
        self._logger.debug(event)
        self._device_version_checker: DeviceVersionCheckRunner = (
            await DeviceVersionCheckRunner.get(self._recording)
        )
        self._recording.device_type = await self._device_version_checker.run()
        self._logger.debug(f"Device type: {self._recording.device_type}")
        if self._recording.device_type is MpsAriaDevice.UNKNOWN:
            self._logger.error("Unknown device type")
    ...
    async def on_enter_VALIDATION(self, event: EventData) -> None:
        ...
        if not self._recording.health_check_path.is_file():
            self._logger.error(
                f"Failed to run VrsHealthCheck for {self._recording.path}"
            )
            self._error_code = ErrorCode.HEALTH_CHECK_FAILURE
        else:
            self._error_code = validate_recording_eligibility(
                self._recording, self._feature
            )
