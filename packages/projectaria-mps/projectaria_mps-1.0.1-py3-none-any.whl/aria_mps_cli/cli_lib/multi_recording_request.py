...
from aria_mps_cli.cli_lib.base_state_machine import BaseStateMachine
from aria_mps_cli.cli_lib.common import Config
...
from aria_mps_cli.cli_lib.device_version_check import DeviceVersionCheckRunner
from aria_mps_cli.cli_lib.eligibility_check import validate_recording_eligibility
from aria_mps_cli.cli_lib.encryption import VrsEncryptor
from aria_mps_cli.cli_lib.hash_calculator import HashCalculator
from aria_mps_cli.cli_lib.health_check import HealthCheckRunner
...
from aria_mps_cli.cli_lib.uploader import check_if_already_uploaded, Uploader
...
class MultiRecordingRequest(BaseStateMachine):
    ...
    TRANSITIONS: Final[List[List[Any]]] = [
        ...
        ["next", States.DEVICE_TYPE_CHECK, States.PAST_OUTPUTS_CHECK],
...
class MultiRecordingModel:
    ...
    async def on_enter_DEVICE_TYPE_CHECK(self, event: EventData) -> None:
        """
        Check device type for all recordings
        """
        logger.debug(event)
        device_check_tasks: List[asyncio.Task] = []

        async def _check_device_type(rec: AriaRecording) -> None:
            device_version_checker: DeviceVersionCheckRunner = (
                await DeviceVersionCheckRunner.get(rec)
            )
            rec.device_type = await device_version_checker.run()
            logger.debug(f"Device type for {rec.path}: {rec.device_type}")
            if rec.device_type is MpsAriaDevice.UNKNOWN:
                logger.error(f"Unknown device type for {rec.path}")
                self._error_codes[rec.path] = ErrorCode.DEVICE_TYPE_UNSUPPORTED

        for rec in self._recordings:
            device_check_tasks.append(asyncio.create_task(_check_device_type(rec)))

        await asyncio.gather(*device_check_tasks)
        await self.next()
    ...
    async def on_enter_VALIDATION(self, event: EventData) -> None:
        ...
        async def _health_check(rec: AriaRecording) -> None:
            ...
            if not rec.health_check_path.is_file():
                ...
                error_code = validate_recording_eligibility(rec, self._feature)
                if error_code:
                    self._error_codes[rec.path] = error_code
                    raise VrsHealthCheckError()
