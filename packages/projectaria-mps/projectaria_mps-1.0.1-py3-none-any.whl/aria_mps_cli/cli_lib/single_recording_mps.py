...
from aria_mps_cli.cli_lib.single_recording_request import (
    ...
    async def _process_through_requestor(self) -> None:
        ...
        while requestor_tasks:
            ...
            for t in done:
                ...
                # Cache device type from the first completed model
                if self._device_type is None and model.recording.device_type:
                    self._device_type = model.recording.device_type
    ...
    def get_device_type(self):
        ...
        for model in self._model_by_feature.values():
            if isinstance(model, SingleRecordingModel):
                if model.recording.device_type is not None:
                    self._device_type = model.recording.device_type
                    return self._device_type
