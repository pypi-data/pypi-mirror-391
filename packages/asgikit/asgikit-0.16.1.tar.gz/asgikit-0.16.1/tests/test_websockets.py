...
async def test_iter():
    ...
    _events = iter([
        ...
        {"type": "websocket.disconnect", "code": "1000", "reason": "client disconnect"},
    ])
...
async def test_close_already_closed_should_fail():
    scope = {"type": "websocket", "headers": []}
...
async def test_close_websocket_not_yet_accepted():
    scope = {"type": "websocket", "headers": []}
    ...
    assert websocket.ws_state == WebSocket.State.NEW

    await websocket.close()

    assert websocket.ws_state == WebSocket.State.CLOSED
