...
class JupyterLabSubmission(NotebookExtractor):
    ...
    def setup_ui(self):
        if self.use_sidecar:
            ...
            self.sidecar = None
            # For inline mode, we'll display the layout directly (not using Output widget)
            # Output widgets are for display only, not for interactive widgets
        self.create_widgets()
        self.setup_layout()
    ...
    def setup_layout(self):
        layout = widgets.VBox(
            [
                ...
                widgets.HBox(
                    [self.submission_start_date_input, self.benchmark_input],
                    layout=widgets.Layout(margin="0 0 10px 0"),
                ),
        ...
        if self.use_sidecar and self.sidecar is not None:
            ...
            # Display directly in cell output (not using Output widget)
            # Interactive widgets need to be at top level for events to work
            display(layout)
    ...
    def submit(self, model_name=None, model_universe=None, gpu=None, bypass_validate=None, ignore_local_validation=None, submission_start_date=None, benchmark=None):
        self.reinitialize(
    ...
    def submit_model(self):
        ...
        if self.submission_start_date_input.value.strip():
            try:
                self.model_info["start"] = int(self.submission_start_date_input.value.strip())
            except ValueError:
                raise ValueError(f"Invalid start date format: {self.submission_start_date_input.value}. Expected YYYYMMDD as integer.")
        ...
        with self.status_output:
            ...
            if self.submission_start_date_input.value.strip():
                print(f"Start Date: {self.submission_start_date_input.value.strip()}")
            if self.benchmark_input.value.strip():
    ...
    def proceed_with_submission(self):
        ...
        with self.status_output:
            ...
            # Perform local validation unless ignored
