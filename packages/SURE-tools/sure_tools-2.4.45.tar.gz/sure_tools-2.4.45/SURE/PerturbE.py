...
class PerturbE(nn.Module):
    ...
    def model1(self, xs):
        ...
        with pyro.plate('data'):
            ...
            if self.loss_func == 'negbinomial':
                ...
                pyro.sample('x', dist.Multinomial(total_count=int(1e8), logits=log_mu), obs=xs)
    ...
    def predict(self, xs, perturbs_us, library_sizes=None):
        ...
        dzs = self.get_cell_shift(perturbs_us)          
        zs = zs + dzs
    ...
    def _get_log_mu(self, delta_zs):
        return self.decoder_log_mu(delta_zs)
    ...
    def get_log_mu(self, 
        ...
        with tqdm(total=len(dataloader), desc='', unit='batch') as pbar:
            for delta_Z_batch, _ in dataloader:
                r = self._get_log_mu(delta_Z_batch)
    ...
    def get_counts(self, zs, library_sizes,
        ...
        with tqdm(total=len(dataloader), desc='', unit='batch') as pbar:
            for Z_batch, L_batch, _ in dataloader:
                log_mu = self._get_log_mu(Z_batch)
