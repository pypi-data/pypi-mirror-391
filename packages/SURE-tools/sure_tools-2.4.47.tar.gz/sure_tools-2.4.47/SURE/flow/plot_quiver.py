...
def plot_quiver(z_points, delta_z, method='pca', figsize=(6,4), dpi=200, 
    ...
    if (original_color_by is not None) and (unique_labels is not None):
        for label in unique_labels:
            ...
            plt.scatter(z_2d_all[ind, 0], z_2d_all[ind, 1], 
                   c=colors_all[ind], alpha=point_alpha, s=point_size, label=label)
    else:
        plt.scatter(z_2d_all[:, 0], z_2d_all[:, 1], 
                   c=colors_all, alpha=point_alpha, s=point_size, label='All data points')
    ...
    plt.legend(
        ...
        markerscale=min(2*point_size,5)           # 关键：调整图例中点的大小（2倍）
        )
    plt.tight_layout()
    plt.show()
