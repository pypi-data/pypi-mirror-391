...
from shared_memory_lock import SharedMemoryMutex
...
class TestAsyncMutex:
    """Async mutex functionality tests."""

    @pytest.mark.asyncio
    async def test_async_acquire_release_basic(self, check, unique_run_id, mutex_name):
        """Test basic async acquire and release operations."""
        mutex = SharedMemoryMutex(name=mutex_name, create=True, run_id=unique_run_id)

        try:
            # Should be able to acquire
            result = await mutex.acquire_async()
    ...
    @pytest.mark.asyncio
    async def test_async_context_manager(self, check, unique_run_id, mutex_name):
        """Test async mutex works correctly as async context manager."""
        mutex = SharedMemoryMutex(name=mutex_name, create=True, run_id=unique_run_id)
    ...
    @pytest.mark.asyncio
    async def test_async_acquire_timeout(self, check, unique_run_id, mutex_name):
        """Test async acquire with timeout."""
        mutex = SharedMemoryMutex(name=mutex_name, create=True, run_id=unique_run_id)

        try:
            # Acquire mutex first (synchronously)
            mutex.acquire()

            # Async acquire with timeout should fail after timeout
            timeout_duration = 0.2
            start_time = time.time()
            result = await mutex.acquire_async(timeout=timeout_duration)
            elapsed = time.time() - start_time

            check.is_false(result, "Timed async acquire on held mutex should fail")
            check.greater_equal(
                elapsed, timeout_duration * 0.9, "Should wait at least most of timeout"
            )
            check.less(
    ...
    @pytest.mark.asyncio
    async def test_async_concurrent_tasks(self, check, unique_run_id, mutex_name):
        """Test multiple async tasks competing for the same mutex."""
        mutex = SharedMemoryMutex(name=mutex_name, create=True, run_id=unique_run_id)

        try:
            counter = {"value": 0}

            async def increment_with_mutex(task_id: int, iterations: int):
                """Task that increments counter with mutex protection."""
            ...
            tasks = [increment_with_mutex(i, iterations_per_task) for i in range(num_tasks)]

            await asyncio.gather(*tasks)

            # If mutex works correctly, counter should be exactly num_tasks * iterations_per_task
            expected = num_tasks * iterations_per_task
            check.equal(
                counter["value"],
                expected,
                f"Counter should be {expected} if mutex provides mutual exclusion",
            )
    ...
    @pytest.mark.asyncio
    async def test_async_acquire_no_timeout(self, check, unique_run_id, mutex_name):
        """Test async acquire without timeout on free mutex."""
        mutex = SharedMemoryMutex(name=mutex_name, create=True, run_id=unique_run_id)

        try:
            # Acquire without timeout should succeed immediately on free mutex
            start_time = time.time()
            result = await mutex.acquire_async(timeout=None)
            elapsed = time.time() - start_time

            check.is_true(result, "Async acquire should succeed on free mutex")
