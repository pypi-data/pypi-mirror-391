...
class SharedMemoryMutex:
    ...
    def _compare_and_swap(self, expected: int, new: int) -> bool:
        """
        Atomic compare-and-swap operation using CPU-level atomics.

        Returns:
            True if swap succeeded, False otherwise
        """
        result = self._atomic.cmpxchg_weak(expected, new)
    ...
    async def acquire_async(self, timeout: float | None = None) -> bool:
        ...
        This provides async/await support for long waits without blocking the event loop.
        Uses asyncio.sleep() to yield control while waiting for the mutex.
        ...
        while True:
            ...
            # For async, we can't use futex_wait directly as it blocks the thread.
            # Instead, we poll with small async sleeps to yield to the event loop.
            # This is less efficient than sync acquire but necessary for async.
            await asyncio.sleep(0.001)  # 1ms sleep, yield to event loop

            # Try to acquire after sleeping
            if self._compare_and_swap(UNLOCKED, LOCKED):
                return True
