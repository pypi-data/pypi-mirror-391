...
from biolib._internal.utils.resource_uri import parse_resource_uri
...
class BioLibApp:
    def __init__(
        ...
        if not suppress_version_warning:
            parsed_uri = parse_resource_uri(uri)
...
""")
    ...
    def _run_locally(self, module_input_serialized: bytes) -> Result:
        job_dict = BiolibJobApi.create(
            ...
            app_resource_name_prefix=parse_resource_uri(self._app_uri)['resource_name_prefix'],
        )
        job = Result(job_dict)
