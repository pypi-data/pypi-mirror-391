...
var RulersGuides = function (evt, dragdrop) {
  ...
  var doc = document.documentElement,
    ...
    removeInboundGuide = function (guide, gUid) {
      ...
      if (
        ...
        ((guide.className === "guide h draggable" &&
          guide.offsetTop < hBound + scrollPos[0]) ||
          (guide.className === "guide v draggable" &&
            guide.offsetLeft < vBound + scrollPos[1]))
      ) {
    ...
    OpenGridDialog = function () {
      ...
      this.open = function () {
        ...
        if (gridListLen > 0) {
          ...
          dialog.style.top =
            (doc.clientHeight - dialog.clientHeight) / 2 + "px";
        }
      };
    ...
    showDetailedInfo = function () {
      ...
      for (i = 0; i < hGuides.length - 1; i = i + 1) {
        ...
        for (j; j < vGuides.length - 1; j = j + 1) {
          ...
          text =
            vGuides[j + 1] - vGuides[j] + " x " + (hGuides[i + 1] - hGuides[i]);
    ...
    Menu = function () {
      ...
      this.render = function () {
        ...
        evt.attach("mousedown", menuBtn, function () {
          toggles.rulers.txt.nodeValue =
            rulerStatus === 1 ? "Hide rulers" : "Show rulers";
          ...
          if (guidesCnt > 0) {
            ...
            toggles.guides.txt.nodeValue =
              guideStatus === 1 ? "Hide guides" : "Show guides";
          } else {
          ...
          toggles.all.txt.nodeValue =
            rulerStatus === 1 || guideStatus === 1 ? "Hide all" : "Show all";

          toggles.lock.txt.nodeValue =
            locked === 0 ? "Lock rulers" : "Unlock rulers";
          toggles.details.txt.nodeValue =
            detailsStatus === 0 ? "Show detailed info" : "Hide detailed info";
          toggles.snapdom.txt.nodeValue =
            snapDom === 0 ? "Snap to DOM" : "Turn off snap to DOM";
    ...
    prepare = function () {
      ...
      setTimeout(function () {
        ...
        wrapper = document.getElementById("rg-wrapper");
        gInfoBlockWrapper = document.createElement("div");
        ...
        // Remove the body.appendChild(wrapper) line since it's already in HTML
  ...
  evt.attach("mousedown", document, function (e, src) {
    ...
    if (
      //   ((x > vBound && y < hBound) || (y > hBound && x < vBound)) &&
      y > hBound &&
      x < vBound &&
      rulerStatus === 1
    ) {
