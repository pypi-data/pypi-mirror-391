...
class Bridge(QObject):
    def __init__(self, view, parent_widget):
        ...
        # Keep track of JS calls before page is ready
    ...
    def _on_page_loaded(self, ok):
        if ok:
            ...
            for args in self._pending_js_calls:
                self._run_js(*args)
            self._pending_js_calls.clear()

    def _run_js(self, column_name, column_data, match, match_option):
        js_code = f"initialize_regex_table({json.dumps(column_name)}, {json.dumps(column_data)}, {json.dumps(match)}, {match_option})"
        self.view.page().runJavaScript(js_code)

    def sendDataToJs(self, column_name, column_data, match, match_option):
        # If page is loaded, run immediately; otherwise queue
        if self._page_loaded:
            self._run_js(column_name, column_data, match, match_option)
        else:
            self._pending_js_calls.append(
                (column_name, column_data, match, match_option)
            )
...
class OWRegexExtractor(widget.OWWidget):
    ...
    def on_regex_expression_change(self, pattern):
        ...
        if (
            ...
            df = pd.DataFrame({"No Data Input(Widget is not connected!)": [""]})
            column_name = "No Data Input(Widget is not connected!)"
