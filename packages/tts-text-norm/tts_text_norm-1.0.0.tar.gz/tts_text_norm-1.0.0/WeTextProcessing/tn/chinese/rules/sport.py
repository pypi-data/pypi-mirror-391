...
class Sport(Processor):
    ...
    def build_tagger(self):
        ...
        separator = delete('/')
        score_gang = add_weight(
            rmspace + number + separator + insert('杠') + number + separator + insert('杠') + number + rmspace,
            2.0
        )
        
        all_scores = score_gang | score
        
        team_score_tagger = (insert('team: "') + (country | club) + insert('" score: "') +
                            all_scores + insert('"'))
        
        standalone_score_tagger = add_weight(
            insert('score: "') + all_scores + insert('"'),
            1.8
        )
        combined_tagger = team_score_tagger | standalone_score_tagger
    ...
    def build_verbalizer(self):
        ...
        team_verbalizer = team + score
        standalone_verbalizer = score
        verbalizer = team_verbalizer | standalone_verbalizer
