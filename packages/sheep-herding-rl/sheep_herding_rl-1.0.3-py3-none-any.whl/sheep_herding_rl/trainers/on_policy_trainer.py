"""On-policy training loop for PPO, A2C, TRPO, and other on-policy algorithms."""
...
class OnPolicyTrainer:
    """
    High-level trainer for on-policy RL algorithms (PPO, A2C, TRPO, etc.).
    
    Collects rollouts/episodes before updating the policy.
    Does NOT use replay buffer - only fresh experience.
    """
    ...
    def run(
        ...
        """Run training for on-policy algorithms (collects full episodes before updating).
        
        Args:
            num_episodes: Number of episodes to train
            render_every_steps: Render every N steps (0 disables)
            start_render_enabled: Start with rendering enabled
            allow_render_toggle: Allow toggling render with 'h' key
            log_interval: Log every N episodes
        
        Returns:
            List of episode logs
        """
        ...
        for episode in range(num_episodes):
            ...
            for step in range(self.max_steps):
                ...
                # For on-policy: observe() just stores in rollout buffer (no learning yet)
            ...
            # On-policy: Update policy after full episode
    ...
    def evaluate(
        ...
        """Evaluate agent performance (deterministic policy, no training)."""
