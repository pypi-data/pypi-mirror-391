...
# Combination helpers
# -------------------------
def combine_letter_matches_to_words(letter_matches: List[List[str]], cap: int = 50) -> List[str]:
    """
    DEPRECATED: This function produces garbage output.
    Letter-level combinations don't work for Telugu due to context rules.
    
    Keeping this function for backward compatibility but return empty list.
    Use word-level suggestions instead.
    """
    # DO NOT COMBINE - produces garbage like "ఇఅనతఇకఇ"
    return []
...
def process_english_words_input(word: str, letter_tokenize: bool = True, per_word_limit: int = 8) -> Dict:
    """
    UNIVERSAL FIX: Use transliterator + suggestions, NOT letter combinations.
    
    Priority:
    1. Raw transliteration (handles ALL phonetic rules correctly)
    2. Word-level suggestions (generates quality variants)
    3. DO NOT use letter-level combinations (they produce garbage)
    """
    ...
    # STEP 1: Get raw transliteration (PRIMARY - uses full context)
    raw = eng_to_telugu(word)
    
    # STEP 2: Get word-level suggestions (generates variants using suggest.py)
    # This runs word through BOTH transliterators and roman variants
    word_suggestions = get_word_level_suggestions(word, limit=per_word_limit * 2)
    
    # STEP 3: Tokenize for ANALYSIS ONLY (not for combinations)
    if letter_tokenize:
        tokens = tokenize_with_trie(word) if _TRIE else multi_char_tokens(word)
    else:
        tokens = split_word_to_letters(word)
    
    # Get letter matches for DISPLAY/DEBUG only
    letter_matches = [get_letter_level_matches(tok, max_per_letter=1) for tok in tokens]
    
    # STEP 4: Build final variant list
    all_variants = []
    seen = set()
    
    # Priority 1: Raw transliteration (always first)
    if raw and hard_filter_single_candidate(raw):
        all_variants.append(raw)
        seen.add(raw)
    
    # Priority 2: Word suggestions (filtered for quality)
    ...
    # CRITICAL: DO NOT add letter-level combinations
    # They produce garbage like "ఇఅనతఇకఇ"
    
    # Ensure we have at least the raw transliteration
    ...
    return {
        ...
        "raw": raw,
        "letters": tokens,
        "letter_matches": letter_matches,  # For analysis/display only
...
def process_english_sentence_input(sentence: str, per_word_limit: int = 6, beam: int = 6) -> Dict:
    ...
    sentence_variants_filtered = [s for s in sentence_variants_list if hard_filter_sentence_variant(s, min_word_pass_ratio=0.7)]
...
def process_telugu_word_in_english_script(word: str, letter_tokenize: bool = True, per_word_limit: int = 8) -> Dict:
    """
    UNIVERSAL FIX: For Telugu words in Roman script (e.g., 'namaste', 'intiki').
    Use transliterator + suggestions only.
    """
    word = normalize_input(word)
    """
    UNIVERSAL FIX: For Telugu words in Roman script (e.g., 'namaste', 'intiki').
    ...
    # Raw transliteration (PRIMARY)
    raw = eng_to_telugu(word)
    
    # Word-level suggestions (generates variants)
    word_suggestions = get_word_level_suggestions(word, limit=per_word_limit * 2)
    
    # Tokenize for analysis only
