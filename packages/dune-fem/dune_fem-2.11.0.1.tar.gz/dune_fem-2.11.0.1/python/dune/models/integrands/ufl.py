...
def _compileUFL(integrands, form, *args, tempVars=True):
    ...
    if 'interior_facet' in integrals.keys():
        ...
        predefined[minCellEdgeLength('-')] = integrands.minCellEdgeLength('Side::out') #minEdgeLength(integrands.cellGeometry('Side::out'))
        predefined[facetArea] = integrands.facetArea()
        predefined[maxFacetEdgeLength] = integrands.maxFacetEdgeLength()
        predefined[minFacetEdgeLength] = integrands.minFacetEdgeLength()
    ...
    if dirichletBCs:
        ...
        predefined[x] = UnformattedExpression('auto', 'geometry().global( Dune::Fem::coordinate( ' + integrands.arg_x.name + ' ) )')
