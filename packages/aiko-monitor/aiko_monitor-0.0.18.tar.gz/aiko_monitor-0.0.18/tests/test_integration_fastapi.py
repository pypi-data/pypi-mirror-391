...
def fastapi_fixture():
    ...
    @app.post("/empty")
    async def post_empty(request: Request):
        raw = await request.body()
        return JSONResponse({"message": "POST empty success", "size": len(raw)})
...
def test_post_empty_body_records_none(fastapi_fixture):
    fx = fastapi_fixture

    response = fx["client"].post("/empty")
    assert response.status_code == 200
    ...
    event = fx["mock_server"].received_events[0]
    assert event["method"] == "POST"
    assert event["endpoint"] == "/empty"
    assert event["request_body"] is None


def test_response_body_none_for_204(fastapi_fixture):
    fx = fastapi_fixture

    response = fx["client"].get("/no_content")
    ...
    event = fx["mock_server"].received_events[0]
    assert event["method"] == "GET"
    assert event["endpoint"] == "/no_content"
