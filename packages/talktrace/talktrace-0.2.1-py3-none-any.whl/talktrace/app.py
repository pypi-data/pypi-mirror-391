...
def server(input, output, session):
    ...
    def loc_dynamic_model_select():
        return ui.input_select("model_select", t("sidebar", "model_select"), choices=select_api_choices(), selected="GPT-4o")
...
# DataFrame für qualitative Statistik generieren
    ...
    @render.ui
    def code_legend():
        return ui.markdown(f"**{t("results", "caption")}:** {extract_code_legend()}")
    ...
    def loc_api_key_exists():
        if not input.api_select():
            ...
            return t("options", "api_openai_found") if api_key_openai.get() else t("options", "api_openai_found")
