...
def _raise_async_exception(thread_id, exc_type):
    """Inject an exception into the target thread."""
...
class HandlerWatchdog:
    """Timer-based watchdog that interrupts the current thread once the deadline passes."""

    def __init__(self, timeout_seconds: float | int | None):
        self.timeout = float(timeout_seconds) if timeout_seconds else None
        self._timer: threading.Timer | None = None
        self._thread_id: int | None = None

    def _trigger(self):
        _raise_async_exception(self._thread_id, HandlerTimeoutError)
...
class HTTP_Parser():
    ...
    def set_content(self) -> None:
        ...
        if handler_info:
            ...
            context = HandlerWatchdog(handler_timeout) if handler_timeout and handler_timeout > 0 else nullcontext()
            with context:
