...
class RabbitMQBroker(Broker):
    ...
    def __init__(self, rabbitmq_url: Optional[str] = None) -> None:
        ...
        # In-memory tracking for workers, inflight messages, and locks
    ...
    def _has_global_permit(self, topic: str) -> bool:
        """Check if topic has capacity for another inflight message."""
        config = self._topic_configs.get(topic, TopicConfig())
        if config.concurrency_limit is None:
            return True
        
        # Count inflight messages for this topic
        inflight_count = sum(1 for msg in self._inflight.values() if msg.topic == topic)
        return inflight_count < config.concurrency_limit
        ...
        with self._lock:
            ...
            for topic_name in self._topic_configs.keys():
                try:
                    ...
                    # Count inflight for this topic
                    inflight = sum(1 for msg in self._inflight.values() if msg.topic == topic_name)
    ...
    def get_all_active_tasks(self, topic: Optional[str]) -> List[ActiveTaskInfo]:
        ...
        with self._lock:
            ...
            # For now, only return inflight tasks (queued tasks would require consuming from RabbitMQ)
    ...
    def get_topic_inflight(self) -> Dict[str, int]:
        """Return current in-flight counts per topic."""
        with self._lock:
            result = {}
            for topic_name in self._topic_configs.keys():
                result[topic_name] = sum(1 for msg in self._inflight.values() if msg.topic == topic_name)
            return result
