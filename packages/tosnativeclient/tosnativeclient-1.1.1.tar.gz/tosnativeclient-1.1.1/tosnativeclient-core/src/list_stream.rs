...
impl ListStream {
    pub fn next(&mut self) -> Result<Option<ListObjectsType2Output>, TosError> {
    ...
    pub(crate) fn list_background(&self) -> Option<Paginator> {
    ...
    ) -> Result<Option<ListObjectsType2Output>, TosError> {
        match paginator {
            None => Ok(None),
            Some(pg) => {
                match pg.has_next() {
