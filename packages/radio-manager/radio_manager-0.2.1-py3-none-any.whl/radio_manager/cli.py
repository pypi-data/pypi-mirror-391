...
def cmd_reception_info(args):
    ...
    try:
        with radio:
            ...
            MAX_TIMEOUTS = 5  # Adjust this value as needed
            timeout_count = 0
            timeout_notice = 1  # Stay quiet if less timeouts that this

            while True:
                try:
                    info = radio.get_reception_info()

                    # If successful, reset the timeout counter
                    if timeout_count > timeout_notice:
                        print(
                    ...
                    timeout_count += 1
                    backoff_time = 2 ** (timeout_count - 1)

                    # Check if the limit has been reached
                    if timeout_count >= MAX_TIMEOUTS:
                        print(
                            f"\n\n🛑 **ERROR:** Maximum consecutive timeouts "
                            f"({MAX_TIMEOUTS}) reached."
                        )
                        print(
                    ...
                    elif timeout_count > timeout_notice:
                        # Only warn for excessive failures
                        print(
                            f"{radio.name}: Timeout ("
...
Examples:
  ...
  radio-manager raw-command "VOL,3"
    ...
    parser_set_volume.add_argument(
        ...
        type=int,
        help=(
            'Volume level (range depends on model: '
            'SDS200=0-29, BCD996P2=0-15)'
        )
    )
    parser_set_volume.add_argument(
        '--radio', '-r',
