    ...
    def _convert_head_node_config_to_api_model(
        ...
        *,
        ...
        if config is None:
            ...
            # Convert physical_resources to API model (OpenAPI PhysicalResources object)
            # Only pass non-None values to avoid sending null fields to backend
            physical_resources_api = None
            if config.physical_resources is not None:
                pr_dict = config.physical_resources.to_dict()
                # Copy only the fields that are present in the dict
                # To add support for new fields, just add them to this list
                pr_kwargs = {
                    field: pr_dict[field]
                    for field in ["cpu", "gpu", "memory"]
                    if field in pr_dict
                }
                physical_resources_api = PhysicalResources(**pr_kwargs)
            ...
            api_model = ComputeNodeType(
                ...
                physical_resources=physical_resources_api,
