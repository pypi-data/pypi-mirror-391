...
class _NodeConfig(ModelBase):
    ...
    physical_resources: Optional[PhysicalResources] = field(
        default=None,
        repr=False,
        metadata={
    ...
    def _validate_physical_resources(
        self, physical_resources: Optional[PhysicalResources]
    ) -> Optional[PhysicalResources]:
        if physical_resources is None:
            return None
        if isinstance(physical_resources, dict):
            # Convert dict to PhysicalResources object
            return PhysicalResources.from_dict(physical_resources)
        if isinstance(physical_resources, PhysicalResources):
            return physical_resources
        raise TypeError(
            "'physical_resources' must be a PhysicalResources object or dict."
        )
