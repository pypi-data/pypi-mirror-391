...
class LDContext:
    ...
    def __hash__(self, __unit: None=None) -> int:
        ...
        mappings_hash: int = box_hash_seq(map(mapping, sort(this.Mappings.keys(), ObjectExpr825())))
        name_hash: int
    ...
    def add_reverse_mapping(self, key: str, value: str) -> None:
        ...
        if active_pattern_result is not None:
            ...
            pattern_input = (try_get_value(this.mappings_004045, prefix, FSharpRef(_arrow829, _arrow830)), out_arg)
            ...
            def _arrow831(__unit: None=None) -> tuple[str, str]:
                return out_arg_1

            def _arrow832(v_2: tuple[str, str]) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            pattern_input_1 = (try_get_value(this.compact_reverse_mappings, key, FSharpRef(_arrow831, _arrow832)), out_arg_1)
    ...
    def try_find_term(self, term: str) -> str | None:
        ...
        pattern_input = (try_get_value(this.mappings_004045, term, FSharpRef(_arrow834, _arrow835)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        ...
        if definition is None:
            ...
            def _arrow836(__unit: None=None) -> str | None:
                prefix: str = active_pattern_result[0]
                suffix: str = active_pattern_result[1]
            ...
            def _arrow837(__unit: None=None) -> str | None:
                d: str = definition
                return d

            return _arrow836() if (active_pattern_result is not None) else _arrow837()
    ...
    def try_find_iri(self, iri: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow838(__unit: None=None) -> str:
            return out_arg

        def _arrow839(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.reverse_mappings, iri, FSharpRef(_arrow838, _arrow839)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        def chooser(ctx: LDContext) -> str | None:
