...
from ...arctrl_core.data_context import (DataContext__ctor_Z780A8A2A, DataContext)
from ...arctrl_core.data_map import DataMap
from ...fable_library.list import (of_array, FSharpList, exists, iterate, length, head, iterate_indexed)
from ...fable_library.map_util import add_to_dict
from ...fable_library.range import range_big_int
from ...fable_library.seq import (try_find, item, to_array, delay, map, to_list)
...
def compose_columns(columns: IEnumerable_1[FsColumn]) -> Array[DataContext]:
    ...
    def _arrow3327(__unit: None=None, columns: Any=columns) -> IEnumerable_1[DataContext]:
        def _arrow3326(i: int) -> DataContext:
            return DataContext__ctor_Z780A8A2A()

        return map(_arrow3326, range_big_int(0, 1, l - 1))

    dc: Array[DataContext] = list(to_array(delay(_arrow3327)))
