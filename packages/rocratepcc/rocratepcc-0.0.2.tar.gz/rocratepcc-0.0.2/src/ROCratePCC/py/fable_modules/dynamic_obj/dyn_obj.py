...
def format(dyn_obj: DynamicObj) -> str:
    def loop(object_mut: DynamicObj, indentation_level_mut: int, members_left_mut: FSharpList[PropertyHelper], acc_mut: FSharpList[str], dyn_obj: Any=dyn_obj) -> str:
        while True:
            ...
            def _arrow400(__unit: None=None, object: Any=object, indentation_level: Any=indentation_level, members_left: Any=members_left, acc: Any=acc) -> IEnumerable_1[str]:
                def _arrow399(i: int) -> str:
                    return "    "

                return map(_arrow399, range_big_int(0, 1, indentation_level - 1))

            indent: str = join("", to_list(delay(_arrow400)))
