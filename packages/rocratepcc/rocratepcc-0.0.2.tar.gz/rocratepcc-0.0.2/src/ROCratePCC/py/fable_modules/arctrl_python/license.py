...
from ..arctrl_core.Helper.hash_codes import box_hash_array
from ..arctrl_file_system.path import alternative_licensefile_names
from ..fable_library.reflection import (TypeInfo, class_type)
from ..fable_library.seq import (contains, for_all)
from ..fable_library.types import Array
from ..fable_library.util import (string_hash, to_enumerable)

def _expr3708() -> TypeInfo:
...
class License:
    ...
    def GetRenameContracts(self, new_path: str) -> Array[Contract]:
        ...
        this.Path = new_path
        def _arrow3702(__unit: None=None) -> Contract:
            match_value: str = this.Type
            return Contract.create_create(new_path, DTOType(10), DTO(1, this.Content))

        return [delete_contract, _arrow3702()]
    ...
    def get_rename_contracts(new_path: str) -> Callable[[License], Array[Contract]]:
        def _arrow3703(license: License) -> Array[Contract]:
    ...
    def try_from_read_contract(c: Contract) -> License | None:
        def _arrow3706(__unit: None=None) -> bool:
            txt: str = c.DTO.fields[0]
            class ObjectExpr3705:
                @property
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow3704(x: str, y: str) -> bool:
                        return x == y
            ...
            return True if (c.Path == "LICENSE") else contains(c.Path, alternative_licensefile_names, ObjectExpr3705())

        def _arrow3707(__unit: None=None) -> License | None:
            txt_1: str = c.DTO.fields[0]
            return License.init_fulltext(txt_1, c.Path)

        return (((((_arrow3707() if _arrow3706() else None) if (c.DTO.tag == 1) else None) if (c.DTO is not None) else None) if (c.DTOType.tag == 10) else None) if (c.DTOType is not None) else None) if (c.Operation == "READ") else None
