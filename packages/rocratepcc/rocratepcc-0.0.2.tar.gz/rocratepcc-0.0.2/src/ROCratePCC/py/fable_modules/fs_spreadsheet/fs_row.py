    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsRow:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(cells.GetCellsInRow(index)) == 0:
                ...
                def _arrow2937(c: FsCell, f: Any=f) -> int:
                    return c.Address.ColumnNumber

                return f(_arrow2937)(cells.GetCellsInRow(index)).Address.ColumnNumber


        def _arrow2940(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2939(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2938:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr2938())

            return _arrow2939

        min_col_index: int = get_index_by(_arrow2940) or 0
        def _arrow2943(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2942(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2941:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr2941())

            return _arrow2942

        max_col_index: int = get_index_by(_arrow2943) or 0
