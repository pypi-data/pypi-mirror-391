...
from ...arctrl_core.comment import (Remark, Comment)
from ...arctrl_core.Helper.collections_ import Dictionary_init
from ...fable_library.list import (FSharpList, singleton, empty, append, cons, find, exists, initialize as initialize_1, map as map_2)
from ...fable_library.map import (of_seq, try_find)
from ...fable_library.map_util import (get_item_from_dict, add_to_dict)
from ...fable_library.option import (map as map_1, default_arg)
from ...fable_library.reflection import (TypeInfo, string_type, int32_type, tuple_type, class_type, list_type, record_type)
from ...fable_library.seq import (indexed, map, choose, initialize, max_by, try_pick, fold, iterate, is_empty, length as length_1, try_item, delay, append as append_1)
from ...fable_library.string_ import (to_fail, printf)
from ...fable_library.types import (Record, Array)
from ...fable_library.util import (IEnumerable_1, compare_primitives, equals, IEnumerator)
from ...fs_spreadsheet.Cells.fs_cell import (FsCell, DataType)
from ...fs_spreadsheet.DSL.cell_builder import (CellBuilder__AsCellElement_6F87C2ED, CellBuilder__ctor)
from ...fs_spreadsheet.DSL.row_builder import (RowBuilder__ctor, RowBuilder__Combine_19F30600, RowBuilder_get_Empty)
from ...fs_spreadsheet.DSL.types import (SheetEntity_1_some_2B595, Messages_format)
from ...fs_spreadsheet.DSL.row_builder import RowBuilder
from ...fs_spreadsheet.DSL.types import (SheetEntity_1, RowElement, ColumnIndex)
...
def SparseRowModule_fromAllValues(v: IEnumerable_1[str | None]) -> IEnumerable_1[tuple[int, str]]:
    def chooser(tupled_arg: tuple[int, str | None], v: Any=v) -> tuple[int, str] | None:
        def _arrow3127(v_1: str, tupled_arg: Any=tupled_arg) -> tuple[int, str]:
            return (tupled_arg[0], v_1)

        return map_1(_arrow3127, tupled_arg[1])

    return choose(chooser, indexed(v))


def SparseRowModule_getAllValues(i: IEnumerable_1[tuple[int, str]]) -> IEnumerable_1[str | None]:
    class ObjectExpr3128:
        @property
        def Compare(self) -> Callable[[int, int], int]:
            return compare_primitives

    m: Any = of_seq(i, ObjectExpr3128())
    def projection(tuple: tuple[int, str], i: Any=i) -> int:
        return tuple[0]

    class ObjectExpr3129:
        @property
        def Compare(self) -> Callable[[int, int], int]:
            return compare_primitives

    def _arrow3130(i_1: int, i: Any=i) -> str | None:
        return try_find(i_1, m)

    return initialize(max_by(projection, i, ObjectExpr3129())[0] + 1, _arrow3130)
...
SparseTable_reflection = _expr3131
...
def SparseTable_AddRow(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    ...
    def _arrow3132(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
        return tuple[0]

    class ObjectExpr3133:
        @property
        def Compare(self) -> Callable[[int, int], int]:
            return compare_primitives

    length: int = (0 if is_empty(values) else (1 + max_by(_arrow3132, values, ObjectExpr3133())[0])) or 0
...
def SparseTable_AddComment(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    if length_1(values) == 0:
        ...
        iterate(action, values)
        def _arrow3134(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
            return tuple[0]

        class ObjectExpr3135:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        length: int = (0 if is_empty(values) else (1 + max_by(_arrow3134, values, ObjectExpr3135())[0])) or 0
                ...
                if en.System_Collections_IEnumerator_MoveNext():
                    ...
                    if pattern_matching_result == 0:
                        ...
                        if active_pattern_result_1 is not None:
                            ...
                            if vals is not None:
                                def _arrow3138(__unit: None=None, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                    v_2: IEnumerable_1[tuple[int, str]] = vals
                                    k_1: str = key
                                    def _arrow3137(label: str) -> bool:
                                        return k_1 == (prefix_1 + label)

                                    return exists(_arrow3137, labels)
                        ...
                        if pattern_matching_result_1 == 0:
                            ...
                            def _arrow3136(label_1: str, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                return k_3 == (prefix_1 + label_1)

                            matrix_mut = SparseTable_AddRow(find(_arrow3136, labels), v_3, matrix)
                            remarks_mut = remarks
                            line_number_1_mut = line_number_1 + 1
                            continue
...
def SparseTable_ToRows_759CAFC1(matrix: SparseTable, prefix: str | None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    prefix_1: str = "" if (prefix is None) else (prefix + " ")
    def _arrow3144(__unit: None=None, matrix: Any=matrix, prefix: Any=prefix) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow3140(key: str) -> IEnumerable_1[tuple[int, str]]:
            def _arrow3139(i: int) -> str:
                return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key, i + 1))

            return SparseRowModule_fromValues(cons(prefix_1 + key, initialize_1(matrix.ColumnCount - 1, _arrow3139)))

        def _arrow3143(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow3142(key_1: str) -> IEnumerable_1[tuple[int, str]]:
                def _arrow3141(i_1: int) -> str:
                    return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key_1, i_1 + 1))

                return SparseRowModule_fromValues(cons(Comment_wrapCommentKey(key_1), initialize_1(matrix.ColumnCount - 1, _arrow3141)))

            return map(_arrow3142, matrix.CommentKeys)

        return append_1(map(_arrow3140, matrix.Keys), delay(_arrow3143))

    return delay(_arrow3144)
