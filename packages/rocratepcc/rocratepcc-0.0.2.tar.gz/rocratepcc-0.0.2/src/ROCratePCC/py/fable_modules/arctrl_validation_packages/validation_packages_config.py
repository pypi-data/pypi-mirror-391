...
from ..arctrl_core.Helper.hash_codes import (box_hash_array, box_hash_option, box_hash_seq)
from ..fable_library.array_ import sort_by
from ..fable_library.option import value
from ..fable_library.range import range_big_int
from ..fable_library.reflection import (TypeInfo, class_type)
from ..fable_library.seq import (to_list, delay, append, singleton, empty, map, length, fold, item)
from ..fable_library.string_ import join
from ..fable_library.types import (Array, to_string)
from ..fable_library.util import (IEnumerable_1, compare_arrays, equals)
from .validation_package import ValidationPackage

def _expr376() -> TypeInfo:
...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow372(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow371(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow370(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow369(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow368(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow367(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow367))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow368))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow369))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow370))

            return append(singleton("{"), delay(_arrow371))

        return join("\n", to_list(delay(_arrow372)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        def sort(arg: Array[ValidationPackage]) -> Array[ValidationPackage]:
            ...
            return sort_by(projection, list(arg), ObjectExpr373())
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow375(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow374(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow374, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow375))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr376
