...
from ..fable_library.list import (FSharpList, empty, of_seq)
from ..fable_library.seq import (delay, append, singleton, iterate_indexed, map, try_find, try_pick)
from ..fable_library.string_ import (to_fail, printf, to_console)
from ..fable_library.util import (get_enumerator, IEnumerable_1, IEnumerator, to_enumerable)
...
def ArcWorkflow_toRows(workflow: ArcWorkflow) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow3338(__unit: None=None, workflow: Any=workflow) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow3337(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow3336(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow3335(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    return to_rows_1("Workflow Person", of_seq(workflow.Contacts))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["WORKFLOW CONTACTS"]))), delay(_arrow3335))

            return append(to_rows(workflow), delay(_arrow3336))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["WORKFLOW"]))), delay(_arrow3337))

    return delay(_arrow3338)
