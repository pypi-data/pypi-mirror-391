...
TemplateReadError_reflection = _expr3365
...
Metadata_Template_TemplateInfo_reflection = _expr3366
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow3368(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if starts_with_exact(_arrow3367(), "MISSING_IDENTIFIER_") else _arrow3368())
    ...
    class ObjectExpr3370:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3369(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr3370())), matrix.ColumnCount)
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow3378(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow3377(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    ...
                    def _arrow3374(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow3373(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow3372(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                ...
                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow3371))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow3372))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow3373))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow3374))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow3375))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow3376))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow3377))

    return delay(_arrow3378)
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    def _arrow3379(__unit: None=None, doc: Any=doc) -> ArcTable:
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow3379(), now())
