...
from ..arctrl_core.comment_list import try_item
from ..arctrl_core.ontology_annotation import OntologyAnnotation
from ..arctrl_core.Process.component import (Component, Component_fromISAString_7C9A7CF8)
from ..arctrl_core.Process.factor import Factor
from ..arctrl_core.Process.factor_value import FactorValue
from ..arctrl_core.Process.material_attribute import (MaterialAttribute, MaterialAttribute_fromString_5980DC03)
...
from ..fable_library.option import (bind, value)
from ..fable_library.seq import find_index
from ..fable_library.types import (FSharpRef, Array)
from ..fable_library.util import (int32_to_string, equals)

def try_int(str_1: str) -> int | None:
    match_value: tuple[bool, int]
    out_arg: int = 0
    def _arrow3718(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3719(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3718, _arrow3719)), out_arg)
    if match_value[0]:
...
def try_get_parameter_column_index(param_value: ProcessParameterValue) -> int | None:
    def _arrow3720(param: ProtocolParameter, param_value: Any=param_value) -> int | None:
        return try_get_parameter_index(param)

    return bind(_arrow3720, param_value.Category)
...
def try_get_factor_column_index(factor_value: FactorValue) -> int | None:
    def _arrow3721(factor: Factor, factor_value: Any=factor_value) -> int | None:
        return try_get_factor_index(factor)

    return bind(_arrow3721, factor_value.Category)
...
def try_get_characteristic_column_index(characteristic_value: MaterialAttributeValue) -> int | None:
    def _arrow3722(characteristic: MaterialAttribute, characteristic_value: Any=characteristic_value) -> int | None:
        return try_get_characteristic_index(characteristic)

    return bind(_arrow3722, characteristic_value.Category)
