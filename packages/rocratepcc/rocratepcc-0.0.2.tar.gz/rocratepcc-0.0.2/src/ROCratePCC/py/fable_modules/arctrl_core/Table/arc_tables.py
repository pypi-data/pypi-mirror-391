...
from ...fable_library.array_ import insert_range_in_place
from ...fable_library.list import FSharpList
from ...fable_library.map import (of_seq as of_seq_1, try_find)
from ...fable_library.map_util import (add_to_dict, add_to_set)
from ...fable_library.option import (value as value_2, default_arg, map as map_2, bind)
from ...fable_library.reflection import (TypeInfo, class_type)
from ...fable_library.seq import (try_find_index, find_index, length, to_list, delay, map as map_1, choose, reduce, append)
from ...fable_library.seq2 import distinct
from ...fable_library.set import (intersect, of_seq, FSharpSet__get_IsEmpty)
from ...fable_library.types import Array
from ...fable_library.util import (get_enumerator, dispose, compare, string_hash, IEnumerable_1, compare_primitives, equals, ignore, IEnumerator, to_iterator)
...
def ArcTablesAux_SanityChecks_validateSheetIndex(index: int, allow_append: bool, sheets: Array[ArcTable]) -> None:
    ...
    def _arrow259(__unit: None=None, index: Any=index, allow_append: Any=allow_append, sheets: Any=sheets) -> bool:
        x: int = index or 0
...
def ArcTablesAux_SanityChecks_validateNamesUnique(names: IEnumerable_1[str]) -> None:
    class ObjectExpr261:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow260(x: str, y: str) -> bool:
                return x == y
    ...
    if not (length(names) == length(distinct(names, ObjectExpr261()))):
...
def ArcTablesAux_SanityChecks_validateNewNameUnique(new_name: str, existing_names: IEnumerable_1[str]) -> None:
    def _arrow262(x: str, new_name: Any=new_name, existing_names: Any=existing_names) -> bool:
        return x == new_name

    match_value: int | None = try_find_index(_arrow262, existing_names)
    if match_value is None:
        pass
...
def ArcTablesAux_SanityChecks_validateNewNameAtUnique(index: int, new_name: str, existing_names: IEnumerable_1[str]) -> None:
    def _arrow263(x: str, index: Any=index, new_name: Any=new_name, existing_names: Any=existing_names) -> bool:
        return x == new_name

    match_value: int | None = try_find_index(_arrow263, existing_names)
    if match_value is None:
        pass
...
def ArcTablesAux_SanityChecks_validateNewNamesUnique(new_names: IEnumerable_1[str], existing_names: IEnumerable_1[str]) -> None:
    ...
    same: Any = intersect(of_seq(new_names, ObjectExpr264()), of_seq(existing_names, ObjectExpr265()))
...
class ArcTables:
    def __init__(self, init_tables: Array[ArcTable]) -> None:
        ...
        self.tables: Array[ArcTable] = _expr289()
    ...
    def TableNames(self, __unit: None=None) -> FSharpList[str]:
        this: ArcTables = self
        def _arrow267(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow266(s: ArcTable) -> str:
                return s.Name

            return map_1(_arrow266, this.Tables)

        return to_list(delay(_arrow267))
    ...
    def AddColumnAt(self, table_index: int, header: CompositeHeader, cells: Array[CompositeCell] | None=None, column_index: int | None=None, force_replace: bool | None=None) -> None:
        this: ArcTables = self
        def _arrow273(table: ArcTable) -> None:
            table.AddColumn(header, cells, column_index, force_replace)

        this.MapTableAt(table_index, _arrow273)
    ...
    def RemoveColumnAt(self, table_index: int, column_index: int) -> None:
        this: ArcTables = self
        def _arrow275(table: ArcTable) -> None:
            table.RemoveColumn(column_index)

        this.MapTableAt(table_index, _arrow275)
    ...
    def UpdateColumnAt(self, table_index: int, column_index: int, header: CompositeHeader, cells: Array[CompositeCell] | None=None) -> None:
        this: ArcTables = self
        def _arrow277(table: ArcTable) -> None:
            table.UpdateColumn(column_index, header, cells)

        this.MapTableAt(table_index, _arrow277)
    ...
    def AddRowAt(self, table_index: int, cells: Array[CompositeCell] | None=None, row_index: int | None=None) -> None:
        this: ArcTables = self
        def _arrow279(table: ArcTable) -> None:
            table.AddRow(cells, row_index)

        this.MapTableAt(table_index, _arrow279)
    ...
    def RemoveRowAt(self, table_index: int, row_index: int) -> None:
        this: ArcTables = self
        def _arrow281(table: ArcTable) -> None:
            table.RemoveRow(row_index)

        this.MapTableAt(table_index, _arrow281)
    ...
    def UpdateRowAt(self, table_index: int, row_index: int, cells: Array[CompositeCell]) -> None:
        this: ArcTables = self
        def _arrow282(table: ArcTable) -> None:
            table.UpdateRow(row_index, cells)

        this.MapTableAt(table_index, _arrow282)
    ...
    def update_reference_tables_by_sheets(reference_tables: ArcTables, sheet_tables: ArcTables, keep_unused_ref_tables: bool | None=None) -> ArcTables:
        ...
        reference_table_map: Any = of_seq_1(choose(chooser, reference_tables.Tables), ObjectExpr284())
        def _arrow288(__unit: None=None) -> IEnumerable_1[ArcTable]:
            ...
            def _arrow286(__unit: None=None) -> Array[ArcTable]:
                a: Array[ArcTable] = sheet_tables.Tables
                return ResizeArray_collect(ArcTable.SplitByProtocolREF(), a)

            s: Array[ArcTable] = ResizeArray_map(f_3, ResizeArray_groupBy(f_2, ResizeArray_map(f_1, _arrow286())))
        ...
        return ArcTables(list(_arrow288()))
