...
def _arrow2149(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2151(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2153(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2149, ROCrate_decoder_1), map(_arrow2151, ROCrate_decoder_2), map(_arrow2153, ROCrate_decoder_3)]))
...
def _arrow2156(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2157(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2159(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2156, ISAJson_decoder_1), map(_arrow2157, ISAJson_decoder_2), map(_arrow2159, ISAJson_decoder_3)]))
