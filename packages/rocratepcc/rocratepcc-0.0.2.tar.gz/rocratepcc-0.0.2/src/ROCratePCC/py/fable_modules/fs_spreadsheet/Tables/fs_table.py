...
class FsTable(FsRangeBase):
    def __init__(self, name: str, range_address: FsRangeAddress, show_totals_row: bool | None=None, show_header_row: bool | None=None, field_names: IEnumerable_1[str] | None=None) -> None:
        ...
        def _arrow2968(__unit: None=None) -> Any:
            fns: IEnumerable_1[str] = field_names
            if length(fns) != super().ColumnCount():
                def _arrow2967(__unit: None=None) -> str:
                ...
                raise Exception(_arrow2967(), "fieldNames")
        ...
        self._fieldNames: Any = dict([]) if (field_names is None) else _arrow2968()
    ...
    def GetFields(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsTableField]:
        this: FsTable = self
        def _arrow2955(i: int) -> FsTableField:
            return this.GetFieldAt(i, cells_collection)

        return initialize(super().ColumnCount(), _arrow2955)
    ...
    def GetColumns(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsColumn]:
        this: FsTable = self
        def _arrow2957(__unit: None=None) -> IEnumerable_1[FsColumn]:
            def _arrow2956(i: int) -> IEnumerable_1[FsColumn]:
            ...
            return collect(_arrow2956, range_big_int(this.RangeAddress.FirstAddress.ColumnNumber, 1, this.RangeAddress.LastAddress.ColumnNumber))

        return delay(_arrow2957)

    def GetRows(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsRow]:
        this: FsTable = self
        def _arrow2959(__unit: None=None) -> IEnumerable_1[FsRow]:
            def _arrow2958(i: int) -> IEnumerable_1[FsRow]:
            ...
            return collect(_arrow2958, range_big_int(this.RangeAddress.FirstAddress.RowNumber, 1, this.RangeAddress.LastAddress.RowNumber))

        return delay(_arrow2959)
    ...
    def GetBodyRows(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsRow]:
        ...
        def _arrow2961(__unit: None=None) -> IEnumerable_1[FsRow]:
            def _arrow2960(i: int) -> IEnumerable_1[FsRow]:
                return singleton(FsRow(FsRangeAddress(FsAddress(i, this.RangeAddress.FirstAddress.ColumnNumber), FsAddress(i, this.RangeAddress.LastAddress.ColumnNumber)), cells_collection))

            return collect(_arrow2960, range_big_int(first_row_index, 1, this.RangeAddress.LastAddress.RowNumber))

        return delay(_arrow2961)

    def GetRowAt(self, index: int, cells_collection: FsCellsCollection) -> FsRow:
        this: FsTable = self
        if True if (index <= 0) else (index > super().RowCount()):
            def _arrow2962(__unit: None=None) -> str:
                arg: int = super().RowCount() or 0
                return to_text(printf("Row index must be between 1 and %i"))(arg)

            raise Exception(int32_to_string(index), _arrow2962())
    ...
    def Field(self, name: str, cells_collection: FsCellsCollection) -> FsTableField:
        ...
        if match_value is None:
            def _arrow2964(__unit: None=None) -> int:
                def mapping(v: FsTableField) -> int:
                    return v.Index

                s: IEnumerable_1[int] = map(mapping, this._fieldNames.values())
                class ObjectExpr2963:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return 0 if (length(s) == 0) else max(s, ObjectExpr2963())

            def _arrow2965(__unit: None=None) -> FsRangeAddress:
            ...
            new_field: FsTableField = FsTableField(name, _arrow2964() + 1, FsRangeColumn(_arrow2965()), some(None), some(None))
