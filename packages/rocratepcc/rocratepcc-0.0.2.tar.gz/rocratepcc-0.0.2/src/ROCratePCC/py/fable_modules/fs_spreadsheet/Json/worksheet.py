...
def encode_rows(no_numbering: bool, sheet: FsWorksheet) -> IEncodable:
    sheet.RescanRows()
    def _arrow3086(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[IEncodable]:
        def _arrow3085(r: int) -> IEncodable:
            def _arrow3084(__unit: None=None) -> IEnumerable_1[FsCell]:
                def _arrow3083(c: int) -> IEnumerable_1[FsCell]:
                ...
                return collect(_arrow3083, range_big_int(1, 1, sheet.MaxColumnIndex))

            return encode_no_numbers(to_list(delay(_arrow3084)))

        return map(_arrow3085, range_big_int(1, 1, sheet.MaxRowIndex))

    def mapping(row_1: FsRow, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEncodable:
        return encode(row_1)

    j_rows: IEncodable = seq(to_list(delay(_arrow3086))) if no_numbering else seq(map(mapping, sheet.Rows))
    def _arrow3091(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow3088(__unit: None=None) -> IEncodable:
        ...
        def _arrow3090(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow3089(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                return singleton(("rows", j_rows))

            return append(singleton(("tables", seq(map(encode_1, sheet.Tables)))) if (not is_empty(sheet.Tables)) else empty(), delay(_arrow3089))

        return append(singleton(("name", _arrow3088())), delay(_arrow3090))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow3091))
...
def _arrow3103(builder: IGetters) -> FsWorksheet:
    row_index: int = 0
    n: str
    ...
    def _arrow3102(__unit: None=None) -> IEnumerable_1[tuple[int | None, IEnumerable_1[FsCell]]] | None:
    ...
    rs: IEnumerable_1[tuple[int | None, IEnumerable_1[FsCell]]] = default_arg(_arrow3102(), empty())
...
decode_rows: Decoder_1[FsWorksheet] = object(_arrow3103)

def encode_columns(no_numbering: bool, sheet: FsWorksheet) -> IEncodable:
    sheet.RescanRows()
    def _arrow3107(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[IEncodable]:
        def _arrow3106(c: int) -> IEncodable:
            def _arrow3105(__unit: None=None) -> IEnumerable_1[FsCell]:
                def _arrow3104(r: int) -> IEnumerable_1[FsCell]:
                    match_value: FsCell | None = sheet.CellCollection.TryGetCell(r, c)
                    return singleton(FsCell("")) if (match_value is None) else singleton(match_value)

                return collect(_arrow3104, range_big_int(1, 1, sheet.MaxRowIndex))

            return encode_no_numbers_1(to_list(delay(_arrow3105)))

        return map(_arrow3106, range_big_int(1, 1, sheet.MaxColumnIndex))

    def mapping(col_1: FsColumn, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEncodable:
        return encode_2(col_1)

    j_columns: IEncodable = seq(to_list(delay(_arrow3107))) if no_numbering else seq(map(mapping, sheet.Columns))
    def _arrow3112(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow3109(__unit: None=None) -> IEncodable:
            value: str = sheet.Name
            class ObjectExpr3108(IEncodable):
                def Encode(self, helpers: IEncoderHelpers_1[Any]) -> Any:
                    return helpers.encode_string(value)

            return ObjectExpr3108()

        def _arrow3111(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow3110(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                return singleton(("columns", j_columns))

            return append(singleton(("tables", seq(map(encode_1, sheet.Tables)))) if (not is_empty(sheet.Tables)) else empty(), delay(_arrow3110))

        return append(singleton(("name", _arrow3109())), delay(_arrow3111))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow3112))
...
def _arrow3114(builder: IGetters) -> FsWorksheet:
    col_index: int = 0
    n: str
    object_arg: IRequiredGetter = builder.Required
    n = object_arg.Field("name", string)
...
decode_columns: Decoder_1[FsWorksheet] = object(_arrow3114)
