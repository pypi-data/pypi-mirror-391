    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(cells.GetCellsInColumn(index)) == 0:
                ...
                def _arrow2945(c: FsCell, f: Any=f) -> int:
                    return c.Address.RowNumber

                return f(_arrow2945)(cells.GetCellsInColumn(index)).Address.RowNumber


        def _arrow2948(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2947(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2946:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr2946())

            return _arrow2947

        min_row_index: int = get_index_by(_arrow2948) or 0
        def _arrow2951(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2950(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2949:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr2949())

            return _arrow2950

        max_row_index: int = get_index_by(_arrow2951) or 0
