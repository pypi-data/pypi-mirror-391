...
from ..arctrl_core.Helper.collections_ import StringDictionary_ofSeq
from ..arctrl_core.Helper.hash_codes import (box_hash_seq, merge_hashes, hash_1 as hash_1_1)
from ..arctrl_core.Helper.regex import ActivePatterns__007CRegex_007C__007C
from ..fable_library.map_util import (add_to_dict, get_item_from_dict, try_get_value)
from ..fable_library.option import default_arg
from ..fable_library.reflection import (TypeInfo, class_type)
from ..fable_library.reg_exp import (get_item, groups)
from ..fable_library.seq import (reduce, map, sort, try_pick)
...
from ..fable_library.util import (IEnumerable_1, get_enumerator, dispose, safe_hash, compare_primitives, string_hash)
...
class LDContext:
    ...
    def PropertyNamesMatch(self, p1: str, p2: str) -> bool:
        ...
        if p1 == p2:
            ...
            def _arrow814(__unit: None=None) -> bool:
                p2def_2: str = p2def
                return p1 == p2def_2

            def _arrow815(__unit: None=None) -> bool:
                p1def_2: str = p1def
                return p1def_2 == p2

            def _arrow816(__unit: None=None) -> bool:
                p1def_1: str = p1def
                p2def_1: str = p2def
                return p1def_1 == p2def_1

            return (_arrow814() if (p2def is not None) else False) if (p1def is None) else (_arrow815() if (p2def is None) else _arrow816())
    ...
    def try_combine_optional(base_context: LDContext | None=None, specific_context: LDContext | None=None) -> LDContext | None:
        def _arrow817(__unit: None=None) -> LDContext | None:
            s_1: LDContext = specific_context
            return s_1

        def _arrow818(__unit: None=None) -> LDContext | None:
            f_1: LDContext = base_context
            return f_1

        def _arrow819(__unit: None=None) -> LDContext | None:
            f: LDContext = base_context
            s: LDContext = specific_context
            return LDContext.combine(f, s)

        return (_arrow817() if (specific_context is not None) else None) if (base_context is None) else (_arrow818() if (specific_context is None) else _arrow819())
    ...
    def __hash__(self, __unit: None=None) -> int:
        ...
        mappings_hash: int = box_hash_seq(map(mapping, sort(this.Mappings.keys(), ObjectExpr820())))
        name_hash: int
    ...
    def add_reverse_mapping(self, key: str, value: str) -> None:
        ...
        if active_pattern_result is not None:
            ...
            out_arg: str = None
            def _arrow821(__unit: None=None) -> str:
                return out_arg

            def _arrow822(v: str) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(this.mappings_004045, prefix, FSharpRef(_arrow821, _arrow822)), out_arg)
            ...
            def _arrow823(__unit: None=None) -> tuple[str, str]:
                return out_arg_1

            def _arrow824(v_2: tuple[str, str]) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            pattern_input_1 = (try_get_value(this.compact_reverse_mappings, key, FSharpRef(_arrow823, _arrow824)), out_arg_1)
    ...
    def try_find_term(self, term: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow825(__unit: None=None) -> str:
            return out_arg

        def _arrow826(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.mappings_004045, term, FSharpRef(_arrow825, _arrow826)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        ...
        if definition is None:
            ...
            def _arrow827(__unit: None=None) -> str | None:
                prefix: str = active_pattern_result[0]
                suffix: str = active_pattern_result[1]
            ...
            def _arrow828(__unit: None=None) -> str | None:
                d: str = definition
                return d

            return _arrow827() if (active_pattern_result is not None) else _arrow828()
    ...
    def try_find_iri(self, iri: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow829(__unit: None=None) -> str:
            return out_arg

        def _arrow830(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.reverse_mappings, iri, FSharpRef(_arrow829, _arrow830)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        def chooser(ctx: LDContext) -> str | None:
