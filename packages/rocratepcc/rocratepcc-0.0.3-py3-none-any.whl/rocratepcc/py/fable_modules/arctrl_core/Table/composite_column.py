...
from ...fable_library.reflection import (TypeInfo, array_type, record_type)
from ...fable_library.seq import (exists, to_array, delay, collect, singleton, empty, is_empty, fold)
from ...fable_library.types import (Array, Record)
...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow185(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow184(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow184, this.Cells)

        arr: Array[OntologyAnnotation] = list(to_array(delay(_arrow185)))
...
CompositeColumn_reflection = _expr186
