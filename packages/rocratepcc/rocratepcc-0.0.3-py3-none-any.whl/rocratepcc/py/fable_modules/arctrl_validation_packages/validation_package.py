...
from ..arctrl_core.Helper.hash_codes import (box_hash_array, box_hash_option)
...
from ..fable_library.util import (IEnumerable_1, equals, string_hash)
...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow364(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow363(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow362(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow361(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow361))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow362))

            return append(singleton("{"), delay(_arrow363))

        return join("\n", to_list(delay(_arrow364)))
...
ValidationPackage_reflection = _expr365
