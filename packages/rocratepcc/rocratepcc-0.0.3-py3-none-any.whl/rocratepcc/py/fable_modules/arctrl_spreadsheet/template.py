...
from ..arctrl_core.comment import (Remark, Comment, Comment_reflection)
from ..arctrl_core.conversion import Person_orcidKey
...
from ..arctrl_core.template import (Template, Organisation)
from ..fable_library.date import now
from ..fable_library.guid import parse
from ..fable_library.list import (of_array, FSharpList, empty, map, reverse, append as append_1, of_seq)
from ..fable_library.map_util import add_to_dict
from ..fable_library.option import to_array
from ..fable_library.reflection import (TypeInfo, class_type, string_type, list_type, record_type)
from ..fable_library.seq import (map as map_1, exists, head, append, singleton, delay, to_list, try_find, iterate_indexed, try_pick)
from ..fable_library.seq2 import List_distinct
from ..fable_library.string_ import (starts_with_exact, to_console, printf)
from ..fable_library.types import (FSharpException, Record, to_string, Array)
from ..fable_library.util import (IEnumerable_1, IEnumerator, string_hash, to_enumerable, get_enumerator, ignore)
...
TemplateReadError_reflection = _expr3364
...
Metadata_Template_TemplateInfo_reflection = _expr3365
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow3366(__unit: None=None, template: Any=template) -> str:
        copy_of_struct: str = template.Id
        return str(copy_of_struct)

    def _arrow3367(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if starts_with_exact(_arrow3366(), "MISSING_IDENTIFIER_") else _arrow3367())
    ...
    class ObjectExpr3369:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3368(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr3369())), matrix.ColumnCount)
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow3377(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow3376(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow3375(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow3374(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    def _arrow3373(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow3372(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow3371(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                def _arrow3370(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                    return to_rows_1("Author", of_seq(template.Authors))

                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow3370))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow3371))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow3372))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow3373))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow3374))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow3375))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow3376))

    return delay(_arrow3377)
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    def _arrow3378(__unit: None=None, doc: Any=doc) -> ArcTable:
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow3378(), now())
