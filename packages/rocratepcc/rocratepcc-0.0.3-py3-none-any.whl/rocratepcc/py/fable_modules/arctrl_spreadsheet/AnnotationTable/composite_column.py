...
def from_string_cell_columns(columns: Array[Array[str]]) -> CompositeColumn:
    ...
    def _arrow3230(__unit: None=None, columns: Any=columns) -> IEnumerable_1[CompositeCell]:
        def _arrow3229(i: int) -> CompositeCell:
            def mapping_1(c_1: Array[str]) -> str:
                return c_1[i]

            return pattern_input[1](map(mapping_1, columns, None))

        return map_1(_arrow3229, range_big_int(1, 1, l - 1))

    cells: Array[CompositeCell] = list(to_array(delay(_arrow3230)))
...
def to_string_cell_columns(column: CompositeColumn) -> FSharpList[FSharpList[str]]:
    ...
    if has_unit:
        def _arrow3236(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3235(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3234(i: int) -> str:
                    return get_cell_or_default(i, 0, cells)

                return map_1(_arrow3234, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[0]), delay(_arrow3235))

        def _arrow3239(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3238(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3237(i_1: int) -> str:
                    return get_cell_or_default(i_1, 1, cells)

                return map_1(_arrow3237, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[1]), delay(_arrow3238))

        def _arrow3242(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3241(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3240(i_2: int) -> str:
                    return get_cell_or_default(i_2, 2, cells)

                return map_1(_arrow3240, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[2]), delay(_arrow3241))

        def _arrow3245(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3244(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3243(i_3: int) -> str:
                    return get_cell_or_default(i_3, 3, cells)

                return map_1(_arrow3243, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[3]), delay(_arrow3244))

        return of_array([to_list(delay(_arrow3236)), to_list(delay(_arrow3239)), to_list(delay(_arrow3242)), to_list(delay(_arrow3245))])

    elif is_term:
        def _arrow3251(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3250(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3249(i_4: int) -> str:
                    return get_cell_or_default(i_4, 0, cells)

                return map_1(_arrow3249, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[0]), delay(_arrow3250))

        def _arrow3254(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3253(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3252(i_5: int) -> str:
                    return get_cell_or_default(i_5, 1, cells)

                return map_1(_arrow3252, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[1]), delay(_arrow3253))

        def _arrow3257(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3256(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3255(i_6: int) -> str:
                    return get_cell_or_default(i_6, 2, cells)

                return map_1(_arrow3255, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[2]), delay(_arrow3256))

        return of_array([to_list(delay(_arrow3251)), to_list(delay(_arrow3254)), to_list(delay(_arrow3257))])

    elif is_data:
        def _arrow3263(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3262(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3261(i_7: int) -> str:
                    return get_cell_or_default(i_7, 0, cells)

                return map_1(_arrow3261, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[0]), delay(_arrow3262))

        def _arrow3266(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3265(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3264(i_8: int) -> str:
                    return get_cell_or_default(i_8, 1, cells)

                return map_1(_arrow3264, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[1]), delay(_arrow3265))

        def _arrow3269(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3268(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3267(i_9: int) -> str:
                    return get_cell_or_default(i_9, 2, cells)

                return map_1(_arrow3267, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[2]), delay(_arrow3268))

        return of_array([to_list(delay(_arrow3263)), to_list(delay(_arrow3266)), to_list(delay(_arrow3269))])

    else: 
        def _arrow3272(__unit: None=None, column: Any=column) -> IEnumerable_1[str]:
            def _arrow3271(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3270(i_10: int) -> str:
                    return cells[i_10][0]

                return map_1(_arrow3270, range_big_int(0, 1, len(composite_cells) - 1))

            return append(singleton(header[0]), delay(_arrow3271))

        return singleton_1(to_list(delay(_arrow3272)))
...
def ColumnValueRefs_fromStringCellColumns(value_map: Any, columns: Array[Array[str]]) -> tuple[CompositeHeader, ColumnValueRefs]:
    ...
    if l == 1:
        ...
        for i in range(1, (l - 1) + 1, 1):
            ...
            def _arrow3273(hash_1: int, hash_2: int, value_map: Any=value_map, columns: Any=columns) -> int:
                return merge_hashes(hash_1, hash_2)

            def mapping_2(s: str, value_map: Any=value_map, columns: Any=columns) -> int:
                return string_hash(s)

            hash_3: int = reduce(_arrow3273, map_1(mapping_2, strings)) or 0
            match_value: int | None
            pattern_input_1: tuple[bool, int]
            out_arg: int = None or 0
            def _arrow3274(__unit: None=None, value_map: Any=value_map, columns: Any=columns) -> int:
                return out_arg

            def _arrow3275(v: int, value_map: Any=value_map, columns: Any=columns) -> None:
                nonlocal out_arg
                out_arg = v or 0

            pattern_input_1 = (try_get_value(hash_map, hash_3, FSharpRef(_arrow3274, _arrow3275)), out_arg)
            ...
            if match_value is None:
                ...
                def _arrow3276(__unit: None=None, value_map: Any=value_map, columns: Any=columns) -> bool:
