    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsRow:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(cells.GetCellsInRow(index)) == 0:
                ...
                def _arrow2936(c: FsCell, f: Any=f) -> int:
                    return c.Address.ColumnNumber

                return f(_arrow2936)(cells.GetCellsInRow(index)).Address.ColumnNumber


        def _arrow2939(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2938(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2937:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr2937())

            return _arrow2938

        min_col_index: int = get_index_by(_arrow2939) or 0
        def _arrow2942(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2941(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2940:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr2940())

            return _arrow2941

        max_col_index: int = get_index_by(_arrow2942) or 0
