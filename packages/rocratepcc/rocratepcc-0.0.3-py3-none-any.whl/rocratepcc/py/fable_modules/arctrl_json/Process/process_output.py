...
def _arrow2159(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2160(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2156, ROCrate_decoder_1), map(_arrow2159, ROCrate_decoder_2), map(_arrow2160, ROCrate_decoder_3)]))
...
def _arrow2162(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2163(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2164(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2162, ISAJson_decoder_1), map(_arrow2163, ISAJson_decoder_2), map(_arrow2164, ISAJson_decoder_3)]))
