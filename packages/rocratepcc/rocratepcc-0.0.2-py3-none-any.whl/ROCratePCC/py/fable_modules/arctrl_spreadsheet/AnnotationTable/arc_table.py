...
from ...arctrl_core.Table.arc_table_aux import (ColumnValueRefs, ArcTableValues)
...
from ...fable_library.array_ import (map as map_1, unzip)
from ...fable_library.list import (reverse, map, FSharpList, fold, singleton, cons, is_empty, head, tail, empty, of_array, to_array, exists, collect as collect_1, sort_by, of_seq, length, iterate_indexed)
from ...fable_library.map_util import add_to_dict
from ...fable_library.range import range_big_int
from ...fable_library.reflection import enum_type
from ...fable_library.seq import (try_find, to_array as to_array_1, delay, map as map_2, collect, singleton as singleton_1, fold as fold_1)
from ...fable_library.string_ import (starts_with_exact, to_fail, printf)
from ...fable_library.types import Array
from ...fable_library.util import (IEnumerable_1, compare_primitives, max)
...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow3278(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow3278, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow3282(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow3281(c: int) -> Array[str]:
                    def _arrow3280(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3279(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow3279, range_big_int(1, 1, t.RangeAddress.LastAddress.RowNumber))

                    return to_array_1(delay(_arrow3280))

                return map_2(_arrow3281, range_big_int(1, 1, t.RangeAddress.LastAddress.ColumnNumber))

            pattern_input: tuple[Array[CompositeHeader], ArcTableValues] = compose_arc_table_values(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow3282)), None))
...
def to_fs_worksheet(index: int | None, table: ArcTable) -> FsWorksheet:
    ...
    if table.ColumnCount == 0:
        ...
        def _arrow3283(column_1: CompositeColumn, index: Any=index, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow3284(column: CompositeColumn, index: Any=index, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
            return classify_column_order(column)

        class ObjectExpr3285:
            @property
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow3283, sort_by(_arrow3284, of_seq(table.Columns), ObjectExpr3285()))
        table_row_count: int
        def folder(acc: int, c: FSharpList[str], index: Any=index, table: Any=table) -> int:
            def _arrow3286(x_1: int, y_1: int, acc: Any=acc, c: Any=c) -> int:
                return compare_primitives(x_1, y_1)

            return max(_arrow3286, acc, length(c))
