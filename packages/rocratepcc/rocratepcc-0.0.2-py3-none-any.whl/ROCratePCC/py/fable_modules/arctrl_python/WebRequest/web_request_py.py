...
from ...fable_library.async_builder import (singleton as singleton_1, Async)
from ...fable_library.map import of_list
from ...fable_library.option import map
from ...fable_library.reflection import (TypeInfo, int32_type, string_type, class_type, record_type)
from ...fable_library.seq import (to_list, delay, map as map_1, collect, singleton)
from ...fable_library.types import Record
from ...fable_library.util import (create_obj, IEnumerable_1, compare_primitives)
...
def Requests_createHeadersDict_9F3777D(headers: Any | None=None) -> Any | None:
    def mapping(values: Any, headers: Any=headers) -> Any:
        def _arrow3846(__unit: None=None, values: Any=values) -> IEnumerable_1[tuple[str, Any]]:
            def _arrow3845(pair: Any) -> tuple[str, Any]:
                return (pair[0], pair[1])

            return map_1(_arrow3845, values)

        return create_obj(to_list(delay(_arrow3846)))
...
def Requests_mapResponseType_Z730CA7D4(response: InteropResponseType) -> Response:
    ...
    def _arrow3848(__unit: None=None, response: Any=response) -> IEnumerable_1[tuple[str, str]]:
        def _arrow3847(header_name: str) -> IEnumerable_1[tuple[str, str]]:
            return singleton((header_name, response.headers[header_name]))

        return collect(_arrow3847, list(response.headers.keys()))

    class ObjectExpr3849:
        @property
        def Compare(self) -> Callable[[str, str], int]:
            return compare_primitives

    return Response(status_code, text, of_list(to_list(delay(_arrow3848)), ObjectExpr3849()), encoding)
...
def download_file(url: str) -> Async[str]:
    def _arrow3850(__unit: None=None, url: Any=url) -> Async[str]:
        response: Response = Requests_get_43A074E6(url)
        return singleton_1.Return(response.text)

    return singleton_1.Delay(_arrow3850)
