...
from ...fable_library.list import (try_find, FSharpList, exists, append, singleton, filter)
from ...fable_library.option import (default_arg, map)
from ...fable_library.reflection import (TypeInfo, string_type, option_type, array_type, record_type)
from ...fable_library.types import (Array, to_string, Record)
...
class Factor(Record):
    ...
    def try_get_by_name(name: str, factors: FSharpList[Factor]) -> Factor | None:
        def _arrow113(f: Factor) -> bool:
            return equals(f.Name, name)

        return try_find(_arrow113, factors)

    @staticmethod
    def exists_by_name(name: str, factors: FSharpList[Factor]) -> bool:
        def _arrow114(f: Factor) -> bool:
            return equals(f.Name, name)

        return exists(_arrow114, factors)
    ...
    def remove_by_name(name: str, factors: FSharpList[Factor]) -> FSharpList[Factor]:
        def _arrow115(f: Factor) -> bool:
            return not equals(f.Name, name)

        return filter(_arrow115, factors)
