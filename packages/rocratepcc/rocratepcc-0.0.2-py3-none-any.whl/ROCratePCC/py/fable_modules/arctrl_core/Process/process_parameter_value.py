...
from ...fable_library.option import (default_arg, map, bind)
from ...fable_library.reflection import (TypeInfo, option_type, record_type)
...
class ProcessParameterValue(Record):
    ...
    def SetCategory(self, c: OntologyAnnotation) -> ProcessParameterValue:
        this: ProcessParameterValue = self
        def _arrow140(__unit: None=None) -> ProtocolParameter | None:
        ...
        return ProcessParameterValue(_arrow140(), this.Value, this.Unit)
    ...
    def PrintCompact(self, __unit: None=None) -> str:
        ...
        def _arrow141(__unit: None=None) -> str:
            value_2: str = value
            return value_2
        ...
        def _arrow143(__unit: None=None) -> str:
            category_1: str = category
            value_1: str = value
            return (category_1 + ":") + value_1

        return ("" if (value is None) else _arrow141()) if (category is None) else (_arrow142() if (value is None) else _arrow143())
...
ProcessParameterValue_reflection = _expr144
