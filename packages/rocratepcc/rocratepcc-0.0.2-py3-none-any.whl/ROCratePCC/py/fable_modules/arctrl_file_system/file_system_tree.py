...
from ..fable_library.array_ import (try_find, exists, try_pick, head, tail, map, filter, equals_with, iterate, choose, append as append_1)
from ..fable_library.list import (cons, to_array as to_array_1, reverse, FSharpList, empty)
from ..fable_library.option import default_arg
from ..fable_library.reflection import (TypeInfo, string_type, array_type, union_type)
from ..fable_library.seq import (to_array, delay, append, singleton)
from ..fable_library.seq2 import (Array_groupBy, Array_distinct)
from ..fable_library.types import (Array, Union)
from ..fable_library.util import (IEnumerable_1, string_hash, array_hash, curry2)
...
class FileSystemTree(Union):
    ...
    def try_get_child_by_name(name: str) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow3(fst: FileSystemTree) -> FileSystemTree | None:
    ...
    def contains_child_with_name(name: str) -> Callable[[FileSystemTree], bool]:
        def _arrow4(fst: FileSystemTree) -> bool:
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow6(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow5(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow5))

        file_paths: Array[str] = to_array(delay(_arrow6))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow7(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr9:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow8(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr9()), None)
            if parent.tag == 1:
                def _arrow11(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow10(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow10))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow11)))
        ...
        class ObjectExpr14:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow13(x: Array[str], y: Array[str]) -> bool:
                    def _arrow12(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow12, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr14()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def to_file_paths(remove_root: bool | None=None) -> Callable[[FileSystemTree], Array[str]]:
        def _arrow15(root: FileSystemTree) -> Array[str]:
    ...
    def filter_files(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow16(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter_folders(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow17(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow18(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def Union(self, other_fst: FileSystemTree) -> FileSystemTree:
        this: FileSystemTree = self
        def _arrow19(__unit: None=None) -> Array[str]:
        ...
        paths: Array[str] = Array_distinct(_arrow19(), ObjectExpr21())
...
FileSystemTree_reflection = _expr22
