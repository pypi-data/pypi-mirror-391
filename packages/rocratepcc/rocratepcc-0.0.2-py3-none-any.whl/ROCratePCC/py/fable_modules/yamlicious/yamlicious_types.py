...
def Config__get_WhitespaceString(this: Config) -> str:
    def _arrow3383(_arg: int, this: Any=this) -> str:
        return " "

    return initialize(this.Level * this.Whitespace, _arrow3383)


def _expr3384() -> TypeInfo:
...
Preprocessor_reflection = _expr3384
...
class PreprocessorElement(Union):
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def innerprint(next_1: PreprocessorElement, level: int) -> None:
            def _arrow3385(_arg: int, next_1: Any=next_1, level: Any=level) -> str:
                return " "

            indent: str = initialize(level * 2, _arrow3385)
            if next_1.tag == 1:
...
PreprocessorElement_reflection = _expr3386
...
YAMLElement_reflection = _expr3388
