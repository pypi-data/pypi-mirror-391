...
class LDPostalAddress:
    ...
    def gen_id(address_country: str | None=None, postal_code: str | None=None, street_address: str | None=None) -> str:
        def _arrow989(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow988(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow987(__unit: None=None) -> IEnumerable_1[str]:
                    return singleton("streetAddress") if (street_address is not None) else empty()

                return append(singleton("postalCode") if (postal_code is not None) else empty(), delay(_arrow987))

            return append(singleton("addressCountry") if (address_country is not None) else empty(), delay(_arrow988))

        items: FSharpList[str] = to_list(delay(_arrow989))
        def _arrow990(__unit: None=None) -> str:
        ...
        return clean(create_missing_identifier() if is_empty(items) else _arrow990())
...
LDPostalAddress_reflection = _expr991
