...
def encode_rows(cell: FsCell) -> IEncodable:
    def _arrow3050(__unit: None=None, cell: Any=cell) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("column", _arrow3050()), ("value", encode(cell.Value))])
...
def decode_rows(row_number: int | None=None) -> Decoder_1[FsCell]:
    def _arrow3054(builder: IGetters, row_number: Any=row_number) -> FsCell:
        def _arrow3052(__unit: None=None) -> tuple[Any, DataType] | None:
            object_arg: IOptionalGetter = builder.Optional
            return object_arg.Field("value", decode)

        pattern_input: tuple[Any, DataType] = default_arg(_arrow3052(), ("", DataType(4)))
        def _arrow3053(__unit: None=None) -> int | None:
            object_arg_1: IOptionalGetter = builder.Optional
            return object_arg_1.Field("column", int_1)

        c: int = default_arg(_arrow3053(), 0) or 0
        return FsCell(pattern_input[0], pattern_input[1], FsAddress(default_arg(row_number, 0), c))

    return object(_arrow3054)


def encode_cols(cell: FsCell) -> IEncodable:
    def _arrow3056(__unit: None=None, cell: Any=cell) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("row", _arrow3056()), ("value", encode(cell.Value))])
...
def decode_cols(col_number: int | None=None) -> Decoder_1[FsCell]:
    def _arrow3062(builder: IGetters, col_number: Any=col_number) -> FsCell:
        def _arrow3058(__unit: None=None) -> tuple[Any, DataType] | None:
            object_arg: IOptionalGetter = builder.Optional
            return object_arg.Field("value", decode)

        pattern_input: tuple[Any, DataType] = default_arg(_arrow3058(), ("", DataType(4)))
