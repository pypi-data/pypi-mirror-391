...
class FsWorkbook(IDisposable):
    ...
    def RemoveWorksheet(self, name: str) -> None:
        self_1: FsWorkbook = self
        def _arrow2971(__unit: None=None) -> FsWorksheet:
            try: 
                def _arrow2970(ws: FsWorksheet) -> bool:
                    return ws.Name == name

                return default_arg(try_find_1(_arrow2970, self_1._worksheets), None)

            except Exception as match_value:
                raise Exception(("FsWorksheet with name " + name) + " was not found in FsWorkbook.")


        class ObjectExpr2972:
            @property
        ...
        ignore(remove_in_place(_arrow2971(), self_1._worksheets, ObjectExpr2972()))
...
FsWorkbook_reflection = _expr2973
