...
from ...arctrl_core.conversion import (JsonTypes_decomposeTechnologyPlatform, JsonTypes_composeTechnologyPlatform)
from ...arctrl_core.Helper.collections_ import (Option_fromValueWithDefault, ResizeArray_iter)
from ...arctrl_core.Helper.identifier import (create_missing_identifier, Assay_tryIdentifierFromFileName, remove_missing_identifier, Assay_fileNameFromIdentifier)
...
def from_sparse_table(matrix: SparseTable) -> FSharpList[ArcAssay]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow3158(i: int, matrix: Any=matrix) -> ArcAssay:
        ...
        return initialize(matrix.ColumnCount, _arrow3158)
...
def to_sparse_table(assays: FSharpList[ArcAssay]) -> SparseTable:
    ...
    class ObjectExpr3160:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3159(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr3160())), matrix.ColumnCount)
