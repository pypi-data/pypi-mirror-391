...
def _007CStudyPath_007C__007C(input: Array[str]) -> str | None:
    ...
    def _arrow3646(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 3) if (not equals_with(_arrow3646, input, None)) else False:
        if input[0] == "studies":
...
def ARCtrl_ArcStudy__ArcStudy_ToCreateContract_6FCE9E49(this: ArcStudy, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow3651(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow3649(__unit: None=None) -> IEnumerable_1[Contract]:
            ...
            def _arrow3648(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(10))) if ((p != "studies/.gitkeep") if (p != path) else False) else empty()

            return collect(_arrow3648, folder_fs.ToFilePaths(False))

        def _arrow3650(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow3649() if with_folder else empty(), delay(_arrow3650))

    return to_array(delay(_arrow3651))
