...
class FsCellsCollection:
    ...
    def Add(self, cell: FsCell, row: int | None=None, column: int | None=None) -> None:
        ...
        add_to_dict(_arrow2915(), column_1, cell)
    ...
    def RemoveCellAt(self, row: int, column: int) -> None:
        ...
        if (row == this._maxRowUsed) if row_removed else False:
            ...
            this._maxRowUsed = (max(this._rowsUsed.keys(), ObjectExpr2917()) if (not is_empty(this._rowsUsed.keys())) else 0) or 0

        if (column == this._maxColumnUsed) if column_removed else False:
            class ObjectExpr2918:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            this._maxColumnUsed = (max(this._columnsUsed.keys(), ObjectExpr2918()) if (not is_empty(this._columnsUsed.keys())) else 0) or 0
        ...
        if match_value is None:
            ...
            def _arrow2919(__unit: None=None) -> bool:
    ...
    def GetCellsInRangeBy(self, row_start: int, column_start: int, row_end: int, column_end: int, predicate: Callable[[FsCell], bool]) -> IEnumerable_1[FsCell]:
        ...
        def _arrow2923(__unit: None=None) -> IEnumerable_1[FsCell]:
            def _arrow2922(ro: int) -> IEnumerable_1[FsCell]:
                ...
                if match_value is None:
                    ...
                    def _arrow2921(co: int) -> IEnumerable_1[FsCell]:
                    ...
                    return collect(_arrow2921, range_big_int(column_start, 1, final_column))


            return collect(_arrow2922, range_big_int(row_start, 1, final_row))

        return delay(_arrow2923)
    ...
    def GetCellsInRange(self, row_start: int, column_start: int, row_end: int, column_end: int) -> IEnumerable_1[FsCell]:
        ...
        final_row: int = (this._maxRowUsed if (row_end > this._maxRowUsed) else row_end) or 0
        final_column: int = (this._maxColumnUsed if (column_end > this._maxColumnUsed) else column_end) or 0
        def _arrow2927(__unit: None=None) -> IEnumerable_1[FsCell]:
            def _arrow2926(ro: int) -> IEnumerable_1[FsCell]:
                match_value: Any | None = Dictionary_tryGet(ro, this._rowsCollection)
                if match_value is None:
                    return empty()
                    ...
                    columns_collection: Any = match_value
                    def _arrow2925(co: int) -> IEnumerable_1[FsCell]:
                        match_value_1: FsCell | None = Dictionary_tryGet(co, columns_collection)
                        if match_value_1 is not None:
                    ...
                    return collect(_arrow2925, range_big_int(column_start, 1, final_column))


            return collect(_arrow2926, range_big_int(row_start, 1, final_row))

        return delay(_arrow2927)
    ...
    def GetFirstAddress(self, __unit: None=None) -> FsAddress:
        this: FsCellsCollection = self
        class ObjectExpr2928:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        def _arrow2932(__unit: None=None) -> int:
            def projection(d: Any) -> int:
                ...
                return min(d.keys(), ObjectExpr2929())
            ...
            d_1: Any = min_by(projection, this._rowsCollection.values(), ObjectExpr2930())
            class ObjectExpr2931:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            return min(d_1.keys(), ObjectExpr2931())

        return FsAddress(0, 0) if (True if is_empty(this._rowsCollection) else is_empty(this._rowsCollection.keys())) else FsAddress(min(this._rowsCollection.keys(), ObjectExpr2928()), _arrow2932())
...
FsCellsCollection_reflection = _expr2933
