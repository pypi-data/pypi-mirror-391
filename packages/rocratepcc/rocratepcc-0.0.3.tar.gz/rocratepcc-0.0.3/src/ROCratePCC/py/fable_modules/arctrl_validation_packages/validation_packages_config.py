...
class ValidationPackagesConfig:
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow377(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow376(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow376, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow377))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr378
