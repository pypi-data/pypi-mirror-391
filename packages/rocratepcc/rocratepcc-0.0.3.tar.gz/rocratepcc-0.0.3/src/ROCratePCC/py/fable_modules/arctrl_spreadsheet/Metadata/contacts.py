...
from ...arctrl_core.conversion import (Person_setOrcidFromComments, Person_setCommentFromORCID)
from ...arctrl_core.Helper.collections_ import ResizeArray_iter
from ...arctrl_core.ontology_annotation import OntologyAnnotation
from ...arctrl_core.person import Person
from ...fable_library.list import (of_array, FSharpList, length, singleton, initialize, map, empty, iterate_indexed, cons, reverse)
...
def from_string(last_name: str | None, first_name: str | None, mid_initials: str | None, email: str | None, phone: str | None, fax: str | None, address: str | None, affiliation: str | None, role: str, roles_term_accession_number: str, roles_term_source_ref: str, comments: Array[Comment]) -> Person:
    ...
    return Person_setOrcidFromComments(_arrow3117())
...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Person]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow3118(__unit: None=None, matrix: Any=matrix) -> Person:
            return_val: Person = Person.create()
            return_val.Comments = comments
            return return_val

        return singleton(_arrow3118())

    else: 
        def _arrow3119(i: int, matrix: Any=matrix) -> Person:
        ...
        return initialize(matrix.ColumnCount, _arrow3119)
...
def to_sparse_table(persons: FSharpList[Person]) -> SparseTable:
    ...
    class ObjectExpr3121:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3120(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr3121())), matrix.ColumnCount)
