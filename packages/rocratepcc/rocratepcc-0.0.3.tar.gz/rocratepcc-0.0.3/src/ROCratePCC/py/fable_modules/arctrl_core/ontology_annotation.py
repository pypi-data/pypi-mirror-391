...
from ..fable_library.option import (default_arg, value as value_4)
from ..fable_library.reflection import (TypeInfo, class_type)
from ..fable_library.seq import (to_list, delay, append, singleton, empty)
from ..fable_library.string_ import (join, to_text, printf)
from ..fable_library.system_text import (StringBuilder__ctor, StringBuilder__Append_Z721C83C5)
from ..fable_library.types import (Array, to_string)
from ..fable_library.util import (ignore, IEnumerable_1, string_hash, safe_hash)
...
class OntologyAnnotation:
    ...
    def to_string_object(oa: OntologyAnnotation, as_ontobee_purl_url_if_short: bool | None=None) -> dict[str, Any]:
        ...
        def _arrow80(__unit: None=None) -> str:
            url: str = oa.TermAccessionAndOntobeeUrlIfShort
            return default_arg(oa.TermAccessionNumber, "") if (url == "") else url

        return {
            "TermAccessionNumber": _arrow80() if as_ontobee_purl_url_if_short_1 else default_arg(oa.TermAccessionNumber, ""),
            "TermName": TermName,
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow88(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow81(__unit: None=None) -> str:
                arg: str = value_4(this.Name)
                return to_text(printf("Name = %A"))(arg)

            def _arrow87(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow82(__unit: None=None) -> str:
                    arg_1: str = value_4(this.TermSourceREF)
                    return to_text(printf("TSR = %A"))(arg_1)

                def _arrow86(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow83(__unit: None=None) -> str:
                        arg_2: str = value_4(this.TermAccessionNumber)
                        return to_text(printf("TAN = %A"))(arg_2)

                    def _arrow85(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow84(__unit: None=None) -> str:
                            arg_3: Array[Comment] = this.Comments
                            return to_text(printf("Comments = %A"))(arg_3)

                        return singleton(_arrow84()) if (len(this.Comments) != 0) else empty()

                    return append(singleton(_arrow83()) if (this.TermAccessionNumber is not None) else empty(), delay(_arrow85))

                return append(singleton(_arrow82()) if (this.TermSourceREF is not None) else empty(), delay(_arrow86))

            return append(singleton(_arrow81()) if (this.Name is not None) else empty(), delay(_arrow87))

        ignore(StringBuilder__Append_Z721C83C5(sb, join("; ", to_list(delay(_arrow88)))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "}"))
        return to_string(sb)
    ...
    def __hash__(self, __unit: None=None) -> int:
        this: OntologyAnnotation = self
        def _arrow89(__unit: None=None) -> int:
        ...
        def _arrow91(__unit: None=None) -> int:
            ...
            if match_value is not None:
                ...
                def _arrow90(__unit: None=None) -> int:
                    taninfo: dict[str, Any] = match_value_1
                    return merge_hashes(string_hash(taninfo["IDSpace"].lower()), string_hash(taninfo["LocalID"]))

                return 0 if (match_value_1 is None) else _arrow90()


        return merge_hashes(_arrow89(), _arrow91())
    ...
    def compute_tan_info(tan: str | None=None, tsr: str | None=None) -> dict[str, Any] | None:
        if tan is None:
            ...
            def _arrow92(__unit: None=None) -> dict[str, Any] | None:
                tsr_1: str = tsr
                return {
                    "IDSpace": tsr_1,
                    "LocalID": tan_1
            ...
            return (None if (tsr is None) else (None if (tsr == "") else _arrow92())) if (match_value is None) else match_value
...
OntologyAnnotation_reflection = _expr93
