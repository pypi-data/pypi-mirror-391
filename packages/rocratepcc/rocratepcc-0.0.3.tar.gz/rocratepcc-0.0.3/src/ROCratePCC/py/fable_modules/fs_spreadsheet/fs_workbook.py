...
class FsWorkbook(IDisposable):
    ...
    def RemoveWorksheet(self, name: str) -> None:
        self_1: FsWorkbook = self
        def _arrow2970(__unit: None=None) -> FsWorksheet:
            try: 
                def _arrow2969(ws: FsWorksheet) -> bool:
                    return ws.Name == name

                return default_arg(try_find_1(_arrow2969, self_1._worksheets), None)

            except Exception as match_value:
                raise Exception(("FsWorksheet with name " + name) + " was not found in FsWorkbook.")


        class ObjectExpr2971:
            @property
        ...
        ignore(remove_in_place(_arrow2970(), self_1._worksheets, ObjectExpr2971()))
...
FsWorkbook_reflection = _expr2972
