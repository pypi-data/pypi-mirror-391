...
from ...fable_library.list import (is_empty as is_empty_1, tail, FSharpList, head, append, singleton, empty)
from ...fable_library.map_util import (add_to_dict, try_get_value, remove_from_dict)
...
from ...fable_library.seq import (is_empty, iterate, map)
from ...fable_library.seq2 import group_by
from ...fable_library.types import (FSharpRef, Array)
from ...fable_library.util import (equals, structural_hash, IEnumerable_1, ignore, get_enumerator, dispose, min, compare_primitives)
...
def Dictionary_ofSeq(s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr55())
...
def Dictionary_tryFind(key: Any, dict_1: Any) -> Any | None:
    ...
    out_arg: _T = None
    def _arrow56(__unit: None=None, key: Any=key, dict_1: Any=dict_1) -> _T:
        return out_arg

    def _arrow57(v: _T | None=None, key: Any=key, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, key, FSharpRef(_arrow56, _arrow57)), out_arg)
    if pattern_input[0]:
...
def Dictionary_ofSeqWithMerge(merge: Callable[[_T, _T], _T], s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr58())
...
def ResizeArray_map2(f: Callable[[__A, __B], __C], a: Array[Any], b: Array[Any]) -> Array[Any]:
    c: Array[__C] = []
    def _arrow59(x: int, y: int, f: Any=f, a: Any=a, b: Any=b) -> int:
        return compare_primitives(x, y)

    n: int = min(_arrow59, len(a), len(b)) or 0
...
def ResizeArray_distinct(a: Array[Any]) -> Array[Any]:
    ...
    try: 
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if not contains(i, b, ObjectExpr60()):
                (b.append(i))
...
def ResizeArray_take(n: int, a: Array[Any]) -> Array[Any]:
    b: Array[__A] = []
    def _arrow61(x: int, y: int, n: Any=n, a: Any=a) -> int:
        return compare_primitives(x, y)

    n_1: int = min(_arrow61, n, len(a)) or 0
...
def ResizeArray_groupBy(f: Callable[[_T], _A], a: Array[Any]) -> Array[tuple[_A, Array[_T]]]:
    ...
    return list(map(mapping, group_by(f, a, ObjectExpr62())))
...
def ResizeArray_zip(a: Array[Any], b: Array[Any]) -> Array[tuple[_T, _U]]:
    c: Array[tuple[_T, _U]] = []
    def _arrow63(x: int, y: int, a: Any=a, b: Any=b) -> int:
        return compare_primitives(x, y)

    n: int = min(_arrow63, len(a), len(b)) or 0
    for i in range(0, (n - 1) + 1, 1):
        (c.append((a[i], b[i])))
    return c
