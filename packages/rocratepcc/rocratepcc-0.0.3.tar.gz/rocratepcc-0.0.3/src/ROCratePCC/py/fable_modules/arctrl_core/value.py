...
from ..fable_library.reflection import (TypeInfo, int32_type, float64_type, string_type, union_type)
from ..fable_library.string_ import (to_text, printf)
from ..fable_library.types import (FSharpRef, to_string, Array, Union)
...
class Value(Union):
    ...
    def from_string(value: str) -> Value:
        ...
        out_arg: int = 0
        def _arrow74(__unit: None=None) -> int:
            return out_arg

        def _arrow75(v: int) -> None:
            nonlocal out_arg
            out_arg = v or 0

        match_value = (try_parse(value, 511, False, 32, FSharpRef(_arrow74, _arrow75)), out_arg)
        if match_value[0]:
            ...
            out_arg_1: float = 0.0
            def _arrow76(__unit: None=None) -> float:
                return out_arg_1

            def _arrow77(v_2: float) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            match_value_1 = (try_parse_1(value, FSharpRef(_arrow76, _arrow77)), out_arg_1)
    ...
    def from_options(value: str | None=None, term_source: str | None=None, term_accesssion: str | None=None) -> Value | None:
        def _arrow78(__unit: None=None) -> Value | None:
            value_1: str = value
            return Value.from_string(value_1)

        return ((None if (term_accesssion is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (term_source is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (value is None) else ((_arrow78() if (term_accesssion is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion))) if (term_source is None) else Value(0, OntologyAnnotation.create(default_arg(value, ""), term_source, term_accesssion)))
