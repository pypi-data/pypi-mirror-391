...
from ...fable_library.int32 import op_unary_negation_int32
from ...fable_library.list import (cons, is_empty as is_empty_1, tail as tail_1, head, FSharpList, empty as empty_1, of_seq, length as length_1, exists, pick, map_indexed)
from ...fable_library.map_util import (add_to_dict, get_item_from_dict, remove_from_dict, try_get_value)
from ...fable_library.option import (value as value_3, bind)
from ...fable_library.range import range_big_int
from ...fable_library.reflection import (TypeInfo, int32_type, class_type, union_type)
from ...fable_library.seq import (max_by, iterate, length, item, iterate_indexed, max, is_empty, to_list, delay, collect, singleton, empty, map, try_find_index, filter)
from ...fable_library.string_ import (to_fail, printf, to_console)
from ...fable_library.types import (Array, Union, to_string)
from ...fable_library.util import (safe_hash, compare_primitives, get_enumerator, dispose, ignore, IEnumerable_1, IEnumerator, to_iterator, equals, compare, max as max_1)
...
ColumnValueRefs_reflection = _expr192
...
def ColumnValueRefs__get_RowCount(this: ColumnValueRefs) -> Any | None:
    if this.tag == 1:
        ...
        return max_by(projection, this.fields[0], ObjectExpr193())
...
def ColumnValueRefs__ToCellColumn_Z2609C9DD(this: ColumnValueRefs, value_map: Any, row_count: int, header: CompositeHeader) -> Array[CompositeCell]:
    if this.tag == 1:
        ...
        default_cell: CompositeCell = get_empty_cell_for_header(header, None) if (len(values) == 0) else get_empty_cell_for_header(header, get_item_from_dict(value_map, get_item_from_dict(values, max(values.keys(), ObjectExpr194()) - 1)))
        def _arrow195(i_1: int, this: Any=this, value_map: Any=value_map, row_count: Any=row_count, header: Any=header) -> CompositeCell:
            return get_item_from_dict(value_map, get_item_from_dict(values, i_1)) if (i_1 in values) else default_cell

        return ResizeArray_init(row_count, _arrow195)
...
class ArcTableValues:
    ...
    def ColumnCount(self, __unit: None=None) -> int:
        ...
        return 0 if is_empty(this._columns) else (1 + max(this._columns.keys(), ObjectExpr196()))
    ...
    def from_cell_columns(columns: Array[Array[CompositeCell]]) -> ArcTableValues:
        ...
        row_count: int = (0 if (len(columns) == 0) else max(ResizeArray_map(len, columns), ObjectExpr197())) or 0
        value_map: Any = dict([])
    ...
    def ToCellColumns(self, headers: Array[CompositeHeader]) -> Array[Array[CompositeCell]]:
        this: ArcTableValues = self
        def _arrow198(i: int) -> Array[CompositeCell]:
        ...
        return ResizeArray_init(len(headers), _arrow198)
    ...
    def __hash__(self, __unit: None=None) -> int:
        ...
        for i in range(0, (this.ColumnCount - 1) + 1, 1):
            ...
            def _arrow199(__unit: None=None) -> ColumnValueRefs:
                return out_arg

            def _arrow200(v: ColumnValueRefs) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(this._columns, i, FSharpRef(_arrow199, _arrow200)), out_arg)
    ...
    def Item(self, cr_indices: tuple[int, int]) -> CompositeCell:
        ...
        pattern_input: tuple[bool, ColumnValueRefs]
        out_arg: ColumnValueRefs = None
        def _arrow201(__unit: None=None) -> ColumnValueRefs:
            return out_arg

        def _arrow202(v: ColumnValueRefs) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this._columns, col, FSharpRef(_arrow201, _arrow202)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        if match_value is not None:
    ...
    def GetEnumerator(self, __unit: None=None) -> IEnumerator[Any]:
        this: ArcTableValues = self
        def _arrow208(__unit: None=None) -> IEnumerable_1[Any]:
            def _arrow207(col_i: int) -> IEnumerable_1[Any]:
                match_value: ColumnValueRefs | None
                pattern_input: tuple[bool, ColumnValueRefs]
                out_arg: ColumnValueRefs = None
                def _arrow203(__unit: None=None) -> ColumnValueRefs:
                    return out_arg

                def _arrow204(v: ColumnValueRefs) -> None:
                    nonlocal out_arg
                    out_arg = v

                pattern_input = (try_get_value(this._columns, col_i, FSharpRef(_arrow203, _arrow204)), out_arg)
                match_value = pattern_input[1] if pattern_input[0] else None
                if match_value is not None:
                    match_value_1: ColumnValueRefs = match_value
                    if match_value_1.tag == 1:
                        values: Any = match_value_1.fields[0]
                        def _arrow205(row_i_1: int) -> IEnumerable_1[Any]:
                        ...
                        return collect(_arrow205, range_big_int(0, 1, this._rowCount - 1))

                    else: 
                        def _arrow206(row_i: int) -> tuple[tuple[int, int], CompositeCell]:
                            return ((col_i, row_i), get_item_from_dict(this._valueMap, match_value_1.fields[0]))

                        return map(_arrow206, range_big_int(0, 1, this._rowCount - 1))
            ...
            return collect(_arrow207, range_big_int(0, 1, this.ColumnCount - 1))

        return get_enumerator(delay(_arrow208))
...
ArcTableValues_reflection = _expr209
...
def _007CIsUniqueExistingHeader_007C__007C(existing_headers: IEnumerable_1[CompositeHeader], input: CompositeHeader) -> int | None:
    if ((((input.tag == 3) or (input.tag == 2)) or (input.tag == 1)) or (input.tag == 0)) or (input.tag == 13):
        ...
        def _arrow210(h: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h.tag == 12) else False

        return try_find_index(_arrow210, existing_headers)

    elif input.tag == 11:
        def _arrow211(h_1: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return True if (h_1.tag == 11) else False

        return try_find_index(_arrow211, existing_headers)

    else: 
        def _arrow212(h_2: CompositeHeader, existing_headers: Any=existing_headers, input: Any=input) -> bool:
            return equals(h_2, input)

        return try_find_index(_arrow212, existing_headers)
...
def SanityChecks_validateColumnIndex(index: int, column_count: int, allow_append: bool) -> None:
    ...
    def _arrow213(__unit: None=None, index: Any=index, column_count: Any=column_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validateRowIndex(index: int, row_count: int, allow_append: bool) -> None:
    ...
    def _arrow214(__unit: None=None, index: Any=index, row_count: Any=row_count, allow_append: Any=allow_append) -> bool:
        x: int = index or 0
...
def SanityChecks_validateCellColumns(headers: Array[CompositeHeader], columns: Array[Array[CompositeCell]], raise_exception: bool) -> bool:
    ...
    if (len(columns) != 0) if (len(headers) != len(columns)) else False:
        def _arrow215(message: str, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> None:
            raise Exception(message)

        def _arrow217(__unit: None=None, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> Callable[[str], None]:
            clo: Callable[[str], None] = to_console(printf("%s"))
            def _arrow216(arg: str) -> None:
                clo(arg)

            return _arrow216

        (_arrow215 if raise_exception else _arrow217())(((("Invalid table. Number of headers (" + str(len(headers))) + ") does not match number of columns (") + str(len(columns))) + ").")
        is_valid = False
    ...
    while (len(columns) != 0) if (en.System_Collections_IEnumerator_MoveNext() if is_valid else False) else False:
        ...
        while col_en.System_Collections_IEnumerator_MoveNext() if is_valid else False:
            ...
            if (cell_is_not_freetext if (not cell.is_data) else False) if header_is_data else False:
                def _arrow218(message_1: str, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> None:
                    raise Exception(message_1)

                def _arrow220(__unit: None=None, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                    clo_1: Callable[[str], None] = to_console(printf("%s"))
                    def _arrow219(arg_1: str) -> None:
                        clo_1(arg_1)

                    return _arrow219

                (_arrow218 if raise_exception else _arrow220())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Data header should contain either Data or Freetext cells.")
                is_valid = False

            if cell_is_not_freetext if header_is_freetext else False:
                def _arrow221(message_2: str, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> None:
                    raise Exception(message_2)

                def _arrow223(__unit: None=None, headers: Any=headers, columns: Any=columns, raise_exception: Any=raise_exception) -> Callable[[str], None]:
                    clo_2: Callable[[str], None] = to_console(printf("%s"))
                    def _arrow222(arg_2: str) -> None:
                        clo_2(arg_2)

                    return _arrow222

                (_arrow221 if raise_exception else _arrow223())(((("Invalid combination of header `" + str(header)) + "` and cell `") + str(cell)) + "`. Freetext header should not contain non-freetext cells.")
                is_valid = False
...
def SanityChecks_validateArcTableValues(headers: Array[CompositeHeader], values: ArcTableValues, raise_exception: bool) -> bool:
    ...
    if (values.ColumnCount != 0) if (len(headers) != values.ColumnCount) else False:
        def _arrow224(message: str, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
            raise Exception(message)

        def _arrow226(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> Callable[[str], None]:
            clo: Callable[[str], None] = to_console(printf("%s"))
            def _arrow225(arg: str) -> None:
                clo(arg)

            return _arrow225

        (_arrow224 if raise_exception else _arrow226())(((("Invalid table. Number of headers (" + str(len(headers))) + ") does not match number of columns (") + str(values.ColumnCount)) + ").")
        is_valid = False
    ...
    if (values.ColumnCount != 0) if (values.RowCount != 0) else False:
        while (col_index < values.ColumnCount) if is_valid else False:
            ...
            pattern_input: tuple[bool, ColumnValueRefs]
            out_arg: ColumnValueRefs = None
            def _arrow227(__unit: None=None, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> ColumnValueRefs:
                return out_arg

            def _arrow228(v: ColumnValueRefs, headers: Any=headers, values: Any=values, raise_exception: Any=raise_exception) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(values.Columns, key, FSharpRef(_arrow227, _arrow228)), out_arg)
            match_value = pattern_input[1] if pattern_input[0] else None
            if match_value is not None:
                if match_value.tag == 1:
...
def Unchecked_tryGetCellAt(column: int, row: int, values: ArcTableValues) -> CompositeCell | None:
    ...
    def _arrow231(__unit: None=None, column: Any=column, row: Any=row, values: Any=values) -> ColumnValueRefs | None:
        pattern_input: tuple[bool, ColumnValueRefs]
        out_arg: ColumnValueRefs = None
        def _arrow229(__unit: None=None) -> ColumnValueRefs:
            return out_arg

        def _arrow230(v: ColumnValueRefs) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(values.Columns, column, FSharpRef(_arrow229, _arrow230)), out_arg)
        return pattern_input[1] if pattern_input[0] else None

    return bind(binder, _arrow231())
...
def Unchecked_getCellWithDefault(column: int, row: int, headers: Array[CompositeHeader], values: ArcTableValues) -> CompositeCell:
    ...
    pattern_input: tuple[bool, ColumnValueRefs]
    out_arg: ColumnValueRefs = None
    def _arrow232(__unit: None=None, column: Any=column, row: Any=row, headers: Any=headers, values: Any=values) -> ColumnValueRefs:
        return out_arg

    def _arrow233(v: ColumnValueRefs, column: Any=column, row: Any=row, headers: Any=headers, values: Any=values) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(values.Columns, column, FSharpRef(_arrow232, _arrow233)), out_arg)
    match_value = pattern_input[1] if pattern_input[0] else None
    if match_value is None:
        ...
        if col.tag == 1:
            ...
            if row in vals:
                ...
                max_i: int = max(vals.keys(), ObjectExpr234()) or 0
...
def Unchecked_setCellAt(column_index: int, row_index: int, c: CompositeCell, values: ArcTableValues) -> None:
    ...
    pattern_input: tuple[bool, ColumnValueRefs]
    out_arg: ColumnValueRefs = None
    def _arrow235(__unit: None=None, column_index: Any=column_index, row_index: Any=row_index, c: Any=c, values: Any=values) -> ColumnValueRefs:
        return out_arg

    def _arrow236(v: ColumnValueRefs, column_index: Any=column_index, row_index: Any=row_index, c: Any=c, values: Any=values) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(values.Columns, column_index, FSharpRef(_arrow235, _arrow236)), out_arg)
    match_value = pattern_input[1] if pattern_input[0] else None
    if match_value is not None:
        col: ColumnValueRefs = match_value
        if col.tag == 1:
...
def Unchecked_removeCellAt(column_index: int, row_index: int, values: ArcTableValues) -> None:
    ...
    pattern_input: tuple[bool, ColumnValueRefs]
    out_arg: ColumnValueRefs = None
    def _arrow237(__unit: None=None, column_index: Any=column_index, row_index: Any=row_index, values: Any=values) -> ColumnValueRefs:
        return out_arg

    def _arrow238(v: ColumnValueRefs, column_index: Any=column_index, row_index: Any=row_index, values: Any=values) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(values.Columns, column_index, FSharpRef(_arrow237, _arrow238)), out_arg)
    match_value = pattern_input[1] if pattern_input[0] else None
    if match_value is not None:
        col: ColumnValueRefs = match_value
        if col.tag == 1:
...
def Unchecked_moveColumnCellsTo(from_col: int, to_col: int, values: ArcTableValues) -> None:
    ...
    pattern_input: tuple[bool, ColumnValueRefs]
    out_arg: ColumnValueRefs = None
    def _arrow239(__unit: None=None, from_col: Any=from_col, to_col: Any=to_col, values: Any=values) -> ColumnValueRefs:
        return out_arg

    def _arrow240(v: ColumnValueRefs, from_col: Any=from_col, to_col: Any=to_col, values: Any=values) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(values.Columns, from_col, FSharpRef(_arrow239, _arrow240)), out_arg)
    match_value = pattern_input[1] if pattern_input[0] else None
    if match_value is not None:
        col: ColumnValueRefs = match_value
...
def Unchecked_addRefColumn(new_header: CompositeHeader, new_col: ColumnValueRefs, row_count: int, index: int, force_replace: bool, only_headers: bool, headers: Array[CompositeHeader], values: ArcTableValues) -> None:
    ...
    if (has_duplicate_unique is None) if (index_1 < start_col_count) else False:
        def _arrow241(x: int, y: int, new_header: Any=new_header, new_col: Any=new_col, row_count: Any=row_count, index: Any=index, force_replace: Any=force_replace, only_headers: Any=only_headers, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_column_index: int = max_1(_arrow241, start_col_count - 1, 0) or 0
...
def Unchecked_fillMissingCells(headers: Array[CompositeHeader], values: ArcTableValues) -> None:
    if values.RowCount == 0:
        ...
        for i in range(0, (values.ColumnCount - 1) + 1, 1):
            ...
            pattern_input: tuple[bool, ColumnValueRefs]
            out_arg: ColumnValueRefs = None
            def _arrow242(__unit: None=None, headers: Any=headers, values: Any=values) -> ColumnValueRefs:
                return out_arg

            def _arrow243(v: ColumnValueRefs, headers: Any=headers, values: Any=values) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(values.Columns, i, FSharpRef(_arrow242, _arrow243)), out_arg)
            match_value = pattern_input[1] if pattern_input[0] else None
            if match_value is not None:
                if match_value.tag == 1:
                    vals: Any = match_value.fields[0]
                    if len(vals) < values.RowCount:
                        header: CompositeHeader = headers[i]
                        def _arrow245(__unit: None=None, headers: Any=headers, values: Any=values) -> CompositeCell:
                            class ObjectExpr244:
                                @property
                                def Compare(self) -> Callable[[int, int], int]:
                                    return compare_primitives

                            i_1: int = max(vals.keys(), ObjectExpr244()) or 0
                            return get_empty_cell_for_header(header, get_item_from_dict(values.ValueMap, get_item_from_dict(vals, i_1)))

                        default_hash: int = ensure_cell_hash_in_value_map(get_empty_cell_for_header(header, None) if (len(vals) == 0) else _arrow245(), values.ValueMap) or 0
        ...
        try: 
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                if match_value.tag == 1:
                    ...
                    max_index: int = max(dict_1.keys(), ObjectExpr246()) or 0
...
def Unchecked_addRow(index: int, new_cells: Array[CompositeCell], headers: Array[CompositeHeader], values: ArcTableValues) -> None:
    ...
    if index < row_count:
        def _arrow247(x: int, y: int, index: Any=index, new_cells: Any=new_cells, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max_1(_arrow247, row_count - 1, 0) or 0
...
def Unchecked_addRows(index: int, new_rows: Array[Array[CompositeCell]], headers: Array[CompositeHeader], values: ArcTableValues) -> None:
    ...
    increase_row_indices: None
    if index < row_count:
        def _arrow248(x: int, y: int, index: Any=index, new_rows: Any=new_rows, headers: Any=headers, values: Any=values) -> int:
            return compare_primitives(x, y)

        last_row_index: int = max_1(_arrow248, row_count - 1, 0) or 0
        for row_index in range(last_row_index, index - 1, -1):
            for column_index in range(0, (column_count - 1) + 1, 1):
...
def Unchecked_alignByHeaders(keep_order: bool, rows: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]) -> tuple[Array[CompositeHeader], ArcTableValues]:
    ...
    def loop(col_i_mut: int, rows_2_mut: FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]], keep_order: Any=keep_order, rows: Any=rows) -> tuple[Array[CompositeHeader], ArcTableValues]:
        while True:
            ...
            def _arrow249(arg: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> bool:
                return not is_empty_1(arg)

            if not exists(_arrow249, rows_2):
                return (headers, values)

            else: 
                def _arrow250(l: FSharpList[tuple[CompositeHeader, CompositeCell]], col_i: Any=col_i, rows_2: Any=rows_2) -> tuple[CompositeHeader, CompositeCell] | None:
                    return None if is_empty_1(l) else head(l)

                first_elem: CompositeHeader = pick(_arrow250, rows_2)[0]
