...
from ..arctrl_core.Helper.identifier import Assay_fileNameFromIdentifier
from ..arctrl_file_system.file_system_tree import FileSystemTree
from ..arctrl_file_system.path import (combine_many, get_assay_folder_path)
from ..arctrl_spreadsheet.arc_assay import (ARCtrl_ArcAssay__ArcAssay_toFsWorkbook_Static_Z2508BE4F, ARCtrl_ArcAssay__ArcAssay_fromFsWorkbook_Static_32154C9D)
...
def _007CAssayPath_007C__007C(input: Array[str]) -> str | None:
    ...
    def _arrow3626(x: str, y: str, input: Any=input) -> bool:
        return x == y

    if (len(input) == 3) if (not equals_with(_arrow3626, input, None)) else False:
        if input[0] == "assays":
...
def ARCtrl_ArcAssay__ArcAssay_ToCreateContract_6FCE9E49(this: ArcAssay, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow3630(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow3628(__unit: None=None) -> IEnumerable_1[Contract]:
            folder_fs: FileSystemTree = FileSystemTree.create_assays_folder([FileSystemTree.create_assay_folder(this.Identifier)])
            def _arrow3627(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(10))) if ((p != "assays/.gitkeep") if (p != path) else False) else empty()

            return collect(_arrow3627, folder_fs.ToFilePaths(False))

        def _arrow3629(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow3628() if with_folder else empty(), delay(_arrow3629))

    return to_array(delay(_arrow3630))
