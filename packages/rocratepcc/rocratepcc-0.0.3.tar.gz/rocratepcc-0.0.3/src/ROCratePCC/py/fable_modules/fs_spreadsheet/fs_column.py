    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(cells.GetCellsInColumn(index)) == 0:
                ...
                def _arrow2944(c: FsCell, f: Any=f) -> int:
                    return c.Address.RowNumber

                return f(_arrow2944)(cells.GetCellsInColumn(index)).Address.RowNumber


        def _arrow2947(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2946(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2945:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr2945())

            return _arrow2946

        min_row_index: int = get_index_by(_arrow2947) or 0
        def _arrow2950(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow2949(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr2948:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr2948())

            return _arrow2949

        max_row_index: int = get_index_by(_arrow2950) or 0
