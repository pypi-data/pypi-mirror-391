...
class StreamingPipeline:
    ...
    def _initialize_metric_logger(self) -> None:
        ...
        try:
            ...
            self.metric_logger = InferenceMetricLogger(
                ...
                app_deploy_id=self.deployment_instance_id,
    ...
    def _initialize_analytics_publisher(self) -> None:
        """Initialize AnalyticsPublisher to send aggregated stats to Redis/Kafka."""
        ...
        try:
            ...
            # Extract Redis connection from camera configs
            for cfg in self.camera_configs.values():
                sc = cfg.stream_config or {}
                st = sc.get("stream_type", "kafka").lower()
                if st == "redis":
                    ...
                    kafka_bootstrap_servers = sc.get("bootstrap_servers", kafka_bootstrap_servers)

            # Initialize analytics publisher (don't start yet)
            ...
            self.logger.info(
                f"Initialized AnalyticsPublisher (Redis: {redis_host}:{redis_port}, "
                f"Kafka: {kafka_bootstrap_servers}, aggregation: 5min, publish: 60s)"
            )
        except Exception as e:
            self.analytics_publisher = None
