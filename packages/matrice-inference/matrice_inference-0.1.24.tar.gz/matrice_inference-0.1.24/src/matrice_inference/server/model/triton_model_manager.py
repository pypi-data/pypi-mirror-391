    ...
    def _create_postprocess_wrapper(self, postprocess_fn: Callable) -> Callable:
        ...
        def wrapper(output: np.ndarray, **kwargs) -> Dict[str, Any]:
            try:
                ...
                if not isinstance(result, dict) or "predictions" not in result or "model_id" not in result or "timestamp" not in result:
                    raise ValueError("Postprocess function must return dict with 'predictions', 'model_id', 'timestamp'")
