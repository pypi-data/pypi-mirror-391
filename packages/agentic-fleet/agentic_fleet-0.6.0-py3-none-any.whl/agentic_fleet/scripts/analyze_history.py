...
def print_executions(executions: List[Dict[str, Any]], limit: Optional[int] = None):
    ...
    for i, exec in enumerate(to_show, 1):
        task = exec.get("task", "Unknown task")
        task = task[:60] + "..." if len(task) > 60 else task

        print(f"\n{i}. {task}")
        if "total_time_seconds" in exec:
            print(f"   ⏰ Time: {exec['total_time_seconds']:.2f}s")
        if "quality" in exec and "score" in exec["quality"]:
            print(f"   📊 Quality: {exec['quality']['score']}/10")
        if "routing" in exec:
            routing = exec["routing"]
            if "mode" in routing:
                print(f"   🔀 Mode: {routing['mode'].upper()}")
            if "assigned_to" in routing:
                print(f"   👥 Agents: {', '.join(routing['assigned_to'])}")
        if "dspy_analysis" in exec and "complexity" in exec["dspy_analysis"]:
            print(f"   🎯 Complexity: {exec['dspy_analysis']['complexity']}")
...
def print_routing_stats(executions: List[Dict[str, Any]]):
    ...
    for exec in executions:
        if "routing" in exec and "mode" in exec["routing"]:
            mode = exec["routing"]["mode"]
...
def print_agent_usage(executions: List[Dict[str, Any]]):
    ...
    agents: Dict[str, int] = {}
    for exec in executions:
        if "routing" in exec and "assigned_to" in exec["routing"]:
            for agent in exec["routing"]["assigned_to"]:
