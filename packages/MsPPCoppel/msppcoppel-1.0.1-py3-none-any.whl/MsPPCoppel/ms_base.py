...
class KafkaBase:
    ...
    def __connectConsumer(self):
        ...
        try:
            ...
            # CAMBIO CLAVE: Ya no llama a sys.exit(-1)
            self.logs.error('Ocurrio un error al conectar con Kafka Consumer')
            self.logs.error(e)
    ...
    def _send(self, topico, msj, idTransaction):
        ...
        """
        # Se modificó para checar si el productor existe antes de enviar
        if self.__PRODUCER is None:
             self.logs.error("No se puede enviar mensaje. El productor de Kafka no está conectado.")
             return 
        ...
        try:
            ...
            self.logs.error(e)
            # Aunque no es crítico, aquí se puede dejar o manejar de otra forma
            # para evitar que un simple fallo de envío detenga el sistema.
            # sys.exit(-1) 
            
    def validateConnection(self):
        # Se modificó para checar si el productor y el consumidor existen
        try:
            if self.__CONSUMER is None or self.__PRODUCER is None:
                 return False # Si no se pudo conectar inicialmente, fallar
                 
            self.__CONSUMER.topics()
            self.__PRODUCER.send('validateConnection', b'') # El valor debe ser bytes
            return True
        except Exception as e:
            return False