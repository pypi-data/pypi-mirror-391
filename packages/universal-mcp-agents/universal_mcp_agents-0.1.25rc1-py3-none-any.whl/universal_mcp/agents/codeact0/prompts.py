...
- Note that if you use display_output_to_user=True, you must include your full answer in the code output.
"""
...
- Do not print the final output; return it from the orchestrator.
 ...
 - The orchestrator (main driver) function MUST include explicit Python type hints for every parameter. Allowed parameter types are strictly: str, int, float, bool, list[...] and dict[...], where list and dict element types are composed ONLY from str, int, float, bool (no Any, Union, Optional, custom classes, or other types). Defaults must match the annotated types or 'None'.
...
def image_generator(image_prompt: str, style: str = "comic", onedrive_parent_folder: str = "root") -> Dict:
    image_result = _generate_image(image_prompt=image_prompt, style=style)
    temp_file_path = _save_temp_image(image_result=image_result)
    upload_result = _upload_to_onedrive(temp_file_path=temp_file_path, onedrive_parent_folder=onedrive_parent_folder)
...
- The orchestrator (main driver) function must have explicit type hints on all parameters. Allowed parameter types are strictly: str, int, float, bool, list[...] and dict[...], where list/dict element types are composed ONLY from str, int, float, bool. Do not introduce Any/Union/Optional/custom classes for inputs.
...
- Prefer adapting and reusing previously validated logic inside affected functions; do not rewrite unrelated functions.
 ...
 - Environment: Code runs in an async-friendly sandbox; if a function is async, callers will `await` it. Do NOT use `asyncio.run()` or create event loops.
