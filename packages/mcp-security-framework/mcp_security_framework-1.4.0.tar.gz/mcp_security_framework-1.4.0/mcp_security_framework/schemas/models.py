...
class CertificateRole(str, Enum):
    ...
    def from_string(cls, role_str: str) -> "CertificateRole":
        ...
        Raises:
            ValueError: If role string is not a valid role
        """
        role_lower = role_str.lower().strip()
        for role in cls:
            if role.value == role_lower:
                return role
        raise ValueError(f"Invalid role: {role_str}. Valid roles are: {[r.value for r in cls]}")
    ...
    def validate_roles(cls, roles: List[str]) -> List["CertificateRole"]:
        ...
        Raises:
            ValueError: If any role string is not valid
        """
        validated_roles = []
    ...
    def to_json(self) -> str:
        """
        Serialize role to JSON string.
        
        This method serializes a single CertificateRole enum value to a JSON string.
        The role is serialized as its string value.
        
        Returns:
            JSON string representation of the role
            
        Example:
            >>> role = CertificateRole.CHUNKER
            >>> json_str = role.to_json()
            >>> print(json_str)  # "chunker"
        """
        return json.dumps(self.value)
    
    @classmethod
    def from_json(cls, json_str: str) -> "CertificateRole":
        """
        Deserialize role from JSON string.
        
        This method deserializes a JSON string to a CertificateRole enum value.
        The JSON string should contain the role value as a string.
        
        Args:
            json_str: JSON string containing the role value
            
        Returns:
            CertificateRole enum value
            
        Raises:
            ValueError: If JSON string is invalid or role is not valid
        ...
        """
        role_value = json.loads(json_str)
        if not isinstance(role_value, str):
            raise ValueError(f"Expected string in JSON, got {type(role_value).__name__}")
        return cls.from_string(role_value)
    
    @classmethod
    def roles_to_json(cls, roles: List["CertificateRole"]) -> str:
        """
        Serialize list of roles to JSON string.
        
        This method serializes a list of CertificateRole enum values to a JSON array string.
        Each role is serialized as its string value.
        
        Args:
            roles: List of CertificateRole enum values to serialize
            
        Returns:
            JSON array string representation of the roles
            
        Example:
            >>> roles = [CertificateRole.CHUNKER, CertificateRole.EMBEDDER]
            >>> json_str = CertificateRole.roles_to_json(roles)
            >>> print(json_str)  # '["chunker", "embedder"]'
        """
        role_values = [role.value for role in roles]
        return json.dumps(role_values)
    
    @classmethod
    def roles_from_json(cls, json_str: str) -> List["CertificateRole"]:
        """
        Deserialize list of roles from JSON string.
        
        This method deserializes a JSON array string to a list of CertificateRole enum values.
        The JSON string should contain an array of role values as strings.
        
        Args:
            json_str: JSON array string containing role values
            
        Returns:
            List of CertificateRole enum values
            
        Raises:
            ValueError: If any role value is invalid
            json.JSONDecodeError: If JSON string cannot be parsed
            
        Example:
            >>> json_str = '["chunker", "embedder"]'
            >>> roles = CertificateRole.roles_from_json(json_str)
            >>> assert roles == [CertificateRole.CHUNKER, CertificateRole.EMBEDDER]
        """
        role_values = json.loads(json_str)
        if not isinstance(role_values, list):
            raise ValueError(f"Expected array in JSON, got {type(role_values).__name__}")
        return cls.validate_roles(role_values)
    
    def to_dict(self) -> Dict[str, str]:
        """
        Serialize role to dictionary.
        
        This method serializes a single CertificateRole enum value to a dictionary
        with the role name as key and role value as value.
        ...
        Example:
            ...
            >>> print(role_dict)  # {"CHUNKER": "chunker"}
        """
        return {self.name: self.value}
    
    @classmethod
    def from_dict(cls, role_dict: Dict[str, str]) -> "CertificateRole":
        """
        Deserialize role from dictionary.
        
        This method deserializes a dictionary to a CertificateRole enum value.
        The dictionary should contain either:
        - A single key-value pair where the value is the role string
        - A key matching the enum name (e.g., "CHUNKER")
        - A key with any name but value matching a role string
        
        Args:
            role_dict: Dictionary containing role information
            
        Returns:
            CertificateRole enum value
            
        Raises:
            ValueError: If dictionary is invalid or role is not valid
        ...
        if not isinstance(role_dict, dict):
            raise ValueError(f"Expected dictionary, got {type(role_dict).__name__}")
        
        if len(role_dict) == 0:
            raise ValueError("Dictionary cannot be empty")
        
        # Try to find role value in dictionary
        for key, value in role_dict.items():
            if isinstance(value, str):
                try:
                    ...
                    continue
            # Also check if key matches enum name
            try:
                return cls[key]
            except KeyError:
                continue
        ...
        raise ValueError(
            f"Could not find valid role in dictionary: {role_dict}. "
            f"Valid roles are: {[r.value for r in cls]}"
        )
    
    @classmethod
    def roles_to_dict(cls, roles: List["CertificateRole"]) -> Dict[str, List[str]]:
        """
        Serialize list of roles to dictionary.
        
        This method serializes a list of CertificateRole enum values to a dictionary
        with "roles" as key and list of role values as value.
        
        Args:
            roles: List of CertificateRole enum values to serialize
            
        Returns:
            Dictionary representation of the roles
            
        Example:
            >>> roles = [CertificateRole.CHUNKER, CertificateRole.EMBEDDER]
            >>> roles_dict = CertificateRole.roles_to_dict(roles)
            >>> print(roles_dict)  # {"roles": ["chunker", "embedder"]}
        """
        return {"roles": [role.value for role in roles]}
    
    @classmethod
    def roles_from_dict(cls, roles_dict: Dict[str, Union[List[str], str]]) -> List["CertificateRole"]:
        """
        Deserialize list of roles from dictionary.
        
        This method deserializes a dictionary to a list of CertificateRole enum values.
        The dictionary should contain:
        - A "roles" key with a list of role strings, or
        - A single key with a list of role strings, or
        - A single key with a single role string
        
        Args:
            roles_dict: Dictionary containing role information
            
        Returns:
            List of CertificateRole enum values
            
        Raises:
            ValueError: If dictionary is invalid or any role is not valid
        ...
        if not isinstance(roles_dict, dict):
            raise ValueError(f"Expected dictionary, got {type(roles_dict).__name__}")
        
        # Try to find roles list in dictionary
        for key, value in roles_dict.items():
            if isinstance(value, list):
                return cls.validate_roles(value)
            elif isinstance(value, str):
                # Single role string
                return [cls.from_string(value)]
        
        raise ValueError(
            f"Could not find valid roles in dictionary: {roles_dict}. "
            f"Expected a key with list of role strings or single role string. "
            f"Valid roles are: {[r.value for r in cls]}"
        )
