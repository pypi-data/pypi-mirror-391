...
def _find_ui_directory() -> Path:
    ...
    # parent = cli, parent.parent = getupandrun (package root)
    ...
    # When installed: site-packages/getupandrun/ui/
    ...
    # In dev mode: src/getupandrun -> src -> project root
    # Go up from package root (getupandrun) -> src -> project root
    project_root = package_root.parent.parent
    ui_dir_at_root = project_root / "ui"
    if ui_dir_at_root.exists():
        return ui_dir_at_root
    
    # Strategy 5: Fallback - try one more level up (for edge cases)
    ui_dir_fallback = package_root.parent.parent.parent / "ui"
    if ui_dir_fallback.exists():
        return ui_dir_fallback
    
    # Strategy 6: Last resort - return the expected path (will show error if not found)
