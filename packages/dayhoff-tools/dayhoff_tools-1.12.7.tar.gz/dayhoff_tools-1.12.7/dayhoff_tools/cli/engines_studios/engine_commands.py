...
@click.option("--env", default="dev", help="Environment (dev, sand, prod)")
def start_engine(name_or_id: str, no_wait: bool, env: str):
    ...
    try:
        ...
        # Wait for engine to be running and SSM-accessible
        click.echo(f"\n⏳ Waiting for engine to be ready...\n")

        try:

            def check_engine_running():
                """Check if engine is running and SSM is accessible."""
                ...
                if state == "pending":
                    progress = 30
                elif state == "running" and not idle_state:
                    # Running but SSM not yet connected
                    progress = 60
                elif state == "running" and idle_state:
                    # Fully ready
                    progress = 100
                else:
                    progress = 10

                # Ready when running AND we can get idle state via SSM
                ready = state == "running" and idle_state is not None
