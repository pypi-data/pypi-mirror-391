...
**Usage:**
```bash
dh engine2 start <name-or-id> [--env <env>]
```
...
**Usage:**
```bash
dh engine2 stop <name-or-id> [--env <env>]
```
...
Connect to an engine via SSH using AWS SSM Session Manager.

**Usage:**
```bash
dh engine2 ssh <name-or-id> [options]
```
...
**Options:**
- `--admin` - Connect as `ec2-user` instead of engine owner
- `--env <env>` - Target environment

**Examples:**
```bash
# Connect as engine owner
dh engine2 ssh my-engine

# Connect as admin (ec2-user)
dh engine2 ssh my-engine --admin

# Connect to engine in sand environment
dh engine2 ssh test-engine --env sand
```

**Output:**
```
Connecting to i-1234567890abcdef0 as alice...
```

**Requirements:**
- AWS SSM Session Manager plugin must be installed
- Engine must be in `running` state
- Your IAM role must have SSM permissions

**Note:** Uses SSM Session Manager with AWS-StartSSHSession document. No bastion hosts required.
...
**Output:**
```
Creating Golden AMI from 'my-engine'...
✓ AMI creation started: ami-0123456789abcdef0
...
Delete your studio (must be detached first).
...
---------------------------------------------------------------------------
alice                vol-0123456789abcdef0    100GB      in-use         
...
6. **Update State** - Mark studio as `in-use` in DynamoDB (~200ms)
...
**Usage:**
```bash
dh studio2 resize --size <GB> [--env <env>]
```
...
```bash
# Launch engine (v2 command)
dh engine2 launch dev-work --type cpu

# Create studio (first time only)
dh studio2 create --size 100

# Attach studio (with progress tracking)
dh studio2 attach dev-work

# Connect
dh engine2 ssh dev-work
