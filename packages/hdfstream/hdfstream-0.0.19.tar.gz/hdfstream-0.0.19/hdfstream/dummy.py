...
class DummyRemoteDataset(RemoteDataset):
    ...
    Set cache=True to test the case where the dataset body was downloaded
    with the metadata. Numpy handles indexing in this case.

    If cache=False we simulate requests to the server by translating
    the __getitem__ key to a slice descriptor and extracting the
    corresponding slices from the ndarray.

    Tests should be repeated with both settings to ensure behaviour does not
    depend on the download size limit.
    """
