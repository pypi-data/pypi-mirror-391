...
- **Organize memories** hierarchically (app → project → ticket)
...
- 🧠 **Hierarchical Memory**: Flexible app_id → project_id → ticket_id organization with validation
...
Memory Hub enforces a strict hierarchy:
...
When retrieving memories, you automatically get parent-level context:
...
1. **`add_memory`** - Store content with hierarchical metadata
   ...
   # Large structured document (disable chunking for AutoStack plans, specs, etc.)
   add_memory(
