...
async def get_project_memories(request: GetProjectMemoriesRequest, config: AppConfig):
    ...
    try:
        ...
        print(f"DEBUG: cascade={request.cascade}, ticket_id={request.ticket_id}, project_id={request.project_id}")
        print(f"DEBUG: should_conditions count: {len(should_conditions)}")
        for i, condition in enumerate(should_conditions):
            print(f"DEBUG: should_condition[{i}]: {condition}")
