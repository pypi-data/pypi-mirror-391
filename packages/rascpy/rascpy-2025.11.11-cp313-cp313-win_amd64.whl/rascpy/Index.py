...
def KS(target,score,sample_weight=None,target_label={'unevent':0,'event':1}):
    ...
    if target_label is not None:
        target = target.apply(trans_y, y_label=target_label)
...
def AUC(target,score,sample_weight=None,target_label={'unevent':0,'event':1}):
    '''
    计算AUC指标
    支持权重
    支持target取值自定义
...
def LIFTn(target,pred,n=10,weight=None,score_reverse=True,target_label={'unevent':0,'event':1}):
    '''
    计算LIFT指标
    支持权重
    支持target取值自定义
   ...
   target_label : dict
    ...
    perf,_ = gen_perf_table_by_pred(target,pred,0.01,target_label,weight,score_reverse)
