...
class AiiWebSocketClient:
    ...
    async def execute_request(
        ...
        try:
            ...
            logger.debug(f"WebSocket request sent: {request.get('function', 'auto')}")
            ...
            while True:
                ...
                if asyncio.get_event_loop().time() - start_time > self.request_timeout:
                    raise WebSocketTimeout(f"Request timed out after {self.request_timeout}s")
        ...
        try:
            ...
            while True:
                ...
                if elapsed > self.request_timeout:
                    raise WebSocketTimeout(f"Intent recognition timed out after {self.request_timeout}s")
