...
class ExecutionLogger:
    ...
    def _build_metrics(
        ...
        if result.data:
            ...
            # Use actual cost from result if available (most accurate)
            cost_usd = result.data.get("cost", 0.0)

            # Fallback to estimation if no cost provided (backward compatibility)
    ...
    def _calculate_cost(
        ...
        try:
            from ...data.providers.model_pricing import calculate_cost

            return calculate_cost(model, input_tokens, output_tokens)
        except ImportError:
            logger.warning("Model pricing module not available")
            return 0.0
