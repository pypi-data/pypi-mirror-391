            ...
            if field_value is not None:
                ...
                if field_name.lower() == 'sprint':
                    if isinstance(field_value, list) and field_value:
                        # Find sprint with active/future state, or latest completed sprint
                        selected_sprint = None
                        
                        # First, check for active or future sprint
                        ...
                        # If no active/future sprint found, get the one with latest completion date
                        if not selected_sprint:
                            completed_sprints = [
                                s for s in field_value 
                                if isinstance(s, dict) and s.get('completeDate')
                            ]
                            if completed_sprints:
                                # Sort by completion date and get the latest
                                selected_sprint = max(
                                    completed_sprints, 
                                    key=lambda s: Utils.convert_to_utc(s.get('completeDate')) or datetime.min
                                )
                            elif field_value:
                                # Fallback to last sprint if no completion dates
                                selected_sprint = field_value[-1]
                        
                        if selected_sprint and isinstance(selected_sprint, dict):
                            extended_attrs['sprint_id'] = selected_sprint.get('id')
                            extended_attrs['sprint'] = selected_sprint.get('name')
