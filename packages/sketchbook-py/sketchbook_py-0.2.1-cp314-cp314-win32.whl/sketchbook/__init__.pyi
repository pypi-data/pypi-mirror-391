...
class Drawer:
    ...
    - 图层管理：支持基础图层和叠加图层
    - 画布重置：可重置到初始状态继续绘制
    - 双模式输出：render（保持画布）和 finish（重置画布）

    Example:
        >>> drawer = Drawer("bg.png", "font.ttf")
        >>> region1 = DrawerRegion(50, 50, 450, 200)
        >>> region2 = DrawerRegion(50, 250, 450, 400)
        >>> style = TextStyle(color=(255, 0, 0, 255))
        >>>
        >>> # 链式调用
        >>> result = (drawer
        ...     .draw_text("标题文本", region1, style)
        ...     .paste_image("logo.png", region2, PasteStyle())
        ...     .render())
        >>>
        >>> with open("output.png", "wb") as f:
        ...     f.write(result)
    ...
    def paste_image(
        ...
        image: ImageSource,
        region: Optional[DrawerRegion] = None,
        style: Optional[PasteStyle] = None,
    ) -> "Drawer":
        """
        在指定区域粘贴图像（支持链式调用）
        ...
        Raises:
            ValueError: 图像粘贴失败时抛出
        """
        ...

    def reset_base_image(self, new_base_image: ImageSource) -> "Drawer":
        """
        重置基础图层（支持链式调用）

        Args:
            new_base_image: 新的背景图
    ...
    def reset_overlay_image(self, new_overlay_image: Optional[ImageSource]) -> "Drawer":
        """
        重置叠加图层（支持链式调用）

        Args:
            new_overlay_image: 新的叠加层（None 则移除）
    ...
    def reset_font(self, new_font: str) -> "Drawer":
        """
        重置字体（支持链式调用）

        Args:
            new_font: 新的字体文件路径 (.ttf/.otf)
