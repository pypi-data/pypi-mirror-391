...
class BatchInfo:
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> BatchInfo:
        ...
        return cls(
            ...
            expired_at=datetime.fromtimestamp(data["expired_at"]) if data.get("expired_at") else None,
            cancelled_at=datetime.fromtimestamp(data["cancelled_at"]) if data.get("cancelled_at") else None,
...
class OpenAIBatchService:
    ...
    async def _create_batch_from_requests_internal(
        ...
        with open(file_path, "rb") as f:
            file_obj = await self.async_client.files.create(file=f, purpose="batch")
