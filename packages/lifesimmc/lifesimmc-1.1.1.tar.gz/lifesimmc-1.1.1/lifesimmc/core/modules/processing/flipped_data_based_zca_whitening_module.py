...
class FlippedDataBasedZCAWhiteningModule(BaseTransformationModule):
    ...
    def apply(self, resources: list[BaseResource]) -> tuple[DataResource, TemplateResource, TransformationResource]:
        ...
        # Set the observation
        obs = copy(config_in.observation)
        obs.total_integration_time /= 3
        phringe.set(obs)
        phringe.set(config_in.scene)

        # Get the differential counts for the calibration star
        diff_counts1 = phringe.get_diff_counts()
        ...
        # Get the differential counts for the calibration star
        diff_counts2 = phringe.get_diff_counts()

        diff = (diff_counts1 - diff_counts2)[0]
