...
class CalibrationStarZCAWhiteningModule2(BaseTransformationModule):
    ...
    def apply(self, resources: list[BaseResource]) -> tuple[DataResource, TemplateResource, TransformationResource]:
        ...
        for i in range(len(diff_counts)):
            ...
            cov[i] = torch.cov(data_in[i])

            if self.diagonal_only:
                cov[i] = torch.diag(torch.diag(cov[i]))

            i_cov_sqrt[i] = torch.tensor(pinv(sqrtm(cov[i].cpu().numpy())), device=self.device, dtype=torch.float32)
        ...
        for i in range(data_in.shape[0]):
            ...
            diff_counts[i] = i_cov_sqrt[i] @ diff_counts[i]

            plt.imshow(data_in[i].cpu().numpy(), cmap='viridis', aspect='auto')
            plt.colorbar()
            plt.show()

            # plt.imshow(diff_counts[i].cpu().numpy(), cmap='viridis', aspect='auto')
            # plt.colorbar()
            # plt.show()

        # repeat for temporal covariance
        cov2 = torch.zeros(
            (diff_counts.shape[0], diff_counts.shape[2], diff_counts.shape[2]),
            device=self.device,
            dtype=torch.float32
        )
        i_cov_sqrt2 = torch.zeros(
            (diff_counts.shape[0], diff_counts.shape[2], diff_counts.shape[2]),
            device=self.device,
            dtype=torch.float32
        )
        for i in range(len(diff_counts)):
            a = np.cov(data_in[i].T.cpu().numpy())

            if self.diagonal_only:
                cov2[i] = torch.diag(torch.diag(cov2[i]))

            i_cov_sqrt2[i] = torch.tensor(pinv(a), device=self.device, dtype=torch.float32)
            # i_cov_sqrt2[i] = torch.diag(torch.ones(i_cov_sqrt2[i].shape[0], device=self.device, dtype=torch.float32))

            # Apply the whitening matrix to the data and templates
            data_in[i] = (i_cov_sqrt2[i] @ data_in[i].T).T
            data_in[i] /= np.var(data_in[i].cpu().numpy()) ** 0.5
        ...
        if self.n_template_in and self.n_template_out:
            ...
            for i, j in tqdm(
                ...
                for k in range(template_data_in.shape[0]):
                    # template_counts_white[k, :, :, i, j] = (
                    #         i_cov_sqrt2[k] @ (i_cov_sqrt[k] @ template_data_in[k, :, :, i, j]).T).T
                    template_counts_white[k, :, :, i, j] = i_cov_sqrt[k] @ template_data_in[k, :, :, i, j]

                    # Normalize the template counts
                    # template_counts_white[k, :, :, i, j] /= torch.var(template_counts_white[k, :, :, i, j]) ** 0.5
        ...
        def zca_whitening_transformation(data):
            ...
            if isinstance(data, np.ndarray):
                ...
                i3 = i_cov_sqrt2.cpu().numpy()
            else:
                i2 = i_cov_sqrt
                i3 = i_cov_sqrt2
            for l in range(data.shape[0]):
                data[l] = (i3[l] @ (i2[l] @ data[l]).T).T
            return data
