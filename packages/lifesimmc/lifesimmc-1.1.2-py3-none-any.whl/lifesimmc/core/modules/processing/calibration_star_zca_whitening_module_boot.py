...
class CalibrationStarZCAWhiteningModuleBoot(BaseTransformationModule):
    ...
    def apply(self, resources: list[BaseResource]) -> tuple[DataResource, TemplateResource, TransformationResource]:
        ...
        mean = np.zeros(diff_counts.shape[1])
        cov_w = torch.cov(diff_counts[0]).cpu().numpy()
        samples = np.random.multivariate_normal(mean, cov_w, size=diff_counts[0].shape[1])
        diff_counts = torch.tensor(samples, dtype=diff_counts.dtype, device=diff_counts.device).T
        diff_counts = diff_counts[None, :, :]
