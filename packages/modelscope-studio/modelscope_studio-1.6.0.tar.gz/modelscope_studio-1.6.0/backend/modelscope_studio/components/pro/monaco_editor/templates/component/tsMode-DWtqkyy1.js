import { t as I, m as M } from "./editor.main-BMgcVphl.js";
/*!-----------------------------------------------------------------------------
 ...
 *-----------------------------------------------------------------------------*/
var N = Object.defineProperty, R = Object.getOwnPropertyDescriptor, K = Object.getOwnPropertyNames, E = Object.prototype.hasOwnProperty, H = (e, t, r, a) => {
  if (t && typeof t == "object" || typeof t == "function")
    for (let n of K(t)) !E.call(e, n) && n !== r && N(e, n, {
      get: () => t[n],
      enumerable: !(a = R(t, n)) || a.enumerable
    });
  return e;
...
function j(e, t) {
  ...
  if (r != null && r.createTrustedTypesPolicy)
    try {
  ...
  try {
    return (a = globalThis.trustedTypes) == null ? void 0 : a.createPolicy(e, t);
  } catch (n) {
    console.error(n);
    return;
  }
...
function V(e) {
  ...
  if (r) {
    ...
    if (typeof r.getWorkerUrl == "function") {
      ...
      return new Worker(D ? D.createScriptURL(a) : a, {
        name: t,
        type: "module"
      });
    }
...
var B = class {
  ...
  _getClient() {
    ...
    }), this._defaults.getEagerModelSync() ? await this._worker.withSyncedResources(o.editor.getModels().filter((e) => e.getLanguageId() === this._modeId).map((e) => e.uri)) : await this._worker.getProxy()))()), this._client;
  }
...
var y = class {
  ...
  _textSpanToRange(e, t) {
    let r = e.getPositionAt(t.start), a = e.getPositionAt(t.start + t.length), {
      lineNumber: n,
      column: l
    } = r, {
      lineNumber: u,
  ...
  constructor(e, t, r, a) {
    ...
    this._disposables.push(o.editor.onDidCreateModel((s) => n(s))), this._disposables.push(o.editor.onWillDisposeModel(l)), this._disposables.push(o.editor.onDidChangeModelLanguage((s) => {
      l(s.model), n(s.model);
  ...
  async _doValidate(e) {
    ...
    if (!u || e.isDisposed())
      return;
    const s = u.reduce((g, d) => d.concat(g), []).filter((g) => (this._defaults.getDiagnosticsOptions().diagnosticCodesToIgnore || []).indexOf(g.code) === -1), c = s.map((g) => g.relatedInformation || []).reduce((g, d) => d.concat(g), []).map((g) => g.file ? o.Uri.parse(g.file.fileName) : null);
    await this._libFiles.fetchLibFilesIfNecessary(c), !e.isDisposed() && o.editor.setModelMarkers(e, this._selector, s.map((g) => this._convertDiagnostics(e, g)));
  }
  _convertDiagnostics(e, t) {
    const r = t.start || 0, a = t.length || 1, {
      lineNumber: n,
      column: l
    } = e.getPositionAt(r), {
      lineNumber: u,
      column: s
    } = e.getPositionAt(r + a), c = [];
  ...
  _convertRelatedInformation(e, t) {
    ...
    return t.forEach((a) => {
      ...
      if (a.file && (n = this._libFiles.getOrCreateModel(a.file.fileName)), !n)
        return;
      const l = a.start || 0, u = a.length || 1, {
        lineNumber: s,
        column: c
      } = n.getPositionAt(l), {
        lineNumber: g,
...
`)
  ...
  async provideCompletionItems(t, r, a, n) {
    const l = t.getWordUntilPosition(r), u = new o.Range(r.lineNumber, l.startColumn, r.lineNumber, l.endColumn), s = t.uri, c = t.getOffsetAt(r), g = await this._worker(s);
    if (t.isDisposed())
      return;
  ...
  async resolveCompletionItem(t, r) {
    const a = t, n = a.uri, l = a.position, u = a.offset, c = await (await this._worker(n)).getCompletionEntryDetails(n.toString(), u, a.label);
    return c ? {
      uri: n,
      position: l,
      label: c.name,
...
` + c : "")
  ...
  async provideDocumentHighlights(e, t, r) {
    ...
    if (e.isDisposed())
      return;
    const u = await l.getDocumentHighlights(a.toString(), n, [a.toString()]);
    if (!(!u || e.isDisposed()))
  ...
  async provideDefinition(e, t, r) {
    ...
    if (!u || e.isDisposed() || (await this._libFiles.fetchLibFilesIfNecessary(u.map((c) => o.Uri.parse(c.fileName))), e.isDisposed()))
      return;
    const s = [];
    for (let c of u) {
  ...
  async provideReferences(e, t, r, a) {
    ...
    if (!s || e.isDisposed() || (await this._libFiles.fetchLibFilesIfNecessary(s.map((g) => o.Uri.parse(g.fileName))), e.isDisposed()))
      return;
    const c = [];
    for (let g of s) {
...
`,
  ...
  async provideDocumentRangeFormattingEdits(e, t, r, a) {
    ...
    if (e.isDisposed())
      return;
    const c = await s.getFormattingEditsForRange(n.toString(), l, u, k._convertOptions(r));
    if (!(!c || e.isDisposed()))
...
`];
  ...
  async provideOnTypeFormattingEdits(e, t, r, a, n) {
    ...
    if (e.isDisposed())
      return;
    const c = await s.getFormattingEditsAfterKeystroke(l.toString(), u, r, k._convertOptions(a));
    if (!(!c || e.isDisposed()))
      return c.map((g) => this._convertTextChanges(e, g));
  }
}, re = class extends k {
  async provideCodeActions(e, t, r, a) {
    ...
    if (e.isDisposed())
      return;
    const d = await g.getCodeFixesAtPosition(n.toString(), l, u, c, s);
  ...
  async provideInlayHints(e, t, r) {
    ...
    return e.isDisposed() ? null : {
      hints: (await s.provideInlayHints(n, l, u)).map((d) => ({
        ...d,
        label: d.text,
...
function O(e, t) {
  ...
  function u() {
    ...
    ne(r), s.completionItems && r.push(o.languages.registerCompletionItemProvider(t, new G(n))), s.signatureHelp && r.push(o.languages.registerSignatureHelpProvider(t, new J(n))), s.hovers && r.push(o.languages.registerHoverProvider(t, new Q(n))), s.documentHighlights && r.push(o.languages.registerDocumentHighlightProvider(t, new q(n))), s.definitions && r.push(o.languages.registerDefinitionProvider(t, new Y(l, n))), s.references && r.push(o.languages.registerReferenceProvider(t, new X(l, n))), s.documentSymbols && r.push(o.languages.registerDocumentSymbolProvider(t, new Z(n))), s.rename && r.push(o.languages.registerRenameProvider(t, new se(l, n))), s.documentRangeFormattingEdits && r.push(o.languages.registerDocumentRangeFormattingEditProvider(t, new ee(n))), s.onTypeFormattingEdits && r.push(o.languages.registerOnTypeFormattingEditProvider(t, new te(n))), s.codeActions && r.push(o.languages.registerCodeActionProvider(t, new re(n))), s.inlayHints && r.push(o.languages.registerInlayHintsProvider(t, new ie(n))), s.diagnostics && r.push(new z(l, e, t, n));
  }
  return u(), n;
}
function ne(e) {
