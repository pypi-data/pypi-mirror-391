...
def get_bootstrap_config(client: Client):
    ...
    api_url = f"{client.renesas_q_url}/q/bootstrap"
    token = client.load_auth_token()

    response, error = client.request(
        api_url, headers={"Authorization": f"Bearer {token.get('id_token')}"}
    )
    ...
    if not error:
        ...
        print(f"{Fore.RED}Error fetching bootstrap config. {details}{Style.RESET_ALL}")
        sys.exit(1)
