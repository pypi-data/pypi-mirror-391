...
from vaxbnana._utils import set_path_early_data, filter_dict
from vaxbnana.base import BaseConfig
from vaxbnana.data import V2Data, XrayNoise, SplitHttpSettings
from vaxbnana.exceptions import ProtocolNotSupportedError, TransportNotSupportedError
...
class XrayConfig(BaseConfig):
    ...
    def __init__(
        ...
        if not template_path:
            template_path = resources.files("vaxbnana.templates") / "xray.json"
    ...
    def tcp_http_config(header_type=None, host=None, path=None):
        ...
        if header_type != "none" or headers:
    ...
    def splithttp_config(
        ...
        if settings is not None:
            ...
            if settings.padding_bytes is not None:
                splithttp_settings["xPaddingBytes"] = settings.padding_bytes
    ...
    def xhttp_config(
        ...
        xhttp_settings = {"path": path, "headers": headers}
        ...
        if settings is not None:
            if settings.mode is not None:
                xhttp_settings["mode"] = settings.mode
            if settings.no_grpc_header is not None:
                xhttp_settings["noGRPCHeader"] = settings.no_grpc_header
            if settings.padding_bytes is not None:
                xhttp_settings["xPaddingBytes"] = settings.padding_bytes
            if settings.xmux is not None:
                xhttp_settings["xmux"] = filter_dict(
                    {
                        "maxConcurrency": settings.xmux.max_concurrency,
                        "maxConnections": settings.xmux.max_connections,
                        "cMaxReuseTimes": settings.xmux.max_reuse_times,
    ...
    def make_stream_settings(
        ...
        if net:
            if net in {"ws", "websocket"}:
                ...
                stream_settings["xhttpSettings"] = XrayConfig.xhttp_config(
                    path=path,
                    host=host,
                    headers=headers,
                    settings=splithttp_settings,
