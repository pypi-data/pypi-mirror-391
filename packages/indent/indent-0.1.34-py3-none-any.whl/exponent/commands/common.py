...
async def report_sandbox_info(
    ...
    result = await graphql_client.execute(
        REPORT_SANDBOX_INFO_MUTATION, variables, "ReportSandboxInfo"
    )

    data = result["reportSandboxInfo"]
    ...
    if data["__typename"] != "SandboxInfoResponse" or not data.get("success"):
