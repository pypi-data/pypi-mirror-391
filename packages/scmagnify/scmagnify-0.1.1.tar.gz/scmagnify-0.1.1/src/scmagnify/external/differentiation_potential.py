...
def get_embedding_dpt(
    ...
    sc.pl.embedding(
        adata, embedding_key, color=["clusters", "distance"], legend_loc="on data", cmap=cmap, palette=palette
    )


def get_path_matrix(
    adata: AnnData, dpt_var: str, path_var: str, path: list, features: list, split_groups: bool | None = True
):
    """
...
def plot_potential(
    ...
    return_data: bool | None = False,
    gam_smooth: bool | None = True,
    dpt_var: str | None = "distance",
    use_ranked_dpt: bool | None = False,
):
    """
...
def calculate_arrows(df: pd.DataFrame, penal: float | None = 0.03):
    """
    ...
    arrow_map["tf_to_region_match"] = [
        arrow_map["tf_to_region_match"][i] if not isnan(arrow_map["tf_to_region_match"][i]) else arrow_map.index[i]
        for i in range(arrow_map.shape[0])
    ]
    arrow_map["tf_to_gene_match"] = [
        arrow_map["tf_to_gene_match"][i] if not isnan(arrow_map["tf_to_gene_match"][i]) else arrow_map.index[i]
        for i in range(arrow_map.shape[0])
    ]
    arrow_map["region_to_gene_match"] = [
        arrow_map["region_to_gene_match"][i] if not isnan(arrow_map["region_to_gene_match"][i]) else arrow_map.index[i]
        for i in range(arrow_map.shape[0])
    ]
...
def calculate_grid_arrows(
    ...
    minimal_distance = np.mean(
        [y_dist_between_points, x_dist_between_points]
    )  # will be used to mask certain points in the grid
...
def plot_map(
    ...
    embedding_key: str | None = "X_umap",
    window: int | None = 1,
    plot_type: str | None = "tf_to_gene",
    gam_smooth: bool | None = True,
    use_ranked_dpt: bool | None = False,
    tf_traj_thr: float | None = 0.7,
    tf_expr_thr: float | None = 0.2,
    penalization: float | None = 0.03,
    n_grid_cols: int | None = 50,
    n_grid_rows: int | None = 50,
    n_neighbors: int | None = 10,
    offset_frac: float | None = 0.1,
    scale: int | None = 100,
    n_cpu: int | None = 1,
    figsize: tuple[float, float] | None = (10, 10),
    colormap=cm.Greys,
    plot_streamplot: bool | None = True,
    vmax_streamplot: float | None = 0.25,
    linewidth_streamplot: float | None = 0.5,
    arrowsize_streamplot: float | None = 2,
    density_streamplot: float | None = 10,
    return_data: bool | None = False,
    save: str | None = None,
    **kwargs,
):
    ...
    **kwargs
    ...
    for k in ke:
        df = plot_potential(
            ...
            return_data=True,
            show_plot=False,
            gam_smooth=gam_smooth,
            use_ranked_dpt=use_ranked_dpt,
        )
        ...
        df[plot_type + "_match"] = [
            df[plot_type + "_match"][i] if not isnan(df[plot_type + "_match"][i]) else df.index[i]
            for i in range(df.shape[0])
        ]
        df[plot_type + "_match"] = [
            df[plot_type + "_match"][i] if df.iloc[i, 0] > tf_traj_thr else df.index[i] for i in range(df.shape[0])
        ]
        df[plot_type + "_match"] = [
            df[plot_type + "_match"][i] if df.iloc[i, 3] > tf_expr_thr else df.index[i] for i in range(df.shape[0])
        ]
    ...
    grid_xy, uv, mask, color = calculate_grid_arrows(
        embedding, delta_embedding, tf_expr, offset_frac, n_grid_cols, n_grid_rows, n_neighbors, n_cpu
    )
    ...
    if plot_streamplot is True:
        ...
        plt.streamplot(
            ...
            uv.reshape(n_grid_cols, n_grid_cols, 2)[:, :, 1],
            density=density_streamplot,
            color=np.array(scale(distances)).reshape(n_grid_cols, n_grid_cols),
            cmap=colormap,
            zorder=1,
            norm=norm,
            linewidth=linewidth_streamplot,
            arrowsize=arrowsize_streamplot,
        )
    else:
        plt.quiver(
            grid_xy[mask, 0],
...
def cell_forces(
    ...
    tf_traj_thr: float | None = 0.7,
    tf_expr_thr: float | None = 0.2,
    selected_eGRNs: list | None = None,
    penalization: float | None = 0.05,
    n_cpu: int | None = 1,
    **kwargs,
):
    """
    ...
    **kwargs
    ...
    if n_cpu == 1:
        ...
        try:
            ...
            for df in tqdm(to_iterator(jobs), total=len(jobs), desc=f"Running using {n_cpu} cores", smoothing=0.1):
                if df is not None:
                    df_list.append(df)
...
def cell_forces_per_tf(
    ...
    for k in ke:
        if paths_cascade["TF"][k][tf.split("_")[0]].sum() > 0:
            ...
            df[plot_type + "_match"] = [
                df[plot_type + "_match"][i]
                if df.iloc[i, 0] > tf_traj_thr and df.iloc[i, 3] > tf_expr_thr
                else df.index[i]
                for i in range(df.shape[0])
            ]
            df[plot_type + "_length"] = [
                df[plot_type + "_length"][i] if df[plot_type + "_match"][i] != df.index[i] else 0.0
                for i in range(df.shape[0])
            ]
...
def forces_rss(adata: AnnData, df: pd.DataFrame, variable: str):
    """
...
def fitgam(x: np.array, y: np.array, feature_range: tuple[float, float] | None = (0, 1)):
    """
