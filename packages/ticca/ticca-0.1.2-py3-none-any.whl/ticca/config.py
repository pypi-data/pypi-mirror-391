...
from ticca.session_storage import save_session

CONFIG_DIR = os.path.join(os.getenv("HOME", os.path.expanduser("~")), ".ticca")
...
def ensure_config_exists():
    """
    Ensure that the .ticca dir and puppy.cfg exist, prompting if needed.
...
def load_mcp_server_configs():
    """
    Loads the MCP server configurations from ~/.ticca/mcp_servers.json.
    Returns a dict mapping names to their URL or config dict.
    If file does not exist, returns an empty dict.
    """
    from ticca.messaging.message_queue import emit_error
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".ticca_history.txt"
            )
