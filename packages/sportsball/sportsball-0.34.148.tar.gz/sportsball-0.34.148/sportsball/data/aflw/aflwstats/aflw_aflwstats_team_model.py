...
from .aflw_aflwstats_player_model import create_aflw_aflwstats_player_model


def _create_aflw_aflwstats_team_model(
    team_url: str,
    players: dict[str, list[str | float]],
    points: float,
    ...
    return TeamModel(
        ...
        name=name,
        players=[
            create_aflw_aflwstats_player_model(
                player_stats={x: players[x][i] for x in players},
                player_url=player_urls[str(players["Player"][i])],
                session=session,
            )
            for i in range(len(players[list(players.keys())[0]]))
        ],
        odds=[],
        points=points,
        ladder_rank=None,
        location=None,
        news=create_google_news_models(name, session, dt, League.AFLW),
...
def _cached_create_aflw_aflwstats_team_model(
    team_url: str,
    players: dict[str, list[str | float]],
    points: float,
    session: requests_cache.CachedSession,
    ...
    return _create_aflw_aflwstats_team_model(
        team_url=team_url,
        players=players,
        points=points,
        session=session,
...
def create_aflw_aflwstats_team_model(
    team_url: str,
    players: dict[str, list[str | float]],
    points: float,
    session: requests_cache.CachedSession,
    ...
    """Create a team model from AFLW AFLWStats."""
    if not pytest_is_running.is_running():
        return _cached_create_aflw_aflwstats_team_model(
            team_url=team_url,
            players=players,
            points=points,
            session=session,
