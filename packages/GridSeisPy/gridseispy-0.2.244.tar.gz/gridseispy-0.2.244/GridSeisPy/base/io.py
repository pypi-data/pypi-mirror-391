...
def loadgrid_mmapstream(fname: Path|str, dimens: list|np.ndarray|tuple, encoding: str='utf-8',
    ...
    """
    fname = fname if isinstance(fname, Path) else Path(fname)
    fmmapstream = fname.with_suffix('.mmapstream') if fmmapstream is None else fmmapstream
    if fmmapstream.exists() and not force_reload and fmmapstream.stat().st_size == np.prod(dimens, dtype=int) * np.dtype(dtype).itemsize:
        return np.memmap(fmmapstream, dtype=np.dtype(dtype), mode='r', shape=(ax for ax in dimens))
