...
class PerpetualContext:
    ...
    def __getSummaryText(self, text='', max_tokens=1):
        try:
            summary_result = ''
            text = original_text = str(text).replace('  ', ' ').strip()
            max_tokens = max((1, int(max_tokens))) if type(max_tokens) in (int, float) else 1
            ...
            new_tokens_number = self.__countTokens(string=summary_result)
            if new_tokens_number > tokens_number: summary_result = original_text
    ...
    def __getBeginningAndEnd(self, string='', max_tokens=1, separator=''):
        try:
            ...
            tokens_number_1 = self.__countTokens(string=string)
            half = int(max_tokens/2)
            result_text = string[:half]+separator+string[::-1][:half][::-1]
            tokens_number_2 = self.__countTokens(string=result_text)
            if tokens_number_2 > tokens_number_1: result_text = string
            return result_text
        except Exception as error:
    ...
    def __getBeginningMiddleAndEnd(self, string='', max_tokens=1, separator=''):
        try:
            ...
            result_text = beginning+separator+middle+separator+end
            tokens_number_2 = self.__countTokens(string=result_text)
            if tokens_number_2 > tokens_number_1: result_text = string
            return result_text
        except Exception as error:
    ...
    def getSummaryCode(self, text='', max_tokens=1):
        try:
            text = str(text).strip()
            max_tokens = max((1, int(max_tokens))) if type(max_tokens) in (int, float) else 1
            tokens_number_1 = self.__countTokens(string=text)
            if tokens_number_1 <= max_tokens: return text
            ...
            tokens_number_2 = self.__countTokens(string=summary_result)
            if tokens_number_2 > tokens_number_1: summary_result = text
            return summary_result.strip()
        except Exception as error:
    ...
    def getSummaryYouTube(self, file_path='', max_tokens=1):
        try:
            ...
            try: transcript, transcription = self.__YouTubeTranscriptApi().fetch(file_path, languages=self.__languages), ''
            except: transcript, transcription = self.__YouTubeTranscriptApi.get_transcript(file_path, languages=self.__languages), ''
            for speech in transcript:
                try: transcription += f'**SPEECH:** {speech.text} - **START:** {speech.start} - **DURATION:** {speech.duration}\n'
                except: transcription += f'**SPEECH:** {speech["text"]} - **START:** {speech["start"]} - **DURATION:** {speech["duration"]}\n'
            ...
            def getListOfObjects(file_path=''):
                try:
                    ...
                    with open(dnull, 'w') as devnull:
                        with redirect_stdout(devnull), redirect_stderr(devnull): model = self.__models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
                    model.eval()
