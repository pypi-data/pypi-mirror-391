...
fn test_simulation_with_barrels_and_tight_bounds() {
    ...
    schematic.set_block_from_string(0, 1, 0, "minecraft:barrel[facing=north]{signal=1}")
        .expect("Failed to set first barrel");
    
    let dims1 = schematic.get_dimensions();
    println!("After first barrel at (0, 1, 0): dimensions = {:?}", dims1);
    
    schematic.set_block_from_string(0, -1, 0, "minecraft:barrel[facing=north]{signal=2}")
        .expect("Failed to set second barrel");
    
    let dims2 = schematic.get_dimensions();
    println!("After second barrel at (0, -1, 0): dimensions = {:?}", dims2);
    ...
    println!("\nDimension info:");
    println!("  Allocated: {:?}", schematic.default_region.get_dimensions());
    ...
    let result = std::panic::catch_unwind(|| {
        MchprsWorld::new(schematic.clone())
    });
...
fn test_simulation_with_simple_barrel() {
    ...
    schematic.set_block_from_string(0, 0, 0, "minecraft:barrel[facing=north]{signal=5}")
...
fn test_simulation_after_export_reload() {
    ...
    let result = std::panic::catch_unwind(|| {
        MchprsWorld::new(reloaded.clone())
    });
