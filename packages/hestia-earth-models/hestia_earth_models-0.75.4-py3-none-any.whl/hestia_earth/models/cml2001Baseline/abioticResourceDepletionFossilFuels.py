...
def _should_run(impact_assessment: dict) -> tuple[bool, list]:
    ...
    energy_resources_in_mj = [
        energy_input["value-in-MJ"] for energy_input in resource_uses_unpacked
    ]
    valid_energy_resources_in_mj = [
        value for value in energy_resources_in_mj if value is not None
    ]
    all_inputs_have_valid_mj_value = all(
        [energy is not None for energy in energy_resources_in_mj]
        + [bool(energy_resources_in_mj)]
    )
