...
def _as_tuple(input_: Any) -> tuple:
    return tuple(input_) if is_iterable(input_) else (input_,)


def _get_group_by_field(node: dict, *, field: str, default: Hashable = None):
    value = get(node, field, default=default)
    return "-".join(str(v) for v in value) if is_iterable(value) else value
...
def _get_group_keys(
    ...
    return tuple(product(*(_as_tuple(grouper(node)) for grouper in groupers)))
...
def _get_groupers(by: list[Union[str, _Grouper]]) -> list[_Grouper]:
    return (
        [
            ...
            for x in _as_tuple(by)
        ]
        if by
        else _as_tuple(_FIELD_TO_GROUPER[None])
    )
...
def _sort_grouped(grouped: dict) -> dict:
    ...
    return dict(
        sorted(
            grouped.items(),
            key=lambda kv: _as_tuple((x is None, x) for x in _as_tuple(kv[0])),
        )
    )
...
def _group_nodes_by_consecutive_dates(nodes: list[dict]) -> list[list[dict]]:
    ...
    for node in sorted(
        nodes,
        key=lambda n: (parse_node_date(n, "startDate"), parse_node_date(n, "endDate")),
    ):
        if not group or (
