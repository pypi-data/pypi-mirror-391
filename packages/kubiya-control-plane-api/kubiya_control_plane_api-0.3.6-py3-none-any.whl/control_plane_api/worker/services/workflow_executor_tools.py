...
"""
    ...
    def _execute_json_workflow_specific(
        ...
        # Publish final workflow completion to control plane
        if self.control_plane and self.execution_id and workflow_message_id:
            try:
                completion_message = f"\n✅ Workflow '{workflow_data.get('name', 'unknown')}' completed successfully\nDuration: {duration:.2f}s\n"
                self.control_plane.publish_event(
                    execution_id=self.execution_id,
                    event_type="message_chunk",
                    data={
                        ...
                        "is_chunk": False,  # Final message - not a chunk
                        "message_id": workflow_message_id,
                        "source": "workflow",
                        "metadata": {
                ...
                logger.debug(f"Failed to publish workflow completion: {e}")
