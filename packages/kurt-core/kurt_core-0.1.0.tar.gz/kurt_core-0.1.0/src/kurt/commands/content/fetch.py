...
@click.option("--url", hidden=True, help="[DEPRECATED: use positional IDENTIFIER] Single source URL (auto-creates if doesn't exist)")
@click.option(
    "--urls", help="FILTER: Comma-separated list of source URLs (auto-creates if don't exist)"
)
@click.option(
    "--file", "file_path", hidden=True, help="[DEPRECATED: use positional IDENTIFIER] Local file path to index (skips fetch, only indexes)"
)
@click.option(
    ...
    try:
        ...
        with LiveProgressDisplay(console) as display:
            ...
            for result in successful:
                ...
                size_kb = result.get("content_length", 0) / 1024 if result.get("content_length") else 0
                # Show size and estimated timing
                display.log(f"✓ Fetched [{doc_id[:8]}] {title[:50]} ({size_kb:.1f}KB, ~{avg_time:.1f}s)", style="green")
            ...
            if not skip_index and successful:
                ...
                async def batch_index_with_progress():
                    result = await batch_extract_document_metadata(
                        doc_ids, max_concurrent=concurrency, progress_callback=update_index_progress,
        ...
        if not skip_index and successful:
            ...
            if indexed > 0 and 'kg_result' in locals() and kg_result and "error" not in kg_result:
                console.print(f"✓ KG Entities: {kg_result['entities_created']} created, {kg_result['entities_linked']} linked")
                if kg_result.get('relationships_created', 0) > 0:
                    console.print(f"✓ KG Relationships: {kg_result['relationships_created']} created")
