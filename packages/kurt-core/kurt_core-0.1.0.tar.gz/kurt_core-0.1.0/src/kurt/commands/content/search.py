...
def search_cmd(
    ...
    if output_format == "json":
        ...
        console.print()
        console.print(f"[bold cyan]Search Summary[/bold cyan]")
        console.print(f"[dim]Query: '{query}'[/dim]")
        if include_pattern:
            console.print(f"[dim]Pattern: {include_pattern}[/dim]")
        console.print(f"[dim]Found {len(matches)} matches across {len(doc_summaries)} documents[/dim]")
        console.print()
        ...
        for doc in doc_summaries:
            table.add_row(
                ...
                doc["content_path"][:50] + "..." if len(doc["content_path"]) > 50 else doc["content_path"],
                doc["document_id"][:8] + "...",
            )
    ...
    if len(matches) >= max_results:
        console.print(f"[dim]Showing first {max_results} results. Use --max-results to see more.[/dim]")
