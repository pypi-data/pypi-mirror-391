...
def get_document_stats(
    ...
    if in_cluster:
        ...
        stmt = (
            stmt.join(ClusterMembership, Document.id == ClusterMembership.document_id)
            .where(ClusterMembership.cluster_name == in_cluster)
        )
    ...
    if with_content_type:
        ...
        stmt = (
            stmt.join(DocumentClassification, Document.id == DocumentClassification.document_id)
...
def list_documents_for_indexing(
    ...
    if not ids and not include_pattern and not in_cluster and not with_status and not with_content_type and not all_flag:
        raise ValueError("Must provide either ids, include_pattern, in_cluster, with_status, with_content_type, or all_flag=True")

    # Mode 1: Documents by IDs (single or multiple, supports partial UUIDs/URLs/file paths)
    if ids:
        from kurt.commands.content._identifiers import resolve_ids_to_uuids
    ...
    if include_pattern or in_cluster or with_status or with_content_type or all_flag:
        ...
        if with_status:
            try:
                ...
                raise ValueError(f"Invalid status: {with_status}. Must be one of: NOT_FETCHED, FETCHED, ERROR")
        else:
        ...
        if with_content_type:
            ...
            classified_ids = set()
            for doc in docs:
                classification = session.query(DocumentClassification).filter(
