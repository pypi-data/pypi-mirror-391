...
from kurt.content.entity_resolution import (
...
from kurt.db.models import Document, Entity, DocumentEntity, EntityRelationship, SourceType
...
def mock_embeddings():
    ...
    with patch('kurt.content.entity_resolution._generate_embeddings') as mock:
        ...
        def generate_consistent_embeddings(texts):
            ...
            for text in texts:
                ...
                # Create 384-dim embedding (typical for text-embedding-ada-002)
                embedding = [(hash_val >> i) % 100 / 100.0 for i in range(384)]
                embeddings.append(embedding)
            return embeddings

        mock.side_effect = generate_consistent_embeddings
        yield mock
...
def test_two_documents_same_entity_single_creation(test_document, test_document_2, mock_embeddings):
    """
...
def test_similar_entity_names_merged(test_document, mock_embeddings):
    """
    ...
    with patch('kurt.content.entity_resolution.dspy.ChainOfThought') as mock_cot, \
         patch('kurt.content.entity_resolution.DBSCAN', return_value=mock_dbscan):
        mock_cot.return_value.side_effect = resolve_entities
...
def test_reindexing_no_duplicates(test_document, mock_embeddings):
    """
    ...
    mock_resolution = Mock()
    mock_resolution.resolutions = GroupResolution(resolutions=[EntityResolution(
        entity_name="Django",
        resolution_decision="CREATE_NEW",
...
def test_entity_type_mismatch_no_merge(test_document, test_document_2, mock_embeddings):
    """
    ...
    mock_resolution_1.resolutions = GroupResolution(resolutions=[EntityResolution(
    ...
    mock_resolution_2.resolutions = GroupResolution(resolutions=[EntityResolution(
...
def test_alias_matching_links_to_existing(test_document, mock_embeddings):
    """
...
def test_orphaned_entity_cleanup(test_document, mock_embeddings):
    """
...
def test_relationship_creation_no_duplicates(test_document, mock_embeddings):
    """
...
def test_empty_entity_names_handled(test_document, mock_embeddings):
    """
...
def test_link_existing_entities_creates_edges(test_document, test_document_2):
    ...
    # Verify second link
    stmt = select(DocumentEntity).where(
        DocumentEntity.document_id == test_document_2.id,
        DocumentEntity.entity_id == entity.id
    )
...
def test_finalize_knowledge_graph_end_to_end(test_document, test_document_2, mock_embeddings):
    """
...
def test_complex_grouping_mixed_resolutions(test_document, mock_embeddings):
    """
    ...
    def side_effect_resolutions(*args, **kwargs):
        ...
        group_entities = kwargs.get('group_entities', [])
        existing_candidates = kwargs.get('existing_candidates', [])
    ...
    assert len(all_entities) == 4, f"Should have 4 entities (2 existing + 2 new), got {len(all_entities)}"
    ...
    stmt = select(DocumentEntity).where(DocumentEntity.document_id == test_document.id)
    doc_links = session.exec(stmt).all()
    assert len(doc_links) == 4, f"Should have 4 document-entity links (React, Django, DjangoREST, FastAPI), got {len(doc_links)}"
...
def test_circular_relationships(test_document, mock_embeddings):
    """
    ...
    with patch('kurt.content.entity_resolution.dspy.ChainOfThought') as mock_cot:
        ...
        new_entities = [
            {"name": "React", "type": "Technology", "description": "UI library", "aliases": [], "confidence": 0.95},
            {"name": "Redux", "type": "Technology", "description": "State manager", "aliases": [], "confidence": 0.93},
        ]
...
def test_unicode_entity_names(test_document, mock_embeddings):
    """
    ...
    with patch('kurt.content.entity_resolution.dspy.ChainOfThought') as mock_cot:
        ...
        new_entities = [
            {"name": "Café.js", "type": "Technology", "description": "Framework", "aliases": [], "confidence": 0.90},
            {"name": "AI/ML", "type": "Topic", "description": "AI and ML", "aliases": [], "confidence": 0.88},
            {"name": "C++", "type": "Technology", "description": "Language", "aliases": [], "confidence": 0.95},
        ]
