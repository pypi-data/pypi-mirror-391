...
def _add_hadoop(b):
    return (b
            ...
            .config(f"{_HADOOP}.javax.jdo.option.ConnectionURL", NDP_sys.get_v("ConnectionURL", "jdbc:postgresql://localhost:5432/emdm_hd"))
            .config(f"{_HADOOP}.javax.jdo.option.ConnectionUserName", NDP_sys.get_v("ConnectionUserName", "test"))
            .config(f"{_HADOOP}.javax.jdo.option.ConnectionPassword", NDP_sys.get_v("ConnectionPassword", "test"))
            ...
            .config(f"{_HADOOP}.fs.s3a.endpoint", NDP_sys.get_v("endpoint", "http://localhost:8000"))
...
def _add_bricks(b):
    return (b
            ...
            .config(f"{_CORES}", int(NDP_sys.get_v("Cores", 3)))
            )
