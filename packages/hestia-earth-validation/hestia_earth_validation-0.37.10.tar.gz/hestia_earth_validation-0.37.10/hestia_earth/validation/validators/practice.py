...
def validate_liveAnimal_system(data: dict):
    ...
    For animal production Cycles, it is recommended to add a `system` Practice.
    """
    has_animal = (
        len(
            filter_list_term_type(
...
def validate_primaryPercent(cycle: dict, site: dict, list_key: str = "practices"):
    ...
    This validation prevents using `primaryPercent` on practices when the `siteType` is not `agri-food processor`.
    """
    site_type = site.get("siteType")
