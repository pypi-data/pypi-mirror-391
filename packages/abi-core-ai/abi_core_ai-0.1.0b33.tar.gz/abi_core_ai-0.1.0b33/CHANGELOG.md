...
### Changed
- **Default LLM Model**: Changed from `llama3.2:3b` to `tinyllama:latest` for faster development
  - 3x smaller download (~637 MB vs ~2 GB)
  - 3x faster inference
  - 4x less RAM usage (~1 GB vs ~4 GB)
  - Users can still specify `llama3.2:3b` or any other model via `--model` flag
