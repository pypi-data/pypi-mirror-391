...
def main():
    ...
    parser.add_argument(
        "--max_rank", type=float, default=None, help="Max rank to process"
    )
    ...
    parser.add_argument("--hres", type=float, default=250.0, help="Height resolution")
    ...
    if args.days is not None:
        ...
        for time_dir in time_dir_list:
            trt_list.extend(glob.glob(args.trtbase + time_dir + "/TRTC_cell/*_tt.trt"))
    ...
    for fname in trt_list:
        ...
        try:
            ...
            pyrad_main(cfgfile_proc, trajfile=fname, infostr=infostr, trajtype=trajtype)
    ...
    for i, trt_cell_id in enumerate(trt_cell_id_list):
        ...
        for j, datatype in enumerate(datatype_list):
            ...
            if args.path_structure == 1:
                file_base2 = (
                    args.radarbase + time_dir + "/" + dataset + "_trt_center_traj/"
                )
            ...
            if not flist:
                ...
                if args.path_structure == 1:
                    ...
                    if datatype == "RhoHVc":
                        ...
                        labels = [
                            ...
                            "3rd most common",
                            "% points mode",
                            "% points 2nd most common",
                            "% points 3rd most common",
                        ]
                    elif datatype == "entropy" or "prop" in datatype:
                        labels = ["Mean", "Min", "Max"]

                tbin_edges, hbin_edges, _, data_ma, start_time = read_profile_ts(
                    flist, labels, hres=args.hres, t_res=None
                )
                ...
                if datatype == "hydro":
                    (
                        ...
                        _,
                        _,
                    ) = read_trt_traj_data(trt_file_list[i])
            ...
            if not flist:
                warn(
                    "No histogram files found in "
                    + file_base2
                    + "HISTOGRAM/ for TRT cell "
                    + trt_cell_id
                )
            else:
                ...
                data_ma[data_ma == 0.0] = np.ma.masked
                xlabel = (
                    "time (s from " + start_time.strftime("%Y-%m-%d %H:%M:%S") + ")"
                )
                _plot_time_range(
            ...
            if not flist:
                warn(
                    "No quantiles files found in "
                    + file_base2
                    + "QUANTILES/ for TRT cell "
                    + trt_cell_id
                )
                continue

            tbin_edges, qbin_edges, data_ma, start_time = read_quantiles_ts(
                flist, step=5.0, qmin=0.0, qmax=100.0, t_res=None
            )

            basepath_out = os.path.dirname(flist[0])
            fname = (
                basepath_out + "/" + trt_cell_id + "_trt_QUANTILES_" + datatype + ".png"
            )
            ...
            if datatype == "RhoHVc":
                vmin = 0.95
                vmax = 1.00
            xlabel = "time (s from " + start_time.strftime("%Y-%m-%d %H:%M:%S") + ")"
            _plot_time_range(
                tbin_edges,
                qbin_edges,
                data_ma,
...
def get_graupel_column(tbin_edges, hbin_edges, data_ma, start_time, yyyymmddHHMM):
    """
