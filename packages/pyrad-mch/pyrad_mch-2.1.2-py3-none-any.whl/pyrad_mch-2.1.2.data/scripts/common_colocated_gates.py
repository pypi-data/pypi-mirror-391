...
def main():
    ...
    file_path = "/srn/analysis/pyrad_products/rad4alp_intercomp/colocated_gates/"
    rad1_vec = ["A", "A", "A", "A", "D", "D", "D", "L", "L", "P"]
    rad2_vec = ["D", "L", "P", "W", "L", "P", "W", "P", "W", "W"]

    print(
        "====== common colocated gates started: %s"
        % datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    )
    atexit.register(_print_end_msg, "====== common colocated gates finished: ")
    ...
    for i, rad1 in enumerate(rad1_vec):
        ...
        print(
            "rad1 elev min/max",
            np.min(common_dict["rad1_ele"]),
            np.max(common_dict["rad1_ele"]),
        )
        print(
            "rad2 elev min/max",
            np.min(common_dict["rad2_ele"]),
        ...
        fname1_out = (
            ...
            + "PL"
            + rad2
            + "/info_common_COLOCATED_GATES_PL"
            + rad1
            + "_PL"
            + rad2
            + ".csv"
