...
def main():
    ...
    parser.add_argument("--hres", type=int, default=200, help="Height resolution [m]")
    ...
    for datatype in datatype_list:
        ...
        for i in range(ndays):
            time_dir = (proc_starttime + datetime.timedelta(days=i)).strftime(
                "%Y-%m-%d"
            )
            ...
            if datatype == "dBZc":
                filepath = (
                    file_base
                    + time_dir
                    + "/velFilter/PROFILE_dBZc/"
                    + "*_rhi_profile_*_dBZc_hres"
                    + str(hres)
                    + ".csv"
                )
                ...
                filepath = (
                    file_base
                    + time_dir
                    + "/bird_density/PROFILE/"
                    + "*_rhi_profile_*_bird_density_hres"
                    + str(hres)
                    + ".csv"
                )
                labels = ["50.0-percentile", "25.0-percentile", "75.0-percentile"]
        ...
        titl = (
            ...
            + args.starttime
            + "\n"
            + get_field_name(field_dict, "number_of_samples")
        )
