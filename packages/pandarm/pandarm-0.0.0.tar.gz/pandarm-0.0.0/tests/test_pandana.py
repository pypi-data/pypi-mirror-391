...
def test_pois(sample_osm):
    ...
    sample_osm.set_pois(category="restaurants", maxdist=2000, maxitems=10, x_col=x, y_col=y)

    pois = sample_osm.nearest_pois(2000, "restaurants", num_pois=10, include_poi_ids=True)
    assert pois.shape == (1498, 10)
...
def test_pois_pandana3(second_sample_osm):
    ...
    pois = net2.nearest_pois(2000, "restaurants", num_pois=10)
    assert pois.shape == (1498, 10)
