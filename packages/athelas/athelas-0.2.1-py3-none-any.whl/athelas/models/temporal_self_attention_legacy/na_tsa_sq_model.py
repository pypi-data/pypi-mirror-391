...
@MODSTemplate(
    author="maxueyu", description="NA TSA Suspect Queue Model", version="1.0.0"
)
class NATSASuspectQueueModel:
    def __init__(self, sagemaker_session=None):
        ...
        self.preprocessing_step_training_merge = (
            self.create_preprocessing_step_training_merge()
        )
    ...
    def create_preprocessing_step_training_merge(self):
        ...
        return ProcessingStep(
            ...
            processor=sklearn_processor,
            code=str(
                Path(__file__).parent / "scripts" / "preprocess_train_na_merge.py"
            ),
            inputs=[
                ProcessingInput(
