...
class HeirarchicalMoE(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        dispatch_tensor_outer, combine_tensor_outer, loss_outer = self.gate_outer(
            inputs
        )
        ...
        importance = 0.5 * ((importance > 0.5).float() + (importance > 0.0).float())
        ...
        output = torch.einsum(
            "ebcd,bnec->bnd", expert_outputs_outer, combine_tensor_outer
        )
