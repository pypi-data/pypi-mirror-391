...
def find_lib():
    libpath_py = os.path.join(CURRENT_DIR, "lightgbmmt", "libpath.py")
        ...
        if (
            ...
            def get_cmake_opts(openmp_include_dir, openmp_library):
                if openmp_include_dir and openmp_library:
                    return [
                        "-DOpenMP_C_FLAGS=-Xpreprocessor -fopenmp -I{0}".format(
                            openmp_include_dir
                        ),
                        "-DOpenMP_C_LIB_NAMES=omp",
                        "-DOpenMP_CXX_FLAGS=-Xpreprocessor -fopenmp -I{0}".format(
                            openmp_include_dir
                        ),
            ...
            status = silent_call(
                cmake_cmd + get_cmake_opts(openmp_include_dir, openmp_library)
            )
...
class CustomInstallLib(install_lib):
    def install(self):
        ...
        src = find_lib()[0]
        dst = os.path.join(self.install_dir, "lightgbmmt")
    ...
    def run(self):
        ...
        if not self.precompile:
            ...
            compile_cpp(
                ...
                openmp_include_dir=self.openmp_include_dir,
                openmp_library=self.openmp_library,
                nomp=self.nomp,
                bit32=self.bit32,
            )
        install.run(self)
...
class CustomSdist(sdist):
    def run(self):
        ...
        if os.path.exists(os.path.join(CURRENT_DIR, "lightgbmmt", "Release")):
            shutil.rmtree(os.path.join(CURRENT_DIR, "lightgbmmt", "Release"))
        if os.path.exists(os.path.join(CURRENT_DIR, "lightgbmmt", "windows", "x64")):
            shutil.rmtree(os.path.join(CURRENT_DIR, "lightgbmmt", "windows", "x64"))
        if os.path.isfile(os.path.join(CURRENT_DIR, "lightgbmmt", "lib_lightgbm.so")):
            os.remove(os.path.join(CURRENT_DIR, "lightgbmmt", "lib_lightgbm.so"))
        sdist.run(self)
...
if __name__ == "__main__":
    ...
    if os.path.isfile(os.path.join(CURRENT_DIR, os.path.pardir, "VERSION.txt")):
        distutils.file_util.copy_file(
            ...
            os.path.join(CURRENT_DIR, "lightgbmmt", "VERSION.txt"),
            verbose=0,
        )
    version = (
        io.open(
            os.path.join(CURRENT_DIR, "lightgbmmt", "VERSION.txt"), encoding="utf-8"
        )
        .read()
        .strip()
    )
