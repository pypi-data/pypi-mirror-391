...
# 默认配置
# ========================
STATIC_FOLDER = os.path.abspath("dist")   # Vue/React打包路径（可动态修改）
...
def start_proxy(admin_port=55502, api_port=56883, proxy_port=8081, static_folder=None):
    """
    启动本地代理服务器
    ...
    static_folder : str
        前端静态资源路径（默认 ./dist）
    """
    global API_ADMIN_MC, API_MULTICHANEL, STATIC_FOLDER
    ...
    if static_folder:
        ...
        if not os.path.isdir(STATIC_FOLDER):
            logger.info(f"⚠️ 静态资源路径无效：{STATIC_FOLDER}")
        else:
            logger.info(f"📦 使用静态资源目录：{STATIC_FOLDER}")
    ...
    logger.info(f"   → Static Dir: {STATIC_FOLDER}")
