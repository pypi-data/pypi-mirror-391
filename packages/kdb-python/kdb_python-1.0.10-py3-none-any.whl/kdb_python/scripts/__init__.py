...
import kdb_python
from kdb_python import report
from kdb_python.common.profiles import Profiles
from kdb_python.common.utils import WebDriverUtil
from kdb_python.webdriver import kdb_driver
...
class BaseAPI(_Base):
    ...
    def __init__(self, profile: Profiles, path: str):
        ...
        self.response_schema: dict | str | None = None

    def _execute(self, data: dict | str | Row, method: str = 'POST', headers: dict = None,
                 override_profile_data: dict = None,
                 url_params: dict = None, **kwargs):
        url = self._host
        if url_params is not None:
            url = self._host.format(**url_params)
        ...
        if data is not None:
            ...
            self._request_data.update(data_dict)
        ...
        if self.POST_METHOD == method:
            self._response = kdb_driver.requests.post(url, json=self._request_data, headers=self._headers, **kwargs)
        elif self.GET_METHOD == method:
            self._response = kdb_driver.requests.get(url, self._request_data, **kwargs)
        elif self.PUT_METHOD == method:
            self._response = kdb_driver.requests.put(url, self._request_data, **kwargs)
        elif self.OPTIONS_METHOD == method:
            self._response = kdb_driver.requests.options(url, self._request_data, **kwargs)
        elif self.PATCH_METHOD == method:
            self._response = kdb_driver.requests.patch(url, self._request_data, **kwargs)
        elif self.DELETE_METHOD == method:
            self._response = kdb_driver.requests.delete(url, self._request_data, **kwargs)
        elif self.HEAD_METHOD == method:
            self._response = kdb_driver.requests.head(url, self._request_data, **kwargs)
        else:
            raise InvalidArgumentException('Method is invalid.')

        return self._response

    def verify_response(self, json_path_expressions, value_expected):
        kdb_driver.json_path.verify_value(self._response.json(), json_path_expressions, value_expected)
        return self

    def verify_response_with_row(self, row: Row):
        json_path = self._response.json()
        for key in row.headings:
            kdb_driver.json_path.verify_value(json_path, key, row.get(key))
        return self

    def verify_response_keys(self, keys: set | str = None, expressions=None):
    ...
    def verify_response_success_keys(self, keys: set | str = None, expressions=None):
    ...
    def verify_response_schema(self, schema: dict | str = None):
