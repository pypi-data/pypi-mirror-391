...
from kdb_python.common import random_util
from kdb_python.common.constants import InfoMessage
from kdb_python.common.mobile_manager import MobileManager
from kdb_python.common.utils import TimeUtil, CommandLine, DateTimeUtil, WebDriverUtil, FileUtil
from kdb_python.config.settings import DriverSettings, IESettings
from kdb_python.webdriver import actions
from kdb_python.webdriver.alert import Alert
from kdb_python.webdriver.common import report_passed_test_step, report_failed_test_step, log_start, \
    report_warning_test_step
from kdb_python.webdriver.cookies import Cookies
from kdb_python.webdriver.files import Files
from kdb_python.webdriver.json_path import JsonPath
from kdb_python.webdriver.keys import Keys
from kdb_python.webdriver.mobile_gestures import MobileGestures
from kdb_python.webdriver.requests import Requests
from kdb_python.webdriver.video import Video
from kdb_python.webdriver.webdriver_generator import create_driver
from kdb_python.webdriver.windows import Windows
...
class KDBWebDriver:
    ...
    def start_browser(self, browser_name=None, proxy_name=None, private_mode=False, log=True):
        ...
        if browser_name is None:
            browser_name = kdb_python.BROWSER
    ...
    def open_url(self, url, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.open_url.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(url):
    ...
    def verify_url_contains(self, url, exactly=False, reverse=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT,
        ...
        """
        args_passed = locals()
        action_name = self.verify_url_contains.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(url):
                report_warning_test_step(action_name, args_passed, start_time,
    ...
    def verify_state(self, locator, checked, in_frame_on_ios=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT,
        ...
        """
        args_passed = locals()
        action_name = self.verify_state.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(checked):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
    ...
    def execute_script(self, script, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.execute_script.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(script):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return
    ...
    def execute_async_script(self, script, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.execute_async_script.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(script):
                report_warning_test_step(action_name, args_passed, start_time,
    ...
    def set_element_attribute(self, locator, name, value, in_frame_on_ios=False,
        ...
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(value) or _is_ignore_value(name):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
    ...
    def verify_element_attribute(self, locator, name, value, check_contains=False, reverse=False, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        action_name = self.verify_element_attribute.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(value) or _is_ignore_value(name):
                report_warning_test_step(action_name, args_passed, start_time,
            ...
            report_failed_test_step(self._driver, action_name, args_passed, start_time, error_msg % (name, value))
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
    ...
    def verify_element_text(self, locator, text_expected, check_contains=False, reverse=False, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        action_name = self.verify_element_text.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(text_expected):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
            ...
            report_failed_test_step(self._driver, action_name, args_passed, start_time, error_msg % text_expected)
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
    ...
    def press_keys(self, keys, locator=None, in_frame_on_ios=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT,
        ...
        """
        args_passed = locals()
        action_name = self.press_keys.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            ...
            report_passed_test_step(action_name, args_passed, start_time, InfoMessage.ACTION_SUCCESS % "Press keys")
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
    ...
    def upload_file(self, locator, file_path, in_frame_on_ios=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT,
        ...
        """
        args_passed = locals()
        action_name = self.upload_file.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(file_path):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
            actions.perform(actions.upload_file, self._driver, locator, timeout, in_frame_on_ios, file_path, extra_time)
            report_passed_test_step(action_name, args_passed, start_time, InfoMessage.ACTION_SUCCESS % "Upload a file")
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
    ...
    def verify_string_contains(self, input_string, expected_string, exactly=False, reverse=False, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.verify_string_contains.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(expected_string) or _is_ignore_value(input_string):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
            if exactly:
    ...
    def execute_command_line(self, command, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.execute_command_line.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(command):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return
            result = CommandLine.execute(command, timeout)
            report_passed_test_step(action_name, args_passed, start_time, "Execute a command line successfully")
            return result
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
    ...
    def get_json_path(self, json_path, expression, log=True):
        ...
        """
        return self.json_path.get(json_path, expression, log)
    ...
    def verify_activity(self, activity, reverse=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.verify_activity.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(activity):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
    ...
    def verify_string_not_empty(self, input_string, reverse=False, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.verify_string_not_empty.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(input_string):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
            assert (input_string is not None and len(str(input_string).strip()) > 0) != reverse
            report_passed_test_step(action_name, args_passed, start_time, "Verifying '%s' is not empty" % input_string)
        except AssertionError:
            error_msg = "The '%s' is empty"
            if reverse:
                error_msg = "The '%s' is NOT empty"
            report_failed_test_step(self._driver, action_name, args_passed, start_time, error_msg % input_string)
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
    ...
    def verify_json_schemas(self, instance, schema, cls: Validator = None, log=True):
        ...
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(schema) or _is_ignore_value(instance):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
        ...
        except ValidationError as validate_ex:  # the instance is invalid
            error_msg = "The instance <'%s'> is invalid. Error details: %s"
            report_failed_test_step(self._driver, action_name, args_passed, start_time,
                                    error_msg % (str(instance), str(validate_ex)))
        except SchemaError as schema_ex:  # the schema itself is invalid
            error_msg = "The schema <'%s'> itself is invalid. Error details: %s"
            report_failed_test_step(self._driver, action_name, args_passed, start_time,
                                    error_msg % (str(schema), str(schema_ex)))
        except Exception as ex:
            report_failed_test_step(self._driver, action_name, args_passed, start_time, str(ex))
        return self
