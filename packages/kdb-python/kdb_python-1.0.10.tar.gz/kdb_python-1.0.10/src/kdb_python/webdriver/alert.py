...
from kdb_python.webdriver.actions import alert
...
class Alert:
    ...
    def send_keys(self, key_to_send, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.send_keys.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(key_to_send):
    ...
    def verify_text_alert(self, text_to_verify, reverse=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT, log=True):
        ...
        """
        args_passed = locals()
        action_name = self.verify_text_alert.__name__
        start_time = log_start(action_name, args_passed, log)
        try:
            if _is_ignore_value(text_to_verify):
                report_warning_test_step(action_name, args_passed, start_time,
                                         _WARNING_IGNORE_VALUE.format(action_name))
                return self
