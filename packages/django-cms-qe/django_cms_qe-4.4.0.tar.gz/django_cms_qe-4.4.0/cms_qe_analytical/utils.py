...
def get_required_setting(setting, value_re, invalid_msg):
    ...
    try:
        ...
        raise AnalyticalException(f"{setting} setting: not found")  # pylint: disable=raise-missing-from
    if not value:
        raise AnalyticalNotSet(f"{setting} setting is not set")
    value = str(value)
    if not value_re.search(value):
        raise AnalyticalException(f"{setting} setting: {invalid_msg}: '{value}'")
    return value
...
def get_identity(context, prefix=None, identity_func=None, user=None):
    ...
    if prefix is not None:
        try:
            return context[f'{prefix}_identity']
        except KeyError:
            pass
    try:
...
def get_domain(context, prefix):
    ...
    """
    domain = context.get(f'{prefix}_domain')
    if domain is None:
        domain = context.get('analytical_domain')
    if domain is None:
        domain = getattr(settings, f'{prefix.upper()}_DOMAIN', None)
    if domain is None:
...
def is_internal_ip(context, prefix=None):
    ...
    try:
        ...
        internal_ips = None
        if prefix is not None:
            internal_ips = getattr(settings, f'{prefix}_INTERNAL_IPS', None)
