# pylint: disable=protected-access,unused-variable,invalid-name,fixme
#
...
def do_macro(parser, token):
    try:
        ...
        _, macro_name, args = args[0], args[1], args[2:]
    except IndexError:
        m = f"'{token.contents.split()[0]}' tag requires at least one argument (macro name)"
        raise template.TemplateSyntaxError(m)  # pylint: disable=raise-missing-from
...
def do_usemacro(parser, token):
    try:
        args = token.split_contents()
        _, macro_name, values = args[0], args[1], args[2:]
    except IndexError:
        m = f"'{token.contents.split()[0]}' tag requires at least one argument (macro name)"
        raise template.TemplateSyntaxError(m)  # pylint: disable=raise-missing-from
    try:
        ...
        m = f"Macro '{macro_name}' is not defined"
        raise template.TemplateSyntaxError(m)  # pylint: disable=raise-missing-from
