...
def matrix_to_anndata(adata, matrix_key):
    """Creates a new anndata object with the given matrix (any layer, obsm, or X) as adata.X and returns it.
       ...
       the var_names of adata.X and the matrix match exactly in order, values and length.
    ...
    if matrix_key == "X":
        ...
        mat = pd.DataFrame(
            adata.layers[matrix_key].toarray() if hasattr(adata.layers[matrix_key], "toarray") else adata.layers[matrix_key],
            index=adata.obs_names,
            columns=adata.var_names
        )
    elif matrix_key in adata.obsm:
        mat = adata.obsm[matrix_key].copy() if type(adata.obsm[matrix_key]) == pd.DataFrame else pd.DataFrame(adata.obsm[matrix_key])
    else:
