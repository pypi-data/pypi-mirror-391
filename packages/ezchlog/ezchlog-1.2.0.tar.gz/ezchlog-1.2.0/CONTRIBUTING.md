...
You can create merge requests with:
- python code only
- rust code only
- both

But each submitted code should have its test files.

The commit message should start with summary in one line starting by a reference to the **gitlab issue** as `Ref #18` for instance.
Example: `Ref #25: Allow to edit part changelog files`
