...
class ReviewHandler:
    ...
    def do_advance_review(self, diff: str, is_frontend: bool) -> str:
        ...
        with yaspin(Spinners.clock, text=spinner_text, timer=True) as sp:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                ...
                sp.write("Stage 1 (intent and sanity) completed.")
                result2 = future2.result()
                sp.write("Stage 2 (core practices and robustness) completed.")
                result3 = future3.result()
                sp.write("Stage 3 (risk check) completed.")
            ...
            sp.write("Stage 4 (final report generation) completed.")

        return final_result

    def consolidate_reviews(self, reviews: list[str]) -> str:
        spinner_text = "正在整合审查结果..."
        with yaspin(Spinners.dots, text=spinner_text, timer=True) as sp:
            partial_reviews = "\n\n---\n\n".join(reviews)
            prompt = Prompts.build_internal(
                template_name="consolidate_reviews",
                partial_reviews=partial_reviews
            )
            consolidated_result = self.do_review(prompt, model=Model.G_2_5_FLASH)
    ...
    def do_normal_review(self, copy_prompt: Optional[bool], prompt_templates: list[str]):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=len(prompts_with_name) or 1) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_template):
                    ...
                    try:
                        ...
                        logger.error("review for {} generated an exception: {}", template_name, exc)
        ...
        display_and_submit_review(results[0], self.ri.mr, self.ri.jira_key)
...
def split_code(diff: str) -> Optional[dict[str, list[dict]]]:
    ...
    if json_match:
        ...
        json_string = "{}"  # Fallback to an empty JSON object to avoid JSONDecodeError
...
def display_and_submit_review(result: str, mr: MergeRequest, jira_key: str):
    Console().print(Markdown(f"# Code Review Result\n\n{result}", code_theme='rrt'))
    print()
    ...
    """).strip()
    gitlab_comment = gitlab_comment.format(result=result)
    ...
    prompt_text = "✨ For the review: {add_prompt}{add_options}， or {other_options}\n👉 ".format(
    ...
    if selected.lower() == 'y':
        ...
        print(f"✅ Review 已添加到 MR")
    elif selected.lower() == 'c':
        pyperclip.copy(result)
        print("✅ 已复制到剪贴板")
    elif selected.lower() == 's':
        file_path = os.path.expanduser(f'~/Downloads/magic_code_review_{jira_key}.md')
...
def code_review(
    ...
    logger.info("✨ code diff line length: {}", len(ri.mr_diff.splitlines()))
    logger.info("✨ code diff file count: {}", len(ri.mr_diff_by_file))

    if len(ri.mr_diff.splitlines()) < 1000 or len(ri.mr_diff_by_file) == 1:
        result = rh.do_advance_review(ri.mr_diff, is_frontend=False)
        display_and_submit_review(result, ri.mr, ri.jira_key)
        return
    ...
    for i, chunk in enumerate(chunks):
        print(f"🤖 Reviewing chunk {i + 1}/{len(chunks)}: {chunk['summary']}")
        chunk_diff = "".join(ri.mr_diff_by_file.get(file, "") for file in chunk["files"])
        if chunk_diff:
            result = rh.do_advance_review(chunk_diff, is_frontend=False)
            results.append(result)

    if not results:
        logger.warning("没有生成任何审查结果。")
        return

    if len(results) > 1:
        logger.info("🤝 正在整合所有审查结果...")
        final_result = rh.consolidate_reviews(results)
    else:
        final_result = results[0]

    logger.info("✅ 整合完成。")
    display_and_submit_review(final_result, ri.mr, ri.jira_key)
