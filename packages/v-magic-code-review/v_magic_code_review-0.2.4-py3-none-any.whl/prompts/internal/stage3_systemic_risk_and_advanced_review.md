...
- **团队最佳实践 (本阶段聚焦)**:
    ...
    * **警惕隐式范围扩大**
        * 仔细审查循环（for, while）内部的逻辑变更。特别是，警惕那些删除或弱化 `if ... continue` 等提前退出条件的改动。这种变更会扩大循环处理的数据范围，可能会将之前被特意忽略的边缘情况（edge cases）引入执行路径，从而导致潜在的回归（regression）或非预期的行为。当发现此类变更时，请仔细评估其影响，并质疑这是否是有意为之的范围扩大。
    * **字符串比较**
