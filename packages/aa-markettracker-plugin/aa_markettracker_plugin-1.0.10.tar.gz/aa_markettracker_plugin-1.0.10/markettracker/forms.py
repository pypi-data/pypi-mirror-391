...
class TrackedItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "item": forms.Select(
                attrs={
                    "class": "select-search js-item-select",
                    "data-placeholder": "Search...",
                    "data-url": "/markettracker/api/item-search/",
                }
            )
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Najważniejsze dla szybkości: absolutnie NIC nie ładujemy tutaj
        self.fields["item"].queryset = EveType.objects.none()
        self.fields["item"].empty_label = "Search..."
        inst = getattr(self, "instance", None)
        if inst and getattr(inst, "pk", None) and getattr(inst, "item_id", None):
            self.fields["item"].queryset = EveType.objects.filter(pk=inst.item_id)
            self.initial.setdefault("item", inst.item_id)
