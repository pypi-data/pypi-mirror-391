...
class BaseCollector(ABC):
    ...
    def __init__(
        ...
        version: str,
        data_type: str,
        rate_limit: tuple[int, float] = (10, 60),
        ...
        try:
            if url.endswith(".pdf"):
                ...
                if self.enable_deep_crawl and hasattr(crawl_result, "crawled_pages") and crawl_result.crawled_pages:
                    logger.info("Processing %d pages from deep crawl of %s", len(crawl_result.crawled_pages), url)
                    ...
                    for page_result in crawl_result.crawled_pages:
    ...
    async def _process_urls_batch(
        self, urls: list[str], result: CollectionResult
    ) -> list[dict[str, Any]]:
    ...
    async def collect_async(self) -> CollectionResult:
        ...
        for batch in tqdm(batches, desc=f"Collecting {self.collector_name}"):
            batch_items = await self._process_urls_batch(batch, result)
