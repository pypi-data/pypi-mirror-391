...
def download_cos_file_via_coscli(
    ...
    download_command_str = \
        f"{config['coscli_path']} cp \
    ...
    if result.returncode != 0:
        print(f"Error downloading {cos_path} to {local_file_path}: {result.stderr}, result: {result}")
        return result.returncode
    return result.returncode
...
def upload_file2cos_via_coscli(
    ...
    upload_command_str = f"{config['coscli_path']} cp \
        ...
        --part-size {part_size} \
        --thread-num {thread_num} \
        --routines {parallel_file_upload_num} > /dev/null 2>&1"
    result = subprocess.run(upload_command_str, shell=True, capture_output=False, text=True)
    if result.returncode != 0:
        raise RuntimeError(f"Error uploading {local_file_path} to {cos_save_path}: {result}")
    return result.returncode
