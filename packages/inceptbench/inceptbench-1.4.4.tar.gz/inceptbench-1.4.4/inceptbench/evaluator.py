...
class UniversalEvaluationRequest(BaseModel):
    ...
    submodules_to_run: Optional[List[Literal["ti_question_qa", "answer_verification", "external_edubench", "reading_question_qc", "math_content_evaluator", "text_content_evaluator", "math_image_judge_evaluator", "image_quality_di_evaluator", "article_holistic_evaluator", "article_evaluator"]]] = None
    ...
    def _determine_submodules(self) -> List[str]:
        ...
        if has_articles or self.type == "article":
            ...
            # Article evaluator - ALWAYS run for articles (evaluates with 7-metric system)
            modules.append("article_evaluator")
...
def _load_edu_agents_module(module_name: str, file_path: Path):
    import importlib.util
...
def _initialize_edu_agents_namespaces():
    for namespace in ['edu_agents', 'edu_agents.core', 'edu_agents.eval', 'edu_agents.tools']:
        if namespace not in sys.modules:
            sys.modules[namespace] = type(sys)(namespace)


def _run_article_evaluator_task(item_idx: int, parsed_article: Dict[str, Any]) -> Dict[str, Any]:
    logger.debug(f"Running article evaluation for item {item_idx}")

    try:
        _agentic_src = Path(__file__).parent / "submodules" / "agentic-incept-reasoning" / "src"

        _initialize_edu_agents_namespaces()

        dependencies = [
            ('edu_agents.core.api_key_manager', _agentic_src / "edu_agents" / "core" / "api_key_manager.py"),
            ('edu_agents.core.tool_types', _agentic_src / "edu_agents" / "core" / "tool_types.py"),
            ('edu_agents.tools.vector_store', _agentic_src / "edu_agents" / "tools" / "vector_store.py"),
            ('edu_agents.tools.curriculum_search', _agentic_src / "edu_agents" / "tools" / "curriculum_search.py"),
            ('edu_agents.eval.object_counter', _agentic_src / "edu_agents" / "eval" / "object_counter.py"),
        ]

        for mod_name, mod_path in dependencies:
            _load_edu_agents_module(mod_name, mod_path)

        edubench_module = _load_edu_agents_module(
            'edu_agents.eval.edubench_evaluator',
            _agentic_src / "edu_agents" / "eval" / "edubench_evaluator.py"
        )

        if not edubench_module:
            raise ImportError("edubench_evaluator module not found")

        if hasattr(edubench_module, 'ArticleEvaluationResult'):
            edubench_module.ArticleEvaluationResult.model_rebuild()

        evaluate_article = edubench_module.evaluate_article
        article_obj = parsed_article['article']
        result = asyncio.run(evaluate_article(article_obj.content))
        ...
        logger.error(f"Error running article evaluation for item {item_idx}: {e}")
        import traceback
        traceback.print_exc()
        return {
            'question_idx': item_idx,
...
def universal_unified_benchmark(request: UniversalEvaluationRequest) -> UniversalEvaluationResponse:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # Submit article evaluation tasks if enabled (7-metric system)
        if "article_evaluator" in modules_to_use and parsed_articles:
            logger.info(f"Submitting {len(parsed_articles)} article evaluation tasks")
            for i, parsed_article in enumerate(parsed_articles):
                idx = len(questions) + len(text_content) + i
                future = executor.submit(_run_article_evaluator_task, idx, parsed_article)
                all_futures.append(('article_evaluator', idx, future))
        ...
        if all_futures:
            ...
            with tqdm(total=len(all_futures), desc="Running All Evaluation Tasks") as pbar:
                for future_info in all_futures:
                    ...
                    elif module_type == 'article_evaluator':
                        _, item_idx, future = future_info
                        result = future.result()
                        article_evaluator_results.append((item_idx, result))
    ...
    # Process article evaluator results (7-metric system)
    if "article_evaluator" in modules_to_use and article_evaluator_results:
        logger.info(f"Processing {len(article_evaluator_results)} article evaluator results")

        # Sort by item index to maintain order
        article_evaluator_results.sort(key=lambda x: x[0])

        for item_idx, result_dict in article_evaluator_results:
            # Articles are offset by questions + text_content
            article_idx = item_idx - len(questions) - len(text_content)
            item = parsed_articles[article_idx]['article']
            ...
            if item.id in evaluations:
                try:
                    eval_result = result_dict.get('result')
                    if eval_result and 'error' not in result_dict:
                        # Store the full result dict (ArticleEvaluationResult structure)
                        evaluations[item.id].article_evaluator = eval_result
                    else:
                        logger.warning(f"Article evaluation result for item {item_idx} is None or has error: {result_dict.get('error')}")
                        evaluations[item.id].article_evaluator = None
                except Exception as e:
                    logger.error(f"Error converting article evaluation result for item {item_idx}: {e}")
                    evaluations[item.id].article_evaluator = None

        logger.info(f"Assigned article evaluation results to {len(article_evaluator_results)} items")
    ...
    for question_id, evaluation in evaluations.items():
        ...
        # Article evaluator (7-metric system): convert overall rating to 0-1 scale
        if evaluation.article_evaluator:
            overall_rating = evaluation.article_evaluator.get('overall', {}).get('rating', 'INFERIOR')
            article_eval_score = ARTICLE_RATING_SCORES.get(overall_rating, ARTICLE_RATING_SCORES['INFERIOR'])
            scores_to_combine.append(article_eval_score)
            logger.info(f"  - Article evaluator: {article_eval_score:.3f} ({overall_rating})")
    ...
    for question_id, evaluation in evaluations.items():
        ...
        if not request.verbose:
            ...
            if "article_evaluator" in modules_to_use and evaluation.article_evaluator is not None:
                eval_dict["article_evaluator"] = evaluation.article_evaluator
