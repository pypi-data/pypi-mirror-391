...
openai.api_base = os.getenv("OPENAI_API_BASE")  # 可选，若使用默认则留空

# Prompt templates for Chinese and English
prompt_template_zh = """
你是一个精通各个学科的教师，帮助学生进行问题的纠错。请针对以下学科和难度级别自由生成一个问题和学生的错误回答，并给出纠错。问题类型为{question_type}。
...
prompt_template_en = """
You are an expert teacher in various subjects, helping students correct errors in their answers. Please freely generate a question and an incorrect student answer for the given subject and difficulty level. The question type is {question_type}.
If the type is a short-answer question, provide code or mathematical calculations where necessary for certain subjects. Do not include any extra content.
...
def validate_response(response: str) -> bool:
    ...
    try:
        ...
        required_keys = ["Question", "Original Answer", "Corrected Answer", "Correction Explanation"]
...
def get_question_and_answer(subject: str, level: str, question_type: str, lang: str = "en") -> Optional[dict]:
    """Get a question and its corresponding answers based on language selection."""
    ...
    try:
        ...
        return {
            ...
            "Original Answer": qa_data.get("Original Answer", "") or qa_data.get("原答案", ""),
            "Corrected Answer": qa_data.get("Corrected Answer", "") or qa_data.get("纠错后答案", ""),
            "Correction Explanation": qa_data.get("Correction Explanation", "") or qa_data.get("纠错说明", ""),
...
def process_subjects(subject_list: List[Tuple[str, str]], output_file: str, lang: str = "en"):
    """Process all subject and difficulty combinations, generating five questions for each question type."""
    ...
    with open(output_file, 'a', encoding='utf-8') as outfile:
        for idx, (subject, level) in enumerate(subject_list, 1):
            ...
            for question_type in question_types:
                successful_attempts = 0  # Track successful generation attempts

                while successful_attempts < 5:  # Generate 5 questions per type
                    result = get_question_and_answer(subject, level, question_type, lang)
                    ...
                    if result:
                        try:
                            ...
                            outfile.flush()  # Flush immediately

                            successful_attempts += 1
                            print(f"Saved successfully: {subject}-{level}-{question_type}-{lang} ({successful_attempts}/5)")
                        except Exception as e:
...
def main(lang: str = "en"):
    ...
    output_file = os.path.join(output_dir, f"{lang}_error_correct_{current_time}.jsonl")
