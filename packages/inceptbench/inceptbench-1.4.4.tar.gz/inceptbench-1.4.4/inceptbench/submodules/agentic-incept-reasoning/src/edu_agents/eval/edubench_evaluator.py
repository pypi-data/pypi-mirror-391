...
from edu_agents.eval.object_counter import (
...
def _prepare_content_for_curriculum_search(content: str, max_length: int = 3500) -> str:
    ...
    # Build search query with metadata + truncated content
    metadata_text = '\n'.join(metadata_lines)
    remaining_length = max_length - len(metadata_text) - 10  # Leave some buffer
    
    # Take as much content as possible
    truncated_content = '\n'.join(content_lines)
    if len(truncated_content) > remaining_length:
        truncated_content = truncated_content[:remaining_length] + "..."
