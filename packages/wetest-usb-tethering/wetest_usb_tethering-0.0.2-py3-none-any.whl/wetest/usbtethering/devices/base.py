...
class BaseDevice(ABC):
    ...
    def __init__(self, manager: UIAutomatorManager, timeout: float = 5.0):
        """初始化并连接设备
        ...
        self._fastpusher = FastpusherManager(self.android_host, self.real_serial, timeout)
        self.timeout = timeout
        logger.info(
            f"Initializing {self.__class__.__name__} with model {self.device_info.get('model', 'unknown')}. "
            f"fastpusher process PID: {self._fastpusher.pid if self._fastpusher.is_running() else 'N/A'}. "
    ...
    def pc_host_ip(self) -> str:
        ...
        return self._fastpusher.pc_host_ip
    ...
    def _download_normal_file(self, local_file_path: str, device_file_path: str) -> Tuple[List[str], int]:
        ...
        download_url = self._fastpusher.get_download_url(local_file_path)
        cmd = [
            "adb",
            "-s",
            self.serial,
    ...
    def download_file(
        ...
        if not self._fastpusher.is_running():
            self._fastpusher.start()
