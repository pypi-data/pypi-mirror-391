...
def _handle_docker_v2(*, sbom_config: SbomConfig, is_daemon: bool = False) -> dict[str, Any]:  # type: ignore[explicit-any]
    return get_docker_sbom(  # type: ignore[misc]
        sbom_config.source,
...
def _handle_ecr_v2(sbom_config: SbomConfig) -> dict[str, Any]:  # type: ignore[explicit-any]
...
def handle_ecr_with_credentials_v2(  # type: ignore[explicit-any]
    image_uri: str,
...
def resolve_sbom_source_v2(sbom_config: SbomConfig) -> dict[str, Any]:  # type: ignore[explicit-any]
    match sbom_config.source_type:
        case SourceType.DOCKER:
            return _handle_docker_v2(sbom_config=sbom_config)  # type: ignore[misc]
        case SourceType.DOCKER_DAEMON:
            return _handle_docker_v2(sbom_config=sbom_config, is_daemon=True)  # type: ignore[misc]
        case SourceType.ECR:
            return _handle_ecr_v2(sbom_config)  # type: ignore[misc]
        case SourceType.ECR_WITH_CREDENTIALS:
            if not sbom_config.aws_credentials:
                raise AwsCredentialsNotFoundError

            return handle_ecr_with_credentials_v2(  # type: ignore[misc]
                image_uri=sbom_config.source,
                aws_credentials=sbom_config.aws_credentials,
            )
        case _:
