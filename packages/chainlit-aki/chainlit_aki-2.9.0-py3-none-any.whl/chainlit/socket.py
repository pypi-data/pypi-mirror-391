...
def _validate_knowledge_config(knowledge_types: List[Dict[str, Any]], config: Dict[str, Any], knowledge_type_id: str) -> Optional[Dict[str, str]]:
    """Validate knowledge config against widget requirements.

    Returns dict of field errors if validation fails, None if valid.
    """
    # Find the matching knowledge type definition
    knowledge_type = None
    for kt in knowledge_types:
        if kt.get("type") == knowledge_type_id:
            knowledge_type = kt
            break

    if not knowledge_type:
        return None  # Can't validate without type definition

    errors = {}

    # Validate required fields
    for input_widget in knowledge_type.get("inputs", []):
        field_id = input_widget.get("id")
        is_required = input_widget.get("required", False)
        ...
        if is_required:
            ...
            if value is None or value == "" or (isinstance(value, list) and len(value) == 0):
                label = input_widget.get("label", field_id)
...
async def create_knowledge(sid, data: Dict[str, Any]):
    ...
    knowledge_type_id = data.get("type")
    config_data = data.get("config", {})

    # Get knowledge types for validation
    if hasattr(context.session, 'knowledge_types'):
        validation_errors = _validate_knowledge_config(
            context.session.knowledge_types,
            config_data,
            knowledge_type_id
        )
...
async def update_knowledge(sid, knowledge_id: str, updates: Dict[str, Any]):
    ...
    # Note: For updates, we could validate if knowledge type info is available
    # but since we don't have it readily, we'll let backend handler deal with it
    # Client-side validation in the form will catch most issues
