...
async def toggle_mcp(
    ...
    callback = config.code.on_mcp_toggle
    if callback:
        mcp_params = {
            "name": payload.name,
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("success") is False:
                raise HTTPException(
                    status_code=400,
                    detail=result.get("message", "Toggle failed"),
                )
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Toggle callback failed: {e!s}",
            )

    return JSONResponse(
        content={"success": False, "message": "No toggle handler configured"}
    )
