...
def on_mcp_toggle(func: Callable[[Dict[str, Any]], None]) -> Callable:
    """
    Called when an MCP server's enabled/disabled state is toggled.
    Use this to persist the state without actually connecting/disconnecting.

    Args:
        func: Callback function that receives MCP parameters dict containing:
            - name: str - MCP server name
            - enabled: bool - New enabled state (True = enabled, False = disabled)

    Example:
        @cl.on_mcp_toggle
        async def handle_toggle(params: dict):
            server_name = params['name']
            enabled = params['enabled']
            
            # Save state to config/database
            save_mcp_state(server_name, enabled)
            
            # Update UI
            await cl.mcp_status_update('update', {
                'name': server_name,
                'enabled': enabled
            })
            ...
            return {
                'success': True,
                'message': f'Server {"enabled" if enabled else "disabled"}'
            }
    """

    config.code.on_mcp_toggle = wrap_user_function(func)
    return func
...
def on_knowledge_create(
    func: Callable[[Dict[str, Any]], str],
) -> Callable[[Dict[str, Any]], str]:
    """
    Hook to handle knowledge creation.

    Args:
        func: Function that receives knowledge data and returns knowledge_id
    ...
    Example:
        ...
        async def handle_create(data: Dict[str, Any]) -> str:
            ...
            return knowledge_id
    """
    config.code.on_knowledge_create = wrap_user_function(func, with_task=True)
    return func
...
def on_knowledge_update(
    ...
    """
    Hook to handle knowledge updates.

    Args:
        func: Function that receives knowledge_id and updates dict
    ...
    Example:
        ...
        async def handle_update(knowledge_id: str, updates: Dict[str, Any]):
            ...
            pass
    """
    config.code.on_knowledge_update = wrap_user_function(func, with_task=True)
    return func


def on_knowledge_delete(
    func: Callable[[str], Any],
) -> Callable[[str], Any]:
    """
    Hook to handle knowledge deletion.

    Args:
        func: Function that receives knowledge_id
    ...
    Example:
        ...
        async def handle_delete(knowledge_id: str):
            ...
            pass
    """
    config.code.on_knowledge_delete = wrap_user_function(func, with_task=True)
    return func


def on_knowledge_select(
    func: Callable[[List[str]], Any],
) -> Callable[[List[str]], Any]:
    """
    Hook to handle knowledge selection/activation.

    Args:
        func: Function that receives list of selected knowledge_ids
    ...
    Example:
        ...
        async def handle_select(knowledge_ids: List[str]):
            # Activate selected knowledge
            pass
    """
    config.code.on_knowledge_select = wrap_user_function(func, with_task=True)
    return func
