...
def pub(new_persons: List[tdf.TableFrame]) -> tdf.TableFrame | None:
    # If there are no files matching the pattern, the publisher function
    # receives an empty list for the 'new_persons' parameter.

    if new_persons:
        # Concatenate all the TableFrames in the list into a single
        # TableFrame.
        return tdf.concat(new_persons)

    else:
        # If the publisher returns a None, the new version of the
        # Tabsdata table 'new_persons' is empty and without columns.
        return None
