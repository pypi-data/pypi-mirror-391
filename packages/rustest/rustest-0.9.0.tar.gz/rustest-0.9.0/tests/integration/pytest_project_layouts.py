...
def test_src_layout(src_layout_project):
    ...
    assert "3 passed" in result.stdout, f"Expected 3 tests to pass: {result.stdout}"
...
def test_flat_layout(flat_layout_project):
    ...
    assert result.returncode == 0, f"rustest failed: {result.stderr}"
    assert "1 passed" in result.stdout, f"Expected 1 test to pass: {result.stdout}"
...
def test_nested_packages(nested_package_project):
    ...
    assert result.returncode == 0, f"rustest failed: {result.stderr}"
    assert "2 passed" in result.stdout, f"Expected 2 tests to pass: {result.stdout}"
