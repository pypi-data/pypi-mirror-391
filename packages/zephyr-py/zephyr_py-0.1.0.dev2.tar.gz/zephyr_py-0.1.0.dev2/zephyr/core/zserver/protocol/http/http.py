...
from zephyr.core import logging
from zephyr.core.logging import Logger, TRACE_LOG_LEVEL
from zephyr.core.zserver.config import ServerConfig
from zephyr.core.zserver.protocol.http.flow_control import (
    FlowControl,
    CLOSE_HEADER,
    HIGH_WATER_LIMIT,
    service_unavailable,
)
from zephyr.core.zserver.protocol.http.parser import parser
from zephyr.core.zserver.protocol.http.exceptions import HTTPParserError, HTTPParserUpgrade
from zephyr.core.zserver.protocol.http.utils import (
    get_local_addr,
    get_remote_addr,
    get_client_addr,
...
class HTTPProtocol(asyncio.Protocol):
    def __init__(
        ...
        self.access_log = self.logger.hasHandlers()
        ...
        try:
            from zephyr.core.zserver.protocol.websocket import WebSocketProtocol

            self.ws_protocol_class = WebSocketProtocol
            self.logger.debug("WebSocket protocol loaded successfully")
        except ImportError as exc:
            self.ws_protocol_class = None
            self.logger.error("Failed to import WebSocket protocol: %s", exc)
    ...
    def handle_websocket_upgrade(self) -> None:
        self.logger.info("WebSocket upgrade requested for path: %s", self.scope.get("path"))

        if self.logger.level <= TRACE_LOG_LEVEL:
            prefix = "%s:%d - " % self.client if self.client else ""
            self.logger.log(TRACE_LOG_LEVEL, "%sUpgrading to WebSocket", prefix)

        # Send 101 Switching Protocols response BEFORE switching protocols
        ws_key = None
        for name, value in self.headers:
        ...
        if not ws_key:
            ...
            self.send_400_response("Missing Sec-WebSocket-Key")
            return
        ...
        self.logger.debug("Sent 101 Switching Protocols response")

        # Now remove from HTTP connections
        self.connections.discard(self)
        ...
        ws_scope = {
            "type": "websocket",
            "asgi": self.scope.get("asgi", {"version": "3.0"}),
            "http_version": self.scope.get("http_version", "1.1"),
            "scheme": self.scope.get("scheme", "ws"),
            "server": self.scope.get("server"),
            "client": self.scope.get("client"),
            "root_path": self.scope.get("root_path", ""),
            "path": self.scope.get("path", "/"),
            "raw_path": self.scope.get("raw_path", b"/"),
        ...
        # Set scope on protocol
        protocol.scope = ws_scope
        protocol.handshake_complete = True  # Handshake already done by HTTP protocol

        # Transfer connection to WebSocket protocol
        protocol.connection_made(self.transport)
        self.transport.set_protocol(protocol)

        # Start ASGI task
        loop = asyncio.get_event_loop()
        protocol.asgi_task = loop.create_task(protocol.run_asgi())
    ...
    def on_error(self, error: str) -> None:
        ...
        """
        self.logger.error("HTTP parser error: %s", error)
        self.send_400_response(error)
    ...
    def on_headers_complete(self) -> None:
        ...
        # Parse URL first (needed for both HTTP and WebSocket)
        parsed_url = self.parser.parse_url(self.url.decode())
        path = parsed_url.path
        if "%" in path:
            path = urllib.parse.unquote(path)
        full_path = self.root_path + path
        full_raw_path = (self.root_path + path).encode("ascii")
        self.scope["path"] = full_path
        self.scope["raw_path"] = full_raw_path
        self.scope["query_string"] = (parsed_url.query or "").encode("ascii")
...
class RequestResponseCycle:
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.response_started:
            ...
            headers = self.default_headers + list(((i, v) for i, v in message.get("headers", [])))
