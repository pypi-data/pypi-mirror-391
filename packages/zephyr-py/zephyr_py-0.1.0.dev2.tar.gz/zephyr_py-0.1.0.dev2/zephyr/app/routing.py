...
from zephyr.app.responses import PlainTextResponse, Response, JSONResponse, RedirectResponse, WebSocketClose
from zephyr.app.datastructures import URL
...
class _DefaultLifespan:
    ...
    async def __aenter__(self) -> None:
        await self._router.startup(self._app)

    async def __aexit__(self, *exc_info: object) -> None:
        await self._router.shutdown(self._app)
...
class WebSocketRoute(BaseRoute):
    ...
    def url_path_for(self, name: str, /, **path_params: Any) -> URLPath:
        ...
        path, remaining_params = replace_params(self.path_format, self.param_convertors, path_params)
        assert not remaining_params
        return URLPath(path=path, protocol="ws")
...
class Mount(BaseRoute):
    ...
    def __init__(
        ...
        """Initialize a Mount route.

        Args:
            path: The path to mount at (e.g., "/static", "/admin")
    ...
    def matches(self, scope: Scope) -> tuple[Match, Scope]:
        """Check if this mount route matches the request scope.

        Args:
            scope: ASGI scope dict

        Returns:
            Tuple of (Match enum, updated scope)
        """
        if scope["type"] in ("http", "websocket"):
            path = scope["path"]
            if path.startswith(self.path + "/") or path == self.path:
                # Adjust the path for the mounted app
                remaining_path = path[len(self.path) :] or "/"
                return Match.FULL, {"path": remaining_path}
        return Match.NONE, {}

    def url_path_for(self, name: str, /, **path_params: Any) -> URLPath:
        """Get URL path for a named route (not supported for mounts)."""
        msg = f"Mount does not support url_path_for: {name}"
        raise NoMatchFound(msg)

    async def handle(self, scope: Scope, receive: Receive, send: Send) -> None:
        """Handle the request by delegating to the mounted app.
    ...
    def __repr__(self) -> str:
        return f"Mount(path={self.path!r}, name={self.name!r})"
...
class Router:
    ...
    async def shutdown(self, app: Any = None) -> None:
        ...
        # Try new hook system first
        if app is not None and hasattr(app, "run_before_shutdown_hook"):
            await app.run_before_shutdown_hook()
        # Fallback to old event handlers for backward compatibility
        elif app is not None and hasattr(app, "shutdown_handlers"):
            from zephyr.app._callables import run_callable
