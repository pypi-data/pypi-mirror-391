...
def generate_taskflow_stubs(backend_path: Path, output_dir: Path) -> bool:
    ...
    key_modules = ["app.py", "config.py", "database.py", "auth/", "models/", "routes/"]
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("stubs"), help="Output directory for stub files (default: stubs/)"
    )
    parser.add_argument("--zephyr-only", action="store_true", help="Generate only Zephyr framework stubs")
    parser.add_argument("--examples-only", action="store_true", help="Generate only example application stubs")
    parser.add_argument("--taskflow-only", action="store_true", help="Generate only TaskFlow OIDC example stubs")
