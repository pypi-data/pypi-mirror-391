...
def _format_log_prefix(stream_id: Optional[str], capability_name: Optional[str]) -> str:
    """Return a consistent log prefix for stream/capability context."""
    stream_info = f"Stream {stream_id}" if stream_id else ""
    cap_info = f"[{capability_name}]" if capability_name else ""
    return f"{stream_info} {cap_info}: " if stream_info or cap_info else ""
...
class VideoReader:
    ...
    def __init__(
        ...
        if self.source_url is not None:
            ...
            self.logger.info(
                f"{prefix}Using source URL with PyAV. output_type: {self.output_type}, sample_fps: {self._sample_fps}, resize_to: {self._resize_to}, retry: {self.retry_enabled}"
            )
            self._source = self.source_url
        elif self.source_buffer is not None:
            prefix = self._get_log_prefix()
            self.logger.info(
                f"{prefix}Using source buffers with PyAV. output_type: {self.output_type}, sample_fps: {self._sample_fps}, resize_to: {self._resize_to}, retry: {self.retry_enabled}"
            )
    ...
    def _get_log_prefix(self) -> str:
        """Generate log prefix with stream and capability info."""
        return _format_log_prefix(self._stream_id, self._capability_name)
    ...
    def _open(self):
        ...
        prefix = self._get_log_prefix()
        self.logger.info(f"{prefix}Opening video source: {self._source}")
        ...
        try:
            ...
            if self.start_time is None:
                ...
                prefix = self._get_log_prefix()
                self.logger.info(
                    f"{prefix}No metadata timestamp found, using current time: {self.start_time}"
                )

            open_duration = time.monotonic() - start_open
            prefix = self._get_log_prefix()
            self.logger.info(f"{prefix}Successfully opened container in {open_duration:.2f}s")
    ...
    def _print_container_info(self):
        ...
        prefix = self._get_log_prefix()
        self.logger.info(f"{prefix}Container: fmt={fmt}, duration={dur_s:.2f}s, br={br_kbps:.0f}kbps")
        ...
        for idx, stream in enumerate(self._container.streams):
            ...
            if stype == "video":
                ...
                self.logger.info(
                    f"{self._get_log_prefix()}Stream #{idx} [video]: codec={codec}, "
                    f"{w}×{h}@{fps:.2f}fps, br={sbr_kbps:.0f}kbps"
                )
            elif stype == "audio":
                sr = cc.sample_rate
                ch = cc.channels
    ...
    def get_video_frame_generator(self, stream):
        ...
        if "source_paths_generator" in stream.variables:
            ...
            for source_url in source_urls:
                prefix = _format_log_prefix(stream.stream_id, self.__class__.__name__)
                self.logger.info(f"{prefix}Processing {source_url}")
                vfi = VideoReader(
                    source_buffer=None,
