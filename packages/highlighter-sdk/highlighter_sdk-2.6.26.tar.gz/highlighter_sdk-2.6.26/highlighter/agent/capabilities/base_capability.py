...
class Capability(PipelineElement, _BaseCapability):
    ...
    def recording_enabled(self, stream_id) -> bool:
        ...
        if not all(required_args.values()):
            prefix = self._get_log_prefix(stream_id)
            self.logger.warning(
                f"{prefix}Recorder args are missing required parameters: {[name for name, value in required_args.items() if not value]}. recording disabled"
            )
            return False

        return True

    def _get_log_prefix(self, stream_id: Optional[str]) -> str:
        stream_info = f"Stream {stream_id}" if stream_id else ""
        cap_info = f"[{self.__class__.__name__}]" if self.__class__.__name__ else ""
    ...
    def start_stream(self, stream, stream_id, use_create_frame=True):
        if self.recording_enabled(stream_id):
            ...
            if record is not RecordMode.OFF:
                self.logger.info(f"{self._get_log_prefix(stream_id)}Recording to {record}")
    ...
    def record_outputs(self, stream_id: str, frame_data_out: Dict[str, Any], stream_event: StreamEvent):
        ...
        for output in frame_data_out.values():
            ...
            next_elapsed = time.perf_counter() - next_start

            total_elapsed = append_elapsed + next_elapsed
            if total_elapsed > 0.05:  # Only log if > 50ms
                self.logger.debug(
                    f"Stream {stream_id} [{self.__class__.__name__}]: record_outputs append={append_elapsed:.3f}s, next={next_elapsed:.3f}s"
                )
...
class DataSourceCapability(AikoDataSource, _BaseCapability):
    ...
    def frame_generator(self, stream, pipeline_iter_idx):
        ...
        for _ in range(batch_size):
            try:
                ...
                if sample_elapsed > 0.1:  # Only log if > 100ms
                    self.logger.debug(
                        f"Stream {stream.stream_id} [{self.__class__.__name__}]: get_next_data_sample took {sample_elapsed:.3f}s"
                    )
        ...
        if frame_gen_elapsed > 0.1:  # Only log if > 100ms
            self.logger.debug(
                f"Stream {stream.stream_id} [{self.__class__.__name__}]: frame_generator took {frame_gen_elapsed:.3f}s for {batch_size} samples"
            )
