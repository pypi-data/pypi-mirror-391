...
class HLClient(object):
    ...
    def execute(self, request_str: str, variable_values=None) -> Dict[str, dict]:
        ...
        def _execute(request_str: str, variable_values=None):
            ...
            LOGGER.debug(f"[Thread {thread_id}/{thread_name}] _execute: {request_str[:100]}...")

            gql_parse_start = time.perf_counter()
            request_gql = gql(request_str)
            gql_parse_elapsed = time.perf_counter() - gql_parse_start
            LOGGER.debug(f"[Thread {thread_id}/{thread_name}] GQL parse took {gql_parse_elapsed:.3f}s")
            ...
            loop_check_start = time.perf_counter()
            if self._async == "AUTO":
            ...
            loop_check_elapsed = time.perf_counter() - loop_check_start
            LOGGER.debug(
                f"[Thread {thread_id}/{thread_name}] Loop check took {loop_check_elapsed:.3f}s, loop={'exists' if loop else 'None'}"
            )

            client_execute_start = time.perf_counter()
            if loop is None:
                LOGGER.debug(
                    f"[Thread {thread_id}/{thread_name}] Using sync execute (will create new event loop)"
                )
                ...
                LOGGER.debug(
                    f"[Thread {thread_id}/{thread_name}] Using async execute with ThreadPoolExecutor"
                )
            ...
            LOGGER.debug(f"[Thread {thread_id}/{thread_name}] Total _execute took {execute_total:.3f}s")
            return result

        decorator_start = time.perf_counter()
        result = decorators.network_fn_decorator(_execute)(request_str, variable_values=variable_values)
        decorator_elapsed = time.perf_counter() - decorator_start
        LOGGER.debug(f"Total execute (with decorator) took {decorator_elapsed:.3f}s")
        return result
