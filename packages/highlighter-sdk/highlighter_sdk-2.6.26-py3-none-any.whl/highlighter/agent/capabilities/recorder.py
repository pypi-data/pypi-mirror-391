...
class Recorder:
    ...
    def _flush_buffer_async(self, processor_state: Optional["ProcessorState"] = None):
        ...
        try:
            ...
            stream_info = f"Stream {self._stream_id}" if self._stream_id else "Stream <unknown>"
            cap_info = f"[{self._capability_name}]" if self._capability_name else ""
            self.logger.debug(
                f"{stream_info} {cap_info}: queue size: {self._q.qsize()}, queue_put blocked for {queue_elapsed:.3f}s"
            )
        except queue.Full:
    ...
    def _process_data_file(
        ...
        match self._record_mode:
            ...
            case RecordMode.LOCAL:
                ...
                cap_info = f"[{self._capability_name}]" if self._capability_name else ""
                self.logger.info(
                    f'{stream_info} {cap_info}: DataFile("{data_file.file_id}")#save_local {samples_length} samples to {data_file.path_to_content_file} in {save_elapsed:.3f}s'
                )
            ...
            case RecordMode.OUTPUT_FOLDER:
                ...
                self.logger.info(
                    f'DataFile("{data_file.file_id}")#save_local {save_elapsed:.3f}s, copy {copy_elapsed:.3f}s'
                )
            ...
            case RecordMode.CLOUD:
                ...
                try:
                    ...
                    cap_info = f"[{self._capability_name}]" if self._capability_name else ""
                    self.logger.info(
                        f'{stream_info} {cap_info}: DataFile("{data_file.file_id}")#save_to_cloud '
                        f"({samples_length} samples, {file_size_mb:.2f} MB) "
                        f"save_local {save_elapsed:.3f}s, upload {upload_elapsed:.3f}s "
            ...
            case RecordMode.BOTH:
                ...
                cap_info = f"[{self._capability_name}]" if self._capability_name else ""
                self.logger.info(
                    f'{stream_info} {cap_info}: DataFile("{data_file.file_id}")#save_local '
                    f"{samples_length} samples ({file_size_mb:.2f} MB) to {data_file.path_to_content_file} in {save_elapsed:.3f}s"
                )
                try:
                    upload_start = time.perf_counter()
        ...
        cap_info = f"[{self._capability_name}]" if self._capability_name else ""
        self.logger.debug(
            f'{stream_info} {cap_info}: DataFile("{data_file.file_id}") total processing time: {process_elapsed:.3f}s'
        )
