...
def resample_mask(image_input_path, prediction_path):
    """
    Redimensionne le masque de prédiction selon la taille de l'image d'entrée.

    Args:
        image_input_path (str): Chemin de l'image d'entrée
        prediction_path (str): Chemin du fichier de prédiction
    """
    image = sitk.ReadImage(image_input_path)
    mask = sitk.ReadImage(prediction_path)

    resampled_mask = sitk.Resample(mask, image, sitk.Transform(),
                               sitk.sitkNearestNeighbor,
                               0, mask.GetPixelID())

    sitk.WriteImage(resampled_mask, prediction_path)
...
def run_nnunet_prediction(mode, structure, input_path, output_dir, models_dir, animal):
    ...
    resample_mask(input_path, prediction_file)
