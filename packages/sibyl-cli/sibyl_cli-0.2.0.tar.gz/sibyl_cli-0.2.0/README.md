...
### クイックスタート
```bash
pipx install sibyl-cli==0.2.0     # 既存インストールを上書きする場合は --force
sibyl                             # 起動、Ctrl+Q で終了
```

`sibyl-cli` パッケージには `sibyl` コマンドのほか互換目的で `pdev` / `parallel-dev` も同梱されています。バージョン更新時は `pipx upgrade sibyl-cli` を実行してください。pipx 自体がまだなら、macOS なら `brew install pipx && pipx ensurepath`、その他環境なら `python3 -m pip install --user pipx` のあと `pipx ensurepath` を実行し、シェルを再起動して PATH を反映させてください。
