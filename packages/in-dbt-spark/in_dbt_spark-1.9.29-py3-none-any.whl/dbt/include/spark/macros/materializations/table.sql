{% materialization table, adapter = 'spark', supported_languages=['sql', 'python'] %}
   {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Starting table materialization", info=True) %}
   {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Checking for custom Spark table materialization!", info=True) %}
  {%- set file_format = config.get('file_format', 'openhouse') -%}
  {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: file_format = " ~ file_format, info=True) %}
  {% if file_format == 'openhouse' %}
      {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Using LI table materialization (openhouse)", info=True) %}
      {{ return(adapter.dispatch('indbt_table_materialization')()) }}
  {% endif %}
  {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Using standard table materialization", info=True) %}
  ...
  -- build model
  {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Building model with create_table_as", info=True) %}
  {%- call statement('main', language=language) -%}
    {{ create_table_as(False, target_relation, compiled_code, language) }}
  {%- endcall -%}
  {% do log("[DBT_MACRO] TABLE_MATERIALIZATION: Model built successfully", info=True) %}
