{% macro get_insert_overwrite_sql(source_relation, target_relation, existing_relation) %}
  {% do log("[DBT_MACRO] INSERT_OVERWRITE_STRATEGY: Starting insert overwrite strategy", info=True) %}
  {% do log("[DBT_MACRO] INSERT_OVERWRITE_STRATEGY: source_relation = " ~ source_relation ~ ", target_relation = " ~ target_relation, info=True) %}

  {%- set dest_columns = adapter.get_columns_in_relation(target_relation) -%}
  {%- set dest_cols_csv = dest_columns | map(attribute='quoted') | join(', ') -%}
  
  {#-- Set WAP ID configuration before INSERT OVERWRITE --#}
  {{ set_wap_id_config() }}
...
{% macro get_insert_into_sql(source_relation, target_relation) %}
  {% do log("[DBT_MACRO] INSERT_INTO_STRATEGY: Starting insert into strategy", info=True) %}
  {% do log("[DBT_MACRO] INSERT_INTO_STRATEGY: source_relation = " ~ source_relation ~ ", target_relation = " ~ target_relation, info=True) %}

  {%- set dest_columns = adapter.get_columns_in_relation(target_relation) -%}
  {%- set dest_cols_csv = dest_columns | map(attribute='quoted') | join(', ') -%}
  
  {#-- Set WAP ID configuration before INSERT INTO --#}
  {{ set_wap_id_config() }}
