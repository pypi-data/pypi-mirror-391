...
{% macro spark__tblproperties_clause() -%}
  {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: Macro called", info=true) -%}
  {%- set tblproperties = config.get('tblproperties') -%}
  {%- set wap_id_check = env_var('wap_id', 'NOT_FOUND') -%}
  {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: tblproperties from config: " ~ tblproperties, info=true) -%}
  {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: wap_id: '" ~ wap_id_check ~ "'", info=true) -%}
  
  {%- if tblproperties is not none or wap_id_check != 'NOT_FOUND' -%}
    {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: Generating TBLPROPERTIES clause", info=true) -%}
    tblproperties (
      ...
      '{{ prop }}' = '{{ tblproperties[prop] }}'{% if not loop.last %},{% endif %}
        {%- endfor -%}
      {%- endif -%}
      {%- if (wap_id_check != 'NOT_FOUND') and tblproperties is not none -%},{% endif -%}
      {%- if wap_id_check != 'NOT_FOUND' -%}
      {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: Adding WAP properties", info=true) -%}
      'write.wap.enabled' = 'true'
      {%- endif -%}
    )
  {%- else -%}
    {%- do log("[DBT_MACRO] TBLPROPERTIES_CLAUSE: No TBLPROPERTIES needed", info=true) -%}
  {%- endif %}
{%- endmacro -%}
...
{%- macro spark__create_table_as(temporary, relation, compiled_code, language='sql') -%}
  {%- do log("[DBT_MACRO] SPARK_CREATE_TABLE_AS: Macro called - temporary: " ~ temporary ~ ", relation: " ~ relation, info=true) -%}
  {%- if language == 'sql' -%}
    {%- if temporary -%}
...
{% macro test_env_vars() %}
  {%- do log("[DBT_MACRO] TEST_ENV_VARS: Testing environment variable access", info=true) -%}
  {%- do log("[DBT_MACRO] TEST_ENV_VARS: PATH env var: " ~ env_var('PATH', 'NOT_FOUND'), info=true) -%}
  {%- do log("[DBT_MACRO] TEST_ENV_VARS: HOME env var: " ~ env_var('HOME', 'NOT_FOUND'), info=true) -%}
  {%- do log("[DBT_MACRO] TEST_ENV_VARS: wap_id env var: " ~ env_var('wap_id', 'NOT_FOUND'), info=true) -%}
{% endmacro %}

{% macro set_wap_id_config() %}
  {%- do log("[DBT_MACRO] SET_WAP_ID_CONFIG: Macro called", info=true) -%}
  {%- set tablename = load_relation(this) -%}
  {% do log("[DBT_MACRO] SET_WAP_ID_CONFIG: tablename = '" ~ tablename ~ "'", info=true) -%}
  {%- set wap_id = env_var('wap_id', 'NOT_FOUND') -%}
  {%- do log("[DBT_MACRO] SET_WAP_ID_CONFIG: wap_id = '" ~ wap_id ~ "'", info=true) -%}
  
  {%- if wap_id != 'NOT_FOUND' -%}
    {% do log("[DBT_MACRO] SET_WAP_ID_CONFIG: Setting spark.wap.id to: " ~ wap_id, info=true) %}
    {% do log("[DBT_MACRO] SET_WAP_ID_CONFIG: Checking if table exists before setting write.wap.enabled to true: " ~ tablename, info=true) %}
    {%- if tablename is not none -%}
      {% call statement('set_wap_id_table_properties') -%}
        ALTER TABLE {{ tablename }} SET TBLPROPERTIES ('write.wap.enabled' = 'true')
      {%- endcall %}
    {% endif %}
    {% call statement('set_wap_id') -%}
      SET spark.wap.id = '{{ wap_id }}'
    {%- endcall %}
  {%- else -%}
    {%- do log("[DBT_MACRO] SET_WAP_ID_CONFIG: No WAP ID found", info=true) -%}
    {% do log("[DBT_MACRO] SET_WAP_ID_CONFIG: Checking if table exists before setting write.wap.enabled to false: " ~ tablename, info=true) -%}
    {%- if tablename is not none -%}
      {% call statement('unset_wap_id_table_properties') -%}
        ALTER TABLE {{ tablename }} SET TBLPROPERTIES ('write.wap.enabled' = 'false')
      {%- endcall %}
    {%- endif %}
  {%- endif -%}
{% endmacro %}
