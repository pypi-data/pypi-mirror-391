...
# Try to find realign-hook-pre-commit in PATH first
if command -v realign-hook-pre-commit >/dev/null 2>&1; then
    exec realign-hook-pre-commit "$@"
fi

# Fallback: Try using the Python that was used to initialize ReAlign
# This path is captured during 'realign init'
PYTHON_EXEC="{python_path}"
if [ -x "$PYTHON_EXEC" ]; then
    exec "$PYTHON_EXEC" -m realign.hooks --pre-commit "$@"
fi

# Try other common Python executables
for PYTHON_CMD in python3 python; do
    if command -v "$PYTHON_CMD" >/dev/null 2>&1; then
        if "$PYTHON_CMD" -c "import realign.hooks" 2>/dev/null; then
            exec "$PYTHON_CMD" -m realign.hooks --pre-commit "$@"
        fi
    fi
done

# If all else fails, print an error
echo "Error: Cannot find realign-hook-pre-commit or a Python environment with realign installed" >&2
echo "Please ensure ReAlign is properly installed with: pipx install realign-git" >&2
exit 1
'''
...
# Try to find realign-hook-prepare-commit-msg in PATH first
...
PYTHON_EXEC="{python_path}"
if [ -x "$PYTHON_EXEC" ]; then
    exec "$PYTHON_EXEC" -m realign.hooks --prepare-commit-msg "$@"
fi
...
for PYTHON_CMD in python3 python; do
    if command -v "$PYTHON_CMD" >/dev/null 2>&1; then
        if "$PYTHON_CMD" -c "import realign.hooks" 2>/dev/null; then
            exec "$PYTHON_CMD" -m realign.hooks --prepare-commit-msg "$@"
        fi
    fi
done

# If all else fails, print an error
echo "Error: Cannot find realign-hook-prepare-commit-msg or a Python environment with realign installed" >&2
echo "Please ensure ReAlign is properly installed with: pipx install realign-git" >&2
exit 1
'''
