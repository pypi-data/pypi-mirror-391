...
class SignalGenerator(SignalBuilder):
    ...
    def __init__(self, dp: DataProvider, occupancy_profile: OccupancyProfile = None) -> None:
...
class TemperatureController:
    ...
    def hvac_power_k(self, k: int, temperature_setpoint: float, state_model_k: StateModel, state_k: np.matrix, name_inputs_k: dict[str, float], name_inputs_kp1: dict[str, float] = None) -> float:
        ...
        if self.controller_delay == 0:
            hvac_power_k: float = la.inv(T_matrix * state_model_k.D * S_matrix.transpose()) * (temperature_setpoint - T_matrix * state_model_k.C * state_k - T_matrix * state_model_k.D * S_bar_matrix.transpose() * inputs_k)
            ...
            hvac_power_k: float = la.inv(T_matrix * state_model_k.C * state_model_k.B * S_matrix.transpose()) * (temperature_setpoint - T_matrix * state_model_k.C * state_model_k.A * state_k - T_matrix * state_model_k.C * state_model_k.B * S_bar_matrix.transpose() * S_bar_matrix * inputs_k - T_matrix * state_model_k.D * S_bar_matrix.transpose() * S_bar_matrix * inputs_kp1)
