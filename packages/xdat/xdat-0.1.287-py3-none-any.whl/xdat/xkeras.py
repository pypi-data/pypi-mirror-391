...
class SafePredictor:
    """
    Keras model.predict() has a memory leak  (at least for TF 2.13.1)
    This wrapper gets rid of it.
    predict() accepts both a numpy array X, or a generator, and should work with models with 1 or multiple outputs.
    """
    ...
    def predict(self, X, **kwargs):
        ...
        X may be:
          • a NumPy array
          • a tuple/list of NumPy arrays (multi-input)
          • a generator yielding batches (NumPy arrays or tuples)
        """

        # -------- Case 1: X is a single batch (numpy array or list/tuple of numpy arrays) --------
        if not callable(getattr(X, "__iter__", None)) or isinstance(X, (np.ndarray, dict)):
            # Non-generator case: treat X as a single input batch
            return self._predict_single_batch(X)

        # Check if we received a generator (Python iterables with no length)
        if self._looks_like_generator(X):
            return self._predict_generator(X)

        # If it's an iterable but not a generator (e.g., list of arrays), treat as a multi-input batch
        return self._predict_single_batch(X)

    def _looks_like_generator(self, obj):
        return (hasattr(obj, "__iter__") and
                not hasattr(obj, "__len__") and
                not isinstance(obj, (np.ndarray, dict)))

    def _predict_single_batch(self, batch):
        # Multi-input case
        if isinstance(batch, (tuple, list)):
            inputs = [tf.convert_to_tensor(x) for x in batch]
        else:
    ...
    def _predict_generator(self, generator):
        ...
        for batch in generator:
            ...
            for i, out_tensor in enumerate(y):
                outputs[i].append(out_tensor.numpy())
