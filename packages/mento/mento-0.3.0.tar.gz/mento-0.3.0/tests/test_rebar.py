...
def beam_example_imperial() -> RectangularBeam:
    ...
    beam_settings = BeamSettings(
        stirrup_diameter_ini=0.5 * inch, minimum_longitudinal_diameter=1 * inch
    )
...
def test_beam_transverse_rebar_ACI_318_19_imperial(
    ...
    assert beam_example_imperial._A_v.to("cm**2/m").magnitude == pytest.approx(
        11.2214, rel=1e-3
    )
...
def test_beam_transverse_rebar_ACI_318_19_metric(
    ...
    assert beam_example_metric._A_v.to("cm**2/m").magnitude == pytest.approx(
        7.14, rel=1e-3
    )
