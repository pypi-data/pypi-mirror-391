...
```
Usage: plly [OPTIONS] PATH

  Log Linter for Python

Options:
  --config PATH  Path to configuration file (default: plly.yml)
...
plly . --config=custom-plly.yml
```

## Configuration

Create a `plly.yml` in your project root. This is where you declare war on bad logging:
...
jobs:
  plly:
    ...
    steps:
      ...
      - run: uv pip install plly
      - run: plly . --config=plly.yml  # Fail the build. Block the merge. Save the future.
```
