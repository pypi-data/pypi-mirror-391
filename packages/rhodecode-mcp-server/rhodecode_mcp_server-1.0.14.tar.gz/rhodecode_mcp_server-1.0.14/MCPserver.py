...
def get_recent_changesets(
    ...
    try:
        ...
        repo_info = _normalize_result(repo_result)
        
        if not isinstance(repo_info, dict) or "error" in repo_info:
            return {
                "error": repo_info.get("error", "Error al obtener información del repositorio"),
                "changesets": []
            }
        ...
        changesets = _normalize_result(changesets_result)
        
        # Si changesets no es una lista, intentar extraerla
        if isinstance(changesets, dict) and "changesets" in changesets:
