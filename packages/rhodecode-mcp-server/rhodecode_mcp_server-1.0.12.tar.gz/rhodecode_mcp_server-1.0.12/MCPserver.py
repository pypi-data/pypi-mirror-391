...
def get_recent_changesets(
    ...
    """
    try:
        # Obtener información del repositorio para conocer el último commit
        repo_info = get_repo(repoid=repoid, cache=True)
        
        if not isinstance(repo_info, dict):
            return {
                "error": "Error al obtener información del repositorio",
                "changesets": []
            }
        ...
        # En RhodeCode: revision es el número del último commit (base-0 o base-1 según tipo)
        # Para SVN/Git/Hg: total_commits = revision + 1
        ...
        # Calcular start_rev para obtener los últimos N commits
        # start_rev = total_commits - limit (pero no menor que 0)
        requested_limit = min(limit, total_commits)
        start_rev = max(0, total_commits - requested_limit)
        
        # Obtener los changesets
        changesets = get_repo_changesets(
            repoid=repoid,
        ...
        return {
            "changesets": changesets if isinstance(changesets, list) else [],
            "metadata": {
                ...
                "showing": len(changesets) if isinstance(changesets, list) else 0,
