...
RC_TIMEOUT_MS = int(os.getenv("RC_TIMEOUT_MS", "8000"))
...
def get_repos(root: str = None, traverse: bool = True) -> Dict[str, Any]:
    ...
    return {"repos": result if isinstance(result, list) else []}
...
def search(
    ...
    Args:
        ...
        search_type (str): Search type. Valid options: 'commit', 'content', 'path'
        page_limit (int): Page item limit, from 1 to 500. Default 10 items.
        page (int): Page number. Default first page (1).
        search_sort (str): Search sort order. Must start with 'asc:' or 'desc:'. 
                          Default 'desc:date'. Valid options:
                          - asc|desc:message.raw
                          - asc|desc:date
                          - asc|desc:author.email.raw
                          - newfirst (legacy, equal to desc:date)
                          - oldfirst (legacy, equal to asc:date)
        repo_name (str): Filter by one repo. Default is all.
        repo_group_name (str): Filter by one repo group. Default is all.
    
    Returns:
        Dict con resultados de búsqueda
    
    Ejemplo:
        search(search_query="django", search_type="content", page_limit=20, page=1)
        search(search_query="fix bug", search_type="commit", repo_name="myrepo")
    """
    args = {
    ...
    # Agregar parámetros opcionales solo si están especificados
    if repo_name:
    ...
    if isinstance(result, str):
        # Si el resultado es un string, intenta parsear como JSON
        try:
            import json
            result = json.loads(result)
        except:
            result = {"results": result, "error": "Could not parse search results"}
    elif isinstance(result, list):
        # Si es una lista, envolverla en dict
        result = {"results": result, "total": len(result)}
    elif not isinstance(result, dict):
        # Otros tipos, convertir a dict
        result = {"results": str(result), "error": "Unexpected result type"}
    ...
    tools_info = {
        ...
        "categories": {
            ...
            "search": {
                ...
                "tools": [
                    {"name": "search", "readonly_safe": True, "description": "Busca commits, archivos y documentación"}
                ]
            },
            "meta": {
