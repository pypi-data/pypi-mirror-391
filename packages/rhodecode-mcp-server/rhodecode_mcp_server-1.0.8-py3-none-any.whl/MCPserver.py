...
def search(
    ...
    """
    Busca contenido en RhodeCode (commits, archivos, documentación).
    
    Args:
        query: Término de búsqueda
        search_type: Tipo de búsqueda ('content', 'commit', 'path')
        docs: Incluir documentación en búsqueda
        repos: Incluir repositorios
        commits: Incluir commits
        path: Path específico para filtrar (opcional)
        start: Índice de inicio para paginación (default: 0)
        size: Número de resultados (default: 10)
    
    Returns:
        Dict con resultados de búsqueda, total de items y hits
    
    Ejemplo:
        search(query="django", search_type="content", docs=True, repos=True, size=20)
    """
    args = {
        "query": query,
