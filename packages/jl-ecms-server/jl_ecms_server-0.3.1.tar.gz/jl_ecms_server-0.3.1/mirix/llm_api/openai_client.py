...
class OpenAIClient(LLMClientBase):
    def _prepare_client_kwargs(self) -> dict:
        ...
        logger.debug(
            f"OpenAI Client Config - base_url: {self.llm_config.model_endpoint}, api_key: {masked_api_key}"
        )
        ...
        if model_settings.openai_extra_headers:
            try:
                ...
                # Log all headers with masked sensitive values
                logger.debug(
                ...
                for header_name, header_value in headers.items():
                    ...
                    if header_name.lower() in ["authorization", "x-api-key", "api-key"]:
                        masked_value = (
                            f"{str(header_value)[:10]}...{str(header_value)[-4:]}"
                            if len(str(header_value)) > 14
                            else "***"
                        )
