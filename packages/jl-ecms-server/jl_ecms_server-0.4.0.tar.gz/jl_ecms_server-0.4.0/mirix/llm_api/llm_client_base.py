...
class LLMClientBase:
    ...
    async def _get_auth_headers(self) -> Optional[Dict[str, str]]:
        """
        Get authentication headers from configured auth provider (async).

        Returns:
            Dict of headers if auth provider is configured and available, None otherwise.
        """
        if (
            not hasattr(self.llm_config, "auth_provider")
            or not self.llm_config.auth_provider
        ):
            return None
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(auth_headers)} auth header(s) from provider '{self.llm_config.auth_provider}'"
            )
            return auth_headers
