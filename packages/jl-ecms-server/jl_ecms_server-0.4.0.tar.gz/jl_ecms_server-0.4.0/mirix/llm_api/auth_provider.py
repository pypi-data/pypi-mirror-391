...
Example:
    >>> from mirix.llm_api.auth_provider import register_auth_callable
    >>>
    >>> async def get_my_auth_headers():
    ...     token = await get_fresh_token()
    ...     return {"Authorization": f"Bearer {token}"}
    >>>
    >>> register_auth_callable("my_provider", get_my_auth_headers)
"""
...
class AuthProvider(ABC):
    ...
    All auth providers must implement get_auth_headers() as an async method since
    most authentication operations involve I/O (HTTP requests, database queries, etc.).
    """

    @abstractmethod
    async def get_auth_headers(self) -> Dict[str, str]:
        """
        Get authentication headers for the current request (async).
        ...
        should handle token refresh logic internally.
