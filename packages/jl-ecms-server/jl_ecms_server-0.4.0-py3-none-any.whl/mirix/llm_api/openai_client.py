...
class OpenAIClient(LLMClientBase):
    def _prepare_client_kwargs(self) -> dict:
        ...
        # Note: Auth providers are NOT supported in sync mode (_prepare_client_kwargs).
        # Use the async methods (request_async, stream_async) for auth provider support.
        # This is intentional since auth providers now require async operations.
        if hasattr(self.llm_config, "auth_provider") and self.llm_config.auth_provider:
            logger.warning(
                f"Auth provider '{self.llm_config.auth_provider}' configured but not supported "
                "in synchronous OpenAI client initialization. Use async methods (request_async, "
                "stream_async) for auth provider support."
            )
    ...
    async def _prepare_client_kwargs_async(self) -> dict:
        """
        Prepare kwargs for AsyncOpenAI client initialization, including async auth provider headers.
        
        This is the async version that supports auth providers.
        """
        # Start with the base kwargs (API key, base URL, etc.)
        kwargs = self._prepare_client_kwargs()
        
        # Handle auth provider (async)
        if hasattr(self.llm_config, "auth_provider") and self.llm_config.auth_provider:
            ...
            if auth_provider:
                try:
                    ...
                    # Merge auth provider headers into existing headers
                    if "default_headers" in kwargs:
                        kwargs["default_headers"].update(auth_headers)
                    else:
                        kwargs["default_headers"] = auth_headers
    ...
    async def request_async(self, request_data: dict) -> dict:
        ...
        """
        client_kwargs = await self._prepare_client_kwargs_async()
