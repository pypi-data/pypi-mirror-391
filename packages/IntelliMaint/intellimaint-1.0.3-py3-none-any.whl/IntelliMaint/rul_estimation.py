...
Author: rameshbk
Last modified: Shweta, 13th November, 2025
"""
import os
import glob
import re
...
class GPParticleFilter:
    """
    Gaussian Process Particle Filter (GPPF) model for degradation trajectory prediction
    and Remaining Useful Life (RUL) estimation.

    This class combines a Gaussian Process Regression (GPR) model with a particle
    filtering approach to simulate degradation evolution over time. The GPR learns
    the degradation rate (dy/dt) as a function of the current health indicator (HI),
    while the particle filter performs stochastic forward simulations to predict
    the mean degradation path and estimate RUL.

    Typical use cases include:
    - Predicting bearing or tool wear progression based on measured degradation signals.
    - Simulating multiple degradation trajectories to assess uncertainty in RUL.
    - Early warning prediction when HI crosses a failure threshold.

    Example usage:
    ```python
    import numpy as np
    from my_module import GPParticleFilter

    # Sample synthetic degradation data
    y = np.linspace(0.2, 0.9, 100) + np.random.randn(100) * 0.02

    # Initialize and fit the GP Particle Filter
    model = GPParticleFilter(length_scale=0.3, alpha=1e-5, noise_std=0.02)
    model.fit(y)

    # Predict degradation trajectory and RUL
    results = model.predict_rul(y_segment=y, steps=200, N=300)

    # Access RUL estimates for each starting point
    for start_idx, res in results.items():
        print(f"Start index {start_idx} -> Estimated RUL: {res['rul_estimate']} steps")
    ```
    """

    def __init__(self, length_scale: float = 0.3, alpha: float = 1e-5, noise_std: float = 0.02, verbose: bool = False):
        """
        Initialize the Gaussian Process Particle Filter model.

        Args:
            length_scale (float, optional): Length scale parameter for the RBF kernel.
                Controls smoothness of the degradation rate function. Default is 0.3.
            alpha (float, optional): Added noise level in GPR for numerical stability.
                Default is 1e-5.
            noise_std (float, optional): Standard deviation of process noise used during
                particle simulation. Higher values increase trajectory variability.
                Default is 0.02.
        """
        ...
        self.logger.info(f"Initialized {self.__class__.__name__} "
                         f"with length_scale={self.length_scale}, alpha={self.alpha}, noise_std={self.noise_std}")
        if self.verbose:
            print(f"[DEBUG] {self.__class__.__name__} initialized with length_scale={self.length_scale}, "
    ...
    def fit(self, y: np.ndarray):
        """
        Fit the underlying Gaussian Process to model the degradation rate.

        Args:
            y (np.ndarray): One-dimensional array of degradation signal (e.g., health
                indicator values) over time. The GP learns dy/dt as a function of y.
        ...
        if hasattr(y, "values"):
            # Select numeric columns only (ignore strings)
            if hasattr(y, "select_dtypes"):
                y = y.select_dtypes(include=[np.number])
                if y.shape[1] > 1:
                    y = y.mean(axis=1)  # Combine multiple numeric features
                else:
                    y = y.iloc[:, 0]
            y = y.values
        ...
        self.gp = GaussianProcessRegressor(kernel=RBF(self.length_scale), alpha=self.alpha)
        self.gp.fit(X, Y)

        self.logger.info("GP model fitted successfully")
        self.logger.debug(f"Training data shape: X={X.shape}, Y={Y.shape}")
        self._print_debug(f"GP trained with length_scale={self.length_scale}, alpha={self.alpha}")

    def particle_filter(self, y0: float, threshold: float, steps: int = 200, N: int = 200,
                        clip_dy: Tuple[float, float] = (-0.01, 0.05)):
        """
        Perform particle filter simulation using the trained GP model.

        Args:
            y0 (float): Initial health indicator value at the starting point.
            threshold (float): Failure threshold (maximum degradation value to stop simulation).
            steps (int, optional): Maximum number of simulation steps. Default is 200.
            N (int, optional): Number of particles for the Monte Carlo simulation.
                Larger N increases stability but adds computation cost. Default is 200.
            clip_dy (Tuple[float, float], optional): Bounds for predicted degradation rate (dy)
                to prevent unrealistic GP predictions. Default is (-0.01, 0.05).

        Returns:
            Tuple[np.ndarray, np.ndarray]:
                - means: Mean degradation trajectory over time.
                - stds: Standard deviation of the simulated trajectories.
        """
        if self.gp is None:
            raise RuntimeError("GP not fitted before particle filter simulation")

        self.logger.info("Starting particle filter simulation")
        self.logger.debug(f"y0={y0}, threshold={threshold}, steps={steps}, particles={N}")

        particles = y0 + np.random.randn(N, 1) * 0.05
        means, stds = [], []

        for t in range(steps):
            dy_hat, dy_std = self.gp.predict(particles, return_std=True)
            dy_hat = np.clip(dy_hat, clip_dy[0], clip_dy[1]).reshape(-1, 1)
            particles = particles + dy_hat + np.random.randn(N, 1) * self.noise_std

            means.append(np.mean(particles))
            stds.append(np.std(particles))

            if np.mean(particles) >= threshold:
                self.logger.info(f"Threshold reached at step {t}")
                break

        self.logger.info("Particle filter simulation completed")
        return np.array(means), np.array(stds)

    def predict_rul(self, y_segment: np.ndarray, start_rel_idx: Optional[List[int]] = None,
                    steps: int = 300, N: int = 200):
        """
        Predict Remaining Useful Life (RUL) using GP-based particle filtering.

        This method repeatedly applies the particle filter from different points
        along the degradation signal to estimate RUL distributions.

        Args:
            y_segment (np.ndarray): Degradation signal or health indicator time series.
            start_rel_idx (List[int], optional): Indices (relative to y_segment)
                from which to start RUL simulations. If None, three evenly spaced
                start points are automatically selected.
            steps (int, optional): Number of forward simulation steps per start point.
                Default is 300.
            N (int, optional): Number of particles per simulation. Default is 200.

        Returns:
            Dict[int, Dict[str, Any]]:
                A dictionary where each key is a start index and each value contains:
                    - 'mean': Mean simulated trajectory.
                    - 'std': Standard deviation across particles.
                    - 'rul_estimate': Estimated RUL in time steps.
        """
    
        # Handle pandas DataFrame or Series inputs safely
        if hasattr(y_segment, "values"):
            import pandas as pd
            if isinstance(y_segment, pd.DataFrame):
                # Keep only numeric columns
                y_segment = y_segment.select_dtypes(include=[np.number])
                if y_segment.empty:
                    raise ValueError("Input DataFrame has no numeric columns.")
                # Average across columns if multiple numeric features
                if y_segment.shape[1] > 1:
                    y_segment = y_segment.mean(axis=1)
                else:
                    y_segment = y_segment.iloc[:, 0]
            elif isinstance(y_segment, pd.Series):
                y_segment = y_segment.astype(float)
            y_segment = y_segment.values  # Convert to numpy array
                
        y_segment = np.asarray(y_segment).reshape(-1)
        self.fit(y_segment)
        threshold = np.max(y_segment)
        results = {}

        if start_rel_idx is None:
            start_rel_idx = [0, max(0, len(y_segment)//3), max(0, 2*len(y_segment)//3)]

        self.logger.info("Starting RUL prediction")
        self.logger.debug(f"Start indices for RUL estimation: {start_rel_idx}")

        for sp in start_rel_idx:
            y0 = y_segment[sp]
            mu, sigma = self.particle_filter(y0, threshold=threshold, steps=steps, N=N)
            results[int(sp)] = {"mean": mu, "std": sigma, "rul_estimate": len(mu)}
            self.logger.debug(f"RUL estimated from index {sp}: {len(mu)} steps")

        self.logger.info("RUL prediction completed for all start points")
        return results