...
def pydantic_to_zod(
    input: Type[BaseModel] | PRIMITIVE_TYPE | COLLECTION_TYPE | None,
    path: str,
    ...
    if origin is Union:
        ...
        return pydantic_to_zod(non_none_args[0], path, tag) + '.optional()'

    if isinstance(input, type) and issubclass(input, BaseModel):
        # Help 'mypy' narrow the type.
        input_model: Type[BaseModel] = input
        ...
        zod_element = pydantic_to_zod(
            element_type,
            f"{path}.[element]",
        )

        output = f'z.array({zod_element})'
        # The 'tag' might be missing if it is a nested type inside a
        # collection.
        if tag is not None:
            output += f'.meta({{ tag: {tag} }})'
