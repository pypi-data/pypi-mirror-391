...
class ServiceServer(Server):
    ...
    async def start(self):
        ...
        async def wait_for_channel_manager_then_ready():
            """
            Helper that waits for the channel manager to have at least
            our local servicers before setting `ready`, at which point
            tasks can be dispatched.
            """
        ...
        self._wait_for_channel_manager_then_ready_task = (
            asyncio.create_task(
                wait_for_channel_manager_then_ready(),
                name=f'wait_for_channel_manager_then_ready() in {__name__}',
            )
        )
