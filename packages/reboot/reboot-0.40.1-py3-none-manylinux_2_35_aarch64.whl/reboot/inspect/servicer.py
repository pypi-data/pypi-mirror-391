...
from rebootdev.aio.types import ApplicationId, ServerId, StateId, StateTypeName
...
class InspectServicer(
    ...
    async def _aggregate_actor_ids(
        ...
        parts: dict[ServerId, list[StateId]] = {}
        parts_changed = asyncio.Event()

        async def call_other_server(server_id: ServerId):
            """
            Calls `GetStateIds` on the given server, and updates its
            ...
            server_request = GetStateIdsRequest()
            ...
            async for response in stub.GetStateIds(
                ...
                parts[server_id] = list(response.state_ids)
                parts_changed.set()
    ...
    async def GetStateIds(
        ...
        # If this call is not specifically for this server, act as an
        # aggregator.
