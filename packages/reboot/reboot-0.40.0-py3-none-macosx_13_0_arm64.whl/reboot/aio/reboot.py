...
# tests.
#
# Note that this number does NOT influence the number of servers in
# a Reboot Cloud deployment, since it gets its `ApplicationConfig` from
# elsewhere. It may still follow
# `reboot.controller.plan_makers.DEFAULT_NUM_SERVERS`.
...
class Reboot:
    ...
    def create_external_context(
        ...
        *,
        ...
        if app_internal:
            app_internal_authorization = self._server_manager.app_internal_api_key_secret
    ...
    async def up(
        ...
        *,
        ...
        servers: the number of serving servers (think of these
          as "servers") to create.
        """
        ...
        if len(serviceables) > 0:
            ...
            servers = servers or DEFAULT_NUM_SERVERS
