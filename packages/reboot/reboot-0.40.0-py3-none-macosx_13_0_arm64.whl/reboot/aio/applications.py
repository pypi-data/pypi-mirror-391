...
class Library(AbstractLibrary):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not hasattr(cls, 'name'):
            raise NotImplementedError("Please set `name` as a class variable.")

    async def pre_run(self, application: Application) -> None:
        """
        A hook to allow for any library setup before the `Application` runs.
        """
        pass
...
class Application:
    ...
    def __init__(
        ...
        *,
        ...
        """
        # Get all libraries including dependent libraries.
        if libraries is not None:
            ...
            # Verify we have all the dependencies for libraries.
            dependency_names = set(
                dependency for library in libraries
                for dependency in library.dependencies()
            )
            needed_dependencies = dependency_names - library_names

            if len(needed_dependencies) > 0:
                raise ValueError(
                    "Missing dependent libraries: "
                    f"{', '.join(needed_dependencies)}"
