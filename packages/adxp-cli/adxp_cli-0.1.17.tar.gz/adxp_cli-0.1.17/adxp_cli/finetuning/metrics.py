...
def get_finetuning_hub(use_backend_ai: bool = False):
    ...
    if hasattr(config, 'token') and config.token:
        return AXFineTuningHub(headers=headers, base_url=config.base_url, use_backend_ai=use_backend_ai)
    else:
        ...
        return AXFineTuningHub(credentials, use_backend_ai=use_backend_ai)
