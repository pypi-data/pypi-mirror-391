...
class QRadarAPIClient:
    ...
    def __init__(
        ...
        logger.info("QRadar API client initialized for %s", self.base_url)
    ...
    def _make_request(
        ...
        try:
            ...
            raise QRadarAPIException(f"Request failed: {str(e)}") from e
    ...
    def execute_aql_query(
        ...
        while True:
            ...
            if status == "ERROR":
                ...
                raise QRadarAPIException(f"Search failed: {error_msg}")
            if status == "CANCELED":
    ...
    def _initiate_search(self, aql_query: str) -> str:
        ...
        if not isinstance(response, dict):
            raise QRadarAPIException(f"Unexpected response type: {type(response)}")
    ...
    def _get_search_results(
        ...
        if return_format.lower() == "csv":
            ...
            if not isinstance(response, str):
                raise QRadarAPIException(f"Expected CSV string response, got {type(response)}")
    ...
    def get_events(
        ...
        if fields:
            ...
            # Default fields matching client requirements
            select_clause = """
