...
class QRadarEvent(BaseModel):
    ...
    magnitude: Optional[int] = Field(
        default=5, alias="magnitude", ge=0, le=10, description="Event magnitude/severity level (0-10)"
    )
    ...
    @field_validator("source_ip", "dest_ip", "username", mode="before")
    @classmethod
    ...
    def get_primary_asset_identifier(self) -> str:
        ...
        Prefers source IP, falls back to destination IP, then username.

        Returns:
            str: Asset identifier (IP address or username)
        """
...
if __name__ == "__main__":
    ...
    logger.info("Parsed QRadar Event:")
    logger.info(json.dumps(event.model_dump(), indent=2))
    logger.info(f"\nEffective Severity: {event.get_severity_value()}")
    logger.info(f"Primary Asset: {event.get_primary_asset_identifier()}")
    logger.info(f"Valid Source Asset: {event.is_valid_source_asset()}")
    logger.info(f"Valid Dest Asset: {event.is_valid_dest_asset()}")
