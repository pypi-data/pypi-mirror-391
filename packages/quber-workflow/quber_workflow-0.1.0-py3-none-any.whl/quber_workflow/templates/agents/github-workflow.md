...
```bash
# 1. Ensure main workspace is up to date (at /home/mande/repo/quber-excel/)
cd /home/mande/repo/quber-excel
git checkout main
git pull origin main

# 2. Create worktree with feature branch in /tmp/
git worktree add /tmp/quber-excel-QUE-XXX -b <type>/QUE-XXX-<description>

# 3. IMMEDIATELY push to remote (even if empty)
cd /tmp/quber-excel-QUE-XXX
...
**Worktree Locations**:
- Main workspace: `/home/mande/repo/quber-excel/` (always stays on `main` branch)
- Feature branches: `/tmp/quber-excel-QUE-XXX/` (isolated workspace per ticket)
...
git worktree add /tmp/quber-excel-QUE-XXX -b feature/QUE-XXX-brief-description
