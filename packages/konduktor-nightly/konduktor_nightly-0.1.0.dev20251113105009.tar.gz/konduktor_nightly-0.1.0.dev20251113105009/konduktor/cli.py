...
def _get_auth_clients() -> Tuple[AuthService, AuthStore]:
    service = build_default_auth_service()
    store = build_default_auth_store()
    return service, store


def _format_auth_user(user: AuthUser) -> str:
    return user.email or user.name or user.subject


def _require_auth_session(store: AuthStore) -> AuthState:
    session = store.load()
    if session:
        return session
    raise click.ClickException(
        'No active auth session found. Run '
        f'{colorama.Style.BRIGHT}konduktor auth login{colorama.Style.RESET_ALL} first.'
    )


def _handle_auth_service_error(exc: AuthServiceError, store: AuthStore) -> None:
    message = str(exc)
    if 'HTTP 401' in message:
        store.clear()
        raise click.ClickException(
            'Your Trainy Auth session expired. Run '
            f'{colorama.Style.BRIGHT}konduktor auth login{colorama.Style.RESET_ALL} again.'
        ) from exc
    raise click.ClickException(message) from exc


def _render_clusters_table(clusters: List[ClusterInfo]) -> None:
    table_kwargs = {
    ...
    table = log_utils.create_table(
        ['Cluster', 'Teleport', 'Description'],
        **table_kwargs,
    )
    for cluster in sorted(clusters, key=lambda item: item.name):
        table.add_row(
            [
                cluster.name,
...
def _list_clusters_command() -> None:
    service, store = _get_auth_clients()
    session = _require_auth_session(store)
    try:
        clusters = service.list_clusters(session.token)
    except AuthServiceError as exc:
        _handle_auth_service_error(exc, store)
    if not clusters:
        click.echo(
            'No Kubernetes clusters are currently available for '
            f'{_format_auth_user(session.user)}.'
        )
        return
    click.secho(
        f'Clusters visible to {_format_auth_user(session.user)}:',
        fg='green',
        bold=True,
    )
    _render_clusters_table(clusters)
...
def auth():
    """Authenticate with Trainy Auth and manage kubeconfigs."""
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help='Ignore cached credentials and start a fresh login.',
)
def login(force: bool):
    """Run a headless Dex device login for the CLI."""
    service, store = _get_auth_clients()
    existing = store.load()
    if existing and not force:
        click.secho(
            f'Already logged in as {_format_auth_user(existing.user)}.',
            fg='green',
            bold=True,
        )
        click.echo('Use --force to start a new login session.')
        return

    click.secho(f'Using Trainy Auth endpoint: {service.base_url}', fg='cyan')
    try:
        device = service.start_device_authorization()
    except AuthServiceError as exc:
        raise click.ClickException(str(exc)) from exc

    url_hint = device.verification_uri_complete or device.verification_uri
    click.secho('Complete the login in your browser:', fg='cyan', bold=True)
    click.echo(
        f'  Open: {colorama.Style.BRIGHT}{url_hint}{colorama.Style.RESET_ALL}'
    )
    if not device.verification_uri_complete:
        click.echo(
            f'  Code: {colorama.Style.BRIGHT}{device.user_code}{colorama.Style.RESET_ALL}'
        )
    click.echo('  Approve the Konduktor CLI request.')
    minutes = max(device.expires_in // 60, 1)
    click.echo(f'This link expires in about {minutes} minute(s).')
    click.echo('Waiting for approval...')

    poll_interval = max(device.interval, 5)
    deadline = time.time() + device.expires_in
    while time.time() < deadline:
        time.sleep(poll_interval)
        try:
            status = service.poll_device_session(device.session_id)
        except AuthServiceError as exc:
        ...
        if status.status == 'approved':
            if not status.session_token or not status.user:
                raise click.ClickException(
                    'Auth service returned an incomplete response.'
                )
            ...
            click.secho(
                f'Logged in as {_format_auth_user(status.user)}',
                fg='green',
                bold=True,
            )
            click.echo(
                f'Session expires at {expires_at.astimezone().isoformat()}'
            )
            return
        if status.status == 'failed':
            detail = status.detail or 'Login was declined.'
            raise click.ClickException(detail)
        if status.status == 'expired':
            detail = status.detail or 'Device code expired. Run the command again.'
            raise click.ClickException(detail)

    raise click.ClickException(
        'Device authorization timed out before approval. Please rerun '
        'konduktor auth login.'
    )


@auth.group(cls=_NaturalOrderGroup, invoke_without_command=True)
@click.pass_context
def clusters(ctx):
    """Inspect accessible clusters and fetch kubeconfigs."""
    if ctx.invoked_subcommand is None:
        _list_clusters_command()


@clusters.command(name='list')
def clusters_list():
    """List clusters available to the logged-in user."""
    _list_clusters_command()


@clusters.command(name='get-credential')
@click.argument('cluster_name', required=True)
@click.option(
    '--ttl',
    type=str,
    default=None,
    help='Optional Teleport TTL override such as 8h or 30m.',
)
@click.option(
    '--kubeconfig',
    'kubeconfig_path',
    ...
    default=None,
    help='Path to merge into (defaults to ~/.kube/config).',
)
def clusters_get_credential(
    cluster_name: str, ttl: Optional[str], kubeconfig_path: Optional[str]
):
    """Request and merge credentials for a cluster."""
    service, store = _get_auth_clients()
    session = _require_auth_session(store)
    try:
        credential = service.issue_kubeconfig(session.token, cluster_name, ttl=ttl)
    except AuthServiceError as exc:
        _handle_auth_service_error(exc, store)

    try:
        merged_path = merge_kubeconfig(
            credential.kubeconfig,
            destination=kubeconfig_path,
        )
    except AuthServiceError as exc:
        raise click.ClickException(str(exc)) from exc

    click.secho(
        f'Merged credentials for {cluster_name} into {merged_path}',
        fg='green',
        bold=True,
    )
    click.echo(f'Expires at {credential.expires_at.astimezone().isoformat()}')
