...
Optimizations (no boto3 required):
- Key discovery uses s3fs.glob (server-side filtering) across only hour
  prefixes that can contain the target time, executed concurrently.
- Downloads executed in parallel with s3fs.get.
- Existing files are skipped if their size matches the remote object.
"""
...
def _build_s3_filesystem() -> s3fs.S3FileSystem:
    """
    Construct an anonymous S3FileSystem with a larger connection pool.
    """
    return s3fs.S3FileSystem(
        anon=True,
        # Bigger pool helps when we parallelize
...
# -----------------------------------------------------------------------------
# Fast key discovery (glob + concurrency)
# -----------------------------------------------------------------------------
...
def find_ash_keys_for_datetime(
    ...
    *,
    ...
    -------
    (keys, target_dt)
        keys : dict  -> {channel: s3_key}   (e.g., "noaa-goes16/ABI-L1b-RadF/...")
        target_dt : datetime
    """
    ...
    # Build per-channel candidate lists using glob (fast server-side filtering)
    ch_to_candidates: Dict[str, List[str]] = {ch: [] for ch in channels}

    def _glob_one(prefix: str, ch: str) -> List[str]:
        # Keys look like: {bucket}/{product}/YYYY/DOY/HH/{product}-M6C{ch}_...nc
        pattern = f"{prefix}{product}-M6C{ch}_*.nc"
        try:
            return fs.glob(pattern)  # lets S3 do the filtering
    ...
    if max_time_diff is None and all(len(v) == 0 for v in ch_to_candidates.values()):
        ...
        # List hour dirs under day, then glob within them
        ...
        # Extract only directory-like entries
...
def download_ash_bands_for_datetime(
    ...
    *,
    ...
    Optimized:
      - Parallel downloads (ThreadPoolExecutor) with s3fs.get
      - Skip re-download when local file size matches remote
    """
    _log(
    ...
    def _remote_size(key: str) -> Optional[int]:
        try:
            info = fs.info(key)  # {'Key':..., 'Size':...} depending on s3fs version
            # s3fs typically uses 'Size' (capital S) or 'size'
            return info.get("Size") or info.get("size")
        except Exception:
            return None

    def _needs_download(key: str, local_path: str) -> bool:
        rsize = _remote_size(key)
        if rsize is None:
            return True
    ...
    def _download_one(ch_key_pair: Tuple[str, str]) -> Tuple[str, str, str]:
        ch, key = ch_key_pair
        fname = os.path.basename(key)
        local_path = os.path.join(subdir, fname)
        if not _needs_download(key, local_path):
            _log(f"Skipping download (already up-to-date) for band {ch}: {local_path}")
            return ch, local_path, "skipped"

        _log(f"Downloading band {ch} from s3://{key}")
        # Ensure parent exists (safe under parallel execution)
        os.makedirs(os.path.dirname(local_path), exist_ok=True)
        fs.get(key, local_path)
        return ch, local_path, "downloaded"

    # Build tasks and run in parallel (I/O-bound → threads work well)
    items = list(keys.items())
    max_workers = min(8, len(items)) or 1
    local_paths: Dict[str, str] = {}

    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futs = {ex.submit(_download_one, kv): kv[0] for kv in items}
        for fut in as_completed(futs):
            ch = futs[fut]
            try:
                _ch, lpath, status = fut.result()
                local_paths[_ch] = lpath
                _log(f"Band {_ch} {status}: {lpath}")
            except Exception as e:
