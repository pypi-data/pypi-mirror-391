...
async def main() -> None:
    response = await client.customer_portal.create_session(
        return_url="https://example.com/dashboard",
        subject_id="subj_VyX6Q96h5avMho8O7QWlKeXE",
    )
    print(response.subject_id)
...
try:
    client.customer_portal.create_session(
        return_url="https://example.com/dashboard",
        subject_id="subj_VyX6Q96h5avMho8O7QWlKeXE",
    )
...
client.with_options(max_retries=5).customer_portal.create_session(
    return_url="https://example.com/dashboard",
    subject_id="subj_VyX6Q96h5avMho8O7QWlKeXE",
)
```
...
client.with_options(timeout=5.0).customer_portal.create_session(
    return_url="https://example.com/dashboard",
    subject_id="subj_VyX6Q96h5avMho8O7QWlKeXE",
)
```
...
customer_portal = response.parse()  # get the object that `customer_portal.create_session()` would have returned
print(customer_portal.subject_id)
```
