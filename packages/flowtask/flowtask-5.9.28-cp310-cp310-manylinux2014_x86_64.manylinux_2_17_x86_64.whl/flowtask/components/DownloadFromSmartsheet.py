...
class DownloadFromSmartsheet(SmartSheetClient, FlowComponent):
    ...
    ```yaml
    name: Download Approved HVAC Documents
    description: Download PDFs from approved HVAC vendors
    steps:
      - DownloadFromSmartsheet:
          sheet_id: '8504624500658052'
          directory: /home/ubuntu/data/hvac_approved/
          attachment_filter:
            - .pdf
          filename_filter: 'W-9\.pdf$|Full Packet\.pdf$'
          column_filter:
            Vendor Type: 'HVAC'
    ...
    ```yaml
    # Match files ending with "Full Packet.pdf"
    filename_filter: 'Full Packet\.pdf$'

    # Match files starting with "Invoice"
    filename_filter: '^Invoice'

    # Match multiple patterns (W-9 OR Full Packet)
    filename_filter: 'W-9\.pdf$|Full Packet\.pdf$'

    # Match any PDF with "2024" in the name
    filename_filter: '2024.*\.pdf$'
    ```
