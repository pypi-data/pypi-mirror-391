...
class MLOpsManager:
    ...
    def get_user_info(self):
        base_url = self.USER_MANAGEMENT_API.rstrip('/') + "/"
        user_path = f"user/{self.CLEARML_USERNAME}/metadata?token={self.USER_TOKEN}"
    ...
    def add_model(self, source_type, model_name=None, source_path=None, code_path=None,
        ...
        if not model_name or not isinstance(model_name, str):
            ...
            logger.error(error_msg) # Use logger here as it's a validation error in MLOpsManager
        ...
        if self.get_model_id_by_name(model_name):
            ...
            logger.warning(warning_msg) # Use logger for warnings
        ...
        try:
            ...
            if model_id != temp_model_id:
                ...
                self.ceph.delete_folder(dest_prefix) # This might be redundant if move_folder deletes source
            ...
            logger.error(error_msg, exc_info=True) # Use logger for exceptions
            print(f"[ERROR] {error_msg}")
            ...
            if 'model_id' in locals():
                try:
                    ...
                    logger.error(error_cleanup) # Use logger for cleanup errors
