...
# Add the lager module to path
sys.path.insert(0, '/opt/lager')
...
def main():
    """Main BLE function - dispatches to scan or connect based on arguments"""
    try:
        # Parse arguments
        if len(sys.argv) < 2:
        ...
        if action == 'scan':
            ...
            print(f"{RED}Error: Unknown action '{action}'. Use 'scan' or 'connect'{RESET}")
            sys.exit(1)
