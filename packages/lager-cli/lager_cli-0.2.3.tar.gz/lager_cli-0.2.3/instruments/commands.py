...
@click.pass_context
def instruments(ctx, box: str | None, dut: str | None = None) -> None:
    """List attached instruments"""
    # Resolve box/dut (box takes precedence, dut is for backward compatibility)
    resolved = box or dut

    # Check if resolved is a local DUT name first, then gateway
    if resolved:
        local_ip = get_dut_ip(resolved)
        if local_ip:
            dut = local_ip
        else:
            dut = resolved
