...
def add_dut(name: str, ip: str, user: Optional[str] = None) -> None:
    """Add a DUT to the local storage.

    Args:
        name: DUT name
        ip: IP address
        user: Optional username (if None, stores in legacy format)
    """
    duts = load_duts()
    if user:
        # New format with user
        duts[name] = {"ip": ip, "user": user}
    else:
