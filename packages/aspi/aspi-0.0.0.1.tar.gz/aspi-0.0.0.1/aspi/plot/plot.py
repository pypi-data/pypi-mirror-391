...
def plot3d(d3d,frames=None,z=None,x=None,y=None,vmin=None,vmax=None,dz=0.01,dx=0.01,dy=0.01,nlevel=100,figsize=(8, 6),ifnewfig=True,figname=None,showf=True,close=True,**kwargs):
	'''
	...
	vmin, vmax: clip value minimum and maximum (e.g., clim=[vmin,vmax])
...
#,
	...
	kw = {
	'vmin': [d3d.min(),vmin][1-int(vmin==None)],
	'vmax': [d3d.max(),vmax][1-int(vmax==None)],
...
def framebox(x1,x2,y1,y2,c=None,lw=None):
	...
	EXAMPLE I
	from aspi.plot import framebox
	from aspi.seistr import gensyn
	from matplotlib import pyplot as plt
	d=gensyn();
	plt.imshow(d);
...
def wiping(inpath, outpath, duration=500, number=30):
	...
	from aspi import genplane3d,plot3d
	data,datan=genplane3d(noise=True,seed=202425,var=0.1);
	dn=datan[:,:,10]
	import aspi.drr as pd
