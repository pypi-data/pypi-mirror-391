...
from ..sheets.file_browser_sheet import FileBrowserSheet
...
def _run_classic(argv: Sequence[str]) -> int:
    ...
    launch_browser = args.path is None and expr_text is None
    if launch_browser:
    ...
    try:
        try:
            if launch_browser:
                session = _create_file_browser_session(runtime, recorder, args)
            else:
...
def _create_file_browser_session(
    ...
    start_dir = Path.cwd()
    sheet = FileBrowserSheet(start_dir, scanners=runtime.scanners)
    session = Session(
        None,
        ...
        runtime=runtime,
        initial_sheet=sheet,
        source_label=str(start_dir),
    )
    message = getattr(sheet, "status_message", None)
    if message:
        session.viewer.status_message = message
    with suppress(Exception):
