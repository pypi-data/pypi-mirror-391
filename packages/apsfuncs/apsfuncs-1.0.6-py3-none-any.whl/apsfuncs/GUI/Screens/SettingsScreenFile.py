...
class ResetSettingsConfDialog(QDialog):
    # Init
    def __init__(self, parent_widget, logger):
        super().__init__()
    ...
    def confirm_reset(self):
        ...
        self.logger.info("Reset settings confirmed by user")
...
class SaveChangesDialog(QDialog):
    ...
    def confirm_save(self):
        # Call the parent reset and close the dialog
        self.logger.info("Save changes confirmed by user")
...
class SettingsScreenAPSBase(QWidget):
    ...
    # Init
    def __init__(self, main_window, config_dict, logger):
        super().__init__()
        self.logger = logger
    ...
    def return_clicked(self):
        ...
        if self.holding_dict != self.config_dict:
            self.logger.info("Setting changes detected")

            # Show confirmation dialog to confirm if the user wants to save their changes
            conf_dialog = SaveChangesDialog(parent_widget=self, logger=self.logger)
            conf_dialog.exec()
            self.logger.info("Save change dialog closed")
    ...
    def scene_opened(self):
        ...
        self.holding_dict = self.config_dict.copy()
        self.logger.info("Current settings stored")
    ...
    def update_alpha_opt_in(self, new_state):
        ...
        self.logger.info("Alpha opt in updated to {}".format(bool_state))
    ...
    def update_maintain_config(self, new_state):
        ...
        self.logger.info("Maintain config updated to {}".format(bool_state))
    ...
    def update_crash_repo_location(self):
        ...
        if len(new_location) > 0:
            self.logger.info("Crash repo location updated to {}".format(new_location))
    ...
    def update_feedback_repo_location(self):
        ...
        if len(new_location) > 0:
            self.logger.info("Feedback repo location updated to {}".format(new_location))
    ...
    def update_server_auth_repo_location(self):
        ...
        if len(new_location) > 4:
            self.logger.info("Server auth location updated to {}".format(new_location))
    ...
    def confirm_reset_config(self):
        self.logger.info("Reset to deaults selected")
        # Create and open a confirmation dialog
        conf_dialog = ResetSettingsConfDialog(parent_widget=self, logger=self.logger)
        conf_dialog.exec()
        self.logger.info("Reset setting dialg closed")
    ...
    def reset_config(self):
        ...
        self.logger.info("Config settings read from defualt config")
    ...
    def update_config_json(self):
        ...
        self.logger.info("Holding dictionary dumped to config.json")
        ...
        for key in self.config_dict.keys():
            if key in self.holding_dict:
                self.config_dict[key] = self.holding_dict[key]
    ...
    def refresh_base_aps_settings(self):
        ...
        self.alpha_opt_in_box.setChecked(self.config_dict["Use_dev"])
        self.alpha_opt_in_box.blockSignals(False)
        
        # Update the maintain config checkbox
        self.maintain_config_box.blockSignals(True)
        self.maintain_config_box.setChecked(self.config_dict["Keep_config"])
        self.maintain_config_box.blockSignals(False)

        # Update the crash repo location labels
        self.crash_repo_label.setText("Crash repo location: {}".format(self.config_dict["Crash_log_repo"]))
        self.feedback_repo_label.setText("Feedback repo location: {}".format(self.config_dict["Feedback_repo"]))
        self.server_auth_repo_label.setText("Server auth location: {}".format(self.config_dict["Server_auth_location"]))
