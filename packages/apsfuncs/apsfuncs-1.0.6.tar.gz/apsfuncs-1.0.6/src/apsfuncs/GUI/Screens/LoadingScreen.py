...
class LoadingScreen(QWidget):
    ...
    # Init class
    def __init__(self, main_window, config_dict, data_logger):
        super().__init__()
        ...
        self.logger.info("Checcking for held crash logs")
        self.check_for_held_files(file_type="crash logs", folder_path=get_crash_log_path(), external_repo=self.config_dict["Crash_log_repo"])
        self.logger.info("Checcking for held feedback files")
        self.check_for_held_files(file_type="feedback files", folder_path=get_held_feedback_path(), external_repo=self.config_dict["Feedback_repo"])

        # Check for software updates
        self.logger.info("Checcking for available updates")
    ...
    def check_for_held_files(self, file_type, folder_path, external_repo):
        ...
        if os.path.exists(folder_path):
            ...
            if len(file_list) > 0:
                self.logger.info("Held {} have been found, trying to move them to external repo".format(file_type))
                for file in file_list:
                    try:
                        ...
                        os.remove(src_file)
                        self.logger.info("{} has been moved sucessfully".format(file))

                    except Exception as e:
                        self.logger.exception("Failed to move {} into external repo, error {}".format(file, e))
    ...
    def check_for_updates(self):
        ...
        self.update_worker = UpdateThreadWorker(config_dict=self.config_dict, logger=self.logger)
        ...
        self.logger.info("Dynamic load timer started")
    ...
    def load_complete(self, update_data):
        ...
        self.logger.info("Auto update check complete")
        ...
        if update_available:    
            ...
            update_dialog = UpdateDialog(new_version=latest_version, new_version_url=latest_version_url, datalog=self.logger, updater_name=updater_name, prog_name=prog_name)
            update_dialog.exec()
        else:
            # Otherwise just start the main program
            self.logger.info("Running the latest version")
