...
from seeq.spy import _common, _compatibility, _login, _search, Session
from seeq.spy._errors import *
from seeq.spy.tests import test_common, test_vantage
...
@pytest.mark.system
def test_table_definition_search():
    if not _compatibility.is_vantage_supported():
        pytest.skip('Server version does not support Vantage')

    session = test_common.get_session_copy(Sessions.admin)
    vantage_room = test_vantage.create_vantage_room(session)

    search_df = spy.search({
        'Scoped To': vantage_room.id,
    }, session=session)

    types_returned = set(search_df['Type'].to_list())

    # Without specifying the Table type, it won't be returned
    assert 'Table' not in types_returned
    ...
    table_definition = search_df.iloc[0].to_dict()
    assert table_definition['Name'].startswith('My Vantage Table')

    # Can't push these yet
    with pytest.raises(spy.SPyRuntimeError, match="Items with no valid type specified cannot be pushed"):
        spy.push(metadata=search_df, session=session)
