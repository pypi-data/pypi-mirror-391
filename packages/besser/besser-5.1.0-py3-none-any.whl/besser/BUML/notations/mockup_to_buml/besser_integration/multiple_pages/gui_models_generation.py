...
def gpt4_self_improvement_gui_models(api_key, mockup_image_path, base64_metamodel, base64_mockup, metamodel_text_path, python_code, structural_model_path):
...
@retrying.retry(stop_max_attempt_number=3, wait_fixed=2000)
def gpt4o_call_gui_models(api_key, mockup_image_path, base64_metamodel, base64_mockup, prompt, first_example_code_path, second_example_code_path, metamodel_text_path, structural_model_path):
...
def direct_prompting_gui_models(api_key, metamodel_image_path, mockup_image_path, first_example_code_path, second_example_code_path, metamodel_text_path, structural_model_path):
...
def run_pipeline_gui_models_generation(api_key: str, mockup_image_path: str, output_folder: str):

    structural_dir = os.path.join(output_folder, "buml")
    structural_model_path = os.path.join(structural_dir, "model.py")

    # List all files in the folder
    mockup_image_paths = [os.path.join(mockup_image_path, f) for f in os.listdir(mockup_image_path) if os.path.isfile(os.path.join(mockup_image_path, f))]
    ...
    for index, mockup_image_path in enumerate(mockup_image_paths, start=1):
        # Generate the Python code using the direct prompt method
        python_code = direct_prompting_gui_models(api_key, metamodel_image_path, mockup_image_path, first_example_code_path, second_example_code_path, metamodel_text_path, structural_model_path)
        ...
        improved_code = gpt4_self_improvement_gui_models(api_key, mockup_image_path, metamodel_image_path, mockup_image_path, metamodel_text_path, python_code, structural_model_path)
