...
from edgefirst.validator.datasets.utils.transformations import (letterbox,
                                                                resize, pad)
    ...
    def run_single_instance(
        ...
        with InferVStreams(
            ...
            if self.parameters.common.preprocessing == "letterbox":
                image, label_ratio, shapes = letterbox(
                    image, new_shape=self.get_input_shape()[1:3], auto=False, scaleup=False)
                self.shapes = [shapes, label_ratio]
            elif self.parameters.common.preprocessing == "pad":
                image, label_ratio, shapes = pad(
                    image, self.get_input_shape()[1:3])
                self.shapes = [shapes, label_ratio]
            else:
                # Take only the (height, width).
                image = resize(image, self.get_input_shape()[1:3])
            image = self.apply_normalization(
                image, self.parameters.common.norm, "float32")
