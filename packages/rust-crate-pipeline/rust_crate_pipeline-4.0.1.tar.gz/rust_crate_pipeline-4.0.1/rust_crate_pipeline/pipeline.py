...
class CrateDataPipeline:
    ...
    async def _enrich_single_crate(
        ...
        try:
            ...
            if self.skip_source:
                if self.skip_ai:
                    # Create enriched crate without AI enrichment or source analysis
                    ...
                    enriched = await self.enricher.enrich_crate(crate)
            ...
            # Now enrich with AI (skip if skip_ai is True)
            if self.skip_ai:
                # Create enriched crate without AI enrichment
                enriched_dict = crate.to_dict()
