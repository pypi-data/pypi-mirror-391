...
def get_gene_sequence(gene_name):
    try:
        ...
        raise Exception("could not find gene sequence file %s, please see examples and generate one for your gene as needed, with this filename" % gene_file) # type: ignore
    return seq



# # NOTE: overload for tool integration purposes
# def get_or_compute(file: str, force_compute=False):
#     try:
#         if os.path.exists(file) and not force_compute:
#             with open(file, mode='rb') as f:
#                 return pickle.load(f)
#         else:
#             raise FileNotFoundError(f"Cannot find model file: {file}")
#     except (FileNotFoundError, IOError) as e:
#         raise Exception(f"An error occurred while loading pickle model file: {file}") from e
...
def execute_parallel(farg_pairs, num_procs=None, verbose=False):
    ...
    results = list(zip(*sorted(results, key=lambda x: x[0])))[1]
    return results
