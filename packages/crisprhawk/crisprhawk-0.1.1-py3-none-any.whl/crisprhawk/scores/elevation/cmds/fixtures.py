...
from .. load_data import load_hauessler_minus_guideseq
...
class Fixtures(Command):
    ...
    def get_hmg(self):
        hmg = load_hauessler_minus_guideseq(options.learn_options)
    ...
    def generate_prediction_fixture(self):
        ...
        df.to_excel(settings.pred_default_fixture_file)

    def generate_aggregation_fixtures(self):
        with open(settings.agg_model_file, mode="rb") as fh:
