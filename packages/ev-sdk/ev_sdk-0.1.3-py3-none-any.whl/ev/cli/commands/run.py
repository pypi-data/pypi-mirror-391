...
class DataSourceType(Enum):
    """Enum for different data source types that can be injected as function arguments."""
...
def _is_daft_dataframe_type(annotation: Any) -> bool:
    """Check if an annotation is daft.DataFrame by comparing fully qualified type name.
    ...
    if isinstance(annotation, str):
        return annotation in ("daft.dataframe.dataframe.DataFrame", "daft.DataFrame", "DataFrame")

    # Handle type objects
    if not hasattr(annotation, "__module__") or not hasattr(annotation, "__qualname__"):
        return False

    return bool(annotation.__module__ == "daft.dataframe.dataframe" and annotation.__qualname__ == "DataFrame")


def _get_data_sources_from_module(module_name: str | Path, function_name: str) -> dict[str, DataSourceType]:
    """Extract data source argument types by dynamically importing the module and inspecting the function."""
    ...
    try:
        ...
        for param_name, annotation in type_hints.items():
            if _is_daft_dataframe_type(annotation):
                data_sources[param_name] = DataSourceType.DAFT_DATAFRAME
        ...
        console.print(f"Error getting data sources from module '{module_name}' and function '{function_name}': {e}")
        if cleanup_module_name:
            sys.modules.pop(cleanup_module_name, None)
        return {}
...
def _resolve_data_source_args(
    ctx: Context,
    data_sources: dict[str, DataSourceType],
    kwargs: dict[str, Any],
) -> dict[str, str]:
    """Resolve data source arguments by fetching them from the API.

    Args:
        ctx: Context with client and workspace/project info
        data_sources: Dict mapping parameter names to their DataSourceType
        kwargs: Dict of keyword arguments from the command line

    Returns:
        Dict mapping parameter names to data source configurations
    """
    data_source_configurations: dict[str, str] = {}

    for param_name in data_sources:
        if param_name in kwargs:
            ds_name = kwargs[param_name]
            console.print(f"[dim]Resolving data source '{ds_name}' for parameter '{param_name}'...[/dim]")

            try:
                data_source = ctx.client.get_data_source(ctx.workspace_id, ctx.project_id, ds_name)

                if data_source is None:
                    raise click.ClickException(
                        f"Data source '{ds_name}' not found in project. " f"Please create it first or check the name."
                    )

                data_source_configurations[param_name] = data_source.configuration.model_dump_json()
                console.print(f"[dim green]✓ Found data source '{ds_name}' (id: {data_source.id})[/dim green]")

            except Exception as e:
                raise click.ClickException(
                    f"Failed to resolve data source '{ds_name}' for parameter '{param_name}': {e}"
                ) from e
...
def _parse_entrypoint(ctx: Context, entrypoint: str, argv: list[str]) -> RunEntrypoint:
    ...
    if ":" in entrypoint:
        ...
        if module_or_file.endswith(".py"):
            ...
            data_sources = _get_data_sources_from_module(module_or_file, symbol)
        ...
        # Resolve data source arguments if any.
        if data_sources:
            data_source_configurations = _resolve_data_source_args(ctx, data_sources, kwargs)
            kwargs.update(data_source_configurations)
