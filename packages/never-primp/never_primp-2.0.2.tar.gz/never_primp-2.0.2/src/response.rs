...
impl Response {
    ...
    fn json<'rs>(&mut self, py: Python<'rs>) -> Result<Bound<'rs, PyAny>> {
        ...
        let json_value: serde_json::Value = from_slice(raw_bytes)
            .map_err(|e| {
                // 尝试截取部分内容作为预览
                let preview = if raw_bytes.len() > 100 {
                    format!("{}...", String::from_utf8_lossy(&raw_bytes[..100]))
                } else {
    ...
    fn get_headers<'rs>(&mut self, py: Python<'rs>) -> Result<Bound<'rs, PyDict>> {
        ...
        for (key, value) in self.resp.headers() {
            let value_str = value.to_str()
                .map_err(|e| ClientError::HeaderError {
                    message: format!("Header值包含无效字符: {}", e),
                    header_name: Some(key.as_str().to_string()),
                })?;
            new_cookies.set_item(key.as_str(), value_str)?;
        }
