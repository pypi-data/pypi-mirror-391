...
def _segment_body_http(image_url: str, ak: str, sk: str) -> str:
    # 使用官方 SDK Advance 调用人体分割（以文件流传入，避免URL域名限制）
    ...
    try:
        ...
        req = imageseg_models.SegmentBodyAdvanceRequest()
        # 远程URL：使用BytesIO作为流，避免必须同地域OSS
        if s.lower().startswith("http://") or s.lower().startswith("https://"):
            try:
                ...
                for field in [
                    ...
                    try:
                        ...
                        pass
                res = seg_client.segment_body_advance(req, runtime)
            except Exception:
