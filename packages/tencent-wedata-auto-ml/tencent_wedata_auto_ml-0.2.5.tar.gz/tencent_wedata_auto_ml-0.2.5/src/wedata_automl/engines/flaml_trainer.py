    ...
    def train(
        ...
        if self.experiment_id:
            ...
            if experiment is None:
                # 创建实验时不传递 tags 参数，避免与装饰器冲突
                try:
                    experiment_id = mlflow.create_experiment(experiment_name)
                except TypeError as e:
                    # 如果有装饰器自动添加 tags，使用 set_experiment 代替
                    if "multiple values for argument 'tags'" in str(e):
                        safe_print(f"⚠️  Detected MLflow decorator conflict, using set_experiment() instead")
                        mlflow.set_experiment(experiment_name)
                        experiment = mlflow.get_experiment_by_name(experiment_name)
                        experiment_id = experiment.experiment_id
                        safe_print(f"✅ Created/using experiment: '{experiment_name}' (ID: {experiment_id})")
                    else:
                        raise
                else:
