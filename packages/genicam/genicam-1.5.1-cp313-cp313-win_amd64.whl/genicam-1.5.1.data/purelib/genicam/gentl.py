...
class STREAM_INFO_CMD_LIST(IntEnum):
    ...
    STREAM_INFO_FLOW_TABLE                 = 14  #: Current state of flow mapping table in GenDC format (if available). Return the GC_ERR_NO_DATA error code if the stream will not use flow mechanism in current configuration.
    STREAM_INFO_GENDC_PREFETCH_DESCRIPTOR  = 15  #: Prefetch version of the GenDC descriptor corresponding to the current stream status, if available and if the stream/device is currently configured to stream in GenDC format. Return the GC_ERR_NO_DATA error code if the stream will not output GenDC payload in current configuration.
...
class BUFFER_INFO_CMD_LIST(IntEnum):
    ...
    BUFFER_INFO_IS_COMPOSITE                    = 31  #: Indicates whether this is a composite buffer, announced using DSAnnounceCompositeBuffer
...
class FLOW_INFO_CMD_LIST(IntEnum):
    """
    Defines commands to retrieve the Flow information.
    """
    FLOW_INFO_SIZE         = 0  #: Size of the flow in bytes. This information is essential for GenTL Consumer to be able to allocate suitable buffers for flow acquisition using DSAnnounceCompositeBuffer. In case of GenDC streaming this directly corresponds to the information from GenDC mapping table. Note: querying the flow structure through FLOW_INFO_SIZE and through STREAM_INFO_FLOW_TABLE must yield same results.

    FLOW_INFO_CUSTOM_ID    = 1000  #: Starting value for GenTL Producer custom IDs which are implementation specific.


class SEGMENT_INFO_CMD_LIST(IntEnum):
    """
    Defines commands to retrieve the Flow information.
    """
    SEGMENT_INFO_BASE                     = 0  #: Base address of the buffer segment memory as passed to the DSAnnounceCompositeBuffer function. This is also the address where the payload within the segment starts.
    SEGMENT_INFO_SIZE                     = 1  #: Size of the buffer segment in bytes as passed to the DSAnnounceCompositeBuffer function.
    SEGMENT_INFO_IS_INCOMPLETE            = 2  #: Flag to indicate that the buffer segment was filled but an error occurred during that process. For technologies or use cases where this is difficult to track, it is valid leave the command not imlemented.
    SEGMENT_INFO_SIZE_FILLED              = 3  #: Number of bytes written into the buffer segment the last time it has been filled. This value is reset to 0 when the buffer is placed into the Input Buffer Pool. Used similar way as BUFFER_INFO_SIZE_FILLED at buffer level. For technologies or use cases where this is difficult to track, it is valid leave the command not imlemented.
    SEGMENT_INFO_DATA_SIZE                = 4  #: Size of the data intended to be written to the buffer last time it has been filled. This value is reset to 0 when the buffer is placed into the Input Buffer Pool. Used similar way as BUFFER_INFO_DATA_SIZE at buffer level. For technologies or use cases where this is difficult to track, it is valid leave the command not imlemented.

    SEGMENT_INFO_CUSTOM_ID                = 1000  #: Starting value for GenTL Producer custom IDs which are implementation specific.


class ComponentIDValue(IntEnum):
    """
    Unique Identifier value that corresponds to type of the component.
    This is the value of GenDC Component TypeId
    """
    Undefined     = 0  #: Udefined Component Type.
    Intensity     = 1  #: Intensity image component.
    Infrared      = 2  #: Infrared image component.
    Ultraviolet   = 3  #: Ultraviolet image component.
    Range         = 4  #: Range image component.
    Reflectance   = 5  #: Reflectance image component.
    Confidence    = 6  #: Confidence image component.
    Scatter       = 7  #: Scatter image component.
    Disparity     = 8  #: Disparity image component.
    Multispectral = 9  #: Multispectral image component.

    Metadata      = 32769  #: The component contains only metadata and no image data.

    Reserved      = 65535  #: Reserved Component Type value.


class GDC_PART_HEADER(IntEnum):
    """
    Defines GenDC part header types
    """
    GDC_METADATA_GENICAM_CHUNK = 16384  #: GenICam Chunk Metadata. This is only available when ComponentIDValue us equal to "Metadata".
    GDC_METADATA_GENICAM_XML   = 16385  #: GenICam Metadata XML. This is only available when ComponentIDValue us equal to "Metadata"
    GDC_1D                     = 16640  #: 1D array (such as 3D Point Cloud).
    GDC_2D                     = 16896  #: Rectangular uncompressed image (monochrome or none planar color).
    GDC_2D_JPEG                = 16897  #: JPEG compressed Image.
    GDC_2D_JPEG2000            = 16898  #: JPEG 2000 compressed Image.
    GDC_2D_H264                = 16898  #: H.264 compressed Image.
...
class LibraryManager(object):
    ...
    def ds_announce_composite_buffer(self, h_data_stream, iNumSegments, ppSegments, piSizes, user_data):
        return _gentl.LibraryManager_ds_announce_composite_buffer(self, h_data_stream, iNumSegments, ppSegments, piSizes, user_data)
    ...
    def ds_get_num_flows(self, h_data_stream):
        return _gentl.LibraryManager_ds_get_num_flows(self, h_data_stream)

    def ds_get_num_buffer_segments(self, h_data_stream, h_buffer):
        return _gentl.LibraryManager_ds_get_num_buffer_segments(self, h_data_stream, h_buffer)
...
class Components(object):
    ...
    def iterator(self):
        return _gentl.Components_iterator(self)
    def __iter__(self):
        return self.iterator()

    def __nonzero__(self):
        return _gentl.Components___nonzero__(self)

    def __bool__(self):
        return _gentl.Components___bool__(self)

    def __len__(self):
        return _gentl.Components___len__(self)

    def __getslice__(self, i, j):
        return _gentl.Components___getslice__(self, i, j)

    def __setslice__(self, *args):
        return _gentl.Components___setslice__(self, *args)

    def __delslice__(self, i, j):
        return _gentl.Components___delslice__(self, i, j)

    def __delitem__(self, *args):
        return _gentl.Components___delitem__(self, *args)

    def __getitem__(self, *args):
        return _gentl.Components___getitem__(self, *args)

    def __setitem__(self, *args):
        return _gentl.Components___setitem__(self, *args)

    def pop(self):
        return _gentl.Components_pop(self)

    def append(self, x):
        return _gentl.Components_append(self, x)

    def empty(self):
        return _gentl.Components_empty(self)

    def size(self):
        return _gentl.Components_size(self)

    def swap(self, v):
        return _gentl.Components_swap(self, v)

    def begin(self):
        return _gentl.Components_begin(self)

    def end(self):
        return _gentl.Components_end(self)

    def rbegin(self):
        return _gentl.Components_rbegin(self)

    def rend(self):
        return _gentl.Components_rend(self)

    def clear(self):
        return _gentl.Components_clear(self)

    def get_allocator(self):
        return _gentl.Components_get_allocator(self)

    def pop_back(self):
        return _gentl.Components_pop_back(self)

    def erase(self, *args):
        return _gentl.Components_erase(self, *args)

    def __init__(self, *args):
        _gentl.Components_swiginit(self, _gentl.new_Components(*args))

    def push_back(self, x):
        return _gentl.Components_push_back(self, x)

    def front(self):
        return _gentl.Components_front(self)

    def back(self):
        return _gentl.Components_back(self)

    def assign(self, n, x):
        return _gentl.Components_assign(self, n, x)

    def resize(self, *args):
        return _gentl.Components_resize(self, *args)

    def insert(self, *args):
        return _gentl.Components_insert(self, *args)

    def reserve(self, n):
        return _gentl.Components_reserve(self, n)

    def capacity(self):
        return _gentl.Components_capacity(self)
    __swig_destroy__ = _gentl.delete_Components

# Register Components in _gentl:
_gentl.Components_swigregister(Components)
class Parts(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
...
class Buffer(object):
    ...
    def _get__num_buffer_components(self):
        return _gentl.Buffer__get__num_buffer_components(self)

    def _get_components(self):
        return _gentl.Buffer__get_components(self)
    ...
    _num_buffer_components = property(_get__num_buffer_components)
...
class Component(object):
    ...
    def __init__(self, buffer, gdcContainer, gdcComponent):
        _gentl.Component_swiginit(self, _gentl.new_Component(buffer, gdcContainer, gdcComponent))
    __swig_destroy__ = _gentl.delete_Component

    def _get_group_id(self):
        return _gentl.Component__get_group_id(self)

    def _get_source_id(self):
        return _gentl.Component__get_source_id(self)

    def _get_region_id(self):
        return _gentl.Component__get_region_id(self)

    def _get_x_offset(self):
        return _gentl.Component__get_x_offset(self)

    def _get_y_offset(self):
        return _gentl.Component__get_y_offset(self)

    def _get_timestamp_ns(self):
        return _gentl.Component__get_timestamp_ns(self)

    def _get_component_id_value(self):
        return _gentl.Component__get_component_id_value(self)

    def _get_data_format(self):
        return _gentl.Component__get_data_format(self)

    def _get_data_format_namespace(self):
        return _gentl.Component__get_data_format_namespace(self)

    def _get__num_parts(self):
        return _gentl.Component__get__num_parts(self)

    def _get_parts(self):
        return _gentl.Component__get_parts(self)

    group_id = property(_get_group_id)


    source_id = property(_get_source_id)


    region_id = property(_get_region_id)


    component_id_value = property(_get_component_id_value)
    ...
    _num_parts = property(_get__num_parts)


    parts = property(_get_parts)


# Register Component in _gentl:
_gentl.Component_swigregister(Component)
class Part(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    ...
    def _get_part_type(self):
        return _gentl.Part__get_part_type(self)
    ...
    part_type = property(_get_part_type)
...
class MultiPart(Part):
    ...
    def __init__(self, buffer, part_index):
        _gentl.MultiPart_swiginit(self, _gentl.new_MultiPart(buffer, part_index))
    __swig_destroy__ = _gentl.delete_MultiPart
    ...
    def _get_data_offset(self):
        ...
        """
        return _gentl.MultiPart__get_data_offset(self)
    ...
    def _get_data_size(self):
        ...
        """
        return _gentl.MultiPart__get_data_size(self)
    ...
    def _get_data_type(self):
        ...
        """
        return _gentl.MultiPart__get_data_type(self)
    ...
    def _get_data_format(self):
        ...
        """
        return _gentl.MultiPart__get_data_format(self)
    ...
    def _get_data_format_namespace(self):
        ...
        """
        return _gentl.MultiPart__get_data_format_namespace(self)

    def _get_part_type(self):
        return _gentl.MultiPart__get_part_type(self)
    ...
    def _get_width(self):
        ...
        """
        return _gentl.MultiPart__get_width(self)
    ...
    def _get_height(self):
        ...
        """
        return _gentl.MultiPart__get_height(self)
    ...
    def _get_x_offset(self):
        ...
        """
        return _gentl.MultiPart__get_x_offset(self)
    ...
    def _get_y_offset(self):
        ...
        """
        return _gentl.MultiPart__get_y_offset(self)
    ...
    def _get_x_padding(self):
        ...
        """
        return _gentl.MultiPart__get_x_padding(self)
    ...
    def _get_source_id(self):
        ...
        """
        return _gentl.MultiPart__get_source_id(self)
    ...
    def _get_delivered_image_height(self):
        ...
        """
        return _gentl.MultiPart__get_delivered_image_height(self)

# Register MultiPart in _gentl:
_gentl.MultiPart_swigregister(MultiPart)
class GenDCPart(Part):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, buffer, gdcContainer, gdcComponent, gdcPart):
        _gentl.GenDCPart_swiginit(self, _gentl.new_GenDCPart(buffer, gdcContainer, gdcComponent, gdcPart))
    __swig_destroy__ = _gentl.delete_GenDCPart
    ...
    def _get_data_offset(self):
        ...
        """
        return _gentl.GenDCPart__get_data_offset(self)
    ...
    def _get_data_size(self):
        ...
        """
        return _gentl.GenDCPart__get_data_size(self)
    ...
    def _get_data_type(self):
        ...
        """
        return _gentl.GenDCPart__get_data_type(self)
    ...
    def _get_data_format(self):
        ...
        """
        return _gentl.GenDCPart__get_data_format(self)
    ...
    def _get_data_format_namespace(self):
        ...
        """
        return _gentl.GenDCPart__get_data_format_namespace(self)

    def _get_part_type(self):
        return _gentl.GenDCPart__get_part_type(self)
    ...
    def _get_width(self):
        ...
        """
        return _gentl.GenDCPart__get_width(self)
    ...
    def _get_height(self):
        ...
        """
        return _gentl.GenDCPart__get_height(self)
    ...
    def _get_x_offset(self):
        ...
        """
        return _gentl.GenDCPart__get_x_offset(self)
    ...
    def _get_y_offset(self):
        ...
        """
        return _gentl.GenDCPart__get_y_offset(self)
    ...
    def _get_x_padding(self):
        ...
        """
        return _gentl.GenDCPart__get_x_padding(self)
    ...
    def _get_source_id(self):
        ...
        """
        return _gentl.GenDCPart__get_source_id(self)
    ...
    def _get_delivered_image_height(self):
        ...
        """
        return _gentl.GenDCPart__get_delivered_image_height(self)

# Register GenDCPart in _gentl:
_gentl.GenDCPart_swigregister(GenDCPart)
