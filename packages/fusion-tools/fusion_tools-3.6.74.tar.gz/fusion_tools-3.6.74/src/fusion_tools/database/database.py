...
class fusionDB:
    ...
    def get_layers(self, item_id, user_token: Union[str,None] = None) -> list:
        ...
        #print(f'{item_id=}')
        #print(f'{user_token=}')
        #print(f'{self.get_ids(table_name = "item",user_token=user_token)=}')
        ...
        with self.get_db() as session:
            ...
            for l in layer_query.all():
                ...
                feature_collection = {
                    ...
                    'features': [],
                    'properties': {
                        'name': l.to_dict().get('name'),
                        '_id': l.to_dict().get('id')
                    }
                }
                ...
                for s in structure_query.all():
                    feature_collection['features'].append({
                        'type': 'Feature',
                        'geometry': s.to_dict().get('geom'),
                        'properties': s.to_dict().get('properties')
                    })
