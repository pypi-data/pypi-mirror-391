...
Main entry point for  XeSQL/UbiSQL/MySQL DataSource MCP Client server.
"""
import os
import sys
from typing import List
from fastmcp import FastMCP
from xesql_mcp_server import get_base_package_info

project_path=os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
# Add current directory to Python module search path
sys.path.insert(0,project_path)
from xesql_mcp_server.utils.logger_util import logger, db_config_path
from xesql_mcp_server.utils.db_operate import execute_sql
from xesql_mcp_server.utils import load_activate_db_config
from xesql_mcp_server.tools.db_tool import generate_test_data
from xesql_mcp_server.resources.db_resources import generate_database_config, generate_database_tables
...
async def sql_exec(sql: str):
    """
    Universal  XeSQL/UbiSQL/MySQL/Tidb SQL execution tool
    ...
    - sql (str): XeSQL/UbiSQL/MySQL/Tidb SQL statement to execute, supports parameterized queries
...
async def describe_table(table_name: str):
    """
    XeSQL/UbiSQL/MySQL/Tidb Table structure description tool
...
async def generate_demo_data(table_name: str, columns_name: List[str], num: int):
    """
    XeSQL/UbiSQL/MySQL/Tidb Test data generation tool
...
async def get_database_tables():
    """
    XeSQL/UbiSQL/MySQL/Tidb Database table information resource
...
def main():
    ...
    logger.info(f"XeSQL/UbiSQL/MySQL MCP Client version: {get_base_package_info()}")
    logger.info(f"Database configuration file path:{db_config_path}")
    logger.info(f"Current project path:{project_path}")
    logger.info("XeSQL/UbiSQL/MySQL DataSource MCP server is ready to accept connections")
