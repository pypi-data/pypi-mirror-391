...
def create_user_and_group() -> Tuple[str, str]:
    ...
    try:
        ...
        logger.warning(f"Error copying dotfiles: {e}")
    try:
        create_git_config(user)
    except Exception as e:
        logger.warning(f"Error creating git config: {e}")
    try:
        copy_welcome_readme(user)
    except Exception as e:
        logger.warning(f"Error copying welcome readme: {e}")
    os.system(f"chown -R {user}:{group} /home/{user}/")
    return user, group
    ...
    for root, dirs, files in os.walk(skel_dir):
        ...
        for d in dirs:
            ...
            if not os.path.exists(target_dir):
                ...
                os.chown(target_dir, uid, gid)  # root:root
...
def create_git_config(user: str):
    ...
    logger.info(f"Git global config created for user {user}.")
...
def link_script_files():
    ...
    for script_file in script_files:
        ...
        if not os.path.exists(src_path):
            logger.warning(f"Script {script_file} not found in {scripts_location}.")
            continue
        dest_path = os.path.join(user_bin_dir, script_file)
        if not os.path.exists(dest_path):
            os.symlink(src_path, dest_path)
            logger.info(f"Linked script {script_file} to {dest_path}")
...
def copy_welcome_readme(user: str):
    ...
    if not readme_src.exists():
        logger.warning(
            f"Demo README.md not found in datatailr_demo package at: {readme_src}"
        )
        return
