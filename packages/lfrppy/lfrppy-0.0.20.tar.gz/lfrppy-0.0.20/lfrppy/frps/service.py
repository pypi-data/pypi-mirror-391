...
def frps_service_action(action: str, name: str) -> int:
    ...
    if not state:
        print("No frps deployment found; service commands are unavailable.")
        return 1
    ...
    if action in {"start", "enable", "restart"}:
        ...
        if not config_path.exists():
            print(f"Configuration file {config_path} not found. Create it before running `frps {action} {name}`.")
            return 1
