...
def frpc_vim(name: str) -> int:
    state = _load_state_optional()
    install_dir = _resolve_install_dir(state)
    if install_dir is None:
        print("No frpc deployment found; nothing to edit.")
        return 1
...
def frpc_cat(name: str) -> int:
    state = _load_state_optional()
    install_dir = _resolve_install_dir(state)
    if install_dir is None:
        print("No frpc deployment found; nothing to display.")
        return 1
    ...
    return cat_config(config_path, use_sudo_on_posix=True)
