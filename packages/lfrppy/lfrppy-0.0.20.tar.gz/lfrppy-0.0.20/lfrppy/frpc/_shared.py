...
STATE_FILE = Path.home() / ".frppy" / "frpc_state.json"
SERVICE_ACTIONS = ("start", "stop", "restart", "status", "enable", "disable")
FILE_ACTIONS = {"vim", "cat"}
ELEVATION_ENV = "LFRPPY_FRPC_ELEVATED"
...
def _discover_frpc_units(systemd_dir: Path) -> Set[str]:
    units: Set[str] = set()
    if systemd_dir.exists():
        for path in systemd_dir.glob("frpc@*.service"):
            if path.name == "frpc@.service":
                continue
            units.add(path.stem)
    units.update(_systemctl_collect_units("list-units frpc@*.service --type=service --all"))
    units.update(_systemctl_collect_units("list-unit-files frpc@*.service"))
    return units
...
def _load_state() -> dict:
    if not STATE_FILE.exists():
        raise FileNotFoundError("未找到 frpc 的部署记录，请先执行 frpc deploy。")
