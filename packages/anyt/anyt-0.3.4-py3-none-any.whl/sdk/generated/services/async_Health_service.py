...
async def health_check_health_get(api_config_override : Optional[APIConfig] = None) -> app__schemas__responses__HealthResponse:
...
async def healthCheck(api_config_override : Optional[APIConfig] = None) -> app__api__v1__routes__health__HealthResponse:
    api_config = api_config_override if api_config_override else APIConfig()
    ...
    if response.status_code != 200:
        raise HTTPException(response.status_code, f'healthCheck failed with status code: {response.status_code}')
    else:
                body = None if 200 == 204 else response.json()
