...
def generate_pheno_plink_fast(
    ...
    """
    Write per-(measnum, sex) PLINK inputs (PED/MAP/PHENO) with **strain-level** samples.

    Behavior:
      - For each (measnum, sex), compute per-strain mean of raw 'value'.
      - Emit exactly **one** PED row per strain with:
            FID = STRAIN
            IID = STRAIN
            SEX = 2 for 'f', 1 for 'm'
            PHE = per-strain mean raw 'value' (or -9 if missing)
      - Emit PHENO with **one** row per strain: "FID IID value".
    ...
    for col in ("strain", "sex", "measnum", "value"):
        if col not in pheno.columns:
            raise ValueError("pheno must have columns: 'strain', 'sex', 'measnum', 'value'.")
    ...
    # Index reference PED for first occurrence of each strain
    ...
    for (measnum, sex), df in groups.items():
        # Per-strain mean raw phenotype
        strain_mean = (
            df.groupby("strain", sort=False)["value"]
              .apply(lambda s: pd.to_numeric(s, errors="coerce").mean())
              .astype(float)
        )
        strains_in_order = list(strain_mean.index)

        # MAP
        map_out = os.path.join(outdir, f"{measnum}.{sex}.map")
        map_df.to_csv(map_out, sep="\t", index=False, header=False)

        # PED + PHENO (one row per strain, FID=IID=STRAIN)
        ...
        with open(ped_out, "w", encoding="utf-8") as f_ped, open(phe_out, "w", encoding="utf-8") as f_ph:
            for strain in strains_in_order:
                val = strain_mean.loc[strain]

                # Load template line for this strain from reference PED
                with open(ped_file, "rb") as f:
                ...
                # Build PED row: FID=IID=strain; SEX from group; PHE = mean(value)
                meta = parts[:6]
                meta[0] = strain
                meta[1] = strain
                ...
                # PHENO row: FID IID value (strain-level)
                if val is None or (isinstance(val, float) and not math.isfinite(val)):
                    f_ph.write(f"{strain} {strain} -9\n")
                else:
                    f_ph.write(f"{strain} {strain} {val}\n")
...
def fast_prepare_pylmm_inputs(
    ...
    plink2_path: str,
    *,
    ped_pheno_field: str = "value",
) -> None:
    """
    Orchestrate extraction and PLINK file generation for PyLMM inputs, **strain-level**.

    Steps:
      1) Extract phenotype rows for requested measure_ids.
      2) For each (measnum, sex), generate:
           - {measnum}.{sex}.map  (sanitized)
           - {measnum}.{sex}.ped  (one row per strain, FID=IID=STRAIN)
           - {measnum}.{sex}.pheno (FID IID mean_raw_value)
      3) Convert PED/MAP -> BED/BIM/FAM (no --keep).
      4) Compute kinship (--make-rel square) on the same set.
    """
    os.makedirs(outdir, exist_ok=True)
    ...
    for measure_id in measure_ids:
        ...
        for sex in ("f", "m"):
            ...
            generate_bed_bim_fam(
                ...
                maf_threshold=0.05,
                sample_keep_path=None,   # IMPORTANT: no --keep; we already emitted exactly one row per strain
                autosomes_only=True,
            )

            logging.info(f"[fast_prepare_pylmm_inputs] compute kinship (REL) for {base_id}.{sex}")
            calculate_kinship_matrix(
                plink2_path=plink2_path,
                input_prefix=out_prefix,
                output_prefix=os.path.join(outdir, f"{base_id}.{sex}.kin"),
                sample_keep_path=None,   # same set as BED
            )


def generate_pheno_plink_fast_V0(
    ped_file: str,
    map_file: str,
    pheno: pd.DataFrame,
    outdir: str,
...
def fast_prepare_pylmm_inputs_V0(
    ped_file: str,
    map_file: str,
    measure_id_directory: str,
    measure_ids: List,
