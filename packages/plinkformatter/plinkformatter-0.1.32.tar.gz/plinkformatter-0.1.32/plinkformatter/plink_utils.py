...
def generate_bed_bim_fam(
    ...
    maf_threshold: Optional[float] = 0.05,
    ...
    Behavior:
      * Direct PED/MAP -> BED (no intermediate PGEN).
      * Applies --geno 0.1 and (unless relaxed) --mind 0.1.
    ...
    logging.info(
        "[plink_utils:generate_bed_bim_fam] PED/MAP -> BED with %s %s %s %s",
        mind or "no --mind", maf or "no --maf", keep or "no --keep", chrflag or "all chr"
    )

    # Direct conversion, matching Hao’s workflow
    run_plink2(
        f"{plink2_path} --ped {ped_file} --map {map_file} "
