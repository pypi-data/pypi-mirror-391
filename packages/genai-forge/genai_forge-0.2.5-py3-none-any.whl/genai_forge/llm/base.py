...
class BaseLLM(ABC):
    ...
    def __or__(self, other: Any) -> Callable:
        """
        Pipe operator to compose LLM with a parser.
        llm | parser -> returns a callable that processes LLM output through the parser
        """
        if hasattr(other, 'parse'):
            # It's a parser - return a callable that calls LLM and parses
            def llm_parser_chain(prompt: Any) -> Any:
                llm_output = self(prompt)
                return other.parse(str(llm_output))
            return llm_parser_chain
        raise TypeError(f"Cannot pipe LLM to {type(other)}. Expected a parser with 'parse' method.")
