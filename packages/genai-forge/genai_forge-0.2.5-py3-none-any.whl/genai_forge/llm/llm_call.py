...
from prompting_forge.versioning import final_prompt_path
import json
...
class LLMCall:
	...
	def run(self, context: Mapping[str, Any]) -> Tuple[str, Any]:
		...
		if self._enable_versioning:
			# Extract query separately from context variables
			query_value = ctx.get("query")
			context_vars = {k: v for k, v in ctx.items() if k != "query"}
			self._save_llm_call(self._template, chat.system, rendered, context_vars, query_value, response)
	...
	def _save_llm_call(
		...
		Query is saved separately from variables.
		"""
		...
		# Determine prompt reference - the template path
		prompt_ref_path = None
		prompt_ref_type = None
		
		# Check if this is using a final prompt
		if hasattr(prompt_template, "_saved_final_path"):
			# Using a final prompt (FinalPromptTemplate)
			final_path = getattr(prompt_template, "_saved_final_path", None)
			if final_path:
				prompt_ref_path = str(final_path)
				prompt_ref_type = "final"
		# Check if this is using a versioned prompt
		elif hasattr(prompt_template, "_saved_version_path"):
			# Using a versioned prompt (PromptTemplate)
			version_path = getattr(prompt_template, "_saved_version_path", None)
			if version_path:
				prompt_ref_path = str(version_path)
				prompt_ref_type = "version"
		else:
			# Fallback: try to find a final prompt using the LLMCall instance name
			try:
				final_path = final_prompt_path(self._name, root=self._version_root)
