...
class LLMCall:
	...
	def run(self, prompt_template: BasePromptTemplate, context: Mapping[str, Any], *, prompt_version_path: Path | None = None) -> Tuple[str, str]:
		"""
		...
		if self._enable_versioning:
			self._save_llm_call(prompt_template, chat.system, rendered, ctx, response, prompt_version_path)
	...
	def _save_llm_call(
		...
		"""
		root = self._version_root if self._version_root is not None else Path.cwd()
		target_dir = root / ".llm_call" / self._name
		...
		if prompt_version_path is not None:
			...
			try:
				...
				version_paths = [
					str((root / ".prompt" / self._name / f"{self._name}__prompt__v{v}.json"))
					for v in vers
				]
			except Exception:
				version_paths = []
