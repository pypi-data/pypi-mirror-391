...
def pacsq_openmm_rerun_dis(cyc_plus, rep, foldername="MDrun", location=os.getcwd(), crd="qmmm.crd",
                top="qmmm.top", selection1="resid 1", selection2="resid 2", qmmm_int=None, choose=1, **cfg):
    if qmmm_int is None:
    ...
    for i in tqdm(range(died_cyc, cyc_plus + 2)):
        ...
        if i == 0:
            ...
            for j in range(1, rep + 1):
                ...
                os.system(f"mkdir ./{foldername}/{i}/{j}")
                os.chdir(f"{location}/{foldername}/{i}/{j}")
                openmm_run_cyc(crd, top, i, j, location, foldername, ref, **cfg)
                os.chdir(location)
...
def pacsq_openmm_rerun_rmsd(cyc_plus, rep, foldername="MDrun", location=os.getcwd(), crd="qmmm.crd",
                top="qmmm.top", ref_location="F.pdb", selection="resid 2", qmmm_int=None, **cfg):
    if qmmm_int is None:
        write_to_dat("md.in", md_tem_s)
    else:
        qm_int = read_dat(qmmm_int)
