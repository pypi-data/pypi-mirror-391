...
class BitstreamParser:
    ...
    def _parse_vui_parameters(reader: BitReader, details: Dict[str, Any]):
        ...
        try:
            ...
            if reader.read_bit():  # chroma_loc_info_present_flag
                ...
                # Store chroma location (use top field, which is standard for progressive content)
                details["chroma_location"] = chroma_sample_loc_type_top_field
                logger.debug(f"VUI: Chroma location: {chroma_sample_loc_type_top_field}")
    ...
    def _parse_sps_payload(nal_payload: bytes, details: Dict[str, Any]):
        """
        Parses a SPS NAL unit payload to extract VUI parameters, profile, and pixel format info.
        """
        ...
        try:
            ...
            details["sps_max_sub_layers_minus1"] = sps_max_sub_layers_minus1
            ...
            details["hevc_profile_idc"] = general_profile_idc
            logger.debug(f"SPS: HEVC profile_idc: {general_profile_idc}")
            ...
            # Skip sub-layer profile/level info if present
            ...
            details["chroma_format_idc"] = chroma_format_idc
            logger.debug(f"SPS: chroma_format_idc: {chroma_format_idc}")
            ...
            details["bit_depth_luma"] = 8 + bit_depth_luma_minus8
            # bit_depth_chroma_minus8
            bit_depth_chroma_minus8 = reader.read_ue()
            details["bit_depth_chroma"] = 8 + bit_depth_chroma_minus8
            logger.debug(f"SPS: bit_depth_luma: {details['bit_depth_luma']}, bit_depth_chroma: {details['bit_depth_chroma']}")
