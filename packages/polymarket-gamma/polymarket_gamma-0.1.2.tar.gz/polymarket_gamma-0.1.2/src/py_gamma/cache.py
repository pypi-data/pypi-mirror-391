...
# Type aliases for diskcache library to handle unknown types
DiskCacheValue = Any
DiskCacheReturn = bool | int
...
class CacheKey:
    ...
    @staticmethod
    def generate(method: str, url: str, params: dict[str, Any] | None = None) -> str:
...
class CacheManager:
    ...
    def __init__(self, config: GammaConfig) -> None:
        ...
        self._memory_cache: LRUCache[str, CacheValue] | None = None
    ...
    async def get(self, key: str) -> Any:
        ...
        try:
            if self._disk_cache:
                disk_value: DiskCacheValue = self._disk_cache.get(key)
                if disk_value is not None:
                    # Type assertion: disk cache should return our structured cache value
                    if not isinstance(disk_value, dict):
                        logger.warning(
                            f"Unexpected cache value type: {type(disk_value)}"
                        )
                        return None

                    # Store in memory cache for faster future access
                    self._memory_cache[key] = disk_value
                    logger.debug(f"Cache hit (disk): {key}")
                    return disk_value.get("data")
        except Exception as e:
