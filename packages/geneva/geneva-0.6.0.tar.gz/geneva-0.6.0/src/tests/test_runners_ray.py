...
@pytest.mark.multibackfill
def test_backfill_batch_size_overrides_udf_batch_size(tmp_path: Path) -> None:
    """Verify backfill's batch_size overrides the UDF's batch_size across scenarios.

    Scenarios:
    1) UDF default (100), backfill=50 => max observed 50, never exceed 50
    2) UDF default (100), backfill=150 => max observed 150, includes 150
    3) UDF=200, backfill=150 => max observed 150, includes 150
    4) UDF=200, backfill=250 => max observed 250, includes 250
    """
    ...
    def run_case(
        *,
        ...
        tbl.backfill("observed", batch_size=backfill_batch_size)
        ...
        assert max(vals) == backfill_batch_size
        assert all(0 <= v <= backfill_batch_size for v in vals)
