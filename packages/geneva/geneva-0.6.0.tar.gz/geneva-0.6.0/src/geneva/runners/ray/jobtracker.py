...
from geneva.utils import escape_sql_string
...
class JobTracker:
    ...
    _db: AsyncConnection | None = attrs.field(init=False, default=None)
    ...
    async def _save_metrics(self, _metrics: dict[str, dict]) -> None:
        ...
        if not self._db:
            ...
            self._jobs_table = await self._db.open_table(GENEVA_JOBS_TABLE_NAME)
