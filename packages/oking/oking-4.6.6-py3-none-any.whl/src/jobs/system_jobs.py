...
def send_execution_notification(job_config: dict) -> None:
    with lock:
        ...
        try:
            ...
            try:
                # Formato ISO: "2025-11-11 11:29:49.622673"
                if '.' in execution_start:
                    data_objeto = datetime.strptime(execution_start.split('.')[0], '%Y-%m-%d %H:%M:%S')
                # Formato ISO sem microsegundos: "2025-11-11 11:29:49"
                elif '-' in execution_start and 'T' not in execution_start:
                    data_objeto = datetime.strptime(execution_start, '%Y-%m-%d %H:%M:%S')
                # Formato antigo: "11//05//2025 10:30:00"
                else:
                    data_formatada = execution_start.replace('T', ' ')
