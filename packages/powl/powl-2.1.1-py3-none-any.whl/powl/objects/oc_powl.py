from powl.objects.obj import POWL, Transition, SilentTransition
...
class LeafNode(ObjectCentricPOWL):
    ...
    def __init__(self, transition: Transition, related, divergent, convergent, deficient):
        super().__init__()
...
class ComplexModel(ObjectCentricPOWL):
    ...
    def get_type_information(self):
        return {key:value for oc_child in self.oc_children for key,value in oc_child.get_type_information().items()}
...
def load_oc_powl(flat_model: POWL, related, divergence, convergence, deficiency) -> ObjectCentricPOWL:
    if isinstance(flat_model, Transition):
        if isinstance(flat_model, SilentTransition):
            ...
            return LeafNode(SilentTransition(), all_types, all_types, all_types, all_types)
        else:
            activity = flat_model.label
            return LeafNode(flat_model, related[activity], divergence[activity], convergence[activity],
                            deficiency[activity])
    else:
        oc_children = [load_oc_powl(sub, related, divergence, convergence, deficiency) for sub in
                             flat_model.children]
        mapping = {flat_model.children[i]: oc_children[i] for i in range(len(flat_model.children))}
