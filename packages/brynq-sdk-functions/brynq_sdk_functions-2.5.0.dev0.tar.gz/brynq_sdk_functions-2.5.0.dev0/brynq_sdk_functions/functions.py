...
class Functions:
    ...
    def detect_changes_between_dataframes(df_old: pd.DataFrame, df_actual: pd.DataFrame, check_columns: list, unique_key: str | list, keep_old_values: Union[str, bool] = False, detect_column_changes: bool = False, ignore_new_empty_value_in_column: Union[bool, list] = False):
        ...
        for column in check_columns + [unique_key]:
            ...
            # string and object are also compatible as they both represent string data
            if not (df_old[column].dtype in ['int64', 'float64'] and df_actual[column].dtype in ['int64', 'float64']) \
                    and not (df_old[column].dtype in ['string', 'object'] and df_actual[column].dtype in ['string', 'object']) \
    ...
    def validate_data(df: pd.DataFrame, schema: Type[pa.DataFrameModel], debug: bool = False) -> (pd.DataFrame, pd.DataFrame):
        ...
        try:
            ...
            if debug:
                # Print concise summary of schema errors
                total_failures = len(exc.failure_cases)
                unique_columns = exc.failure_cases['column'].unique()
                print(f"\n⚠️  Schema validation errors: {total_failures} failures across {len(unique_columns)} column(s)")

                # Group by column and show summary
                for column in unique_columns:
                    column_failures = exc.failure_cases[exc.failure_cases['column'] == column]
                    total_rows = len(column_failures)
                    first_5_indices = column_failures['index'].head(5).tolist()
                    checks = column_failures['check'].unique()
                    print(f"   • {column}: {total_rows} invalid row(s) (checks: {', '.join(checks)})")
                    print(f"     First 5 indices: {first_5_indices}")
