...
# Import them only when needed to avoid import errors at startup
...
async def split_commit(workspace_root: str = None):
    ...
    try:
        ...
        # Lazy import embed_code to avoid import errors if optional dependencies are missing
        try:
            from src.kite_exclusive.commit_splitter.services.voyage_service import embed_code
        except ImportError as e:
            return (
                f"error: Failed to import voyage_service. This may be due to missing dependencies. "
                f"Original error: {e}. "
                f"Please ensure google-generativeai and other helix-py dependencies are installed."
            )
        
        # Lazy import helix to avoid import errors if optional dependencies are missing
        try:
            import helix
        except ImportError as e:
            return (
                f"error: Failed to import helix. This may be due to missing dependencies. "
                f"Original error: {e}. "
                f"Please ensure google-generativeai and other helix-py dependencies are installed."
            )
