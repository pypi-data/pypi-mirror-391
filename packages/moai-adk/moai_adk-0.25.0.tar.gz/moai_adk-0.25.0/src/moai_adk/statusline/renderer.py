...
class StatuslineRenderer:
    ...
    def render_statusline(self, data: StatuslineData, mode: str = "compact") -> str:
        """
        Alias for render() method for backwards compatibility and convenience.
    ...
    def _build_compact_parts(self, data: StatuslineData) -> List[str]:
        ...
        if self._display_config.version:
            version_str = self._validate_and_format_version(data.version)
            parts.append(f"🗿 Ver {version_str}")
    ...
    def _render_minimal(self, data: StatuslineData) -> str:
        ...
        # Add version if display enabled
        if self._display_config.version:
            parts.append(f"🗿 Ver {self._truncate_version(version_str)}")
    ...
    @staticmethod
    def _validate_and_format_version(version: str) -> str:
        """
        Validate and format version string for display.

        Handles error cases gracefully by replacing error-like versions
        with a placeholder version.

        Args:
            version: Version string (e.g., "v0.20.1" or "0.20.1", "error")
        ...
        # Handle error-like versions
        if version.lower() == "error" or version.lower().startswith("error"):
            return "unknown"

        # Return version as-is, formatting handled by VersionReader
        return version
