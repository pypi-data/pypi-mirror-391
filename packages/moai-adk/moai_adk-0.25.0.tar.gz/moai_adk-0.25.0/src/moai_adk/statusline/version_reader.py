...
class VersionConfig:
    ...
    fallback_version: str = __version__  # Use package version as fallback
...
class VersionReader:
    ...
    def get_version_sync(self) -> str:
        ...
        try:
            # Check cache first (_check_cache increments hits internally)
    ...
    def _check_cache(self) -> Optional[str]:
        ...
        if config_key in self._cache:
            ...
            # Synchronize with backwards compatibility _cache_time if it was manually set
            if self._cache_time is not None and self._cache_time != entry.timestamp:
                # Manual _cache_time update - update entry timestamp to match
                entry.timestamp = self._cache_time
            ...
            if self._is_cache_entry_valid(entry):
                ...
                # Update source-specific hits if tracking is enabled
                if "cache_hits_by_source" in self._cache_stats:
                    if VersionSource.CACHE.value not in self._cache_stats["cache_hits_by_source"]:
    ...
    def _update_cache(self, version: str, source: VersionSource) -> None:
        ...
        self._cache_time = entry.timestamp
    ...
    def _extract_version_from_config(self, config: Dict[str, Any]) -> str:
        ...
        for field_path in self.VERSION_FIELDS:
            ...
            if version:
                ...
                # Validate version format if enabled
                if self.config.enable_validation:
                    if not self._is_valid_version_format(version):
                        logger.warning(
                            f"Invalid version format '{version}', using fallback"
                        )
                        return ""
    ...
    def get_cache_age_seconds(self) -> Optional[float]:
        ...
        if config_key in self._cache:
            entry = self._cache[config_key]
            return (datetime.now() - entry.timestamp).total_seconds()
    ...
    def _is_cache_valid(self) -> bool:
        """
        Check if cache is valid (backwards compatibility).
        ...
        if config_key in self._cache:
            entry = self._cache[config_key]
            return self._is_cache_entry_valid(entry)
