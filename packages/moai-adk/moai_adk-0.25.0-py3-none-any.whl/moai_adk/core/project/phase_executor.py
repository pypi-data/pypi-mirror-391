...
class PhaseExecutor:
    ...
    def execute_configuration_phase(
        ...
        # Clear version reader cache to force fresh read during reinit (Issue #165)
        self._version_reader = None
    ...
    def _ensure_version_consistency(
        ...
        # 1. Already preserved from existing config by merge -> keep it
        # 2. User explicitly set in existing config -> preserve
        # 3. Version from config file -> use
        # 4. Current version from VersionReader -> use
        # 5. Package version -> fallback

        existing_moai = existing_config.get("moai", {})
        config_moai = config["moai"]

        # First check if version was already preserved by merge
        if "version" in config_moai and "version" in existing_moai:
            if config_moai["version"] == existing_moai["version"]:
                # Version already preserved by merge, don't override
                logger.debug(f"Version already preserved by merge: {config_moai['version']}")
                return
