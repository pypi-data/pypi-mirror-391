...
class AutoSpecConfig:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            # Extract auto-spec completion config - support both old and new structure
            # Try new flat structure first
            if "auto_spec_completion" in loaded_config:
                self.config = loaded_config.get("auto_spec_completion", {})
            # Fall back to old nested structure (tags.policy.auto_spec_completion)
            elif "tags" in loaded_config:
                self.config = loaded_config.get("tags", {}).get("policy", {}).get("auto_spec_completion", {})
            else:
                # No valid config found, use empty dict which will trigger defaults
                self.config = {}
    ...
    def save_config(self) -> None:
        ...
        try:
            # Try to load the full config file if it exists
            try:
                with open(self.config_path, "r", encoding="utf-8") as f:
