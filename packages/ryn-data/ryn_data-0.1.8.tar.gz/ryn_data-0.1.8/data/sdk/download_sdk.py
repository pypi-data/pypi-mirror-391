...
USER_MANAGEMENT_API = "http://144.172.105.98:30009"
CLEARML_API_URL = "http://144.172.105.98:30003"

#USER_MANAGEMENT_API = os.environ["USER_MANAGEMENT_API"]

def get_user_info(username, user_token):
    return requests.get(
        os.path.join(
            USER_MANAGEMENT_API, f"user/{username}/metadata?token={user_token}"
        ),
        timeout=100,
    ).json()
...
def clearml_client_session(credentials: dict, log_extra={}):
    ...
    search_payload = {
            ...
            "name": "DatasetIngestion_datauserv3"  # Exact match using regex
        }
    ...
    search_resp = session.post(f"{CLEARML_API_URL}/projects.get_all_ex", 
...
def s3_download(
    ...
    s3_path = get_s3_path_from_clearml_dataset(
        clearml_access_key, clearml_secret_key, dataset_name, user_name=user_name,version=version
    )
