...
from excel_mcp.data_range_analyzer import get_used_range as get_used_range_in_sheet
...
@mcp.tool()
def get_useds_range(filepath: str, sheet_name: str) -> str:
    """Get range cells in a worksheet."""
    try:
        # full_path = get_excel_path(filepath)
        return str(get_used_range_in_sheet(filepath, sheet_name))
    except (ValidationError, SheetError) as e:
        return f"Error: {str(e)}"
    except Exception as e:
        logger.error(f"Error getting merged cells: {e}")
