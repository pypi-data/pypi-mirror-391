...
export async function fetchReplayData(
  ...
  try {
    ...
    // Convert the response to DemoMessage format
    // Assuming the API returns the same format as test_sp.json
    return convertToDemoMessages(data);
  } catch (error) {
...
/**
 * Convert API response to DemoMessage format
 * This follows the same logic as createSampleDemoSequence in demo.ts
 */
function convertToDemoMessages(data: any): DemoMessage[] {
