...
export class JupyterAuthService {
  ...
  private static getJupyterBaseUrl(): string {
    ...
    if (match && match[1]) {
      console.log('[JupyterAuthService] Using base URL with user path:', match[1]);
      return match[1];
    }
    
    // Fallback to origin if no user path found
    console.log('[JupyterAuthService] No user path found, using origin:', window.location.origin);
