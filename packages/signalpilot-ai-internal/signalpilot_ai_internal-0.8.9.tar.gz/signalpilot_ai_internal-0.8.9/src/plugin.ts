  ...
  activate: (
    ...
    const handleEarlyAuth = async () => {
      ...
      try {
        ...
        // Check for replay parameter early
        replayId = urlParams.get('replay');
        if (replayId) {
          console.log(
            '[Replay] Detected replay parameter during plugin initialization:',
            replayId
          );
        }
