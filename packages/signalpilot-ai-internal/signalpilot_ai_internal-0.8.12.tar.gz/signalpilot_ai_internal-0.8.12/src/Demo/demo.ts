...
export async function runDemoSequence(
  ...
  let storedOriginalThreadData = originalThreadData;
  ...
  const startDemo = async (skipMode: boolean = false) => {
    ...
    if (!isDemoAborted) {
      ...
      await replaceTempThreadWithDemoThread(newThread.id, storedOriginalThreadData);
    } else {
 ...
 */
export function createSampleDemoSequence(): { messages: DemoMessage[], originalThreadData: any } {
