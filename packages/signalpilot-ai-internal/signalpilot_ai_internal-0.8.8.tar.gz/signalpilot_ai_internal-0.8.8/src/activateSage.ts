...
export async function activateSage(
  ...
  if (replayId) {
    console.log('[Replay] Replay ID passed to activateSage:', replayId);
  }
      ...
      if (isLauncher) {
        ...
        if (!tourCompleted && isAuthenticated && !isDemoMode) {
          ...
          if (tourCompleted) {
            ...
            console.log(
              '[Startup] Demo mode enabled - skipping welcome tour'
            );
          }
        }
      } else {
  ...
  // Handle replay if replay ID was passed
  if (replayId) {
    console.log('[Replay] Starting replay initialization for ID:', replayId);
    void handleReplayInitialization(replayId);
  }
 ...
 */
async function handleReplayInitialization(replayId: string): Promise<void> {
  ...
  try {
    ...
    if (retries === 0) {
      throw new Error(
        'Timeout waiting for SignalPilot services to initialize'
      );
    }
    ...
    console.log('[Replay] Creating new notebook:', notebookName);
    ...
    // Fetch the replay data from the backend
    console.log('[Replay] Fetching replay data from backend...');
    const demoMessages = await fetchReplayData(replayId);
    ...
    // Start the demo with the fetched data
    await runDemoSequence(demoMessages, 15, true);
