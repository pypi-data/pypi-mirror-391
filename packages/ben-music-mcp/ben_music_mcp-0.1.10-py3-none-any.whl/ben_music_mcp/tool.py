...
def _get_song_info(keywords: str) -> dict:
    ...
    :return: 歌曲信息json
    """
    url = f"{base_url}/search?keywords={keywords}&randomCNIP=true"
    try:
...
def _get_music_url(keywords: str, level: str = "standard") -> dict:
    ...
    url = f"{base_url}/song/url/v1?id={song_id}&level={level}&randomCNIP=true"
    try:
        resp = requests.get(url, timeout=15)
        resp.raise_for_status()
        data = resp.json()
...
def _get_uid(nickname: str) -> dict:
    ...
    :return: 用户uid json
    """
    url = f"{base_url}/get/userids?nicknames={nickname}&randomCNIP=true"
    try:
        resp = requests.get(url, timeout=15)
        resp.raise_for_status()
        data = resp.json()
