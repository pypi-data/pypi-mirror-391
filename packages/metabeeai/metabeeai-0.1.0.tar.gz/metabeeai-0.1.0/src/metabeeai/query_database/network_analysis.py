...
def create_co_occurrence_matrix(bee_df: pd.DataFrame, pesticides_df: pd.DataFrame, papers_with_both: set) -> pd.DataFrame:
    ...
    print(f"  Filtering to {len(bee_genera_multiple_studies)} bee genera with ≥3 studies (removed {len(bee_study_counts) - len(bee_genera_multiple_studies)} with <3 studies)")
