...
def analyze_co_occurrence(bee_df: pd.DataFrame, pesticides_df: pd.DataFrame) -> Dict:
    ...
    nicotinic_pesticides = {
        ...
        'cyantraniliprole', 'chlorantraniliprole'  # diamides, also nAChR modulators
    }
    ...
    excluded_counts = pesticides_df_filtered[~pesticides_df_filtered['pesticide_name'].isin(nicotinic_pesticides)]['pesticide_name'].value_counts().head(10)
    ...
    print(f"  Standardization examples:")
...
def create_visualizations(analysis_data: Dict, output_dir: str):
    ...
    combination_labels = [f"{row['bee_species']} + {row['pesticide']}" 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'top_bee_pesticide_combinations.png'), 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'most_studied_bee_species.png'), 
                dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'most_tested_nicotinic_pesticides.png'), 
                dpi=300, bbox_inches='tight')
    plt.close()
...
def calculate_quantitative_measures(analysis_data: Dict) -> Dict:
    ...
    # Diversity measures
    bee_shannon_diversity = -sum((count/total_combinations) * np.log2(count/total_combinations) 
...
def generate_summary_report(analysis_data: Dict, measures: Dict, output_dir: str):
    ...
    with open(report_file, 'w') as f:
        ...
        f.write("-" * 15 + "\n")
        f.write(f"Most studied combination: {measures['most_studied_combination']['bee_species']} + {measures['most_studied_combination']['pesticide']} ({measures['most_studied_combination']['count']} studies)\n")
        f.write(f"Least studied combination: {measures['least_studied_combination']['bee_species']} + {measures['least_studied_combination']['pesticide']} ({measures['least_studied_combination']['count']} studies)\n")
