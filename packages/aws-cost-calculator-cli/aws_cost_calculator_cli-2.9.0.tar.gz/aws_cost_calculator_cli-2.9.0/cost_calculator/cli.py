...
def calculate_costs(profile_config, accounts, start_date, offset, window):
    ...
    # Start date is window days before end_date (window-1 because end_date is inclusive)
    start_date_calc = end_date - timedelta(days=window - 1)
    ...
    # Get profile name from config if available
    profile_name = profile_config.get('profile_name')
    exclusions = get_exclusions(profile_name)  # Get from DynamoDB
    ...
    # Get support cost - determine which month's support applies
    # Support is charged on the 1st of each month for the previous month's usage
    # If range is within one month, use that month's 1st
    # If range straddles months, use the later month's 1st (which covers the earlier month)
    
    if start_date_calc.month == end_date.month and start_date_calc.year == end_date.year:
        # Range within single month
        support_query_date = start_date_calc.replace(day=1)
    else:
        # Range straddles months - use end_date's month
        support_query_date = end_date.replace(day=1)
    
    support_date_str = support_query_date.strftime('%Y-%m-%d')
    support_date_end = (support_query_date + timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    # Calculate days in the PREVIOUS month (the month that this support charge covers)
    # Support charged on Nov 1 covers October (31 days)
    previous_month = support_query_date - timedelta(days=1)
    import calendar
    days_in_support_month = calendar.monthrange(previous_month.year, previous_month.month)[1]
...
def calculate(profile, start_date, offset, window, json_output, sso, access_key_id, secret_access_key, session_token):
    """
    [DEPRECATED] Calculate AWS costs with daily rate and annual projection.
    
    ⚠️  DEPRECATED: Use 'cc daily-report' instead. This command will be removed in a future version.
    
    The daily-report command provides all the same functionality plus:
    • Detailed daily breakdowns by service and account
    • Interactive shareable charts
    • Support for both offset/window and start-date/end-date
    • Same exclusions, metrics, and support cost calculations
    ...
    click.echo("⚠️  WARNING: 'cc calculate' is deprecated and will be removed in a future version.")
    click.echo("   Use 'cc daily-report' instead for the same functionality plus detailed breakdowns.")
    click.echo("")
...
def list_profiles():
    """
    List all available cost analysis profiles from backend.
    
    Displays all profiles stored in DynamoDB with account counts.
    Profiles contain account lists, exclusions, and Athena configuration.
    ...
    OUTPUT:
    \u2022 Profile names
    \u2022 Number of accounts per profile
    \u2022 Total profile count
    
    \\b
    SEE ALSO:
    \u2022 cc profile - For CRUD operations on profiles
    \u2022 cc calculate --profile NAME - To use a profile
    """
    import requests
...
def configure(api_secret, show):
    """
    Configure Cost Calculator CLI settings and API access.
    
    Set up API secret for backend access to profiles stored in DynamoDB.
    The secret can be configured locally or via COST_API_SECRET environment variable.
    
    \\b
    KEY FEATURES:
    \u2022 Store API secret locally in config file
    \u2022 Show current configuration (masked)
    \u2022 Check environment variable status
    \u2022 Secure storage in ~/.config/cost-calculator/
    ...
    Use environment variable (no config needed):
        export COST_API_SECRET=YOUR_SECRET_KEY
        cc list-profiles
    
    \\b
    CONFIGURATION:
    \u2022 Config file: ~/.config/cost-calculator/config.json
    \u2022 Environment: COST_API_SECRET
    \u2022 Priority: Environment variable > Config file
    
    \\b
    TROUBLESHOOTING:
    If you get 401 errors after configuring, check for stale environment variable:
        echo $COST_API_SECRET
        unset COST_API_SECRET  # Remove old value
        cc configure --show     # Verify config file is used
    
    \\b
    SEE ALSO:
    \u2022 cc list-profiles - Test configuration
    \u2022 cc setup-api - Alternative setup command
    \u2022 AUTHENTICATION_TROUBLESHOOTING.md - Detailed auth guide
    """
    import os
...
def analyze(profile, analysis_type, weeks, pattern, min_cost, json_output):
    """
    Perform pandas-based statistical analysis on cost data.
    
    Advanced analysis using pandas for aggregations, volatility detection,
    trend analysis, and pattern search across services.
    
    \b
    ANALYSIS TYPES:
    • summary - Aggregate statistics (sum, avg, std) across weeks
    • volatility - Identify services with high cost variability
    • trends - Auto-detect increasing/decreasing patterns
    • search - Find services matching pattern or cost threshold
    
    \b
    EXAMPLES:
    
    Summary statistics:
        cc analyze --profile khoros --type summary --weeks 8
    
    Find volatile services:
        cc analyze --profile khoros --type volatility --weeks 12
    
    Search for pattern:
        cc analyze --profile khoros --type search --pattern "EC2" --weeks 4
    
    High-cost services:
        cc analyze --profile khoros --type search --min-cost 1000 --weeks 8
    
    \b
    SEE ALSO:
    • cc trends - For week-over-week comparisons
    • cc drill - For multi-dimensional analysis
    """
...
def trends_detailed(profile, accounts, services, start_date, end_date, granularity, 
    ...
    """
    Detailed cost trends with flexible granularity, filtering, and spike detection.
    
    Advanced trend analysis with customizable date ranges, granularity (daily/hourly),
    account/service filtering, spike detection, and trendline fitting. Perfect for
    investigating specific cost patterns and anomalies.
    ...
    • Trendline fitting with outlier removal
    • CSV and JSON output
    • Chart generation (PNG)
    
    \b
    WHEN TO USE:
    • Investigating specific cost anomalies
    • Analyzing hourly cost patterns
    • Filtering out one-time charges
    • Comparing specific accounts or services
    • Generating smooth trendlines
    • Custom date range analysis
    
    \b
    SPIKE DETECTION:
    • --filter-spikes: Remove anomalous day-over-day increases
    • --spike-threshold: Multiplier for spike detection (2.0 = 100% increase)
    • --trendline: Fit smooth line, removing outliers
    • --outlier-threshold: Std devs from mean (2.5 = 99% confidence)
    
    \b
    EXAMPLES:
    
    Top 20 services, last 30 days:
        cc trends-detailed --profile khoros --sso khoros_umbrella
    
    Specific accounts only:
        cc trends-detailed --profile khoros \\
            --accounts "820054669588,180770971501" --sso khoros_umbrella
    
    Hourly granularity:
        cc trends-detailed --profile khoros --granularity HOURLY \\
            --start-date 2025-11-01 --end-date 2025-11-05 --sso khoros_umbrella
    
    Specific services with chart:
        cc trends-detailed --profile khoros \\
            --services "EC2 - Other,RDS,CloudWatch" --chart --sso khoros_umbrella
    
    Filter spikes and generate trendline:
        cc trends-detailed --profile khoros --filter-spikes --trendline \\
            --sso khoros_umbrella
    
    \b
    OUTPUT:
    • CSV file with date, service, account, cost columns
    • JSON format with structured data
    • PNG chart (with --chart flag)
    • Spike-filtered data (with --filter-spikes)
    • Smooth trendline (with --trendline)
    
    \b
    NOTES:
    • Hourly data available for last 14 days only
    • Default date range: last 30 days (T-2 to T-32)
    • Spike filtering helps identify sustained vs one-time costs
    • Trendline uses statistical outlier removal
    
    \b
    SEE ALSO:
    • cc trends - For standard week-over-week analysis
    • cc drill - For multi-dimensional investigation
    • cc daily-report - For unified daily breakdowns
    """
    import requests
    from datetime import datetime, timedelta
...
@click.option('--profile', required=True, help='Profile name')
@click.option('--offset', default=None, type=int, help='Days back from today (default: 2 = T-2)')
@click.option('--window', default=None, type=int, help='Number of days to query (default: 30)')
@click.option('--start-date', default=None, help='Start date INCLUSIVE (YYYY-MM-DD) - use with --end-date')
@click.option('--end-date', default=None, help='End date INCLUSIVE (YYYY-MM-DD) - use with --start-date')
@click.option('--top-services', default=20, help='Number of top services to include (default: 20)')
@click.option('--top-accounts', default=20, help='Number of top accounts to include (default: 20)')
@click.option('--share', is_flag=True, help='Generate shareable chart URL')
@click.option('--open', 'open_browser', is_flag=True, help='Automatically open chart in browser (requires --share)')
@click.option('--force', is_flag=True, help='Force refresh, bypass S3 cache')
@click.option('--sso', help='AWS SSO profile to use')
@click.option('--access-key-id', help='AWS Access Key ID')
@click.option('--secret-access-key', help='AWS Secret Access Key')
@click.option('--session-token', help='AWS Session Token')
def daily_report(profile, offset, window, start_date, end_date, top_services, top_accounts, share, open_browser, force, sso, access_key_id, secret_access_key, session_token):
    """
    ...
    \b
    DATE RANGE (two methods):
    
    Method 1: offset/window (default)
    • offset: Days back from today (default: 2 = T-2)
    • window: Number of days to analyze (default: 30)
    • Example: --offset 2 --window 30 = last 30 days ending 2 days ago
    
    Method 2: explicit dates
    • start-date: Start date INCLUSIVE (YYYY-MM-DD)
    • end-date: End date INCLUSIVE (YYYY-MM-DD)
    • Example: --start-date 2025-11-01 --end-date 2025-11-01 = Nov 1 only
    • Example: --start-date 2025-11-01 --end-date 2025-11-10 = Nov 1-10 (10 days)
    • Note: Cannot mix with offset/window
    ...
    • Requires AWS credentials with Cost Explorer read access
    • Profile must exist in backend DynamoDB
    • --share uploads via backend API (no S3 permissions needed)
    ...
    import json
    import os
    from cost_calculator.api_client import get_api_config
    ...
    try:
        ...
        # Validate date parameters: either offset/window OR start_date/end_date
        from datetime import datetime, timedelta
        
        if start_date and end_date:
            # Method 2: Explicit dates
            if offset is not None or window is not None:
                raise click.ClickException("Cannot use both offset/window and start-date/end-date. Choose one method.")
            
            # Calculate window for display
            start_obj = datetime.strptime(start_date, '%Y-%m-%d')
            end_obj = datetime.strptime(end_date, '%Y-%m-%d')
            window_calc = (end_obj - start_obj).days
            
            click.echo(f"Fetching daily cost data for {profile}...")
            click.echo(f"Date range: {start_date} to {end_date} (both inclusive) ({window_calc + 1} days)")
            estimated_time = max(4, ((window_calc + 1) // 10) + 4)
            click.echo(f"This will take ~{estimated_time} seconds ({window_calc + 1} parallel queries)...")
            click.echo("")
            
            # Call API with explicit dates
            ...
            end_date_calc = (datetime.now() - timedelta(days=offset)).strftime('%Y-%m-%d')
            start_date_calc = (datetime.now() - timedelta(days=offset + window)).strftime('%Y-%m-%d')
            
            click.echo(f"Fetching daily cost data for {profile}...")
            click.echo(f"Date range: {start_date_calc} to {end_date_calc} ({window} days)")
            estimated_time = max(4, (window // 10) + 4)
            click.echo(f"This will take ~{estimated_time} seconds ({window} parallel queries)...")
            click.echo("")
            
            # Call API with offset/window
            result = call_lambda_api(
                'daily-report',
                credentials,
                profile=profile,
        ...
        if 'support_per_day' in result['summary']:
            click.echo(f"  Support (monthly): ${result['summary']['support_monthly']:,.2f}")
            click.echo(f"  Support (per day): ${result['summary']['support_per_day']:,.2f}")
            click.echo(f"  Daily Rate (with support): ${result['summary']['daily_rate']:,.2f}")
            click.echo(f"  Annual Projection: ${result['summary']['annual_projection']:,.0f}")
        ...
        if share:
            ...
            # Upload via backend API (avoids needing S3 write permissions)
            api_config = get_api_config()
            if not api_config:
                raise click.ClickException("API not configured. Run: cc configure --api-secret YOUR_SECRET")
            
            # Use function URL directly (not through API Gateway)
            upload_url = os.environ.get(
                'SHARE_UPLOAD_URL',
                'https://xwprcnkhft7lrpruqmjflzkhky0tiknj.lambda-url.us-east-1.on.aws/'
            )
            ...
            upload_payload = {
                'data': result,
                'title': f"{profile} Daily Cost Report",
                'description': f"Date range: {result['metadata']['date_range']}"
            }
            
            upload_response = requests.post(upload_url, headers=headers, json=upload_payload, timeout=60)
            
            if upload_response.status_code != 200:
                raise click.ClickException(f"Failed to create shareable chart: {upload_response.status_code} - {upload_response.text}")
            
            share_data = upload_response.json()
            chart_url = share_data['share_url']
            ...
            click.echo("")
            click.echo(f"📋 Share ID: {share_data['share_id']}")
            click.echo("")
            click.echo("📊 Features:")
            ...
            if open_browser:
                ...
                import platform
                click.echo("")
                click.echo("🌐 Opening chart in browser...")
