...
@click.option('--secret-access-key', help='AWS Secret Access Key')
@click.option('--session-token', help='AWS Session Token')
def daily_report(profile, offset, window, top_services, top_accounts, share, open_browser, sso, access_key_id, secret_access_key, session_token):
    """
    ...
    try:
        ...
        # Parallel queries: ~4 seconds for up to 10 days, then +1s per additional 10 days
