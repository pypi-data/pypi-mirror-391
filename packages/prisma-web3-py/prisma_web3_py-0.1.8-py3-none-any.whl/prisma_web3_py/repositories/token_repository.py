...
class TokenRepository(BaseRepository[Token]):
    ...
    async def search_by_symbol(
        ...
        try:
            ...
            self.logger.error(f"Error searching by symbol '{symbol}': {e}")
            return []
    ...
    async def search_by_name(
        ...
        try:
            ...
            self.logger.error(f"Error searching by name '{name}': {e}")
            return []
    ...
    async def search_by_alias(
        ...
        try:
            ...
            # Use JSONB containment operator to search aliases array
            # This checks if the JSONB array contains the given value
            stmt = select(Token).where(
                Token.aliases.contains([normalized_alias])
            )
            ...
            self.logger.error(f"Error searching by alias '{alias}': {e}")
            return []
    ...
    async def fuzzy_search(
        ...
        try:
            ...
            # Try using pg_trgm for fuzzy matching
            try:
            ...
            except Exception as pg_trgm_error:
                # Fall back to LIKE matching if pg_trgm is not available
                self.logger.warning(
                    f"pg_trgm not available, falling back to LIKE matching: {pg_trgm_error}"
                )
            ...
            self.logger.error(f"Error in fuzzy_search('{text}'): {e}")
            return []
