...
"""
MCP RaseSQL/PostgreSQL Server

Main entry point for RaseSQL/PostgreSQL DataSource MCP Client server.
"""
import os
import sys
from typing import List
from fastmcp import FastMCP
from rasesql_mcp_server import get_base_package_info

project_path=os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
# Add current directory to Python module search path
sys.path.insert(0,project_path)
from rasesql_mcp_server.utils.logger_util import logger, db_config_path
from rasesql_mcp_server.utils.db_operate import execute_sql
from rasesql_mcp_server.utils import load_activate_db_config
from rasesql_mcp_server.tools.db_tool import generate_test_data
from rasesql_mcp_server.resources.db_resources import generate_database_config, generate_database_tables
...
async def sql_exec(sql: str):
    """
    Universal RaseSQL/PostgreSQL SQL execution tool
...
async def describe_table(table_name: str):
    """
    RaseSQL/PostgreSQL Table structure description tool
    ...
    Uses PostgreSQL information_schema for compatibility
    
    Parameter description:
    - table_name (str): Table name to describe, supports schema.table format
    ...
    - describe_table("public.users")
    
    Return data example:
    [
        {"Field": "id", "Type": "integer", "Null": "NO", "Key": "PRI", "Default": "nextval('users_id_seq'::regclass)", "Extra": ""},
        {"Field": "name", "Type": "character varying", "Null": "NO", "Key": "", "Default": null, "Extra": ""}
    ]
    """
    logger.info(f"MCP tool: Describe table structure - {table_name}")
    
    # Handle schema.table format or default to public schema
    if '.' in table_name:
...
async def get_database_tables():
    """
   RaseSQL/PostgreSQL Database table information resource
...
async def get_database_config():
    """
    RaseSQL/PostgreSQL Database configuration information resource
...
def main():
    ...
    logger.info(f"RaseSQL MCP Client version: {get_base_package_info()}")
    logger.info(f"Database configuration file path:{db_config_path}")
    logger.info(f"Current project path:{project_path}")
    logger.info("RaseSQL DataSource MCP Client server is ready to accept connections")
