...
class Entity1Repository:
    ...
    def get_all(filters: Optional[dict] = None) -> List[ Entity1Entity ]:
        ...
        raises:
            Entity1ValueError:  Si el valor de entrada no es válido.
        ...
        try:
            ...
            if filters is not None:
                if not isinstance(filters, dict):
                    raise Entity1ValueError(field="filters", detail="filters must be a dict or None")
    ...
    @staticmethod
    def get_by_id(id = None, uuid = None) -> Optional[ Entity1Entity ]:
        """
        ...
        if id is not None: 
            if not is_integer(id):
                raise Entity1ValueError(field="id", detail="ID must be integer.")
        elif uuid is not None:
            if not is_uuid(uuid):
                raise Entity1ValueError(field="uuid", detail="UUID must be valid.")
        else:
            raise Entity1ValueError(field="id/uuid", detail="Either id or uuid must be provided.")

        try:
            if id is not None:
                instance = Entity1.objects.get(id=id)
            else:
                instance = Entity1.objects.get(uuid=uuid)
    ...
    def count_all(filters: Optional[dict] = None) -> int:
        ...
        try:
            ...
            if filters is not None:
                if not isinstance(filters, dict):
                    raise Entity1ValueError(field="filters", detail="filters must be dict or None")               
                if "nombre" in filters and filters["nombre"].strip():
                    instance_list = instance_list.filter(nombre__icontains=filters["nombre"])            
    ...
    def create(entity: Entity1Entity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> Entity1Entity:
        ...
        if not entity or not hasattr(entity, "to_dict"):
            raise Entity1ValueError(field="Entity1", detail="Entity null or without methond 'to_dict'")

        try:
            # Crear el registro a partir de los campos presentes en la 'entity'
            instance = Entity1()
            Mapper.update_model_from_entity(instance, entity, excluded_fields=Entity1Repository.ENTITY_NOT_PERSISTIBLE_FIELDS) 
            ...
            raise Entity1ValueError(field="data", detail=f"Error in the data structure: {str(e)}") from e
        except ValidationError as e:
            raise Entity1ValidationError(f"Validation error: {e.message_dict}") from e
        except IntegrityError as e:
            if 'duplicate' in str(e).lower() or 'unique constraint' in str(e).lower():
                raise Entity1AlreadyExistsError(field='attributeName', detail=instance.attributeName)  # Ajusta según el campo único
            # Otro error de integridad → regla de negocio?
            raise Entity1ValidationError({"integrity": "Duplicated or inconsistent data"})            
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e
        except Exception as e:
    ...
    def update(entity: Entity1Entity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> Entity1Entity:
        ...
        # Validar la entidad de entrada
        if not entity or not hasattr(entity, "to_dict"):
            raise Entity1ValueError(field="Entity1", detail="Entity null or without method 'to_dict'")

        # Validar que tenga el id o uuid
        if not entity.id and not entity.uuid:
            raise Entity1ValueError(field="id/uuid", detail="The id or uuid field is required")
        ...
        try:
            if entity.id:
                ...
                instance = Entity1.objects.get(uuid=entity.uuid)
            ...
            raise Entity1ValueError(field="data", detail=f"Error inthe data structure: {str(e)}") from e
        except ValidationError as e:
            raise Entity1ValidationError(f"Validation error: {e.message_dict}") from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e            
    ...
    def delete(id=None, uuid=None) -> bool:
        ...
        if uuid is not None and not is_uuid(uuid):
            raise Entity1ValueError(field="uuid", detail="El UUID debe ser válido.")
