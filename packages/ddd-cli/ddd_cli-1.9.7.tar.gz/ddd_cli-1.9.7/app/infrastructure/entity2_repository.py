...
from ..domain.entities import Entity2Entity
...
class Entity2Repository:
    """
    Repositorio para manejar la persistencia de datos de la entidad: entity2.
    ...
    @staticmethod
    def get_all(filters: Optional[dict] = None) -> List[ Entity2Entity ]:
        """
        ...
        returns: 
            List[ Entity2Entity ]: Lista de entidades recuperadas.
        raises:
            Entity2ValueError:  Si el valor de entrada no es válido.
        ...
        try:
            instance_list = Entity2.objects.all()    

            # Aplicar filtros si se proporcionan
            if filters is not None:
                if not isinstance(filters, dict):
                    raise Entity2ValueError(field="filters", detail="filters must be a dict or None")
            ...
            return [Mapper.model_to_entity(instance, Entity2Entity) for instance in instance_list]        
    ...
    @staticmethod
    def get_by_id(id = None, uuid = None) -> Optional[ Entity2Entity ]:
        """
        ...
        raises:
            Entity2ValueError: Si el valor de entrada no es válido.        
            Entity2NotFoundError: Si no existe el registro con el ID dado.
        ...
        if id is not None: 
            if not is_integer(id):
                raise Entity2ValueError(field="id", detail="ID must be integer.")
        elif uuid is not None:
            if not is_uuid(uuid):
                raise Entity2ValueError(field="uuid", detail="UUID must be valid.")
        else:
            raise Entity2ValueError(field="id/uuid", detail="Either id or uuid must be provided.")

        try:
            if id is not None:
                instance = Entity2.objects.get(id=id)
            else:
                instance = Entity2.objects.get(uuid=uuid)

            return Mapper.model_to_entity(instance, Entity2Entity)

        except Entity2.DoesNotExist as e:
            raise Entity2NotFoundError(id=id) from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e    
        except Exception as e:
    ...
    def exists_by_field(field_name, value) -> bool:
        ...
        if field_name not in ALLOWED_FIELDS:
            raise Entity2ValueError(field=field_name, detail=f"El campo '{field_name}' no es válido para verificar existencia.")

        try:
            return Entity2.objects.filter(**{field_name: value}).exists()
    ...
    def count_all(filters: Optional[dict] = None) -> int:
        ...
        try:
            ...
            if filters is not None:
                if not isinstance(filters, dict):
                    raise Entity2ValueError(field="filters", detail="filters must be dict or None")               
                if "nombre" in filters and filters["nombre"].strip():
                    instance_list = instance_list.filter(nombre__icontains=filters["nombre"])            
    ...
    @staticmethod
    def create(entity: Entity2Entity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> Entity2Entity:
        """
        ...
        raises:
            Entity2ValueError:  Si el valor de entrada no es válido.
            Entity2ValidationError: Si los datos no son válidos.
            Entity2AlreadyExistsError: Si ya existe un registro con el mismo nombre.
            ConnectionDataBaseError: Si ocurre un error al acceder a la base de datos.   
            RepositoryError: Si ocurre un error inesperado (interno del sistema).     
        """
        ...
        if not entity or not hasattr(entity, "to_dict"):
            raise Entity2ValueError(field="Entity2", detail="Entity null or without methond 'to_dict'")
        ...
        try:
            ...
            data = clean_dict_of_keys(data, keys=Entity2Repository.ENTITY_NOT_PERSISTIBLE_FIELDS)              

            # Crear el registro a partir de los campos presentes en la 'data'
            instance = Entity2(**data)
            ...
            raise Entity2ValueError(field="data", detail=f"Error in the data structure: {str(e)}") from e
        except ValidationError as e:
            raise Entity2ValidationError(f"Validation error: {e.message_dict}") from e
        except IntegrityError as e:
            if 'duplicate' in str(e).lower() or 'unique constraint' in str(e).lower():
                raise Entity2AlreadyExistsError(field='attributeName', detail=instance.attributeName)  # Ajusta según el campo único
            # Otro error de integridad → regla de negocio?
            raise Entity2ValidationError({"integrity": "Duplicated or inconsistent data"})            
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e
        except Exception as e:
    ...
    @staticmethod
    def update(entity: Entity2Entity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> Entity2Entity:
        """
        ...
        # Validar la entidad de entrada
        if not entity or not hasattr(entity, "to_dict"):
            raise Entity2ValueError(field="Entity2", detail="Entity null or without method 'to_dict'")

        # Validar que tenga el id o uuid
        if not entity.id and not entity.uuid:
            raise Entity2ValueError(field="id/uuid", detail="The id or uuid field is required")
        ...
        try:
            if entity.id:
                instance = Entity2.objects.get(id=entity.id)
            else:
                instance = Entity2.objects.get(uuid=entity.uuid)
            ...
            with transaction.atomic():
                ...
                for key, value in entity.to_dict().items():
                    if (key is not None) and (not key in Entity2Repository.ENTITY_NOT_PERSISTIBLE_FIELDS) and hasattr(instance, key):
            ...
            raise Entity2NotFoundError(id=entity.id) from e
        except (TypeError, ValueError) as e:
            raise Entity2ValueError(field="data", detail=f"Error inthe data structure: {str(e)}") from e
        except ValidationError as e:
            raise Entity2ValidationError(f"Validation error: {e.message_dict}") from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e            
    ...
    def delete(id=None, uuid=None) -> bool:
        ...
        if id is not None and not is_integer(id):
            raise Entity2ValueError(field="id", detail="El ID debe ser un entero.")

        # validar el uuid
        if uuid is not None and not is_uuid(uuid):
            raise Entity2ValueError(field="uuid", detail="El UUID debe ser válido.")
        ...
        try:
            ...
            raise Entity2ValidationError(f"Validation error occurred: {e.message_dict}") from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Data base access error") from e            
        except Exception as e:
...
#### 1. 🗣️ **Métodos específicos orientados al dominio**
    En lugar de exponer solo filtros genéricos por `nombre`, puedes agregar métodos que expresen reglas de negocio:
        @staticmethod
        def get_activos():
            return Entity2.objects.filter(estado='activo')

        @staticmethod
        def find_by_slug(slug: str) -> OptionalEntity2Entity:
            try:
                instance = Entity2.objects.get(slug=slug)
                return Mapper.model_to_entity(instance, Entity2Entity)
            except Entity2.DoesNotExist:
                return None
...
#### 2. 🔍 **QuerySets y Managers personalizados**
    Puedes encapsular lógica común (como filtros por estado o relaciones) en un `Manager` personalizado:
        class Entity2Manager(models.Manager):
            def activos(self):
        ...
        class Entity2(models.Model):
            ...
            objects = Entity2Manager()

    Luego, en el repositorio:
        @staticmethod
        def get_all(filters=None):
            instance_list = Entity2.objects.activos()  # Usa tu Manager
            if filters and "nombre" in filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, Entity2Entity) for inst in instance_list]

        @staticmethod
        def get_all_with_relations():
            instance_list = Entity2.objects.activos().con_relacion() # Usa tu Manager
            if filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, Entity2Entity) for inst in instance_list]
...
#### 5. 🧠 **Consultas complejas bien encapsuladas**
    Cuando necesites agregaciones o filtros avanzados, encapsúlalos en métodos del repositorio:
        ...
        def get_con_muchos_externals(min_relaciones=3):
            instances = Entity2.objects.annotate(
                total_externals=Count('externals')
            ).filter(total_externals__gt=min_relaciones)
            return [Mapper.model_to_entity(inst, Entity2Entity) for inst in instances]
...
#### 6. **Uso de `select_related` y `prefetch_related`**
    La plantilla no los usa aún, pero son fáciles de integrar en `get_all()` o nuevos métodos:
        ...
        @staticmethod
        def get_all_with_relations():
            instance_list = Entity2.objects.select_related('external').prefetch_related('externals')
            if filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, Entity2Entity) for inst in instance_list]
...
#### 7. **Consultas RAW o expresiones ORM avanzadas**
    Usa `Q`, `F`, `Subquery`, o SQL crudo **dentro del repositorio** cuando el ORM no alcance:
        ...
        def search_advanced(query):
            instances = Entity2.objects.filter(
        ...
        def reactivar_registros():
            Entity2.objects.filter(estado='inactivo').update(estado=F('estado_anterior'))

        @staticmethod
        def busqueda_compleja_sql():
            from django.db import connection
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM app_entity2 WHERE estado = %s", ['activo'])
                rows = cursor.fetchall()
            return [Mapper.model_to_entity(row, Entity2Entity) for row in rows]
...
#### 8. **Documentación y claridad**
    Los métodos del repositorio deben reflejar intenciones del negocio, no solo operaciones técnicas:
        @staticmethod
        def get_all(filters=None) -> ListEntity2Entity:
            """
            Obtiene todos los entity2 que coincidan con los filtros.
            Usa `.only()` para optimizar rendimiento.
            :param filters: Diccionario con filtros (ej. {"nombre": "juan"}).
            :return: Lista de entidades Entity2.
            """
    ...
    Cada método debe tener pruebas. Ejemplo con `create()`:
        ...
        class UserRepositoryTests(TestCase):
            ...
            def test_create_con_external_y_externals(self):
                entity = Entity2Entity(nombre="Test")
                created = Entity2Repository.create(
                    entity=entity,
                    external_id=1,
                    externals=[1, 2],
                ...
                # Verifica relaciones
                instance = Entity2.objects.get(id=created.id)
