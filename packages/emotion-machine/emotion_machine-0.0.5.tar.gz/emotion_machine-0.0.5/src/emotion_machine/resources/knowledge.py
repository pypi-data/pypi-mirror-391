...
class KnowledgeAPI:
    ...
    def upload_asset(
        ...
        *,
        ...
        with path.open("rb") as handle:
            ...
            response = self._http.request(
                "POST",
                f"/v1/companions/{companion_id}/knowledge-assets",
                files=files,
                expected=(201,),
            )
        if not isinstance(response, dict):  # pragma: no cover - defensive
            raise TypeError("Expected dict payload for knowledge asset response")
        return response

    def list_assets(self, companion_id: str, *, limit: int = 50) -> list[dict[str, Any]]:
        params = {"limit": limit}
        ...
        if not isinstance(data, list):  # pragma: no cover - defensive
            raise TypeError("Expected list payload for knowledge assets response")
        return data
    ...
    def ingest_file(
        ...
        *,
        ...
        asset = self.upload_asset(companion_id, file_path=file_path, mime_type=mime_type)
        job = self.ingest(
            companion_id,
        ...
        return {"asset": asset, "job": final_job}
