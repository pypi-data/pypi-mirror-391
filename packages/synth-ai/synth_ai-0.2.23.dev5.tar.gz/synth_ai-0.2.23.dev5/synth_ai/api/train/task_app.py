...
def check_task_app_health(base_url: str, api_key: str, *, timeout: float = 10.0, max_retries: int = 5) -> TaskAppHealth:
    ...
    if original_hostname and original_hostname not in ("localhost", "127.0.0.1"):
        # Add Host header so server knows which hostname we're requesting
    ...
    ip_task_info_url, _ = _resolve_url_to_ip(task_info_url)
