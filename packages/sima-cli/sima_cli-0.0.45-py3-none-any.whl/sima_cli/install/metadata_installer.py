...
def _run_installation_script(metadata: Dict, extract_path: str = "."):
    ...
    try:
        if os.name == "nt":
            ...
            # --- macOS / Linux / eLxr path ---
            shell = os.environ.get("SHELL", "/bin/bash")
            shell_name = os.path.basename(shell)

            # Determine which RC file to source in safe order
            rc_candidates = [
            ...
            if nested:
                # Nested context → run non-interactive, source RC if found
                if rc_file:
            ...
            subprocess.run(cmd, cwd=extract_path, check=True, env=env)
        ...
        registry.update_state(metadata.get('name'), metadata.get('version'), 'failed-no-shell')
        sys.exit(1)
