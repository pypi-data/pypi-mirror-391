...
from .engine import load_workflow
...
class WorkflowTool:
    ...
    def __init__(
        ...
        # Load workflow
        self.graph, self.engine = load_workflow(yaml_path, checkpointer=checkpointer)
    ...
    def execute(
        ...
        # Check workflow state to determine action
        state = self.graph.get_state(config)

        if state.next:
            # Workflow is interrupted and waiting for input
            print(f"[WorkflowTool] Resuming interrupted workflow {self.name} (thread: {thread_id})")
            result = self.graph.invoke(
                Command(resume=user_message or ""),
                config=config
            )
        else:
            # Workflow is fresh or completed, start/restart
            print(f"[WorkflowTool] Starting fresh workflow {self.name} (thread: {thread_id})")
            context = initial_context or {}
            result = self.graph.invoke(context, config=config)
