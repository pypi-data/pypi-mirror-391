...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "Usage: schemanormalizer schema.json < input.json\n");
        return EXIT_FAILURE;
    }
    ...
    // Use reader to parse the JSON in stdin, and forward SAX events to normalizer
    SchemaNormalizer normalizer(sd);
    Reader reader;
    FileReadStream is(stdin, buffer, sizeof(buffer));
    if (!reader.Parse(is, normalizer) && reader.GetParseErrorCode() != kParseErrorTermination) {
        // Schema normalizer error would cause kParseErrorTermination, which will handle it in next step.
    ...
    if (normalizer.IsValid()) {
        ...
        fprintf(stderr, "Invalid code: %d\n", normalizer.GetInvalidSchemaCode());
        fprintf(stderr, "Invalid message: %s\n", GetValidateError_En(normalizer.GetInvalidSchemaCode()));
        sb.Clear();
        ...
        CreateErrorMessages(normalizer.GetError());
        return EXIT_FAILURE;
    }
}
