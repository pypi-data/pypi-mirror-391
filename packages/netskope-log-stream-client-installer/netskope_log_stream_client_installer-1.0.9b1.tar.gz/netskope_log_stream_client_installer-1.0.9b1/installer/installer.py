...
def display_help():
    ...
    - `--check-beta`        Include beta/alpha/rc versions in update checks.
    ...
    - **--check-beta**
        - Include beta, alpha, and release candidate versions in update checks.
...
def main():
    ...
    try:
        ...
        if "--no-version-check" not in sys.argv and "--help" not in sys.argv:
            ...
            # Check for beta versions if --check-beta flag is present
            include_prereleases = "--check-beta" in sys.argv
            check_for_updates(auto_upgrade=auto_upgrade, include_prereleases=include_prereleases)
