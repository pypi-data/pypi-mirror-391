...
class WindowsTrayActivator:
    ...
    def activate(self, app: AppInfo) -> Dict[str, Any]:  # pragma: no cover - Windows 特有
        ...
        if hwnd:
            ...
            if bring_result:
                ...
                self.steps.append(f"置前窗口失败，尝试其他方法")
            
            # 1.3 尝试 pywinauto
            if self.activate_with_pywinauto(process_name, app.name):
                return self._result(True, "通过 pywinauto 激活窗口")
            
            # 1.4 进程存在但无法激活，返回部分成功（不启动新进程！）
            self.steps.append("进程已运行但无法激活窗口")
            return self._result(True, f"{app.name} 已在运行（无法激活窗口，可能被最小化到托盘）")
    ...
    def bring_window_to_front(hwnd: int) -> bool:
        ...
        try:
            ...
            LOGGER.warning("置顶窗口失败 (hwnd=%s): %s", hwnd, exc)
            return False
        finally:
            if attached:
                try:
