...
class WindowsTrayActivator:
    ...
    def activate(self, app: AppInfo) -> Dict[str, Any]:  # pragma: no cover - Windows 特有
        ...
        if hwnd:
            ...
            if not is_visible:
                # 窗口不可见，很可能是托盘应用
                self.steps.append(f"检测到托盘应用（窗口不可见）")
                if self.activate_tray_app(hwnd):
                    return self._result(True, "成功激活托盘应用")
    ...
    def find_window_by_process(process_name: str):
        ...
        if all_windows:
            ...
            # 调试信息：显示前3个窗口的分数
            # for i, item in enumerate(all_windows[:3]):
            #     print(f"窗口 {i+1}: {item[1]} (score={score_window(item)})")
    ...
    def activate_tray_app(self, hwnd: int) -> bool:
        ...
        try:
            ...
            self.steps.append(f"托盘窗口状态: 可见={is_visible}, 最小化={is_iconic}")
            ...
            try:
                ...
                if fg_hwnd == hwnd:
                    ...
                    self.steps.append("托盘应用已显示（但未获得焦点）")
            ...
            try:
                ...
                self.steps.append("使用 BringWindowToTop 激活")
                return True
            except Exception as e:
    ...
    def simple_activate(self, hwnd: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                win32gui.SetForegroundWindow(hwnd)
                if win32gui.GetForegroundWindow() == hwnd:
                    self.steps.append("BringWindowToTop + SetForegroundWindow 成功")
                    return True
                self.steps.append("BringWindowToTop 成功（但未获得焦点）")
                return True
    ...
    def bring_window_to_front(self, hwnd: int) -> bool:
        ...
        try:
            ...
            # 2. 处理隐藏窗口（托盘应用）
            if not is_visible:
                self.steps.append("窗口不可见（托盘应用），正在显示")
                # 先尝试恢复窗口
                win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
                time.sleep(0.1)
                # 再显示窗口
