...
class WindowsTrayActivator:
        ...
        if hwnd:
            ...
            if not is_visible:
                ...
                if is_iconic:
                    ...
                    # 完全隐藏的托盘应用：尝试通过 Shell 唤起 UI（支持配置与启发式）
                    self.steps.append(f"{app.name} 正在系统托盘中运行（不可见）")
                    try_shell = bool(getattr(app, "relaunch_when_tray_hidden", False))
                    # 启发式：.lnk 或常见托盘 IM 类应用（qq/wechat/tim/钉钉）优先尝试
                    name_l = (app.name or "").lower()
                    path_l = (app.path or "").lower()
                    if path_l.endswith(".lnk") or any(key in name_l for key in ("qq", "wechat", "weixin", "tim", "钉钉")) or any(key in path_l for key in ("qq", "wechat", "weixin", "tim", "dingtalk")):
                        try_shell = True or try_shell

                    if try_shell:
                        self.steps.append("尝试通过 Shell start 重新唤起 UI")
                        try:
                            if self.shell_start(app.path):
                                time.sleep(0.8)
                                # 等待并查找窗口
                                if self.wait_for_window(process_name, timeout=5.0):
                                    hwnd2 = self.find_window_by_process(process_name)
                                    if hwnd2:
                                        self.steps.append(f"重新唤起后找到窗口 hwnd={hwnd2}，尝试置前")
                                        if self.bring_window_to_front(hwnd2):
                                            return self._result(True, "重新唤起并置前成功")
                                        # 置前不成功，尝试后备方法
                                        if self.simple_activate(hwnd2) or self.activate_by_alt_tab(hwnd2):
                                            return self._result(True, "重新唤起后通过后备方法激活")
                                        self.steps.append("重新唤起后仍未获得焦点")
                                else:
                                    self.steps.append("重新唤起后未检测到新窗口（可能仍在托盘）")
                        except Exception as e:
                            self.steps.append(f"Shell 重新唤起失败: {e}")
                    # 如果上述方式仍未成功，提示用户手动点击托盘
                    self.steps.append("提示：请在系统托盘中点击图标打开窗口")
                    return self._result(True, f"{app.name} 已在系统托盘运行（尝试唤起失败，需手动点击托盘图标）")
            ...
            try_shell_fallback = bool(getattr(app, "shell_fallback_on_fail", False))
            # 启发式：.lnk 或常见 IM 应用也尝试 Shell 兜底
            name_l = (app.name or "").lower()
            path_l = (app.path or "").lower()
            if path_l.endswith(".lnk") or any(key in name_l for key in ("qq", "wechat", "weixin", "tim", "钉钉")):
                try_shell_fallback = True or try_shell_fallback
    ...
    def find_window_by_process(process_name: str):
        ...
        # 需要过滤的窗口标题关键词（包括 QQ 的服务窗口）
        IGNORE_TITLES = {
            "缩略图", "thumbnail", "popup", "tooltip", "menu", "context",
            "qmaiservice", "qqexternal", "txguiservice", "qqservice",
            "service", "helper", "daemon"
        }
    ...
    def shell_start(app_path: str) -> bool:
        """通过 cmd 的 start 调用 Shell 打开应用/快捷方式（更接近 TS 的行为）。"""
        try:
            # 注意：start 是 cmd 的内建命令，需通过 cmd /c 调用
            # 第二个参数是空标题 "" 必须保留，以避免把路径当成标题
