...
class WindowsTrayActivator:
    ...
    def activate(self, app: AppInfo) -> Dict[str, Any]:  # pragma: no cover - Windows 特有
        ...
        if hwnd:
            ...
            if not is_visible:
                # 窗口不可见，很可能是托盘应用
                self.steps.append(f"检测到托盘应用（窗口不可见）: '{title}'")
                ...
                if tray_result:
                    ...
                    # 托盘激活失败，可能是不该显示的窗口
                    self.steps.append("托盘激活失败，应用可能已在后台运行")
                    return self._result(True, f"{app.name} 已在后台运行（无法显示窗口）")
