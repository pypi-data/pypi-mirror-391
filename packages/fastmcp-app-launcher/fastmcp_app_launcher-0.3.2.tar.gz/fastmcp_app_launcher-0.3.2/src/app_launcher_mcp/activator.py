        ...
        if hwnd:
            ...
            if not is_visible:
                ...
                if is_iconic:
                    ...
                    # 完全隐藏的托盘应用：可选地通过 Shell 尝试唤起 UI
                    self.steps.append(f"{app.name} 正在系统托盘中运行（不可见）")
                    if getattr(app, "relaunch_when_tray_hidden", False):
                        self.steps.append("尝试通过 ShellExecute 重新唤起 UI")
                        try:
                            # 使用现有的启动方法进行一次“软重启”以唤起 UI（多数单实例应用会显示现有实例）
                            if self.launch_process(app.path):
                                time.sleep(0.6)
                                # 等待并查找窗口
                                if self.wait_for_window(process_name, timeout=4.0):
            ...
            if getattr(app, "shell_fallback_on_fail", False):
