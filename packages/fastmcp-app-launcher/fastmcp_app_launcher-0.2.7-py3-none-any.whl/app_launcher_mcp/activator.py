...
class WindowsTrayActivator:
    ...
    def find_window_by_process(process_name: str):
        ...
        # 需要过滤的窗口标题关键词
        IGNORE_TITLES = {"缩略图", "thumbnail", "popup", "tooltip", "menu", "context"}
        ...
        def is_main_window(item):
            ...
            return has_caption and has_sysmenu
        
        # 优先返回可见的主窗口
        if visible_hwnds:
            main_windows = [item for item in visible_hwnds if is_main_window(item)]
            if main_windows:
                return main_windows[0][0]
            return visible_hwnds[0][0]
        elif hidden_hwnds:
            main_windows = [item for item in hidden_hwnds if is_main_window(item)]
            if main_windows:
                return main_windows[0][0]
            return hidden_hwnds[0][0]
        return None
