...
class Source(RaySource):
    ...
    def __init__(self, position_x=0.0, position_y=0.0, position_z=1.5e8):
    ...
    def rays(self, theta:Union[float, np.ndarray], phi:Union[float, np.ndarray]):
        ...
        if isinstance(theta, np.ndarray) or isinstance(phi, np.ndarray):
            if not isinstance(theta, np.ndarray) and isinstance(phi, np.ndarray):
                theta = theta * np.ones_like(phi)
            if isinstance(theta, np.ndarray) and not isinstance(phi, np.ndarray):
                phi = phi * np.ones_like(theta)
            
            # tuple_val = [(t,p) for t in theta for p in phi] # type: ignore
            tuple_val = list(zip(theta, phi))
            u_dir = list(map(vec_dir, tuple_val)) # type: ignore
            pt_o = [self.pt_origin for i in range(len(tuple_val))]
        else:
            u_dir = vec_dir([theta, phi])
            pt_o = self.pt_origin
        # print("size of u_dir = ", len(u_dir))
        return u_dir, pt_o