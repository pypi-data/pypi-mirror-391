...
class IdentityVerification:
    ...
    def exract_passport_info(self, passport, country):
        ...
        if country.upper() == 'SDN':
            ...
            try:
                ...
                id_infos = self.get_ocr_results(compressed_image_data, country='SDN')
                passport_text = id_infos[0].description
                logging.info(f"extracted passport text: {passport_text}\n")
                logging.info(f'----------------Time taken for vision passport: {time.time() - st} seconds\n')
                ...
                logging.info(f"passport details extracted: {json.dumps(passport_details, indent = 2, ensure_ascii= False)}\n")
                logging.info(
    ...
    def extract_ocr_info(self, data, video, country, report_names, back_img = None):
        ...
        if 'document' in report_names:
            logging.info("Generating Document Report\n")
        ...
        if 'facial_similarity_video' in report_names:
            logging.info("Generating Facial Similarity Report\n")
            if video:
