...
def example_basic_wal_usage(db_path: str | Path = "example_data.json") -> None:
    ...
    db = BearBase(db_path, storage="json", enable_wal=True)
    ...
    records = [{"id": i, "name": f"User{i}", "email": f"user{i}@example.com"} for i in range(1000)]
...
def example_high_throughput_config(db_path: str | Path = "high_throughput.json") -> None:
    ...
    events = [{"id": i, "timestamp": i * 1000, "event_type": "click", "data": f"event_{i}"} for i in range(5000)]
...
def example_custom_wal_config(db_path: str | Path = "custom_config.json") -> None:
    ...
    logs = [{"id": i, "message": f"Log entry {i}"} for i in range(1000)]
...
def example_manual_checkpoint(db_path: str | Path = "manual_checkpoint.json") -> None:
    ...
    # Insert batch 1
    batch1 = [{"id": i, "value": f"batch1_{i}"} for i in range(500)]
    table.insert_all(batch1, flush=False)  # Don't flush yet

    # Insert batch 2
    batch2 = [{"id": i + 500, "value": f"batch2_{i}"} for i in range(500)]
...
def example_crash_recovery(db_path: str | Path = "recovery_example.json") -> None:
    ...
    """
    from bear_dereth.datastore.wal_helper import WALHelper

    wal_path = Path(db_path).parent / "batch_data.wal"

    # Phase 1: Write data with WAL
    db = BearBase(db_path, storage="json", enable_wal=True, wal_dir=str(Path(db_path).parent))
    ...
    db2 = BearBase(db_path, storage="json")
...
# Example using SettingsManager with WAL
def example_settings_manager_with_wal() -> None:
    """Use SettingsManager with WAL enabled for high-volume config updates.

    Note: WAL is disabled by default for SettingsManager since most
    settings updates are small and infrequent.
    """
    from bear_dereth.config.settings_manager import SettingsManager

    # Enable WAL for settings that change frequently
    settings = SettingsManager(
        "app_config",
        enable_wal=True,  # WAL disabled by default, explicitly enable
        flush_mode="buffered",
        flush_interval=5.0,
    )

    # Bulk update many settings
    for i in range(100):
        settings.set(f"setting_{i}", f"value_{i}")

    print(f"Updated {len(settings)} settings with WAL enabled")
    settings.close()
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("\n7. SettingsManager with WAL")
        print("-" * 60)
        example_settings_manager_with_wal()
