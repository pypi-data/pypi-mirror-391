...
```python
from bear_dereth.math import clamp, lerp, map_range, smoothstep
...
```python
from bear_dereth.math import INFINITE, clamp
...
- Re-exported symbols live in `bear_dereth.math.__all__`, so `from bear_dereth import math` exposes the helpers directly.
