...
def test_wal_recovery_idempotence_prevents_infinite_growth(tmp_path: Path):
    ...
    initial_wal_size = wal_file.stat().st_size if wal_file.exists() else 0
    ...
    wal_size_after_first_recovery = wal_file.stat().st_size if wal_file.exists() else 0
    ...
    users3 = db3.table("users")
    users3.wal_helper.recover_from_wal(users3)

    # WAL should still be empty/cleared - NOT GROWING!
    wal_size_after_second_recovery = wal_file.stat().st_size if wal_file.exists() else 0
    ...
    assert len(users3) == 2
    user1 = users3.get(id=1).first()
    assert user1["active"] is True

    user2 = users3.get(id=2).first()
    assert user2["active"] is False
...
def test_wal_recovery_with_updates_doesnt_relog(tmp_path: Path):
    ...
    wal_content_before = wal_file.read_text() if wal_file.exists() else ""
    operations_before = len([line for line in wal_content_before.splitlines() if "UPDATE" in line])

    db.close()

    # First recovery
    db2 = BearBase(str(db_path), storage="json", enable_wal=True, wal_dir=str(wal_dir), flush_mode="immediate")
    table2 = db2.table("data")
    ...
    wal_content_after = wal_file.read_text() if wal_file.exists() else ""
    operations_after = len([line for line in wal_content_after.splitlines() if "UPDATE" in line])
