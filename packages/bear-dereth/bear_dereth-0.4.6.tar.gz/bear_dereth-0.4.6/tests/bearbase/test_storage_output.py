...
def test_generate_sample_output_files(
    ...
    # Generate MsgPack sample
    msgpack_path = data_dir / "sample.msgpack"
    msgpack_storage = MsgPackStorage(msgpack_path, file_mode="w+b")
    msgpack_storage.write(get_unified_data)
    msgpack_storage.close()
    ...
    assert msgpack_path.stat().st_size > 0
