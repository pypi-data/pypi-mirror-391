...
from bear_dereth.query import QueryMapping, query as query_mapping
...
def test_wal_update_batch_basic(wal_db: tuple[BearBase, Path], query: QueryMapping):
    ...
    if table.wal_helper:
        table.wal_helper._wal.clear()

    # Update records
    updates = [{"active": False}]
    updated_count = table.update_all(updates, cond=query.id == 1, flush=False)
...
def test_wal_only_committed_transactions_recovered(tmp_path: Path):
    ...
    from bear_dereth.data_structs.wal import Operation, WriteAheadLog
    ...
    assert len(table) == 2
    all_records = table.all(list_recs=True)
    names = {r["name"] for r in all_records}
...
def test_wal_mixed_operations_recovery(tmp_path: Path):
...
def test_wal_checkpoint_clears_wal(wal_db: tuple[BearBase, Path]):
    ...
    wal_content_after = wal_path.read_text().strip()
...
def test_buffered_vs_immediate_performance_difference(tmp_path: Path):
    ...
    if table_immediate.wal_helper:
        table_immediate.wal_helper._wal.stop()
    db_immediate.close()

    # Test BUFFERED mode
    db_buffered = BearBase(
    ...
    if table_buffered.wal_helper:
        table_buffered.wal_helper._wal.stop()
    db_buffered.close()
...
def test_buffered_recovery_after_incomplete_flush(tmp_path: Path):
    """Test recovery when buffer has unflushed data (simulating crash)."""
    db_path = tmp_path / "crash_buffered.json"
    wal_path = tmp_path / "users.wal"
    ...
    if table.wal_helper:
        ...
        table.wal_helper._wal._stop_event.set()
        if table.wal_helper._wal._thread:
            table.wal_helper._wal._thread.join(timeout=0.001)  # Quick timeout
    db.close()

    # In a real crash scenario, unflushed buffer data would be lost
...
def test_buffered_config_presets(tmp_path: Path):
    ...
    # Test immediate preset
    db_immediate = BearBase(
        str(tmp_path / "immediate.json"),
        storage="json",
        enable_wal=True,
        wal_config=WALConfig.immediate(),
    )
