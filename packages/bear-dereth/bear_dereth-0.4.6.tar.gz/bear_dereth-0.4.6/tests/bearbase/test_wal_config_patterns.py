...
def test_wal_config_pattern_full_config(tmp_path: Path):
    """Test passing full WALConfig object."""
    db_path = tmp_path / "test.json"

    config = WALConfig.high_throughput()
    db = BearBase(str(db_path), storage="json", enable_wal=True, wal_config=config)
...
def test_wal_config_pattern_kwargs_only(tmp_path: Path):
...
def test_wal_config_pattern_mixed_override(tmp_path: Path):
    """Test passing config + kwargs (kwargs should override)."""
    db_path = tmp_path / "test.json"

    base_config = WALConfig.buffered(flush_interval=0.1)
...
def test_wal_config_always_created(tmp_path: Path):
...
def test_wal_config_doesnt_leak_to_storage(tmp_path: Path):
...
def test_wal_config_immediate_mode(tmp_path: Path):
