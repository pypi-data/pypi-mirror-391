...
class RequestBase:
    ...
    def request(self, method, path, **kwargs):
        ...
        if default_kwargs.get('data') or default_kwargs.get('params'):  # 如果data有值json序列化
            data = json.dumps(default_kwargs.get('data',default_kwargs.get('params')))
        if str(method).upper() == 'GET':
            default_kwargs.update({'params': data})
        else:
