...
class CreditScoreClient:
    ...
    def evaluate_batch(
        ...
        """
        Evalúa múltiples solicitantes en lotes seguros (solo tiers premium).
        El SDK se encarga automáticamente del chunking, concatenación y rate limits.
        """
        ...
        # Tamaño seguro para Vercel (evita timeouts)
        CHUNK_SIZE = 10
        ...
        for i, chunk in enumerate(applicant_chunks):
            ...
            # llamada segura (el decorador retry_on_failure ya funciona)
            response = self._make_request(self.ENDPOINT, payload)

            # reconstrucción de índices originales
            offset = i * CHUNK_SIZE

            for key, decision in response["decisions"].items():
                original_index = offset + int(key)

                all_decisions[str(original_index)] = decision
                all_scores[str(original_index)] = response["scores"][key]

            last_threshold = response.get("threshold", last_threshold)

            # CRÍTICO: Pausa para evitar 429 y timeouts
            time.sleep(1.0)
