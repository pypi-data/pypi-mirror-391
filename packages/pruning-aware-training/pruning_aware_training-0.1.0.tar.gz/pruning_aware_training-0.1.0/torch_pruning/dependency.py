...
class Dependency(object):
    ...
    def __call__(self, idxs: list):
        ...
        if len(idxs) > 0 and isinstance(idxs[0], _helpers._HybridIndex):
            idxs = _helpers.to_plain_idxs(
                idxs)  # hybrid indices include root indices. We need to remove them and only pass the plain indices to the handler
...
class Group(object):
    ...
    def prune(self, idxs=None, record_history=True):
        ...
        if idxs is not None:  # prune the group with user-specified indices
            ...
            new_group = self._DG.get_pruning_group(module, pruning_fn,
                                                   idxs)  # create a new group with the specified indices
            new_group.prune()
...
class DependencyGraph(object):
        ...
        for layer_type_or_instance in self.CUSTOMIZED_PRUNERS.keys():
            for m in self.model.modules():
                ...
                if (m == layer_type_or_instance) or (
                        not isinstance(layer_type_or_instance, torch.nn.Module) and isinstance(m,
                                                                                               layer_type_or_instance)):
    ...
    def get_pruning_group(
        ...
        for i in range(len(merged_group)):
            ...
            merged_group[i] = _helpers.GroupItem(merged_group[i].dep,
                                                 idxs)  # transform _helpers._HybridIndex to plain index
    ...
    def _detect_unwrapped_parameters(self, unwrapped_parameters):
        ...
        for m in self.model.modules():
            ...
            if (
                    op_type in prunable_module_types and op_type != ops.OPTYPE.ELEMENTWISE) or m.__class__ in self.CUSTOMIZED_PRUNERS.keys() or m in self.CUSTOMIZED_PRUNERS.keys():
        ...
        if len(unwrapped_detected) > 0 and self.verbose:
            warning_str = "Unwrapped parameters detected: {}.\n Torch-Pruning will prune the last non-singleton dimension of these parameters. If you wish to change this behavior, please provide an unwrapped_parameters argument.".format(
        ...
        for p in unwrapped_detected:
            ...
            if pruning_dim is not None:
                unwrapped_parameters.append(_helpers.UnwrappedParameters(parameters=p,
    ...
    def _build_dependency(self, module2node):
        ...
        for _, node in module2node.items():
            ...
            # --- BEGIN: broadcast-aware tweak for element-wise nodes ---
            is_elem = (node.type == ops.OPTYPE.ELEMENTWISE)

            # Pre-read input channel counts if element-wise (typical 2-input Add/Mul)
            elem_in_nodes = list(node.inputs) if is_elem else []
            elem_in_ch = []
            if is_elem and len(elem_in_nodes) >= 2:
                for _in in elem_in_nodes:
                    # Prefer precise local inference; fall back to generic out_channels
                    ch = self.infer_channels_between(_in, node)
                    if ch is None:
                        ch = self.get_out_channels(_in)
                    elem_in_ch.append(ch)
                # Decide whether it's a broadcast case: exactly one side has C==1 and the other >1
                has_broadcast = (
                    elem_in_ch[0] == 1 and (elem_in_ch[1] is not None and elem_in_ch[1] > 1)
                ) or (
                    elem_in_ch[1] == 1 and (elem_in_ch[0] is not None and elem_in_ch[0] > 1)
                )
            else:
                has_broadcast = False
            # --- END: broadcast-aware pre-read ---

            # Backward deps: inputs <- node
            for in_node in node.inputs:
                ...
                # If element-wise broadcast: skip coupling into the singleton side
                if has_broadcast and is_elem and len(elem_in_nodes) >= 2:
                    # identify index of this input
                    i = 0 if in_node is elem_in_nodes[0] else (1 if in_node is elem_in_nodes[1] else None)
                    if i is not None:
                        ch_i = elem_in_ch[i]
                        ch_other = elem_in_ch[1 - i]
                        # Skip ONLY when this side is singleton and the other side is clearly multi-channel
                        if ch_i == 1 and (ch_other is not None and ch_other > 1):
                            # do NOT add the dependency for this singleton input
                            continue
            ...
            # Forward deps: node -> outputs (unchanged)
            for out_node in node.outputs:
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        if forward_fn is not None:
            if example_inputs[0].shape[1] == 3:
                ...
                if 'seg_fw' in out.keys():
                    out = out['seg_fw']
                elif 'flows_fw' in out.keys():
                    out = out['flows_fw']
                else:
                    raise ValueError("Key has no found")
            else:
        ...
        # For DVNR, takes 4 levels but ignore last 2 outputs
        if len(out) == 6:
            out = out[:4]
        ...
        def create_node_if_not_exists(grad_fn):
            ...
            if module is None:  # a new module
                ...
                if not hasattr(grad_fn, "name"):
                    ...
                    if hasattr(grad_fn, '_saved_start') and hasattr(grad_fn, '_saved_end') and hasattr(grad_fn,
                                                                                                       '_saved_step') and hasattr(
                            grad_fn, '_saved_dim'):
    ...
    def _update_concat_index_mapping(self, cat_node: Node):
        ...
        if hasattr(cat_node.grad_fn,
                   '_saved_dim') and cat_node.grad_fn._saved_dim != 1:  # this only works for Pytorch>=1.12
            return
    ...
    def _update_split_index_mapping(self, split_node: Node):
        ...
        if hasattr(split_node.grad_fn, '_saved_dim'):  # this only works for Pytorch>=1.12
            ...
            if hasattr(split_node.grad_fn, '_saved_self_sym_sizes'):
                if split_node.grad_fn._saved_dim < len(
                        split_node.grad_fn._saved_self_sym_sizes) and split_node.grad_fn._saved_dim != 1:
                    return
            else:
                THRESHOLD = 20
                ...
                for dep in node.dependencies:
                    ...
                    if dep.target == in_node:
                        if node.enable_index_mapping:
                            dep.index_mapping[0] = (
                                _helpers._GQAIndexMapping(repeat=repeat, reverse=True, head_dim=head_dim))
                            addressed_dep.append(dep)
                            break
            ...
            for i, in_node in enumerate(node.inputs):
                for dep in in_node.dependencies:
                    if dep.target == node:
                        ...
                        if node.enable_index_mapping:
                            dep.index_mapping[0] = (
                                _helpers._GQAIndexMapping(repeat=repeat, reverse=False, head_dim=head_dim))
                            addressed_dep.append(dep)
                            break
