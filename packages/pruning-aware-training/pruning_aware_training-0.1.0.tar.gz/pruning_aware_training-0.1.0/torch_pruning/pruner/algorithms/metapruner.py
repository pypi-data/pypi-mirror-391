                    ...
                    for group, ch_groups, group_size, target_pruning_ratio, imp in records:
                        ...
                        if not _is_atten or self.prune_head_dims:
                            ...
                            if len(_pruning_indices) > 0 and self.max_pruning_ratio < 1:
                                # Keep at least minimum amout of channels:
                                # min(get_channel_fn(module) * (1 - self.max_pruning_ratio), self.min_amount_of_channels)
                                # As of now, self.min_amount_of_channels isn't implemented. Instead, it is haed coded to 8.
                                min_amount_of_ch = int(min(get_channel_fn(module) * (1 - self.max_pruning_ratio), 8))
                                n_pruned = len(_pruning_indices)
                                n_ch = get_channel_fn(module)
                                while n_ch - n_pruned < min_amount_of_ch:
