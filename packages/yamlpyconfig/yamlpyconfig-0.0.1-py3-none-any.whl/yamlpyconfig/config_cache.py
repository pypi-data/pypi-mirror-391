...
class ConfigCache:
    ...
    def __init__(self, base_config: Optional[Dict[str, Any]] = None) -> None:
        self._lock = RLock()
        self._base_config: Dict[str, Any] = dict(base_config or {})
        self._order: List[str] = []  # imports 顺序
        self._items: Dict[str, NacosConfigCacheItem] = {}

        # 对外暴露的最终配置快照（不可变）
        self._merged_config: Mapping[str, Any] = MappingProxyType(dict(self._base_config))
    ...
    def _rebuild_merged_unlocked(self) -> None:
        ...
        for data_id in self._order:
            ...
            if not isinstance(cfg, dict):
                ...
                continue
            ConfigMerge.deep_merge(merged, cfg)
