...
def zmq_sync_socket(ctx: zmq.Context, path: str, sock_type: int) -> zmq.Socket:
    """Create a ZMQ socket directly (not a context manager)."""
    return make_zmq_socket(ctx, path, sock_type)


@contextlib.contextmanager
def zmq_sync_socket_ctx(path: str, sock_type: int) -> Iterator[zmq.Socket]:
