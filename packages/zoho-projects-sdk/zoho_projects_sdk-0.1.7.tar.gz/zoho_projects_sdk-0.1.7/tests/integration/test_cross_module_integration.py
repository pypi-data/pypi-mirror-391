...
@pytest.mark.asyncio
async def test_project_bug_integration(
    zoho_client: ZohoProjects, mock_api_client: Mock
) -> None:
    """Test the integration between projects and bugs modules."""
    # Mock the API responses for both projects and bugs
    project_response = {
        "projects": [
            {
                "id": 456,
                "name": "Bug Integration Test Project",
                "status": "active",
                "description": "A project for testing bug integration",
            }
        ]
    }
    bugs_response = {
        "bugs": [
            {
                "id": 1,
                "name": "Integration Test Bug",
                "status": "open",
                "priority": "high",
                "description": "A bug for integration testing",
            }
        ]
    }
    ...
    assert project.name == "Bug Integration Test Project"

    # Test fetching issues for the same project
    bugs = await zoho_client.issues.get_all(456)
    assert len(bugs) == 1
    assert isinstance(bugs[0], Issue)
    assert bugs[0].id == 1
    assert bugs[0].name == "Integration Test Bug"
...
async def test_cross_module_data_consistency(
    ...
    bugs_response = {
        "bugs": [
            {
                "id": 1,
                "name": "Consistency Test Bug",
                "status": "open",
                "priority": "high",
                "description": "A bug for consistency testing",
                "created_time": "2023-01-01T10:00:00Z",
                "updated_time": "2023-01-01T10:00:00Z",
            }
        ]
    ...
    # Test fetching issues for the same project
    bugs = await zoho_client.issues.get_all(project_id)
    assert len(bugs) == 1
    assert isinstance(bugs[0], Issue)
    assert bugs[0].name == "Consistency Test Bug"
...
async def test_multiple_module_concurrent_access(
    ...
    bug_response = {
        "bugs": [
            {"id": 1, "title": "Concurrent Bug", "status": "open", "priority": "high"}
        ]
    }
    ...
    bug_task = asyncio.create_task(zoho_client.issues.get_all(123))

    # Wait for all tasks to complete
    project, tasks, bugs = await asyncio.gather(project_task, task_task, bug_task)
...
async def test_cross_module_pagination_consistency(
    ...
    # Create multiple pages of data for both tasks and bugs
    tasks_page_1 = {
        "tasks": [
            {
                "id": 1,
    ...
    bugs_page_1 = {
        "bugs": [
            ...
            {"id": 2, "title": "Bug 2", "status": "resolved", "priority": "medium"},
        ]
    }
    bugs_page_2 = {
        "bugs": [
            {"id": 3, "title": "Bug 3", "status": "open", "priority": "critical"},
            {"id": 4, "title": "Bug 4", "status": "closed", "priority": "low"},
        ]
    }
...
async def test_cross_module_data_relationship_validation(
    ...
    # Bugs associated with the same project
    bugs_response = {
        "bugs": [
            {
                "id": 11,
                "name": "Bug for Project 303",
                "status": "open",
                "priority": "high",
                "created_time": "2023-01-01T10:00:00Z",
                "updated_time": "2023-01-01T10:00:00Z",
    ...
    bugs = await zoho_client.issues.get_all(project_id)
    assert len(bugs) == 1
    assert bugs[0].name == "Bug for Project 303"
    ...
    assert bugs[0].updated_time == "2023-01-01T10:00:00Z"
...
async def test_cross_module_data_serialization_consistency(
    ...
    bug_response = {
        "bugs": [
            {
                "id": 1,
                "name": "Serialization Test Bug: ñáéíóú 中文 🌍",
                "status": "open",
                "priority": "critical",
                "description": "Bug with special chars: @#$%^&*()",
            }
        ]
    }
    ...
    assert "ñáéíóú" in bugs[0].name
    assert "中文" in bugs[0].name
    assert "🌍" in bugs[0].name
