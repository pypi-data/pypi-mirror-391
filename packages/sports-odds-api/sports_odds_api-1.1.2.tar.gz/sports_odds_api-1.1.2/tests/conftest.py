...
from sports_odds_api import SportsGameOdds, AsyncSportsGameOdds, DefaultAioHttpClient
from sports_odds_api._utils import is_dict
...
logging.getLogger("sports_odds_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[SportsGameOdds]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with SportsGameOdds(base_url=base_url, api_key_header=api_key_header, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncSportsGameOdds]:
    ...
    async with AsyncSportsGameOdds(
        base_url=base_url, api_key_header=api_key_header, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
