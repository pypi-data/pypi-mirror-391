...
def download_part_merge(file_path: pathlib.Path, temp_path: pathlib.Path, file_hash, file_name, total_parts):
    ...
    try:
        with target_path.open("wb") as fwriter:
...
# --------- executor_download ------------
def executor_download(save_path: str, **extra_kwargs):
    ...
    # 下载 meta 信息
    try:
        executor_meta_download(save_path, dir_path=extra_kwargs.get("dir_path"))
    except AssertionError as e:
        print(e)
        return
    except Exception as e:
    ...
    # 下载数据
    try:
        executor_data_download_with_pbar(save_path)
    except AssertionError as e:
        print(e)
    except Exception as e:
        logger.error(e)
...
def executor_data_download_with_pbar(save_path: str):
    import threading
    import pathlib
    ...
    proc_path = pathlib.Path(save_path) / "proc"
    ...
    print(f"cmd_envs, proc_path: {proc_path.absolute()}")
    print("")
...
def executor_meta_download(save_path: str, **extra_kwargs):
    config = Application()
    meta_path = pathlib.Path(save_path) / "meta"
...
def executor_user_signin():
    ...
    # TODO: 使用下载服务调用三方登录
    assert resp.json().get("code") == 200, resp.text
