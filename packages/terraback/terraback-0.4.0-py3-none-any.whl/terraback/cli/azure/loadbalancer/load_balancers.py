...
def _generate_lb_subresource_imports(lb_data: List[Dict[str, Any]], output_dir: Path) -> None:
    ...
    # Write sub-resource import files
    if backend_pool_imports:
        import json
        pool_file = output_dir / "lb_backend_pool_import.json"
        with open(pool_file, 'w') as f:
            json.dump(backend_pool_imports, f, indent=2)
    
    if probe_imports:
        import json
        probe_file = output_dir / "lb_probe_import.json"
        with open(probe_file, 'w') as f:
            json.dump(probe_imports, f, indent=2)
    
    if rule_imports:
        import json
        rule_file = output_dir / "lb_rule_import.json"
        with open(rule_file, 'w') as f:
            json.dump(rule_imports, f, indent=2)
    
    if nat_rule_imports:
        import json
        nat_file = output_dir / "lb_nat_rule_import.json"
