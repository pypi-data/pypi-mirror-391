...
def license_status():
    ...
    if status["has_license"]:
        ...
        if status.get("is_trial"):
            typer.secho(
                f"\n  Free Trial Active! Days remaining: {status.get('days_remaining', 0)}",
                fg=typer.colors.YELLOW,
            )
        if status.get("is_beta"):
            typer.secho(
                f"\n  Beta Program Active! Days remaining: {status.get('days_remaining', 0)}",
                fg=typer.colors.BLUE,
            )
            if status.get("beta_user_id"):
                typer.echo(f"  - Beta User ID: {status.get('beta_user_id')}")
            typer.echo(f"  - Access Level: Professional (Full Features)")
            typer.echo(f"  - Join our community: terraback.slack.com")
        ...
        typer.echo("\nOr join the Beta Program (90 days + community):")
        typer.echo("  terraback beta register <your-email>")
