...
class TerraformImporter:
  ...
  id = "{resource_id}"
    ...
    def bulk_import(
        ...
        if unmatched > 0:
            typer.echo(f"  {unmatched} resources cannot be imported due to missing .tf files")
        ...
        try:
            ...
            plan_result = subprocess.run(
                ["terraform", "plan", "-no-color"] + plan_vars,
                cwd=str(self.terraform_dir),
                capture_output=True,
                text=True,
        ...
        if adds_count > 0 or changes_count > 0 or destroys_count > 0:
            if is_initial_import and destroys_count == 0 and adds_count <= 5 and changes_count <= 10:
                # This is expected for initial import - terraform may want to add/change resources
                # Allow reasonable numbers of adds/changes for initial import, but no destroys
                typer.echo(f"Initial import detected: {imports_count} to import, {adds_count} to add, {changes_count} to change")
                typer.echo("Proceeding with import (configuration drift is normal for first-time import)...")
            else:
                typer.secho("\nSAFETY CHECK FAILED", fg="red", bold=True)
                typer.echo(f"Plan shows operations beyond imports:")
                typer.echo(f"  • {imports_count} to import OK")
                if adds_count > 0:
                    typer.echo(f"  • {adds_count} to add ERROR")
                if changes_count > 0:
                    typer.echo(f"  • {changes_count} to change ERROR")
                if destroys_count > 0:
                    typer.echo(f"  • {destroys_count} to destroy ERROR")
                
                typer.echo("\nImport blocked for safety. The plan must show ONLY imports.")
                typer.echo("This usually means the Terraform configurations don't match the actual resources.")
                typer.echo("\nPossible solutions:")
                typer.echo("1. Re-run 'terraback azure scan-all' to regenerate configurations")
                typer.echo("2. Manually adjust the .tf files to match your actual resources")
                typer.echo("3. Use sequential import mode: terraback azure import --method sequential")
