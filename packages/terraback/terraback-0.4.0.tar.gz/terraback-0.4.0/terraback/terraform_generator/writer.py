...
class AutoDiscoveryTemplateLoader:
    ...
    def get_template_path(self, resource_type: str, provider: str) -> str:
        ...
        Prefers import templates, then v2 templates when available.
        """
        provider_dir = self.template_dir / provider
        if not provider_dir.is_dir():
            raise FileNotFoundError(f"Provider directory not found: {provider_dir}")

        # First, try to find import template (highest priority)
        template_import_name = f"{resource_type}_import.tf.j2"
        found_import_templates = list(provider_dir.rglob(template_import_name))
        
        if found_import_templates:
            relative_path = found_import_templates[0].relative_to(self.template_dir)
            return str(relative_path).replace('\\', '/')

        # Second, try to find v2 template
        template_v2_name = f"{resource_type}_v2.tf.j2"
        found_v2_templates = list(provider_dir.rglob(template_v2_name))
        
        if found_v2_templates:
            relative_path = found_v2_templates[0].relative_to(self.template_dir)
            return str(relative_path).replace('\\', '/')
