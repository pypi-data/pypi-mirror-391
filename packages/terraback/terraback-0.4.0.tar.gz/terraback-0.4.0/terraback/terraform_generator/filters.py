...
def is_gcp_computed_field(field_name: str) -> bool:
    ...
    """
    computed_fields = {
        # Universal computed fields
        'id', 'self_link', 'url', 'creation_timestamp', 'time_created', 
        'updated', 'fingerprint', 'etag', 'project', 'zone', 'region',
        
        # Storage-specific computed fields
        'bucket_policy_only', 'time_created', 'updated', 'metageneration',
        
        # Compute-specific computed fields  
        'instance_id', 'machine_type', 'status', 'cpu_platform',
        'label_fingerprint', 'metadata_fingerprint', 'tags_fingerprint',
        
        # Network-specific computed fields
        'ipv4_range', 'network_id', 'subnetwork_id', 'gateway_address',
        
        # Common read-only attributes across all GCP resources
        'kind', 'creation_timestamp', 'description_fingerprint'
    }
    
    # Convert field name to lowercase for comparison
    field_lower = field_name.lower().strip().strip('"').strip("'")
    
    # Remove common prefixes/suffixes that don't affect the field type
    clean_field = field_lower.replace('_fingerprint', '').replace('_timestamp', '')
    
    return field_lower in computed_fields or any(
        computed in field_lower for computed in [
            'fingerprint', 'timestamp', '_link', '_url', '_id'
        ]
    )
