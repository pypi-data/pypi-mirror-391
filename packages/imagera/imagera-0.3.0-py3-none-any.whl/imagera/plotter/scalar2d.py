...
class Scalar2d:
    ...
    def draw(self, ax, *, params: PlotParams, norm=None,
             annotate_xy=None, plot_label=None, with_colorbar=False)-> Tuple[plt.AxesImage, Optional[Colorbar]]:
        ...
        if params.show_lattice and (params.lattice_pitch is not None and params.lattice_pitch > 0):
            xmin, xmax, ymin, ymax = extent
            pitch = float(params.lattice_pitch)
            ...
            ax.vlines(xs, ymin, ymax, colors=params.lattice_color, linewidth=0.5, alpha=1.0, zorder=5)
            ax.hlines(ys, xmin, xmax, colors=params.lattice_color, linewidth=0.5, alpha=1.0, zorder=5)
        return im, cbar