...
# ============================================================
# STRATEGY PARAMETER MAPPING (centralized configuration)
# ============================================================
STRATEGY_PARAMS_MAP = {
    'iv_lean': {
        'signature': ['z_score_entry', 'z_score_exit', 'lookback_period'],
        'params': [
            {'key': 'z_score_entry',    'code': 'Z',  'format': lambda x: f"{x:.1f}"},
            {'key': 'z_score_exit',     'code': 'E',  'format': lambda x: f"{x:.2f}"},
            {'key': 'lookback_period',  'code': 'L',  'format': lambda x: f"{int(x)}"},
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    
    'iron_condor': {
        'signature': ['wing_width', 'body_width'],
        'params': [
            {'key': 'wing_width',       'code': 'WW', 'format': lambda x: f"{int(x)}"},
            {'key': 'body_width',       'code': 'BW', 'format': lambda x: f"{int(x)}"},
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'profit_target',    'code': 'PT', 'format': lambda x: f"{int(x*100)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    ...
    'credit_spread': {
        ...
        'params': [
            {'key': 'spread_width',     'code': 'SW', 'format': lambda x: f"{int(x)}"},
            {'key': 'delta_target',     'code': 'D',  'format': lambda x: f"{int(x*100)}"},
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'profit_target',    'code': 'PT', 'format': lambda x: f"{int(x*100)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    ...
    'butterfly': {
        ...
        'params': [
            {'key': 'wing_width',       'code': 'WW', 'format': lambda x: f"{int(x)}"},
            {'key': 'center_strike',    'code': '',   'format': lambda x: str(x)},  # ATM/OTM/delta
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    ...
    'calendar_spread': {
        ...
        'params': [
            {'key': 'front_dte',        'code': 'FDT', 'format': lambda x: f"{int(x)}"},
            {'key': 'back_dte',         'code': 'BDT', 'format': lambda x: f"{int(x)}"},
            {'key': 'strike_selection', 'code': '',    'format': lambda x: str(x)},
            {'key': 'delta_target',     'code': 'D',   'format': lambda x: f"{int(x*100)}"},
            {'key': 'position_size_pct','code': '',    'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    ...
    'diagonal_spread': {
        ...
        'params': [
            {'key': 'front_dte',        'code': 'FDT', 'format': lambda x: f"{int(x)}"},
            {'key': 'back_dte',         'code': 'BDT', 'format': lambda x: f"{int(x)}"},
            {'key': 'strike_offset',    'code': 'SO',  'format': lambda x: f"{int(x)}"},
            {'key': 'delta_target',     'code': 'D',   'format': lambda x: f"{int(x*100)}"},
            {'key': 'position_size_pct','code': '',    'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    ...
    'straddle': {
        'signature': ['dte_target'],
        'params': [
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'strike_selection', 'code': '',   'format': lambda x: str(x)},  # ATM/delta
            {'key': 'delta_target',     'code': 'D',  'format': lambda x: f"{int(x*100)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
    ...
    'strangle': {
        ...
        'params': [
            {'key': 'call_delta',       'code': 'CD', 'format': lambda x: f"{int(x*100)}"},
            {'key': 'put_delta',        'code': 'PD', 'format': lambda x: f"{int(x*100)}"},
            {'key': 'dte_target',       'code': 'DT', 'format': lambda x: f"{int(x)}"},
            {'key': 'position_size_pct','code': '',   'format': lambda x: f"{x:.2f}".rstrip('0').rstrip('.')},
        ]
    },
...
def detect_strategy_type(config):
    ...
    """
    for strategy_type, strategy_def in STRATEGY_PARAMS_MAP.items():
        signature = strategy_def['signature']
        # Check if all signature parameters are present
...
def format_params_string(config):
    ...
    - Automatic strategy detection and formatting via STRATEGY_PARAMS_MAP
    ...
    # ═══════════════════════════════════════════════════════════════════════
    strategy_type = detect_strategy_type(config)
    
    if strategy_type and strategy_type in STRATEGY_PARAMS_MAP:
        strategy_def = STRATEGY_PARAMS_MAP[strategy_type]
        parts = []
        
        # Format strategy-specific parameters
        for param_def in strategy_def['params']:
            key = param_def['key']
            code = param_def['code']
            format_func = param_def['format']
...
__all__ = [
    ...
    'STRATEGY_PARAMS_MAP', 'detect_strategy_type', 'format_params_string',
