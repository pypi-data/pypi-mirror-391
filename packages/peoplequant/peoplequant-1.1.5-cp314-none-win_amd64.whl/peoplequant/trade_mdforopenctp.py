...
class PeopleQuantApi():
    ...
    def get_symbol_order(self,InstrumentID:str = "", OrderStatus:str = "",CombOffsetFlag:str="",OrderMemo:str="",wait_return=False) -> List[Order]:
        '''
        获取合约委托单
        Ags:
    ...
    def check_order(self,order,block=True,n_price_tick=1,che_time=0,order_info='无',signal_price=float('nan'),order_close_chan=True,HedgeFlag:str="1",WaitReturn=False,OrderMemo="",**kw):
        '''
        检查委托单,并获取委托单成交结果
        ...
        if isinstance(order,Order): #报单成功发向交易所
            ...
            while True:
                ...
                if order["OrderStatus"] not in finished or order["VolumeTraded"] != sum( trade["Volume"] for trade in trades_list): #平昨单是否完成
                    ...
                    (kaiping in ["kaikong","pingduo"] and quote["AskPrice1"] <= order["LimitPrice"]-PriceTick*n_price_tick)) 
                    and last_price != quote.LastPrice == quote.LastPrice 
                        or (trading_night_time and equal_time and (ctp_time >= night_time > time(20) or time(8) > ctp_time >= night_time))
                    ):
                        self.cancel_order(orderid)  #等待撤单完成，防止重复撤单
                        last_price = quote.LastPrice
                else: break
