...
def Cta(error_queue=None,**kw):  
    try:
        ...
        if (DingDing or QQemail or WeChat) and "backtest" not in kw:
        ...
        if isinstance(DingChan,TqChan): 
            ln = len(e)
            lln = 2000 if "msg" not in  kw else kw["msg"]
            num = int(ln/lln) + 1 if ln/lln - int(ln/lln) > 0 else int(ln/lln)
            for i in range(1,num+1):
                DingChan.send_nowait(f"第{i}部分,共{num}部分\n\n" + e[(i-1)*lln:lln*i])
            DingChan.send_nowait(e)
            if isinstance(DingChan,zhuchannel.ThreadChan): 
                DingChan.send_nowait(content)
                while not DingChan.empty():tm.sleep(5)
    else:
