...
var m = f.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, n = Object.prototype.hasOwnProperty, p$1 = { key: true, ref: true, __self: true, __source: true };
function q(c, a, k) {
  ...
  return { $$typeof: g$1, type: c, key: e, ref: l, props: d, _owner: m.current };
}
...
const name = "@deephaven/js-plugin-python-remote-file-source";
const version = "0.1.2";
const pkg = {
  name,
  version
...
function PythonRemoteFileSourcePluginView(props) {
  const { fetch } = props;
  const [items, setItems] = React.useState([]);
  const [searchText, setSearchText] = React.useState("");
  const [widget, setWidget] = React.useState(null);
  const dh = jsapiBootstrap.useApi();
  const autoFocusRef = React.useCallback((input) => {
    input == null ? void 0 : input.focus();
  }, []);
  const searchTextLc = React.useMemo(() => searchText.toLowerCase(), [searchText]);
  const filteredItems = React.useMemo(
    () => searchTextLc === "" ? items : items.filter((item) => item.toLowerCase().includes(searchTextLc)),
    [items, searchTextLc]
  );
  React.useEffect(() => {
    async function init() {
      log.info("Initializing widget");
      const fetchedWidget = await fetch();
      setWidget(fetchedWidget);
      fetchedWidget.addEventListener(
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(components.Flex, { direction: "column", height: "100%", alignItems: "center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(components.Flex, { alignItems: "center", width: "100%", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(components.Heading, { level: 1, flex: true, margin: "size-160", children: "Remote Modules" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "v",
        pkg.version
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(components.ActionButton, { isQuiet: true, onPress: onRefresh, children: /* @__PURE__ */ jsxRuntimeExports.jsx(components.Icon, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(FontAwesomeIcon, { icon: icons.vsRefresh }) }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      components.DebouncedSearchInput,
      {
        ref: autoFocusRef,
        value: searchText,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(components.ListView, { children: filteredItems })
  ] });
}
...
module.exports = PythonRemoteFileSourcePlugin;
