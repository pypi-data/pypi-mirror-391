...
```env
APITELEMATEL_BASE_URL=http://servidor:5000
...
resultado = cliente.query("SELECT * FROM PUB.gmclien", dsn="tlmplus", max_rows=20)
```

La llamada anterior limita la respuesta a 20 filas usando `max_rows`; si tu servidor lo soporta puedes enviar cualquier valor entero para controlar el tamaño del resultado.
...
| --- | --- | --- |
| `query(sql, connection_key=None, dsn=None, max_rows=None)` | `/query` | SQL completo, `connection_key`/`dsn` y `max_rows` opcional |
