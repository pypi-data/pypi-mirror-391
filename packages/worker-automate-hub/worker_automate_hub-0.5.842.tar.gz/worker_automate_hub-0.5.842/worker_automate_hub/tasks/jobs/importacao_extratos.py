...
async def importacao_extratos(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        log("Parâmetros -> pasta: %s | ignorar se contém: %s | extensões: %s | texto alvo: %s",
            PASTA, IGNORAR_SE_CONTEM, EXT_STR, TEXTO_ALVO)
        ...
        log("Avaliados (ext permitidas): %s | Ignorados por nome: %s | Ignorados por extensão: %s | Lidos: %s | Erros leitura: %s",
            avaliados, ignorados_nome, nao_permitido, lidos, erros)
