...
```python
# Create a new task
response = client.controller_api.task(
    user_id='user123'
)
...
client.controller_api.message(
    task_id=task_id,
    text='Looking for a microwave with at least 20 liters capacity'
)

# Get all tasks for a user
user_tasks = client.controller_api.get_tasks_by_user_id(
    user_id='user123',
    page=1,
    size=10
)
...
async def chat_with_agent():
    ...
    socket = await client.apollo_session.connect()
...
client = ApolloClient(
    ...
    network_api_key: Optional[str],  # Override network API key
    environment: Optional[str],      # Custom environment
...
#### `task(user_id)` - Create Task

```python
response = client.controller_api.task(
    user_id='user123'
)
# Returns: CreateExternalTaskResponse with task_id
```
...
# Returns: List[ExternalTaskMessage]
```

#### `message(task_id, text)` - Submit Message

```python
response = client.controller_api.message(
    task_id='task_id_here',
    text='Your message here',
    is_external_api=True  # Optional
)
# Returns: ExternalTaskMessage
```

#### `get_tasks_by_user_id(user_id, page, size)` - Get User Tasks

```python
tasks = client.controller_api.get_tasks_by_user_id(
    user_id='user123',
    page=1,
    size=10
)
# Returns: ListExternalTasksResponse
```
...
All WebSocket methods are accessed via `client.apollo_session.*`
...
async def search_products(user_id: str, query: str):
    ...
    task_response = client.controller_api.task(user_id=user_id)
    ...
    async def on_message(message):
        if message['type'] == 'streaming_update':
            ...
            if 'product_cards' in message['data']:
                ...
                for i, product in enumerate(message['data']['product_cards'], 1):
...
```python
client = ApolloClient(
    api_key='API_KEY_YOUR_KEY_HERE'  # Double-check this value
)
