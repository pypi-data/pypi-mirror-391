...
class RelationshipPreloader:
    ...
    def _preload_saved_records(
        ...
        from django_bulk_hooks.utils.mti_utils import expand_mti_related_fields

        pks = [r.pk for r in records]

        # Expand MTI child models automatically
        expanded_relationships = expand_mti_related_fields(tuple(relationships), self.model_cls)

        logger.info(
            "SAVED RECORDS PRELOAD: %s records, relationships=%s, expanded=%s",
            len(records),
            list(relationships),
            expanded_relationships
        )

        preloaded = self.model_cls.objects.filter(pk__in=pks).select_related(*expanded_relationships).in_bulk()
    ...
    def _preload_unsaved_records(
        ...
        logger.info(
            "UNSAVED RECORDS PRELOAD: %s records, relationships=%s",
            len(records),
            list(relationships)
        )
    ...
    def _bulk_load_related_objects(
        ...
        from django_bulk_hooks.utils.mti_utils import get_mti_child_models
        ...
        for field, ids in field_ids_map.items():
            ...
            try:
                ...
                if related_model:
                    # Check if this related model has MTI children
                    child_models = get_mti_child_models(related_model)
                    if child_models:
                        # Load with select_related for MTI children
                        child_names = [child._meta.model_name for child in child_models]
                        logger.info(
                            "BULK LOAD WITH MTI: Loading %s %s objects with children: %s",
                            len(ids),
                            related_model.__name__,
                            child_names
                        )
                        queryset = related_model.objects.filter(pk__in=ids).select_related(*child_names)
                        field_objects_map[field] = {obj.pk: obj for obj in queryset}
                    else:
                        # Regular in_bulk loading
                        logger.debug(
                            "BULK LOAD: Loading %s %s objects (no MTI children)",
                            len(ids),
                            related_model.__name__
                        )
...
class HookExecutor:
    ...
    def _preload_method_relationships(
        ...
        try:
            ...
            logger.info(f"METHOD PRELOAD: @select_related fields: {fields}, model: {changeset.model_cls.__name__}, event: {event}")

            # Preload for new_records
            if changeset.new_records:
                logger.info(f"METHOD PRELOAD: Preloading for {len(changeset.new_records)} new records")
                preload_func(
            ...
            if changeset.old_records:
                logger.info(f"METHOD PRELOAD: Preloading for {len(changeset.old_records)} old records")
                preload_func(
...
class HookDispatcher:
    ...
    def _batch_preload_records(
        ...
        # Expand MTI child models before querying
        expanded_relationships = expand_mti_related_fields(tuple(relationships), model_cls)
        
        # Single bulk query for all records
        preloaded_map = model_cls.objects.filter(pk__in=all_pks).select_related(*expanded_relationships).in_bulk()
