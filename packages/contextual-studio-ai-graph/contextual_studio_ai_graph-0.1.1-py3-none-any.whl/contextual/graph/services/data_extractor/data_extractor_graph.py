...
from contextual.graph.models import ModelDataExtractor, State
...
class DataExtractorLG(DataExtractor):
    ...
    async def extract(self, text: str, **kwargs: Dict[Any, Any]) -> ModelDataExtractor:
        ...
        initial_state = State(text=text, model_schema=None, data_extracted={})
        ...
        if isinstance(output, dict):
            ...
            output = State(
                text=output["text"],
                model_schema=output["model_schema"],
                data_extracted=output["data_extracted"],
            )

        if not isinstance(output, State):
            raise TypeError(f"Expected State object, got {type(output)}")

        if output.data_extracted is None:
            raise Exception("Error while extracting the data as the result was found empty.")
