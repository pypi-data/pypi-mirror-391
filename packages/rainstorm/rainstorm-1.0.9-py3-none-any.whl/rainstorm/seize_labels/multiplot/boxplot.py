...
def boxplot_total_exploration_time(
    ...
    if not full_target_names:
        logger.error(f"boxplot_total_exploration_time requires target exploration. Skipping plot for {group}/{trial}.")
        ax.set_title(f"At least one target required for {group}/{trial}", color='red')
        ax.text(0.5, 0.5, 'Requires targets',
    ...
    # Process data to get final exploration times for each subject
    subject_data = []
    for df in raw_dfs:
    ...
    # Generate colors and positions for plotting
...
def boxplot_DI_auc(
    ...
    **kwargs
    ...
    if len(targets) != 2:
        logger.error(f"boxplot_DI_auc requires exactly two targets, but got {len(targets)}: {targets}. Skipping plot for {group}/{trial}.")
        if ax is not None:
            ax.set_title(f"boxplot_DI_auc requires 2 targets for {group}/{trial}", color='red')
...
def boxplot_avg_time_bias(
    ...
    **kwargs
    ...
    if len(targets) != 2:
        logger.error(f"boxplot_avg_time_bias requires exactly two targets, but got {len(targets)}: {targets}. Skipping plot for {group}/{trial}.")
        if ax is not None:
            ax.set_title(f"boxplot_avg_time_bias requires 2 targets for {group}/{trial}", color='red')
            ax.text(0.5, 0.5, 'Requires exactly 2 targets',
                    horizontalalignment='center', verticalalignment='center',
                    transform=ax.transAxes, fontsize=12, color='red')
        return
