...
class ChangeClient(BaseArenaClient):
    ...
    def add_item_to_change(
        ...
        *,
        change_number: str,
        item_number: str,
        revision_selector: Optional[str] = "WORKING",
        ...
        value = revision_selector.strip() if isinstance(revision_selector, str) else ""
        if not value or value.casefold() in {"null", "none"}:
            raw_selector = ""
            selector = "WORKING"
            include_affected_revision = False
        else:
            raw_selector = value
            selector = raw_selector.upper() or "WORKING"
            include_affected_revision = True
        ...
        effective_guid = revision_info.get("EFFECTIVE")
        if effective_guid and include_affected_revision:
            payload["affectedItemRevision"] = {"guid": effective_guid}
        ...
        if new_revision_value:
            ...
            payload["newRevisionNumber"] = raw_selector
