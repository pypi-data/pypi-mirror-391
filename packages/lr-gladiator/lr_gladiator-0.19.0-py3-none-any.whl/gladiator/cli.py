...
def create_change(
    category_name: str = typer.Option(
        ...
        "--category",
        help=(
            "Change category name (default: 'Component Change Order'). "
...
def add_to_change(
    ...
    revision_selector: str = typer.Option(
        "WORKING",
        "--revision",
        "--revision-selector",
        help="Revision selector to add (default: WORKING)",
    ),
    ...
    lifecycle_phase: Optional[str] = typer.Option(
        None,
        "--lifecycle-phase",
        help="Lifecycle phase to assign to the new revision. Abandoned, Deprecated, In Design, In Production, Obsolete, Unreleased (default: auto-detect 'In Production')",
    ),
    format: Optional[str] = typer.Option(
        None,
        "--format",
    ...
    client = _change_client()
    normalized_revision = _normalize_revision_selector(revision_selector)
    spinner_revision = normalized_revision or "auto"
    try:
        with spinner(
            f"Adding {item_number} ({spinner_revision}) to change {change_id}…",
            enabled=sys.stdout.isatty(),
        ):
            result = client.add_item_to_change(
                change_number=change_id,
                item_number=item_number,
                revision_selector=normalized_revision,
