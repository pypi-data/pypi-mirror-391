...
class WindowsActivator:
    ...
    def _activate_existing_window(
        ...
        allow_shell_retry = bool(app.shell_fallback_on_fail) or (tray_hint and app.relaunch_when_tray_hidden)
        ...
        if allow_shell_retry:
            ...
            if tray_hint and not app.relaunch_when_tray_hidden:
                self.steps.append("未启用 relaunch_when_tray_hidden，跳过 Shell 唤起")
            elif not app.shell_fallback_on_fail:
...
'''
    ...
    def _launch_process(app_path: str) -> bool:
        ...
        try:
            ...
            if ext in ['.lnk', '.exe']:
                LOGGER.info(f"使用 os.startfile 启动 {ext} 文件")
