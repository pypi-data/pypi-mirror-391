...
class WindowsTrayActivator:
    ...
    def activate(self, app: AppInfo) -> Dict[str, Any]:
        ...
        if hwnd:
            ...
            # 温和激活失败（可能在托盘），尝试模拟点击托盘图标
            self.steps.append("温和激活失败，尝试模拟托盘双击")
            if self.simulate_tray_click(process_name, app.name):
                self.steps.append("模拟托盘点击成功，等待窗口响应")
                
                # 等待窗口变为可见（最多等待 2 秒）
                for i in range(10):  # 10 次 * 0.2 秒 = 2 秒
                    time.sleep(0.2)
                    
                    # 检查原窗口是否变为可见
                    if win32gui.IsWindowVisible(hwnd):
                        self.steps.append(f"原窗口已变为可见 (hwnd={hwnd})")
                        self.gentle_activate(hwnd)
                        return self._result(True, f"通过模拟托盘点击成功唤起 {app.name}")
                
                self.steps.append("模拟托盘点击后窗口未响应")
            
            # 如果模拟托盘点击也失败，尝试使用 Shell start
            ...
            if self.shell_start(app.path):
                ...
                return self._result(True, f"已尝试唤起 {app.name}")
            
            # 所有方法都失败
            self.steps.append("所有激活方法均失败")
            return self._result(True, f"{app.name} 已在运行（请手动点击托盘图标）")
    ...
    def simulate_tray_click(self, process_name: str, app_name: str) -> bool:
        """模拟点击系统托盘图标"""
        try:
            if not HAS_WIN32:
                return False
            ...
            # 查找系统托盘窗口
            self.steps.append("查找系统托盘窗口")
            tray_hwnd = win32gui.FindWindow("Shell_TrayWnd", None)
            if not tray_hwnd:
                self.steps.append("未找到系统托盘")
                return False
            
            # 查找通知区域
            notification_area = win32gui.FindWindowEx(tray_hwnd, 0, "TrayNotifyWnd", None)
            if not notification_area:
                self.steps.append("未找到通知区域")
                return False
            
            # 尝试发送激活消息到通知区域
            # 注意：这只是一种尝试，不一定总是有效
            self.steps.append(f"尝试激活托盘中的 {app_name}")
            
            # 发送鼠标消息模拟点击
            # 这里使用 PostMessage 发送 WM_LBUTTONDBLCLK 消息
            try:
                win32gui.PostMessage(notification_area, win32con.WM_LBUTTONDBLCLK, 0, 0)
                time.sleep(0.5)
                return True
            except Exception as e:
                self.steps.append(f"发送托盘点击消息失败: {e}")
                return False
            
        except Exception as exc:
            self.steps.append(f"模拟托盘点击失败: {exc}")
            return False
    ...
    def shell_start(app_path: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                command,
                shell=True,  # 使用 shell=True 以正确处理 start 命令
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                check=False  # 不检查返回码，因为 start 命令可能返回非零值
            )
            
            LOGGER.info(f"Shell start 命令执行完成，返回码: {result.returncode}")
            return True
        except Exception as exc:
