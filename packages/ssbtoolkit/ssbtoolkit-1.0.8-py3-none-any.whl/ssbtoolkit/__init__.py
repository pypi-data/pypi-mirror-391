import pkgutil, importlib, sys, os, warnings
...
for module_info in pkgutil.walk_packages(__path__, prefix=__name__ + "."):
    __all__.append(module_info.name.split(".")[-1])
    module = importlib.import_module(module_info.name)
    globals()[module_info.name.split(".")[-1]] = module

# Adjust sys.path and warnings as before
...
class Simulation:
    ...
    class FitModel:
        ...
        def ShowGraphs(self, save=False, filename=None):
            ...
            fig.update_xaxes(title_text="Time (s)", showgrid=False, row=1, col=1, title_font=dict(size=18), 
                            linecolor='black', linewidth=2,
                            ticks='inside', tickfont=dict(size=18), tickcolor='black', ticklen=10, tickwidth=2)

            fig.update_yaxes(title_text=self._observable+' (nM)', title_font=dict(size=18), showgrid=False, row=1, col=1, 
                            linecolor='black', linewidth=2,
                            ticks='inside', tickfont=dict(size=18), tickcolor='black', ticklen=10, tickwidth=2)
            ...
            fig.update_yaxes(showgrid=False, range=[round((min(AMP_values)-min(AMP_values)*0.5)/5)*5,round((max(AMP_values)+max(AMP_values)*0.5)/5)*5 ], row=1, col=2,
                            title_text=self._observable+' (nM)', title_font=dict(size=18),
            ...
            fig.update_yaxes(row=2, col=1,showgrid=False,  title_text=self._observable+' (nM)', range=[round((min(AUC_values)-min(AUC_values)*0.5)/5)*5,round((max(AUC_values)+max(AUC_values)*0.5)/5)*5], 
                            title_font=dict(size=18),linecolor='black', linewidth=2, 
            ...
            fig.update_yaxes(row=2, col=2, showgrid=False, range=[self.pathway_parameters['time_in'],round((max(FWHM_values)+(max(FWHM_values)-self.pathway_parameters['time_in'])*0.5)/5)*5], 
                            title_text='Time (s)', title_font=dict(size=18), linecolor='black', linewidth=2,
