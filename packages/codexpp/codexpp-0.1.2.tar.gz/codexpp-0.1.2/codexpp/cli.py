...
def _build_codex_mcp_block(servers: Dict[str, McpServerDefinition], project_path: Path | None = None) -> str:
    ...
    for server in sorted(servers.values(), key=lambda s: s.identifier):
        ...
        lines.append(f'command = {_toml_quote(server.command)}')
        if server.args:
        # Add the project directory as an argument for the filesystem server
...
def _handle_codex_status(args: argparse.Namespace) -> None:
    ...
    if codex_path is None:
        ...
        print("  Install with: npm i -g @openai/codex  or  brew install --cask codex")
        return
...
def _handle_codex_install(args: argparse.Namespace) -> None:
    ...
    if mcp_servers:
        # Determine the project directory (where codex install was run)
...
def _handle_codex_uninstall(args: argparse.Namespace) -> None:
    ...
    if config_path.exists():
        ...
        cleaned = _strip_codexpp_slash_commands(_strip_codexpp_mcp_servers(original))
        if cleaned != original:
    ...
    if prompts_dir.exists():
        for command in commands.values():
            ...
            if prompt_file.exists():
                prompt_file.unlink()
                removed_prompts.append(prompt_file)
    if removed_prompts:
        print("[codexpp] Removed prompt files:")
        for path in removed_prompts:
            print(f"  - {path}")
    else:
        print("[codexpp] No codexpp prompt files removed (none found).")
    ...
    if removed_mcp:
        print("[codexpp] Removed MCP profiles:")
        for path in removed_mcp:
            print(f"  - {path}")
    else:
        print("[codexpp] No codexpp MCP profiles removed (none found).")
