...
def em_step(params: EMStepParams) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    # Always normalize C for clock-frequency factors to prevent scale issues
    # Even if there are slower-frequency series, we can still normalize clock-frequency factors
    # (tent weight constraints only apply to slower-frequency parts, not clock-frequency parts)
    if num_clock_factors > 0:
        # Only normalize if there are no slower-frequency series
        # (to avoid violating tent weight constraints)
        for j in range(num_clock_factors):
            norm = np.linalg.norm(C_new[:, j])
            if norm > 1e-8:
                # Improved normalization: clip C norm before normalization to prevent Q explosion
                # If C norm is too large, we cap it to prevent Q from becoming extreme
                C_norm_cap = safe_get_attr(config, "max_loading_norm", 10.0) if config is not None else 10.0
                norm_clipped = min(norm, C_norm_cap)
                
                # Normalize C column (using clipped norm to prevent extreme Q)
                C_new[:, j] = C_new[:, j] / norm
                
                # Adjust Q to preserve variance, but use clipped norm to prevent explosion
                # This is a compromise: we normalize C fully but limit Q adjustment
                Q_adjustment = norm_clipped ** 2
                Q_new[j, j] = Q_new[j, j] * Q_adjustment
                
                # Off-diagonal: Q[j,k] and Q[k,j] *= norm_clipped (for k != j)
                for k in range(Q_new.shape[0]):
                    if k != j:
                        Q_new[j, k] = Q_new[j, k] * norm_clipped
                        Q_new[k, j] = Q_new[k, j] * norm_clipped
