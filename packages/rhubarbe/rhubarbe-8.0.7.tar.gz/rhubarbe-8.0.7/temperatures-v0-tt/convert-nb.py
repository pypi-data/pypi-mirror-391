...
loaded['timestamp'] = pd.to_datetime(loaded['timestamp'], format="%Y-%m-%dT%H:%M:%S")
...
def filter(df, duration, resample_period):
    if duration is not None:
        ...
        # print(type(duration))
        time_threshold = pd.to_datetime(pd.Timestamp.now().tz_localize("Europe/Paris")) - duration
