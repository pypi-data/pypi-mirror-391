...
writehosts(){
  echo "\
0.0.0.0  auth.$DOMAIN
0.0.0.0  demoapp1.$DOMAIN
0.0.0.0  demoapp2.$DOMAIN" | sudo tee -a /etc/hosts > /dev/null
}
...
- https://demoapp1.$DOMAIN - Secured with Authelia one-factor authentication
- https://demoapp2.$DOMAIN - Secured with Authelia two-factor authentication (see note below)

You will need to authorize the self-signed certificate upon visiting each domain.
To visit https://demoapp2.$DOMAIN you will need to register a device for second factor authentication and confirm by clicking on a link sent by email. Since this is a demo with a fake email address, the content of the email will be stored in './authelia/notification.txt'.
