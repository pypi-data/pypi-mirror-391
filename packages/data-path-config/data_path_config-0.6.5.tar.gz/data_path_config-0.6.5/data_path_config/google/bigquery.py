...
class BigQueryManager:
    ...
    def upload_dataframe(self, df: pd.DataFrame, dataset_id: str, table_id: str, project_id: Optional[str] = None, if_exists: str = 'replace') -> str:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Dataset {dataset_id} does not exist in project {project}")
            credentials = service_account.Credentials.from_service_account_file(self.credentials_file, scopes=['https://www.googleapis.com/auth/bigquery'])
            to_gbq(df, destination_table=table_id_full, project_id=project, if_exists=if_exists, credentials=credentials)
    ...
    def query_table(self, query: str, project_id: Optional[str] = None) -> pd.DataFrame:
        """
        Execute a query on BigQuery and return the results as a pandas DataFrame.
