...
class Create(Job):
    ...
    def execute(self, resources: dict[str, Resource], **kwargs):
        ...
        if update:
            try:
                update_conditions = {key: kwargs.pop(key) for key in unique_keys if key in kwargs}
                ...
                return Update.execute_now(
                    ...
                    signature=signature,
                    conditions=update_conditions,  # ← Only unique keys
                    **kwargs  # ← Everything else becomes the updates
                )
            except Exception as e:
...
class Delete(Job):
    ...
    def execute(self, resources: dict[str, Resource], **kwargs):
        ...
        with pydb.get_table(table_name) as table:
            original_length = len(table)  # ← Save this FIRST
            ...
            deleted_count = mask.sum()  # ← Count how many matched BEFORE filtering
            result = table[~mask].reset_index(drop=True)
            table.iloc[:] = result  # ← Update the table in place
