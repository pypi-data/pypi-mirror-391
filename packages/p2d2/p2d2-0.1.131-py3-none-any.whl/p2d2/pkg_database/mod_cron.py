    ...
    def __new__(cls, path: Path):
        with cls._lock:
            ...
            if path_key not in cls._instances:
                ...
                log.debug(f"[CronLoader]: Creating NEW instance for path: {path}")
            else:
                log.debug(f"[CronLoader]: Reusing EXISTING instance for path: {path}")
    ...
    def __init__(self, path: Path):
        ...
        if self.path.exists():
            for py_file in self.path.glob("*.py"):
                try:
                    self.bytes[py_file.name] = py_file.read_bytes()
                except Exception as e:
...
class CronManager(ICronManager):
    ...
    def __init__(self, database: Database, cron_loader: type[ICronLoader]):
        with CronManager._lock:
            db_id = id(database)
            if db_id in CronManager._instances:
                existing_count = len(CronManager._instances[db_id])
                log.error(f"DUPLICATE CronManager creation for {database.name} (db instance id: {db_id})")
                log.error(f"  This is manager #{existing_count + 1} for this database instance!")
                log.error(f"  Check if you're accessing database.cron multiple times or if @cached_property is failing")
            if db_id not in CronManager._instances:
                CronManager._instances[db_id] = []
            CronManager._instances[db_id].append(self)
        ...
        self._thread.start()
        log.debug(
            f"{self}: CronManager initialized and started (manager #{len(CronManager._instances[db_id])} for this db)")
