...
class TimelineDirective(Directive):
    ...
    option_spec = {
        ...
        'toggle': directives.flag,       # Show layout toggle switch
    }
    ...
    def run(self):
        ...
        show_toggle = 'toggle' in self.options

        # Parse events from content
        events = self._parse_events()

        # Generate HTML
        html = self._generate_html(events, layout, start_year, end_year, height, width, show_toggle)
    ...
    def _generate_html(self, events: List[Dict[str, str]], layout: str,
                      ...
                      height: str, width: str, show_toggle: bool = False) -> str:
        """Generate HTML for the timeline."""

        # Generate the vertical and horizontal timelines
        vertical_html = self._generate_vertical_timeline(events, height, width)
        horizontal_html = self._generate_horizontal_timeline(events, start_year, end_year, height, width)

        # If toggle is enabled, wrap both versions with a toggle switch
        if show_toggle:
            return self._generate_timeline_with_toggle(vertical_html, horizontal_html, layout)

        # Otherwise, return just the requested layout
        if layout == 'horizontal':
            return horizontal_html
        else:
            return vertical_html

    def _generate_timeline_with_toggle(self, vertical_html: str, horizontal_html: str, default_layout: str) -> str:
        """Generate timeline with a layout toggle switch."""
        ...
        vertical_display = 'block' if default_layout == 'vertical' else 'none'
        horizontal_display = 'block' if default_layout == 'horizontal' else 'none'

        html_parts = [
            f'<div class="timeline-with-toggle" id="{timeline_id}">',
            '  <div class="timeline-controls">',
            '    <div class="timeline-toggle-switch">',
            '      <label class="toggle-label">',
            '        <input type="checkbox" class="toggle-checkbox" />',
            '        <span class="toggle-slider"></span>',
            '        <span class="toggle-text-vertical">Vertical</span>',
            '        <span class="toggle-text-horizontal">Horizontal</span>',
            '      </label>',
            '    </div>',
            '  </div>',
            f'  <div class="timeline-vertical-wrapper" style="display: {vertical_display};">',
            f'    {vertical_html}',
            '  </div>',
            f'  <div class="timeline-horizontal-wrapper" style="display: {horizontal_display};">',
            f'    {horizontal_html}',
            '  </div>',
            '</div>',
            f'''
            ...
            (function() {{
                ...
                const verticalWrapper = container.querySelector('.timeline-vertical-wrapper');
                const horizontalWrapper = container.querySelector('.timeline-horizontal-wrapper');

                // Set initial state based on default layout
                const isHorizontal = {str(default_layout == 'horizontal').lower()};
                checkbox.checked = isHorizontal;

                checkbox.addEventListener('change', function() {{
                    if (this.checked) {{
                        verticalWrapper.style.display = 'none';
                        horizontalWrapper.style.display = 'block';
                    }} else {{
                        verticalWrapper.style.display = 'block';
