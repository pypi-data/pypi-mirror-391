...
class AbstractModuleWeather(ABC):
	...
	def __init__(self,logger:ModuloLogging):
		self._logger=logger
		self.__lastCall:datetime|None=None
	...
	def __makeGenericRequest(self,baseUrl:str,classResponseBean:Type[AbstractWeatherDataBean],
		...
		# not too quick calls
		now=ModuloDate.now()
		if self.__lastCall is not None:
			diff=ModuloDate.secondsDiff(now,self.__lastCall)
			if diff<60:
				raise ExceptionRuntime(f"Too quick call, previous was {diff} seconds ago")
		self.__lastCall=now
		# url
