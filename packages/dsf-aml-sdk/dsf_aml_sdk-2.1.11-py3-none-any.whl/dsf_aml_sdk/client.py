...
class EvaluationResult:
    def __init__(self, score, tier, metrics=None):
        self.score = score
        self.tier = tier
        self.metrics = metrics  # ← Agregar
    ...
    def from_response(resp): 
        ...
        return EvaluationResult(
            ...
            result_data.get("tier", "community"),
            result_data.get("metrics")  # ← Capturar
        )
...
class AMLSDK:
    BASE_URL = os.environ.get(
        "DSF_AML_BASE_URL",
        "https://dsf-hw1vc6a4b-api-dsfuptech.vercel.app/"
    )
    ...
    def batch_evaluate(self, data_points: List[Dict], config: Optional[Union[Dict, Config]] = None) -> List[EvaluationResult]:
        ...
        try:
            ...
            batch_metrics = resp.get("metrics")
            ...
            # Primero creamos la lista de resultados
            results = [
                EvaluationResult(score=float(raw[i]) if i < len(raw) else 0.0, tier=self.tier)
                for i in range(len(data_points))
            ]
            
            if batch_metrics and results:
                results[0].batch_metrics = batch_metrics
