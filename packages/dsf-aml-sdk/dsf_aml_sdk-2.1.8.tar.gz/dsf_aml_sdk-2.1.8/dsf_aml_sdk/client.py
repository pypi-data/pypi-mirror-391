...
class EvaluationResult:
    ...
    def from_response(resp): 
        # Asegura que buscamos el score dentro de 'result' si existe (post-polling)
        result_data = resp.get('result') if resp.get('job_id') else resp
        return EvaluationResult(result_data.get("score", 0.0), result_data.get("tier", "community"))
...
class AMLSDK:
    BASE_URL = os.environ.get(
        "DSF_AML_BASE_URL",
        "https://dsf-lbnba2kzx-api-dsfuptech.vercel.app/"
    )
    ...
    # ---------- NEW: Polling Helper ----------
    def _poll_job_status(self, job_id: str, max_attempts: int = 15, delay: float = 2.0) -> Dict:
        """Polls the job status endpoint until completion or timeout."""
        status_url = (self.base_url or "").rstrip("/") + f"/api/evaluate/status/{job_id}"
        
        for attempt in range(max_attempts):
            time.sleep(delay)
            
            try:
                # Use GET for status check
                ...
                if status == 'completed':
                    # Return the content of the 'result' key
                    return status_resp.get('result', {})
                elif status == 'failed':
                    error_msg = status_resp.get('error', 'Job failed without specific error message.')
                    raise APIError(f"Job failed on worker: {error_msg}")
                # 'queued' or 'processing' continues loop

            except requests.RequestException as e:
                logger.warning(f"Polling failure for job {job_id}: {e}")
                
        raise APIError(f"Job {job_id} timed out after {max_attempts * delay}s.")
    ...
    def evaluate(self, data: Dict[str, Any], config: Optional[Union[Dict, Config]] = None) -> EvaluationResult:
        ...
        # --- START POLLING LOGIC ---
        # The 'evaluate' action is small, but if Redis is slow, Vercel might enqueue it.
        if resp.get('status') == 'queued' and resp.get('job_id'):
            job_id = resp['job_id']
            # Poll the job status (small job, so max 5 attempts/10s total should suffice)
            resp = self._poll_job_status(job_id, max_attempts=5, delay=2.0)
    ...
    def batch_evaluate(self, data_points: List[Dict], config: Optional[Union[Dict, Config]] = None) -> List[EvaluationResult]:
        ...
        try:
            ...
            if resp.get('status') == 'queued' and resp.get('job_id'):
                job_id = resp['job_id']
                # The result is the content of the completed job (using default max attempts)
                resp = self._poll_job_status(job_id)
            ...
            if isinstance(resp, list):
                ...
                # We expect the scores array in the result dictionary (either top-level or under 'scores')
                raw = resp.get("scores")
                
                # Check for the structure returned by the Worker/Polling: {'scores': [...]}
                if raw is None:
                    # Look for scores nested under a 'result' key if the response was pooled through a generic status check
                    raw = resp.get("result", {}).get("scores")
                
                if raw is None:
                     # Still fallback (original code logic)
            ...
            # Fallback to individual evaluation if batch API fails
    ...
    def pipeline_identify_seeds(self, dataset: List[Dict], config: Union[Dict, Config],
        ...
        # --- START POLLING LOGIC (Pipelines are heavy and always enqueued) ---
        if resp.get('status') == 'queued' and resp.get('job_id'):
            job_id = resp['job_id']
            # Pipelines are heavy, use maximum polling attempts
            return self._poll_job_status(job_id, max_attempts=60, delay=5.0) # Poll up to 5 minutes
        # --- END POLLING LOGIC ---
    ...
    def pipeline_identify_seeds_safe(self, dataset: List[Dict], config: Union[Dict, Config], top_k_percent: float = 0.3, chunk_size: int = DEFAULT_CHUNK) -> Dict:
        ...
        for part in _chunked(dataset, chunk):
            # Llama a batch_evaluate, que a su vez llama al worker (y ahora hace polling)
    ...
    def pipeline_generate_critical(self, config, seeds=None, advanced=None, **kwargs):
        ...
        if resp.get('status') == 'queued' and resp.get('job_id'):
            job_id = resp['job_id']
            # Pipelines are heavy, use maximum polling attempts
            resp = self._poll_job_status(job_id, max_attempts=60, delay=5.0) # Poll up to 5 minutes
        # --- END POLLING LOGIC ---
    ...
    def curriculum_status(self) -> Dict:
        ...
        # This is a fast read operation, but we poll just in case it's queued
        resp = self._make_request("api/evaluate", req)
        
        # --- START POLLING LOGIC ---
        if resp.get('status') == 'queued' and resp.get('job_id'):
            job_id = resp['job_id']
            return self._poll_job_status(job_id, max_attempts=5, delay=2.0)
        # --- END POLLING LOGIC ---
    ...
    def distill_train(self, config: Union[Dict, Config], samples: int = 1000, seed: int = 42,
        ...
        if resp.get('status') == 'queued' and resp.get('job_id'):
            job_id = resp['job_id']
            # Distill is heavy, use maximum polling attempts
            return self._poll_job_status(job_id, max_attempts=60, delay=5.0) # Poll up to 5 minutes
        # --- END POLLING LOGIC ---
