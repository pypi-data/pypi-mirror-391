...
# Asumiendo que __version__ está definido
__version__ = "1.0.0" 
...
# Intentar importar numpy para sanitización (esencial para SDK)
try:
    import numpy as np
except ImportError:
    np = None
...
MAX_PARTIAL_VARIANTS = int(os.getenv("DSF_PARTIAL_VARIANTS", "5000"))
...
class AMLSDK:
    ...
    def pipeline_identify_seeds_safe(self, dataset, config, top_k_percent=0.3, chunk_size=DEFAULT_CHUNK):
        """Selecciona semillas localmente, evaluando en lotes para evitar 413s."""
        if self.tier == "community":
            raise LicenseError("Seeds selection requires premium tier")
        chunk = max(1, min(int(chunk_size), MAX_BATCH_EVAL))
        scores = []
        # Evalúa en lotes usando la función batch_evaluate
        for part in _chunked(dataset, chunk):
            scores.extend([r.score for r in self.batch_evaluate(part, config)])
        
        # Lógica de selección del borde de decisión
        idx = _global_near_threshold(scores, top_k_percent)
        
        # Construye la respuesta con la incertidumbre calculada
        median_score = _median(scores)
        seeds = [{"data": dataset[i], "uncertainty": abs(scores[i] - median_score)} for i in idx]
    ...
    def pipeline_generate_critical(self, config, seeds=None, advanced=None, **kwargs):
        ...
        if seeds is None and original_ds:
            ...
            # Usa la versión segura implementada justo arriba
