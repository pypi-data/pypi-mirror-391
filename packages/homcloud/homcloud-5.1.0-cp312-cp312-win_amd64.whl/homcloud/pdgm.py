...
class PDGM(PDGMBase):
    ...
    @cached_property
    def coupled_alpha_relative_coord_resolver(self):
        assert self.filtration_type == "coupled-alpha-relative"

        return geom_resolver.CoupledAlphaRelativeResolver(
            self.pdgmreader.load_simple_chunk("index_to_simplex"),
            self.pdgmreader.load_simple_chunk("vertex_coordintes"),
        )

    @cached_property
    def coupled_alpha_relative_symbol_resolver(self):
        assert self.filtration_type == "coupled-alpha-relative"
    ...
    @cached_property
    def coupled_alpha_relative_vertex_index_resolver(self):
        assert self.filtration_type == "coupled-alpha-relative"
    ...
    def get_geometry_resolver(self, resolver_type="default", pbc_ratio=None):
        match (self.filtration_type, resolver_type):
            ...
            case ("coupled-alpha-relative", "default" | "coordinates"):
                return self.coupled_alpha_relative_coord_resolver
            case ("coupled-alpha-relative", "symbols"):
                return self.coupled_alpha_relative_symbol_resolver
            case ("coupled-alpha-relative", "vertex_indexes" | "vindexes"):
                return self.coupled_alpha_relative_vertex_index_resolver
            case _:
