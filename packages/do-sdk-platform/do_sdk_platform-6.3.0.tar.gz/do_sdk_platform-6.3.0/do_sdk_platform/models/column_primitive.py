...
class ColumnPrimitive(BaseModel):
    """
    ColumnPrimitive
    """ # noqa: E501
    ...
    expression_code: StrictStr = Field(alias="expressionCode")
    expression_return_type: StrictStr = Field(alias="expressionReturnType")
    config_file: Optional[ColumnFileBaseConfigFile] = Field(default=None, alias="configFile")
    config_numeric: Optional[ColumnFloatBaseConfigNumeric] = Field(default=None, alias="configNumeric")
    reference_database_row_id: StrictStr = Field(alias="referenceDatabaseRowId")
    config_select: ColumnSelectBaseConfigSelect = Field(alias="configSelect")
    __properties: ClassVar[List[str]] = ["id", "parentId", "name", "type", "jsonField", "cardinality", "systemType", "isRequired", "enabledViewers", "cellJsonSchema", "inlineViewer", "expressionCode", "expressionReturnType", "configFile", "configNumeric", "referenceDatabaseRowId", "configSelect"]
    ...
    @field_validator('expression_return_type')
    def expression_return_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['text', 'float', 'integer']):
            raise ValueError("must be one of enum values ('text', 'float', 'integer')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ColumnPrimitive from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ColumnPrimitive from a dict"""
        if obj is None:
            return None
