...
class MQTTSerialBridgeApp:
    def __init__(self, master):
        ...
        ttk.Button(frame_conf, text="🔄 Actualiser", command=self.refresh_ports).grid(row=0, column=2, padx=5)

        ttk.Label(frame_conf, text="Baudrate :").grid(row=0, column=3, sticky="w", padx=2)
        self.baud_entry = ttk.Entry(frame_conf, width=10)
        self.baud_entry.insert(0, "9600")
        self.baud_entry.grid(row=0, column=3, sticky="w")  # ← aligné à gauche
        ...
        self.mqtt_port_entry.grid(row=1, column=3, sticky="w")  # ← aligné à gauche
        ...
        ttk.Button(frame_conf, text="Connecter", command=self.start_connection).grid(row=4, column=3, padx=5)
        ttk.Button(frame_conf, text="Déconnecter", command=self.stop_connection).grid(row=4, column=4, padx=5)
