...
from agentsphere.template.dockerfile_parser import parse_dockerfile
from agentsphere.template.types import (
    CopyItem,
    Instruction,
    TemplateType,
    RegistryConfig,
)
from agentsphere.template.utils import (
...
from agentsphere.template.readycmd import ReadyCmd
    ...
    def to_dockerfile(template: "TemplateClass") -> str:
        if template._template._base_template is not None:
            raise ValueError(
                ...
                "Templates based on other templates can only be built using the Agentsphere API."
            )
