...
from agentsphere.api.metadata import package_version
...
class ApiParams(TypedDict, total=False):
    ...
    """Agentsphere API Key to use for authentication, defaults to `AGENTSPHERE_API_KEY` environment variable."""

    domain: Optional[str]
    """Agentsphere domain to use for authentication, defaults to `AGENTSPHERE_DOMAIN` environment variable."""

    debug: Optional[bool]
    """Whether to use debug mode, defaults to `AGENTSPHERE_DEBUG` environment variable."""
...
class ConnectionConfig:
    ...
    @staticmethod
    def _domain():
        return os.getenv("AGENTSPHERE_DOMAIN") or "e2b.app"
    ...
    def sandbox_headers(self):
        """
        # We need this separate as we use the same header for Agentsphere access token to API and envd access token to sandbox.
        """
        return {
            **self.headers,
