...
from agentsphere.sandbox.filesystem.filesystem import WriteEntry

import agentsphere_connect
import httpcore
import httpx
from packaging.version import Version

from agentsphere.envd.versions import ENVD_VERSION_RECURSIVE_WATCH
from agentsphere.exceptions import SandboxException, TemplateException, InvalidArgumentException
from agentsphere.connection_config import (
    ConnectionConfig,
    Username,
    KEEPALIVE_PING_HEADER,
    KEEPALIVE_PING_INTERVAL_SEC,
)
from agentsphere.envd.api import ENVD_API_FILES_ROUTE, handle_envd_api_exception
from agentsphere.envd.filesystem import filesystem_connect, filesystem_pb2
from agentsphere.envd.rpc import authentication_header, handle_rpc_exception
from agentsphere.sandbox.filesystem.filesystem import (
    WriteInfo,
    EntryInfo,
    map_file_type,
)
from agentsphere.sandbox_sync.filesystem.watch_handle import WatchHandle
...
class Filesystem:
    ...
    def __init__(
        ...
        self._rpc = filesystem_connect.FilesystemClient(
            ...
            # compressor=agentsphere_connect.GzipCompressor,
            pool=pool,
            json=True,
    ...
    def list(
        ...
        try:
            res = self._rpc.list_dir(
                filesystem_pb2.ListDirRequest(path=path, depth=1),
    ...
    def exists(
        ...
        try:
            ...
            if isinstance(e, agentsphere_connect.ConnectException):
                if e.status == agentsphere_connect.Code.not_found:
                    return False
            raise handle_rpc_exception(e)
    ...
    def make_dir(
        ...
        try:
            ...
            if isinstance(e, agentsphere_connect.ConnectException):
                if e.status == agentsphere_connect.Code.already_exists:
                    return False
            raise handle_rpc_exception(e)
