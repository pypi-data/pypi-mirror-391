...
class Client:
    ...
    def _get_verify_ssl(self) -> Union[bool, str]:
        """
        ...
        **opts,
        ...
        for _ in range(self._connection_retries):
            try:
                ...
                ssl_context = None if verify_ssl else ssl_module.create_default_context()
                if ssl_context and not verify_ssl:
                    ssl_context.check_hostname = False
                    ssl_context.verify_mode = ssl_module.CERT_NONE
                conn = self.async_pool.create_connection(
                    URL(req_data["url"]).origin,
                    ssl=ssl_context
                )
        ...
        for _ in range(self._connection_retries):
            try:
                ...
                if ssl_context and not verify_ssl:
                    ssl_context.check_hostname = False
                    ssl_context.verify_mode = ssl_module.CERT_NONE
                conn = self.pool.create_connection(
                    URL(req_data["url"]).origin,
                    ssl=ssl_context
                )
