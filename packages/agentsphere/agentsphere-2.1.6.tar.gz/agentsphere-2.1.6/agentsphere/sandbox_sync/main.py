...
from agentsphere.api.client.types import Unset
from agentsphere.connection_config import ConnectionConfig, ApiParams
from agentsphere.envd.api import ENVD_API_HEALTH_ROUTE, handle_envd_api_exception
from agentsphere.exceptions import SandboxException, format_request_timeout_error
from agentsphere.sandbox.main import SandboxOpts
from agentsphere.sandbox.sandbox_api import SandboxMetrics
from agentsphere.sandbox.utils import class_method_variant
from agentsphere.sandbox_sync.filesystem.filesystem import Filesystem
from agentsphere.sandbox_sync.commands.command import Commands
from agentsphere.sandbox_sync.commands.pty import Pty
from agentsphere.sandbox_sync.sandbox_api import SandboxApi, SandboxInfo
...
class Sandbox(SandboxApi):
    ...
    Check docs [here](https://www.agentsphere.run/docs).
    ...
    @overload
    def connect_without_timeout(
        self,
        **opts: Unpack[ApiParams],
    ) -> Self:
        """
        Connect to a sandbox without setting timeout. Only checks if the sandbox exists and is accessible.
        ...
        # Another code block
        same_sandbox = sandbox.connect_without_timeout()
        ```
        """
        ...
    ...
    def connect_without_timeout(
        ...
        **opts: Unpack[ApiParams],
        ...
        # Another code block
        same_sandbox = Sandbox.connect_without_timeout(sandbox.sandbox_id)
        ```
        """
        ...

    @class_method_variant("_cls_connect_without_timeout")
    def connect_without_timeout(
        self,
        **opts: Unpack[ApiParams],
    ) -> Self:
        """
        Connect to a sandbox without setting timeout. Verifies the sandbox is in running state.
        ...
        ```
        """
        # Get sandbox info and verify it's in running state
        info = self.get_info(**opts)
        if info.state != "running":
            raise SandboxException(
                f"Sandbox {self.sandbox_id} is not running (current state: {info.state})"
            )
        return self
    ...
    @classmethod
    def _cls_connect_without_timeout(
        cls,
        sandbox_id: str,
        **opts: Unpack[ApiParams],
    ) -> Self:
        ...
        :return: A running sandbox instance
        """
        # Get sandbox info to verify state and construct the instance
        response = cls._cls_get_info(sandbox_id, **opts)

        # Verify the sandbox is in running state
        if response.state != "running":
            raise SandboxException(
                f"Sandbox {sandbox_id} is not running (current state: {response.state})"
            )
