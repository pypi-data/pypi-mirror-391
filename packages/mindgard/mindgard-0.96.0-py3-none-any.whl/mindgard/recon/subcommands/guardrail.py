...
class GuardrailReconCommand:
    ...
    def start_detect(self, target_name: str) -> uuid.UUID:
        logging.debug(f"Starting guardrail detect reconnaissance for target: {target_name}")
        response = self.service.start_detect(StartDetectRequest(target_name=target_name))
