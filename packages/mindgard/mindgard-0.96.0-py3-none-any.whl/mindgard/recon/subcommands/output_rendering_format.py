...
class OutputRenderingFormatReconCommand:
    ...
    def start_output_rendering_format(self, target_name: str) -> uuid.UUID:
        f"""Starting output rendering format recon for target: {str(target_name)}"""
        response = self.service.start_output_rendering_format(
            StartOutputRenderingFormatRequest(target_name=target_name)
        )
