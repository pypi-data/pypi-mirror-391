...
class InputEncodingReconCommand:
    ...
    def start_input_encoding(self, target_name: str) -> uuid.UUID:
        f"Starting input encoding recon for target: {str(target_name)}"
        response = self.service.start_input_encoding(StartInputEncodingRequest(target_name=target_name))
