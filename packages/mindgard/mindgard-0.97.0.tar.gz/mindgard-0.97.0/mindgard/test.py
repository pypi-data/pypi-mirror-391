...
from azure.messaging.webpubsubclient.models import CallbackType, OnGroupDataMessageArgs, WebPubSubDataType

# Project imports
from mindgard.constants import DEFAULT_RISK_THRESHOLD
from mindgard.exceptions import handle_exception_callback
from mindgard.mindgard_api import AttackResponse, FetchTestAttacksData, MindgardApi
...
logging.getLogger("azure.messaging.webpubsubclient._client").addFilter(
    lambda record: not (
        record.getMessage() == "The client is stopping state. Stop recovery." and record.levelno == logging.WARNING
    )
)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while not finished:
                try:
                    retries += 1
                    test_data = p.poll_test(config=self._config, test_id=test_id)
                except Exception as e:
                    if retries > 9:
                        raise
                else:
