...
class LLMModelWrapper(ABC):
    def to_handler(self) -> RequestHandler:
        ...
        def handler(payload: Any) -> Any:
            ...
            return {"response": prompt_response.response, "duration_ms": prompt_response.duration_ms}
...
class OpenAIWrapper(LLMModelWrapper):
    def __init__(
        ...
        self.client = (
            ...
            if api_url is None
            else OpenAI(api_key=api_key, base_url=api_url, http_client=httpx.Client(follow_redirects=allow_redirects))
        )
...
def openai_call(
    ...
    try:
        ...
        if exception.status_code == 308:
            raise Uncontactable(
                "Failed to contact model: model returned a 308 redirect that couldn't be followed."
            ) from exception
        if exception.status_code == 401:
            raise Unauthorized(
                "Failed to contact model: model returned a 401 (unauthorized).", status_code=401
            ) from exception
        if exception.status_code == 422:
            ...
            try:
                ...
                pass
            raise UnprocessableEntity(f"Received 422 from provider: {err_message}", 422) from exception
...
def get_llm_model_wrapper(
    ...
    if preset == "huggingface-openai" or preset == "openai-compatible":
        ...
        return OpenAIWrapper(api_key=api_key, model_name=model_name, system_prompt=system_prompt, rate_limit=rate_limit)
