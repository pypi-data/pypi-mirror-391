...
from mindgard.types import type_get_request_function, type_orchestrator_source, type_post_request_function
...
def submit_sandbox_test(
    ...
    return get_test_by_id(test_id=id, access_token=access_token, request_function=get_request_function)


def get_tests(access_token: str, request_function: type_get_request_function = api_get) -> GetTestListResponse:
...
def get_test_by_id(
    ...
    try:
        response = request_function(f"{API_BASE}/tests/{test_id}/attacks", access_token)
        data = response.json()
...
def setup_orchestrator_webpubsub_request(
    ...
    try:
        ...
        raise ValueError(f"Failed to get a response from orchestrator ({str(e)})!")
