...
VALID_MULTI_TURNS = ("actor_attack", "crescendo")
...
class MultiturnAttack:
    ...
    def start(self) -> attack_multiturn.StartResponse:
        logger.debug(f"Starting multi-turn {self._attack_name} for target: {self._target_name}")
        return self._maClient.start(
            attack_multiturn.StartRequest(target_name=self._target_name, goal=self._goal, attack_name=self._attack_name)
        )
    ...
    def poll(self, request_uuid: uuid.UUID) -> None:
        ...
        while not completed:
            ...
            if completed:
                break
            wait_time = max(0, (rt + 0.5) - time.monotonic())  # sleep if last request < 0.5s
    ...
    def __poll_inner(self, request_uuid: uuid.UUID) -> tuple[Optional[attack_prompt_events.Event], bool]:
        ...
        error_code = None
        error_message = None
        duration_ms = None  # TODO: can these be None? doesnt allow Optional?
        content = None
        try:
        ...
        logger.debug(f"Pushing prompt result for request")
        if response:
        ...
        self._maClient.push(
            attack_prompt_events.ResultPushRequest(
