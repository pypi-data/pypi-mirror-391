...
class OutputFormatGenerationReconCommand:
    ...
    def start_output_format_generation(self, target_name: str) -> uuid.UUID:
        f"""Starting output format generation recon for target: {str(target_name)}"""
        response = self.service.start_output_format_generation(
            StartOutputFormatGenerationRequest(target_name=target_name)
        )
