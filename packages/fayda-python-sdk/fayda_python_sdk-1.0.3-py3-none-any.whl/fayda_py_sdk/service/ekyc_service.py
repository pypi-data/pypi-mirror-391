...
class EkycService:
    ...
    def _decrypt_identity(self, response_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        partner_id = self.config.get("partnerId", "")
        if not partner_id:
            raise ValueError("partnerId is required for eKYC operations")
    ...
    def _get_certificate_for_encryption(self, data: str, is_internal: bool):
        ...
        reference_id = self.config.get("ida.reference.id", "")
        if not reference_id:
            raise ValueError("ida.reference.id is required for certificate retrieval")
        params = {
            "applicationId": "IDA",
