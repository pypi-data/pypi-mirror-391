...
def analyze_split_pattern():
    ...
    print(f"Y is in position 1 (middle)")
    ...
    print(f"On odd layers, Y is UNCHANGED (only used to transform padding)")
    print("=" * 70)
...
def test_simple_fit():
    ...
    print(f"  Expected MSE (noise only): ~0.01")

    if mse > 0.05:
        print(f"  ⚠️  WARNING: MSE is much higher than expected!")
    ...
    for i in range(len(X_test)):
        print(f"  X={X_test[i, 0]:5.1f}: true={y_test_true[i, 0]:6.2f}, "
