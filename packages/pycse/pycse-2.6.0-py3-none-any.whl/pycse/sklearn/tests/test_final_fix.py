"""Test ConditionalInvertibleNN with all fixes applied from training."""
...
print("=" * 70)
print("Testing ALL Fixes: 5D + Permutation + Correct Sampling")
print("=" * 70)
...
y_pred, y_std = cinn.predict(X, return_std=True, n_samples=1000)
mse = np.mean((y_pred - y_true)**2)

print(f"Final NLL: {cinn.final_nll_:.4f}")
print(f"MSE: {mse:.4f} (expected ~0.01)")

if mse < 0.05:
    print("✓ Linear fit is GOOD!")
else:
    print("❌ Linear fit needs improvement")

# Test 2: Heteroskedastic (the critical test)
...
cinn_het.fit(X_het, y_het, maxiter=3000)  # More iterations

y_pred_het, y_std_het = cinn_het.predict(X_het, return_std=True, n_samples=2000)
...
plt.tight_layout()
plt.savefig('/Users/jkitchin/Dropbox/python/pycse/heteroskedastic_FINAL.png', dpi=150, bbox_inches='tight')
print("\nPlot saved to heteroskedastic_FINAL.png")
plt.close()
...
if mse < 0.5:
    ...
    print(f"❌ Mean prediction needs work (MSE={mse:.4f})")

if 0.7 <= avg_ratio <= 1.3:
    print(f"✓ Uncertainty magnitude is GOOD (avg ratio={avg_ratio:.2f})")
elif avg_ratio < 0.7:
    print(f"⚠️  Uncertainty is underestimated (avg ratio={avg_ratio:.2f})")
else:
    print(f"⚠️  Uncertainty is overestimated (avg ratio={avg_ratio:.2f})")
