...
function Canvas3DGS($$anchor, $$props) {
  ...
  function reset_scene() {
    ...
    scene = new kt();
    camera = new wt();
    renderer = new Mt(get(canvas));
    controls = new Lt(camera, get(canvas));
    ...
    const load = async () => {
      ...
      if (get(url).endsWith(".ply")) {
        await bt.LoadAsync(get(url), scene, void 0);
      } else if (get(url).endsWith(".splat")) {
        await Tt.LoadAsync(get(url), scene, void 0);
      } else {
  ...
  legacy_pre_effect(() => (get(path), deep_read_state(value())), () => {
    (($$value) => {
      set(path, $$value.path);
    })(value() || { path: void 0 });
  });
  legacy_pre_effect(() => (get(canvas), get(mounted), get(path)), () => {
    get(canvas) && get(mounted) && get(path) && reset_scene();
  });
...
//# sourceMappingURL=C5p8OC7C.js.map
