...
class EnhancedFilteringCallback:
    ...
    def __init__(self):
        ...
        self.heavy_data_service = (
            get_heavy_data_filtering_service() if HEAVY_DATA_AVAILABLE else None
        )
        self.websocket_manager = (
            get_websocket_manager() if HEAVY_DATA_AVAILABLE else None
        )
    ...
    def should_use_heavy_data_processing(
        ...
        use_heavy_processing = (
            data_size_mb > 50  # Large datasets
            or filter_params.get("complexity", "medium") == "high"  # Complex operations
            or filter_params.get("use_heavy_processing", False)  # Explicit request
        )

        logger.info(
            f"Data size: {data_size_mb:.1f}MB, Use heavy processing: {use_heavy_processing}"
        )
    ...
    def process_filtering_request_enhanced(
        ...
        try:
            ...
            use_heavy_processing = self.should_use_heavy_data_processing(
                signal_data, filter_params
            )
    ...
    def _process_with_standard_method(
        ...
        try:
            ...
            filtered_signal = self._apply_standard_filter(
                signal_data, sampling_freq, filter_params
            )
...
def integrate_heavy_data_filtering_with_callbacks():
    ...
    try:
            ...
            if isinstance(result, Generator):
                ...
                for chunk_result in result:
                    if (
                        ...
                        and chunk_result.filtered_signal is not None
                    ):
        ...
        logger.info(
            "Successfully integrated heavy data filtering with existing callbacks"
        )
