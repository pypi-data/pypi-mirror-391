...
from vitalDSP_webapp.layout.pages.tasks_page import (
    create_task_item,
    create_task_details,
)
...
def register_tasks_callbacks(app):
    ...
    def update_tasks_display(n_intervals, refresh_clicks):
        ...
        try:
            ...
            running_count = sum(
                1 for task in all_tasks if task.status.value == "running"
            )
            completed_count = sum(
                1 for task in all_tasks if task.status.value == "completed"
            )
    ...
    def update_task_progress(n_intervals, selected_task_data):
        ...
        """
        if not selected_task_data or not selected_task_data.get("task_id"):
            return ({"display": "none"}, 0, "No task selected", [])
        ...
        try:
            ...
            if not task_progress:
                return ({"display": "none"}, 0, "Task not found", [])
            ...
            details = [
                ...
                html.P(
                    f"Status: {task_progress.status.value.title()}", className="mb-1"
                ),
            ...
            return ({"display": "none"}, 0, "Error loading progress", [])
    ...
    def update_task_history(n_intervals):
        ...
        try:
            ...
            return [
                html.P("Error loading history", className="text-danger text-center")
            ]
