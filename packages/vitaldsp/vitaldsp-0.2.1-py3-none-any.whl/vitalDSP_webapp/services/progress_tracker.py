...
class ProgressTracker:
    ...
    def update_progress(
        ...
        logger.debug(
            f"Updated progress for task {task_id}: {progress_percentage:.1f}% - {current_step}"
        )
        return True
    ...
    def complete_task(
        ...
        logger.info(
            f"Task {task_id} completed with status: {progress_info.status.value}"
        )
    ...
    def cleanup_completed_tasks(self, max_age_hours: int = 24):
        ...
        for task_id, progress_info in self.active_tasks.items():
            if progress_info.end_time:
                age_hours = (
                    current_time - progress_info.end_time
                ).total_seconds() / 3600
    ...
    def to_loading_progress(
        self, task_id: str, loading_strategy: str = "standard"
    ) -> Optional[Dict[str, Any]]:
        """
        Convert ProgressInfo to LoadingProgress format for enhanced data service.
        ...
        Returns:
            Dictionary compatible with LoadingProgress or None if task not found
        """
        if task_id not in self.active_tasks:
            return None
        ...
        if progress_info.progress_percentage > 0:
            total_estimated = elapsed / (progress_info.progress_percentage / 100.0)
            remaining = max(0, total_estimated - elapsed)
        else:
            remaining = 0

        # Convert to LoadingProgress-compatible dict
        return {
            "task_id": task_id,
            "progress_percent": progress_info.progress_percentage,
            "bytes_processed": progress_info.metadata.get("bytes_processed", 0),
            "total_bytes": progress_info.metadata.get("total_bytes", 0),
            "chunks_processed": progress_info.metadata.get("chunks_processed", 0),
            "total_chunks": progress_info.metadata.get("total_chunks", 0),
            "elapsed_time": elapsed,
            "estimated_remaining": remaining,
            "current_chunk_size": progress_info.metadata.get("current_chunk_size", 0),
            "loading_strategy": loading_strategy,
            "status": progress_info.status.value,
            "message": progress_info.current_step,
            "data_preview": progress_info.metadata.get("data_preview"),
        }

    def from_loading_progress(
        self,
        loading_progress: Dict[str, Any],
        operation_name: str = "Data Loading",
    ) -> str:
        """
        Create or update a task from LoadingProgress data.

        Args:
            loading_progress: LoadingProgress dictionary
            operation_name: Name of the operation

        Returns:
            task_id: Task identifier
        """
        task_id = loading_progress.get("task_id")
        if not task_id:
            task_id = str(uuid.uuid4())
        ...
        if task_id in self.active_tasks:
            ...
            self.update_progress(
                task_id=task_id,
                progress_percentage=loading_progress.get("progress_percent", 0),
                current_step=loading_progress.get("message", "Processing"),
                metadata={
                    "bytes_processed": loading_progress.get("bytes_processed", 0),
                    "total_bytes": loading_progress.get("total_bytes", 0),
            ...
            progress_info = ProgressInfo(
                ...
                current_step=loading_progress.get("message", "Processing"),
                total_steps=100,
                current_step_number=int(loading_progress.get("progress_percent", 0)),
                start_time=datetime.now(),
                metadata={
                    "bytes_processed": loading_progress.get("bytes_processed", 0),
                    "total_bytes": loading_progress.get("total_bytes", 0),
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            if exc_type is None:
                ...
                self.tracker.complete_task(
                    self.task_id, success=False, error_message=str(exc_val)
                )
