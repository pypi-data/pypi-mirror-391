...
def create_error_alert(
    ...
    if show_details and isinstance(error, Exception):
        alert_content.extend(
            [
                html.Hr(),
                html.P([html.Strong("Error Type: "), error_type], className="mb-1"),
                html.P(
                    [html.Strong("Context: "), getattr(error, "context", "Unknown")],
                    className="mb-1",
                ),
            ]
        )
...
def create_user_friendly_error_message(error: Exception) -> str:
    ...
    if isinstance(error, WebappError):
        ...
        if "upload" in error_type.lower():
            return (
                f"Upload Error: {error.message}. Please check your file and try again."
            )
        ...
        if "file" in error_msg.lower():
            ...
            return (
                f"An error occurred: {error_msg}. Please try again or contact support."
            )
...
def safe_execute(
    func, *args, default_return=None, error_context: str = "Unknown", **kwargs
):
    """
...
def validate_required_fields(
    data: Dict[str, Any], required_fields: list, context: str = "Data validation"
) -> None:
    """
...
def validate_data_types(
    ...
    context: str = "Data type validation",
) -> None:
    """
