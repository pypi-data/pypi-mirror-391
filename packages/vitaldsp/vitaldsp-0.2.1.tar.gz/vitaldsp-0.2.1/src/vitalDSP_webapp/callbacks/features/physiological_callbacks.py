...
def perform_physiological_analysis(
    time_data,
    signal_data,
    signal_type,
    sampling_freq,
    ...
    try:
        ...
        if "hrv" in analysis_categories:
            results["hrv_metrics"] = analyze_hrv(
                signal_data, sampling_freq, hrv_options
            )
        ...
        if "segmentation" in analysis_categories:
            results["segmentation_metrics"] = analyze_segmentation(
                signal_data, sampling_freq
            )
        ...
        if "statistical" in analysis_categories:
            results["statistical_metrics"] = analyze_statistical(
                signal_data, sampling_freq
            )
        ...
        if advanced_features:
            results["advanced_features_metrics"] = analyze_advanced_features(
                signal_data, sampling_freq, advanced_features
            )

        # Signal Quality & Artifact Detection
        if quality_options:
            results["quality_metrics"] = analyze_signal_quality_advanced(
                signal_data, sampling_freq, quality_options
            )

        # Signal Transforms
        if transform_options:
            results["transform_metrics"] = analyze_transforms(
                signal_data, sampling_freq, transform_options
            )

        # Advanced Computation
        if advanced_computation:
            results["advanced_computation_metrics"] = analyze_advanced_computation(
                signal_data, sampling_freq, advanced_computation
            )

        # Feature Engineering
        if feature_engineering:
            results["feature_engineering_metrics"] = analyze_feature_engineering(
                signal_data, sampling_freq, feature_engineering, signal_type
            )

        # Preprocessing
        if preprocessing:
            results["preprocessing_metrics"] = analyze_preprocessing(
                signal_data, sampling_freq, preprocessing
            )
...
def analyze_waveform(signal_data, sampling_freq):
    ...
    try:
        ...
        waveform_metrics = {
            ...
            "skewness": np.mean(
                ((signal_data - np.mean(signal_data)) / np.std(signal_data)) ** 3
            ),
            "kurtosis": np.mean(
                ((signal_data - np.mean(signal_data)) / np.std(signal_data)) ** 4
            ),
            "peak_to_peak": np.max(signal_data) - np.min(signal_data),
        ...
        if "fourier" in transform_options:
            ...
            try:
                ...
                if len(fft_magnitude) > 1:
                    ...
                    transform_metrics["fourier_peak"] = float(
                        dominant_freq_idx * sampling_freq / len(signal_data)
                    )
                else:
            ...
            try:
                ...
                if np.std(signal_data) > 0:
                    ...
                    transform_metrics["hilbert_phase"] = float(
                        np.mean(np.angle(analytic_signal))
                    )
                else:
...
def perform_physiological_analysis_enhanced(
    time_data,
    signal_data,
    signal_type,
    sampling_freq,
    ...
    try:
        ...
        if "hrv" in analysis_categories:
            results["hrv_metrics"] = get_vitaldsp_hrv_analysis(
                signal_data, sampling_freq, hrv_options, signal_type
            )

        # Morphology Analysis with vitalDSP integration
        if "morphology" in analysis_categories:
            results["morphology_metrics"] = get_vitaldsp_morphology_analysis(
                signal_data, sampling_freq, morphology_options, signal_type
            )
        ...
        if quality_options:
            results["quality_metrics"] = get_vitaldsp_signal_quality(
                signal_data, sampling_freq, quality_options, signal_type
            )

        # Signal Transforms with vitalDSP integration
        if transform_options:
            results["transform_metrics"] = get_vitaldsp_transforms(
                signal_data, sampling_freq, transform_options, signal_type
            )

        # Advanced Computation with vitalDSP integration
        if advanced_computation:
            results["advanced_computation_metrics"] = get_vitaldsp_advanced_computation(
                signal_data, sampling_freq, advanced_computation, signal_type
            )
        ...
        if feature_engineering:
            results["feature_engineering_metrics"] = get_vitaldsp_feature_engineering(
                signal_data, sampling_freq, feature_engineering, signal_type
            )
        ...
        try:
            ...
            # Waveform Analysis
            if "waveform" in analysis_categories:
                results["waveform_metrics"] = analyze_waveform(
                    signal_data, sampling_freq
                )
            ...
            # Frequency Analysis
            if "frequency" in analysis_categories:
                results["frequency_metrics"] = analyze_frequency(
                    signal_data, sampling_freq
                )
