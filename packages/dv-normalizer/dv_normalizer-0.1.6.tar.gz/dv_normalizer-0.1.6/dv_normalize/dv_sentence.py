...
class DhivehiSentence:
    ...
    processed: str
    length: int
    is_continuation: bool = False  # Flag to mark continuation chunks from the same sentence
...
class DhivehiTextProcessor:
        ...
        for word in words:
            ...
            for mapping in self.suffix_mappings:
                if word.endswith(mapping["suffix"]):
                    # For exact matches (like standalone abbreviations), replace the whole word
                    if word == mapping["suffix"]:
                        mapped_word = mapping["spoken_as"]
                    else:
    ...
    def replace_dhivehi_consonant_with_full_word(self, sentence: str) -> str:
        ...
        def replace_with_full_form_with_period(match):
            """Handle abbreviations with period: 'ކ. ގުޅީ'"""
            abbr = match.group(1)
            ...
            # Map each letter in the abbreviation individually
            # For single letter: use full mapping
            # For multi-letter: use first word of each mapping and join without spaces
            if len(abbr) == 1:
                # Single letter abbreviation - use full mapping
                if abbr in letter_map:
                    return f"{letter_map[abbr]} {following_word}"
                else:
                    return f"{abbr}. {following_word}"
            else:
                # Multi-letter abbreviation - use first word of each letter's mapping
                mapped_parts = []
                for char in abbr:
                    if char in letter_map:
                        # Get the first word from the mapping
                        mapping = letter_map[char]
                        first_word = mapping.split()[0] if ' ' in mapping else mapping
                        # For multi-word mappings, take only the base form (first 2-3 chars typically)
                        # This handles cases like "ދާލު" -> "ދާ"
                        if len(first_word) > 2 and char in ['ދ', 'ފ', 'ލ', 'ގ']:
                            # Extract just the first part (typically 2 chars for base form)
                            first_word = first_word[:2]
                        mapped_parts.append(first_word)
                    else:
                        mapped_parts.append(char)
                
                # Join without spaces for multi-letter abbreviations
                mapped_abbr = ''.join(mapped_parts)
                return f"{mapped_abbr} {following_word}"
        
        def replace_with_full_form_no_period(match):
            """Handle abbreviations without period: 'ކ ގުޅީ'"""
            abbr = match.group(1)
            following_word = match.group(2)
        ...
        # Pattern to match single or double Dhivehi letter abbreviations
        # Case 1: "ކ. ގުޅީ" - consonant with period, space, and word
        # Case 2: "ކ.ގުޅީ" - consonant with period (no space), and word
        # Case 3: "ކ ގުޅީ" - consonant with space and word
        pattern1 = r'\b([ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ]|[ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ][ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ])\.\s+([^\s]+)'
        pattern1_no_space = r'\b([ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ]|[ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ][ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ])\.([^\s]+)'
        pattern2 = r'\b([ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ]|[ހށނރބޅކއވމފދތލގޏސޑޖޗޒޓޔޕޤޥ])\s+([^\s]+)'
        
        # First handle abbreviations with periods and space followed by words
        formatted_text = re.sub(pattern1, replace_with_full_form_with_period, sentence)
        # Then handle abbreviations with periods (no space) followed by words
        formatted_text = re.sub(pattern1_no_space, replace_with_full_form_with_period, formatted_text)
        # Finally handle abbreviations without periods followed by words
        formatted_text = re.sub(pattern2, replace_with_full_form_no_period, formatted_text)
    ...
    def process_text(self, text: str,consonant_full_word:bool=True, apply_dv_numbers:bool=True) -> List[DhivehiSentence]:
        ...
        for line, has_period in lines_with_periods:
            ...
            # Process each chunk, but only add period to the very last chunk
            # Mark chunks from the same line so process_full_text can handle them correctly
            for i, chunk_text in enumerate(total_chunks):
                result = self.process_sentence(chunk_text, consonant_full_word, apply_dv_numbers)
                if result:
                    # Only add period to the very last chunk of the line
                    is_last_chunk = (i == len(total_chunks) - 1)
                    # Mark if this is a continuation chunk (not the last chunk)
                    if not is_last_chunk:
                        # Store a flag to indicate this is a continuation
    ...
    def process_full_text(self, text: str, consonant_full_word: bool = True, apply_dv_numbers: bool = True, sentence_separator: str = "، ") -> str:
        ...
        # But don't add separator between continuation chunks (chunks from the same sentence)
        cleaned_sentences = []
        current_sentence_parts = []
        
        for sentence in processed_sentences:
            cleaned = sentence.processed.rstrip('.')
            current_sentence_parts.append(cleaned)
            
            # If this is not a continuation, it's the end of a sentence
            # Join all parts and add to cleaned_sentences
            if not sentence.is_continuation:
                if current_sentence_parts:
                    combined = " ".join(current_sentence_parts)
                    cleaned_sentences.append(combined)
                    current_sentence_parts = []
        
        # Handle any remaining parts (shouldn't happen, but just in case)
        if current_sentence_parts:
            combined = " ".join(current_sentence_parts)
            cleaned_sentences.append(combined)
    ...
    def process_full_text_preserve_line_breaks(self, text: str, consonant_full_word: bool = True, apply_dv_numbers: bool = True, sentence_separator: str = "، ") -> str:
        ...
        for line in original_lines:
            ...
            # Combine processed sentences from this line using the separator
            # Handle continuation chunks properly
            cleaned_sentences = []
            current_sentence_parts = []
            ...
            combined_line = sentence_separator.join(cleaned_sentences)
    ...
    # Old Aliases
    def spoken_dv(self, text: str, preserve_line_breaks: bool = True, sentence_separator: str = "، ") -> str:
        """Alias for process_full_text with sentence_separator option.
        
        Args:
            text: The text to process
            preserve_line_breaks: Whether to preserve line breaks (default: True)
            sentence_separator: The separator to use between sentences (default: "، ")
        
        Returns:
            Processed text with the specified sentence separator between sentences
        """
        if preserve_line_breaks:
            return self.process_full_text_preserve_line_breaks(text, sentence_separator=sentence_separator)
        else:
            return self.process_full_text(text, sentence_separator=sentence_separator)
