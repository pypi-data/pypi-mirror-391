...
def check_auth(api_url: str = DEFAULT_API_URL, offline_mode: bool = False) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Unauthorized - check for specific token status
            token_status = response.headers.get("X-Token-Status", "")
            ...
            # Return specific error message based on token status
            if token_status == "revoked":
                return False, "Your API token has been revoked. Please generate a new token."
            elif token_status == "deleted":
                return False, "Your API token has been deleted. Please generate a new token."
            elif token_status == "expired":
                return False, "Your API token has expired. Please generate a new token."
            else:
                return False, error_detail or "Authentication failed"
        else:
...
def require_auth(api_url: str = DEFAULT_API_URL) -> str:
    ...
    """
    is_auth, token_or_error = check_auth(api_url)
    
    if is_auth and token_or_error:
    ...
    if token_or_error and isinstance(token_or_error, str):
        if "revoked" in token_or_error.lower() or "deleted" in token_or_error.lower() or "expired" in token_or_error.lower():
            console.print(f"\n[red]{token_or_error}[/red]")
            console.print("\n[yellow]Your saved token is no longer valid. Please generate a new one.[/yellow]")
        else:
            console.print(f"\n[red]{token_or_error}[/red]")
    else:
