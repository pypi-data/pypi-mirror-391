                ...
                if status_response.status_code == 200:
                    ...
                    if status == 'completed':
                        ...
                        if report_response.status_code == 200:
                            ...
                            # Save report as markdown file in .rohkun/reports/ folder
                            try:
                                rohkun_dir = dir_path / ".rohkun"
                                reports_dir = rohkun_dir / "reports"
                                ...
                                # Write markdown report (convert from text if needed)
                                # If format is json, save as json. Otherwise save as markdown
                                if format == 'json':
                                    report_path = report_path.with_suffix('.json')
                                    report_path.write_text(output_text, encoding='utf-8')
                                else:
                                    # Save as markdown - output_text is already formatted text
                                    report_path.write_text(output_text, encoding='utf-8')
                                
                                console.print(f"\n[green]✓ Report saved to: {report_path.relative_to(dir_path)}[/green]")
                            except Exception as e:
                                # Don't fail the whole operation if saving fails
                                logger.warning(f"Could not save report to .rohkun/reports/: {e}")
                                console.print(f"\n[yellow]Note: Could not save report to .rohkun/reports/: {e}[/yellow]")
                            ...
                            if not no_copy:
                                try:
                                    ...
                                    console.print("[yellow]Note: Install pyperclip to enable clipboard copy (pip install pyperclip)[/yellow]")
                                except Exception as e:
                            ...
                            detail = report_response.json().get('detail', 'Report is being generated') if report_response.headers.get('content-type') == 'application/json' else report_response.text
                            progress.update(task, description=f"Generating report... ({detail[:50]})")
                            time.sleep(2)
                            continue
                        elif report_response.status_code == 404:
                            # Report not found - might still be generating
                            progress.update(task, description="Report not ready yet, waiting...")
                            time.sleep(2)
                            continue
                        else:
                            # Other error
                            error_detail = report_response.json().get('detail', 'Unknown error') if report_response.headers.get('content-type') == 'application/json' else report_response.text
                            console.print(f"[yellow]Warning: Could not fetch report (HTTP {report_response.status_code}): {error_detail}[/yellow]")
                            time.sleep(2)
                            continue
