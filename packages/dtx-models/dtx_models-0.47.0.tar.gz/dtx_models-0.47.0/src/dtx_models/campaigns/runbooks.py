...
from dtx_models.prompt_canvas.attack_strategies.jailbreaks import AttackStrategy as PromptCanvasAttackStrategy
...
class AttackPlugin(BaseModel):
    """Represents an attack plugin identifier.
    
    The plugin field refers to plugin names from the plugin repository.
    """

    plugin: str = Field(..., description="Plugin identifier (e.g., plugin ID like 'ME-001') - refers to plugin names from plugin repo")


# Alias for the prompt canvas AttackStrategy
JailbreakAttackStrategy = PromptCanvasAttackStrategy


class JailbreakStrategyReference(BaseModel):
    """Reference to a jailbreak attack strategy with name and description."""

    name: str = Field(..., description="Name of the attack strategy (e.g., 'crescendo-attack', 'goat-attack')")
    description: Optional[str] = Field(default=None, description="Description of the attack strategy")


class AttackDataset(BaseModel):
    """Represents a dataset of attack plugins with an optional attack strategy."""

    plugins: List[AttackPlugin] = Field(default_factory=list, description="List of attack plugins")
    attack_strategy: Optional[JailbreakStrategyReference] = Field(default=None, description="Optional reference to an attack strategy")
...
class AttackLibraryItem(BaseRepoModel):
    ...
    attack_dataset: Optional[AttackDataset] = Field(default=None, description="Attack dataset containing plugins and optional attack strategy")
...
class AITargetType(BaseRepoModel):
    """Metadata describing types of AI targets that can be tested in campaigns."""


class CampaignType(BaseRepoModel):
    """Metadata describing types of campaigns that can be executed."""
