...
class ModelRegistry:
    ...
    ) -> Optional[Union[HuggingFaceProviderConfig, LitellmProviderConfig]]:
        for repo in self._get_repos_by_provider(provider):
            try:
    ...
    def get_model_by_name(self, model_name: str) -> Optional[Union[HuggingFaceProviderConfig, LitellmProviderConfig]]:
        """
        Search for a model by name in the format 'provider/model_name'.
        If the model is not found, returns None.
        
        Args:
            model_name: Model name in the format 'provider/model_name' (e.g., 'openai/gpt-4', 'groq/llama3-8b-8192')
            
        Returns:
            The model configuration if found, None otherwise
        """
        # Parse the provider/model_name format
        if '/' in model_name:
            provider, actual_model_name = model_name.split('/', 1)
            return self.get_model(actual_model_name, provider)
        else:
