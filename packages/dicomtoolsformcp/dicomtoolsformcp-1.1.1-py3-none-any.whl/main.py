...
from upload import Analysis_dicom_directory_tool, separate_series_by_patient_tool, get_result_tool
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="获文件夹分析结果，返回测量的url，如果没有分析结果，需要进行{Analysis_dicom_directory}工具上传分析，返回测量的url",
            inputSchema={
                "type": "object",
                "properties": {
                    "directory_path": {
                        "type": "string",
                        "description": "查询分析结果的文件夹，使用相对路径"
                    }
                },
                "required": ["directory_path"]
            }
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> list[TextContent]:
    ...
    try:
        ...
        if name == "scan-dicom-directory":
            ...
            args = DirectoryPathArgs(**arguments)
            result = await get_result_tool(args.directory_path)
        else:
