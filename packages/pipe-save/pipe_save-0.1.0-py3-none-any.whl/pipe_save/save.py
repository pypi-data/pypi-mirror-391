...
DEFAULT_CONFIG = "~/.config/pipe-save"
PATH_KEY = "SAVE_PATH"
FRONTMATTER_TAGS = "FABRIC_FRONTMATTER_TAGS"
DATE_FORMAT = "%Y-%m-%d"
load_dotenv(os.path.expanduser(DEFAULT_CONFIG))


def main(tag: str, tags: list[str], silent: bool, fabric: bool):
    ...
    all_tags: list[str] = []
    if fabric:
        frontmatter_tags = os.getenv(FRONTMATTER_TAGS)
        if frontmatter_tags:
            frontmatter_tags_list = frontmatter_tags.split(",")
            all_tags.extend(frontmatter_tags_list)
    if tag:
        all_tags.append(tag)
    if tags:
