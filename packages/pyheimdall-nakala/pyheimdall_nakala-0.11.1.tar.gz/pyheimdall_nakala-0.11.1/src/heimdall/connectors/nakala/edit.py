...
def upload(tree, **options):
    ...
    :Keyword arguments:
        ...
        * **rights** (:py:class:`list`, optional) -- List of :py:class:`dict` for additional rights to give to the created data and collection(s). Each :py:class:`dict` must be something like ``[{'role': 'ROLE_ADMIN', 'id': GROUP_ID}, ]`` ; check Nakala API documentation for a list of available roles.
    """  # nopep8: E501
...
def upload_item(metas, hashes, api_key, collections, rights=None,
                doi=None, private=True, test=True
                ):
    d = Data(
            doi=doi,
