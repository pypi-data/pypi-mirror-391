...
3. **Enter Username**
   ...
   - Navigate to `username_submit_button`, click, then `wait(2)` to ensure the password field is fully rendered.
4. **Enter Password**
   - Navigate to `password_field`, click, and call `type_credential_field(credential_id, "password")`.
   - Navigate to `login_button_final`, click, then `wait(3)` for the dashboard to appear.
...
8. **Download Invoices**
   - For each invoice index `n` from `0` to `(requested_count - 1)`:
     - Compute `normalized_y = 0.412 + (n * 0.065)`, navigate to the download button, click, then `wait(1)` before moving to the next invoice.
