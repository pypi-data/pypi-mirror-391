...
3. **Start Login**
   - Navigate to `login_button_home`, click, then `wait(1)`.
4. **Enter Username**
   - Navigate to `username_field`, click, call `type_credential_field(credential_id, "username")`, then `wait(1)`.
5. **Enter Password**
   - Navigate to `password_field`, click, call `type_credential_field(credential_id, "password")`, then `wait(1)`.
6. **Submit Login**
   - Navigate to `login_button_form`, click, then `wait(3)` for the account page to load.
7. **Navigate to Invoice List**
   - Scroll once with `scroll(-200)` to reveal the invoices section.
   - Navigate to `view_all_invoices_button`, click, then `wait(5)` for the invoice list to render.
8. **Download Invoices**
   - For each invoice index `n` from `0` to `(requested_count - 1)`:
     - Compute `normalized_y = 0.630 + (n * 0.046)`, navigate to the download button, click, then `wait(1)` for the popup.
     - When the popup appears:
       - Navigate to `download_popup_pdf_option`, click to select PDF.
       - Navigate to `download_popup_close`, click to dismiss the popup.
