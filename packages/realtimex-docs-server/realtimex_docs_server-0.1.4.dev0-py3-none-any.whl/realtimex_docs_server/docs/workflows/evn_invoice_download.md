...
7. **Navigate to Invoice List**
   - Ensure the view is at the top by scrolling up if needed (`scroll(200)` once).
   - Perform controlled downward scrolling:
     1. `scroll(-100)` and immediately call `wait_for_element("evn/view_all_invoices_button.png", timeout=6)`.
     2. If not found, scroll an additional `-20` and retry once.
     3. If still not found, scroll another `-20` and retry a final time.
     4. If the button still does not appear, **STOP the workflow** and report that the invoices button was not detected.
   - When the button is detected, use the coordinates returned by `wait_for_element` for the click (no manual offsets).
8. **Download Invoices**
