...
Use the `wait(seconds)` tool to satisfy every pause duration listed below. Each UI action follows this pattern:
1. Call `calculate_screen_coordinates` with the element’s normalized values.
...
2. **Select Credentials**
   - Call `get_credentials()` and identify the credential entry that matches the user’s instruction (e.g., “FPT portal”).
   - Confirm with the user before using the credential reference.
3. **Enter Username**
   - Navigate to `username_field`, click, and call `type_credential_field(credential_id, "username")`.
   - Click `username_submit_button`, then `wait(1)` for the password step to load.
4. **Enter Password**
