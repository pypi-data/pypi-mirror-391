...
3. **Enter Username**
   ...
   - Type: `type_credential_field(credential_id, "username")`.
   - Advance: `calculate_screen_coordinates` for `username_submit_button` → `move_mouse` → `click_mouse` → `wait(2)` to ensure the password field loads.
4. **Enter Password**
   - Focus field: `calculate_screen_coordinates` for `password_field` → `move_mouse` → `click_mouse`.
   - Type: `type_credential_field(credential_id, "password")`.
   - Submit: `calculate_screen_coordinates` for `login_button_final` → `move_mouse` → `click_mouse` → `wait(3)` for the dashboard to appear.
5. **Open Contracts Page**
   - `calculate_screen_coordinates` for `contracts_menu` → `move_mouse` → `click_mouse` → `wait(1)`.
6. **Open Invoices**
   - `calculate_screen_coordinates` for `view_invoices_link` → `move_mouse` → `click_mouse` → `wait(2)`.
7. **Filter Paid Invoices**
   - `calculate_screen_coordinates` for `paid_invoices_tab` → `move_mouse` → `click_mouse` → `wait(2)`.
8. **Download Invoices**
   - For each invoice index `n` from `0` to `(requested_count - 1)`:
     - Compute `normalized_y = 0.412 + (n * 0.065)`.
     - `calculate_screen_coordinates(0.927, normalized_y)` → `move_mouse` → `click_mouse` → `wait(1)` before moving to the next invoice.
