...
def calculate_sps(coord_dict_raw, coord_dict_rand, n_dim, keys=None):
	"""
	calculate STAVAG priority scores (sps)
	"""
	if keys:
		pass
	else:
...
def keep_variant_genes(coord_dict_raw, coord_dict_rand, n_dim, threshold=0.05, keys=None):
	if keys:
		pass
	else:
		keys = ['x', 'y', 'z', 'a'][:n_dim]
	for k in range(n_dim):
		top_percentile = np.percentile(coord_dict_rand[keys[k]], 1-threshold)
		coord_dict_raw[keys[k]] = coord_dict_raw[keys[k]][coord_dict_raw[keys[k]]['Importance']>top_percentile]
	return coord_dict_raw

def DVG_detection(adata, coords, sps=False, threshold=0.05):
	"""
...
def TVG_detection(adata, coords, sps=False, threshold=0.05):
	"""
