...
class AptlyManager:
    ...
    def _create_aptly_config(self, codename: str) -> None:
        ...
        # Publish endpoint for correct directory structure
        # Note: publish_base WITHOUT codename, because prefix in publish includes it
        # Use named endpoint "filesystem:public" instead of empty "" which doesn't work
        publish_root = Path(self.config.publish_base)
    ...
    def delete_repo(self, codename: str, component: str) -> bool:
        ...
        """
        repo_name = self._get_repo_name(codename, component)
        # Use same prefix format as publish (with publish_prefix if set)
        if self.config.publish_prefix:
            prefix_path = f"{self.config.publish_prefix}/{codename}"
        else:
            prefix_path = codename
        prefix = f"filesystem:public:{prefix_path}"
    ...
    def _create_dual_format_symlinks(self, codename: str, component: str) -> None:
        ...
        # Path to new format (actual data location created by aptly)
        # aptly publishes to: {rootDir}/{prefix}/dists/{distribution}/
        # where prefix from publish command = "codename" or "beta/codename"
        # and distribution from -distribution = component
        # Result: /{prefix}/dists/{component}/ BUT aptly adds component dir!
        # Actual: /{prefix}/{component}/dists/{component}/
        if self.config.publish_prefix:
            new_path = (
                publish_base
                / self.config.publish_prefix
                / codename
                / component
                / "dists"
                / component
        ...
        # /opt/repo/public/dists/{codename}/{component}
    ...
    def _publish_snapshot(
        ...
        Example:
            ...
            True
        """
        # With FileSystemPublishEndpoints, prefix depends on publish_prefix:
        # - Empty publish_prefix (stable): prefix = "trixie"
        #   Result: /opt/repo/public/trixie/dists/{component}/
        # - publish_prefix = "beta": prefix = "beta/trixie"
        #   Result: /opt/repo/public/beta/trixie/dists/{component}/
        if self.config.publish_prefix:
            prefix = f"{self.config.publish_prefix}/{codename}"
        else:
            prefix = codename
        ...
        try:
            if is_initial:
                # Initial publish
                # Use named endpoint "public:prefix" to use FileSystemPublishEndpoints["public"]
                self._run_aptly(
                    [
                        "publish",
                        "snapshot",
                        ...
                        snapshot_name,
                        f"filesystem:public:{prefix}",  # "filesystem:public:prefix"
                    ],
                    codename,
                )
