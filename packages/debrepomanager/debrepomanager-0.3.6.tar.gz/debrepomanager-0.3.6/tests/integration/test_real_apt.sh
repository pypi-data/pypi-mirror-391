...
cd /tmp/packages-v1
# Use STABLE (no publish_prefix) for tests to match APT URLs
/app/scripts/repoadd stable bookworm /tmp/packages-v1 jethome-tools > /dev/null 2>&1
...
# NEW format URL: http://localhost:8080/{codename} {component} main
...
cd /tmp/packages-v2
# Use STABLE (no publish_prefix) for tests
/app/scripts/repoadd stable bookworm /tmp/packages-v2 jethome-tools > /dev/null 2>&1
...
# Use STABLE (no publish_prefix) for tests
/app/scripts/repoadd stable noble /tmp/packages-noble jethome-tools > /dev/null 2>&1
...
cd /tmp/pkg-tools
# Use STABLE (no publish_prefix) for tests
/app/scripts/repoadd stable bookworm /tmp/pkg-tools jethome-tools > /dev/null 2>&1
...
# Use STABLE (no publish_prefix) for tests
/app/scripts/repoadd stable bookworm /tmp/pkg-bookworm-comp jethome-bookworm > /dev/null 2>&1
...
cd /tmp/pkg-desktop
# Use STABLE (no publish_prefix) for tests
/app/scripts/repoadd stable bookworm /tmp/pkg-desktop jethome-desktop > /dev/null 2>&1
...
# =============================================================================
# SCENARIO 5: Old APT format compatibility - SKIP FOR NOW
# TODO: Fix dual format symlinks after publish_prefix is working
# =============================================================================
...
echo "  ⏭️ Scenario 5: Old APT format (skipped - TODO)"
...
echo "  ⏭️ Dual format support - TODO (needs fix)"
echo
