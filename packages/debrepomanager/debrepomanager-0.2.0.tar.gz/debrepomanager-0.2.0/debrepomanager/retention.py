...
class RetentionPolicy:
    ...
    def _get_package_upload_date(
        ...
        """Get upload date for a package.

        LIMITATION: Currently returns datetime.now() which makes age_days = 0.
        This means max_age_days cleanup doesn't work in current implementation.
        Only min_versions-based cleanup is functional.
        ...
        Returns:
            Upload date (currently datetime.now() - see LIMITATION)

        TODO for v0.3: Implement proper upload date tracking:
        - Option A: Get file mtime from pool directory
        - Option B: Track upload_date in metadata.json
        - Option C: Parse aptly snapshot creation dates
        """
        # TEMPORARY: Return datetime.now() for MVP
        # This makes all packages appear "new" (age_days = 0)
        # Result: Only min_versions cleanup works, max_age_days is ineffective
        # This is documented limitation for Phase 8 MVP
