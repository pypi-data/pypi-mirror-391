"""
Email activity logs resource for the Naxai SDK.

This module provides methods for retrieving and analyzing email activity logs,
including delivery status, engagement metrics, and event history for emails
sent through the Naxai platform.
...
class ActivityLogsResource:
    ...
    def list(self,
        ...
        Example:
            ...
            ...     sort="createdAt:asc"
            ... )
            >>> print(f"Found {len(delivered_emails.messages)} delivered emails to "
            >>>                 "customer@example.com in the last week")
            >>> 
            >>> # Calculate engagement metrics
            >>> if delivered_emails.messages:
            ...     opened = sum(1 for msg in delivered_emails.messages if \
            ...                 msg.opens and msg.opens > 0)
            ...     clicked = sum(1 for msg in delivered_emails.messages if msg.clicks and \
            ...               msg.clicks > 0)
        ...
        Note:
            ...
            * "status": Sort by delivery status
            ...
            - The client_id and campaign_id parameters help filter emails associated with
              specific entities
            - The status parameter filters by delivery status ("sent", "delivered", "failed")
            - For detailed information about a specific email, use the get() method with
              its message_id
