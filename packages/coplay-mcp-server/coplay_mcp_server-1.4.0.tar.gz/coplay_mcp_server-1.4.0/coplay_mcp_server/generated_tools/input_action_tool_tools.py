...
async def generate_wrapper_code(
    ...
    try:
        logger.debug(f"Executing generate_wrapper_code with parameters: {locals()}")
        ...
        # Execute Unity RPC call
        result = await _unity_client.execute_request('generate_wrapper_code', params)
        logger.debug(f"generate_wrapper_code completed successfully")
        return result

    except Exception as e:
        logger.error(f"Failed to execute generate_wrapper_code: {e}")
        raise RuntimeError(f"Tool execution failed for generate_wrapper_code: {e}")
...
def register_tools(mcp: FastMCP, unity_client: UnityRpcClient) -> None:
    ...
    # Register generate_wrapper_code
    mcp.tool()(generate_wrapper_code)
