...
"""
    ...
    if args.command == 'scan':
        ...
        # Upload to backend if --api-url and --token provided (CI/CD mode)
        ...
        # Legacy upload mode (--upload flag with stored auth)
        elif args.upload:
            print(f"\n📤 Uploading results to backend...")
        ...
        # Return exit code based on --fail-on threshold (doesn't print fail message)
        # The extension/pipeline will decide what to do with the exit code
        if args.fail_on:
            ...
            if found_blocking:
                # Exit with code 1 so pipeline can catch it
                sys.exit(1)
