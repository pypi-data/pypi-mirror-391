...
class WebSapCommonFunctions:
    ...
    def keep_sap_session_alive(self, stop_event: Event):
        ...
        while not stop_event.is_set():  # 检查是否被要求停止
            try:
                ...
                self.browser.execute_script("""
                    fetch(window.location.href, {cache:'no-store'}).then(()=>{}).catch(()=>{}); 
                """)
            except Exception:
                pass
            sleep(300)
    ...
    def wait_element_invisible_by_css_selector(self, css_selector: str, by: By = By.CSS_SELECTOR):
        ...
        """
        stop_event = Event()
        thr = Thread(target=self.keep_sap_session_alive, args=(stop_event,), daemon=True)
        thr.start()
        try:
