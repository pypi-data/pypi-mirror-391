...
class WebSapCommonFunctions:
    ...
    def keep_sap_session_alive(self):
        ...
        js_script = """
        (async () => {
          console.log("🟢 SAP KeepAlive script started.");

          async function sendKeepAlive() {
            try {
              // ① 动态读取 token & host
              const token = window.SEC_SESSTOKEN;
              const origin = window.location.origin;
              if (!token) {
                console.error("❌ 未检测到 window.SEC_SESSTOKEN，可能还未进入 SAP 主界面。");
                return;
              }
              ...
              // ④ 从 HTML 提取 /sap(cz1...)/
              const match = html.match(/action="(\\/sap\\(.*?\\)\\/bc\\/gui\\/sap\\/its\\/webgui\\/)"/);
              if (!match) {
                console.error("❌ 未找到 session handle (sap(...))");
                return;
              }

              const path = match[1].replace("/bc/gui/sap/its/webgui/", "/");
              const url = `${origin}${path}bc/gui/sap/its/webgui/state/ur?~RG_WEBGUI=X&~SEC_SESSTOKEN=${token}`;
              console.log("🔗 Sending keepalive:", url);

              // ⑤ 发出一次心跳请求
              const keep = await fetch(url, {
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                credentials: "include"
              });

              const text = await keep.text();
              console.log(`✅ [${new Date().toLocaleTimeString()}] Keepalive OK, status: ${keep.status}`);
              console.log("📄 响应片段:", text.slice(0, 120));
    ...
    def auto_get_browser_screenshots(self, save_folder_path, time_interval=15, browser_screenshot_tag='', remote_save=False, username='', password='', server_name='',
        ...
        try:
            ...
            smb_client = None
            print('[SMB CONNECTION ERROR]:', traceback.format_exc())
        ...
        while not self.stop_screenshot_thread.is_set():
            ...
            try:
                ...
                sleep(1)
                if remote_save and smb_client is not None:
