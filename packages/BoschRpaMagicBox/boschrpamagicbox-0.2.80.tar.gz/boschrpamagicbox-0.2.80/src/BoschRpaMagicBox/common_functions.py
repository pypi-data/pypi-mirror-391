...
class WebSapCommonFunctions:
    ...
    def keep_sap_session_alive(self):
        ...
        # while not stop_event.is_set():  # 检查是否被要求停止
        self.browser.execute_script("""
        if (!window._sap_webgui_keepalive) {
            console.log("[SAP WebGUI] Injecting keepalive JS loop...");
            window._sap_webgui_keepalive = setInterval(() => {
                try {
                    const base = window.location.href.split('/bc/gui/sap/its/webgui')[0];
                    const url = base + '/bc/gui/sap/its/webgui/batch/json?~RG_WEBGUI=X&sap-statistics=true';
                    fetch(url, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(r => console.log('[SAP KeepAlive]', r.status, new Date().toLocaleTimeString()))
                    .catch(e => console.log('[SAP KeepAlive ERROR]', e));
                } catch (err) {
                    console.error('[SAP KeepAlive EXCEPTION]', err);
                }
            }, 300000);  // 每5分钟执行一次
        }
        """)
