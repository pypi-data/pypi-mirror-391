...
def printer(**kwargs) -> str:
    ...
    try:
        ...
        logging.warning("Ruff is not installed, please install it (uv pip install ruff)")
    except subprocess.CalledProcessError as err:
        logging.warning(f"Ruff formatting failed: {err.stderr.decode()}")
    return code
...
def print_error(**kwargs) -> list[str]:
    """Helper for print_fill_axe."""
    ...
    match (kwargs["window_y"] is None, kwargs["window_x"] is None):
        ...
        case (False, True):
            middle = [
                ...
                "            k: [v for v, m in zip(vs, mask) if m] for k, vs in values.items()",
                "        }",
                f'        axes[0, j].set_title({kwargs["window_y"]!r} f"={{window_y_value}}")',
...
def print_main(**kwargs) -> list[str]:
    ...
    code.extend([
        "",
        "    # save the figure",
        "    plt.savefig(FIGNAME, format=FIGNAME.suffix[1:], transparent=False)",
        "    plt.show()",
        "",
        "",
    ])
...
def print_read_sql(**kwargs) -> list[str]:
    ...
    # get sql query
    atom_names, line_extractor = merge_extractors(set(), alias=alias, select=kwargs["filter"])
    ...
    return [
        *line_extractor,
        ...
        '    with sqlite3.connect(database) as conn, Printer("Read database...") as prt:',
        "        conn.row_factory = sqlite3.Row",
        "        for i, line in enumerate(  # enumerate is for debug",
