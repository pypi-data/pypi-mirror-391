...
class ATsheet(GADataFileFormat):
    ...
    def parse_csv(cls, filename: str) -> ATResults:
        with open(filename, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                sid = int(row[0])
                values = [float(val) for val in row[1:] if val and val[0].isnumeric()]
                results.append((sid, dict(zip(tool_names, values))))
    ...
    def dump_files(
        ...
        for tool in at_data.results.tools:
            ...
            for _, results in at_data.results.results:
                ...
                sheet.append([next(grade_labels), -1 if grade is None else grade])
