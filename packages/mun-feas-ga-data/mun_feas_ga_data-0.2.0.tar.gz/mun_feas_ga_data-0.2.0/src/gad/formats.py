...
class GADataFile(NamedTuple):
    ...
    def course_and_semester(self):
        return f"{self.course} {self.academic_year}-{self.semester:02}"
    ...
    def __str__(self):
        ...
        return f"{course} ({semester_name} {year}): {len(self.results)} results"
...
class FEAMS(GADataFileFormat):
    ...
    def dump_files(
        ...
        if course not in curriculum_map.course_names:
            print(f"{course} not in curriculum map; skipping course")
            return
