...
class FileFormat(abc.ABC):
    ...
    def parse(
        ...
        filename : str
          ...
          We use filenames rather than already-open file buffers because we might need
...
class GADataFile(NamedTuple):
    ...
    # Parser warnings: (filename, message) pairs
...
class ATsheet(GADataFileFormat):
    ...
    def parse(
        ...
        if len(unnamed) > 0:
            warnings.append((filename, f"contains {len(unnamed)} unnamed columns"))
...
class FEAMS(GADataFileFormat):
    ...
    def parse(
        ...
        warnings = [(filename, "missing student IDs")]
...
def parse(
    files_or_directories: Iterable[str],
    curriculum_map: Optional[CurriculumMap],
    validate: bool = False,
    warn_unmapped: bool = False,
) -> list[GADataFile]:
    ...
    for filename in collect_files(files_or_directories):
        ...
        for file_format in all_formats:
            if file_format.filename_matches(os.path.basename(filename)):
                all_files.append(
                    file_format.parse(filename, curriculum_map, validate, warn_unmapped)
                )
