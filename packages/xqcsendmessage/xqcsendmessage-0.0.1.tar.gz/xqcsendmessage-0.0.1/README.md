...
[![PyPI version](https://img.shields.io/badge/PyPI-Not%20Published-lightgrey)](https://github.com/xiaoqiangclub/XQCSendMessage) [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT) [![Python Versions](https://img.shields.io/badge/Python-3.10%2B-blue)](https://github.com/xiaoqiangclub/XQCSendMessage)
...
- `**config`: 关键字参数，用于配置特定发送器。这些参数会传递给底层发送器的 `__init__` 方法。同时，您也可以在此处设置消息的默认参数，这些默认参数可以在调用 `.send()` 方法时被覆盖。
  ...
  - **钉钉 (`sender_type="dingtalk"`)**:
    ...
    - `at` (Dict[str, Any], optional): 默认 @ 人参数，例如 `{"isAtAll": True}` 或 `{"atMobiles": ["139xxxx0000"]}`。
  - **企业微信 Webhook (`sender_type="wecom_webhook"`)**:
    - `webhook` (str): 企业微信机器人的 Webhook 地址。
    - `message` (Union[str, Dict[str, Any]], optional): 默认消息内容或消息体。
    - `touser` (str, optional): 默认接收成员，例如 `"@all"` 或 `"USERID1|USERID2"`。
    - `toparty` (str, optional): 默认接收部门，例如 `"PARTYID1|PARTYID2"`。
    - `totag` (str, optional): 默认接收标签，例如 `"TAGID1|TAGID2"`。
...
- `**kwargs`: 关键字参数，用于覆盖客户端初始化时设置的默认消息参数，或者传递给底层发送器的额外参数。
  ...
  - **钉钉**: 可以覆盖 `at` 参数 (`{"isAtAll": True}` 或 `{"atMobiles": ["139xxxx0000"]}` 等)。
  - **企业微信**: 可以覆盖 `touser`, `toparty`, `totag`, `safe`, `enable_duplicate_check`, `duplicate_check_interval` 等。
...
def main_dingtalk_sync():
    ...
    client = SyncClient(
        ...
        webhook=WEBHOOK_URL,
        secret=SECRET,
        # 可以在初始化时设置默认消息内容和 @ 人参数，这些参数会在 send 方法中作为默认值，但可以被 send 方法中的 kwargs 覆盖
        message="【XQCSendMessage】这是一条来自 XQCSendMessage 的同步测试消息。",
        at={"isAtAll": True} # 默认发送给所有人
    )

    # 2. 发送消息
    try:
        # 发送字符串消息，此时会使用初始化时设置的默认 @ 人参数（发送给所有人）
        result_default_at = client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的同步测试消息 (使用默认 @ 人)。"
        )
        print(f"✅ 钉钉消息发送成功 (默认 @ 人): {result_default_at}")

        # 发送字符串消息，并覆盖 @ 人参数（指定手机号）
        result_override_at = client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的同步测试消息 (覆盖 @ 人，指定手机号)。",
            at={"atMobiles": ["13900139000"], "isAtAll": False}
        )
        print(f"✅ 钉钉消息发送成功 (覆盖 @ 人): {result_override_at}")

        # 发送字典消息，并覆盖 @ 人参数（指定手机号）
        result_dict_override_at = client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条来自 XQCSendMessage 的同步测试消息 (字典，覆盖 @ 人，指定手机号)。"},
                "at": {"atMobiles": ["13700137000"], "isAtAll": False}
            }
        )
...
def main_wecom_webhook_sync():
    ...
    client = SyncClient(
        ...
        webhook=WEBHOOK_URL,
        # 可以在初始化时设置默认消息内容和参数，这些参数会在 send 方法中作为默认值，但可以被 send 方法中的 kwargs 覆盖
    ...
    try:
        ...
        result_override_str = client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的同步 Webhook 测试消息 (覆盖字符串内容，指定用户)。",
        ...
        result_override_dict = client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条来自 XQCSendMessage 的同步 Webhook 测试消息 (覆盖字典内容，指定用户)。"}
            },
            touser="USERID" # 可以在这里覆盖 touser
        )
...
def main_wecom_app_sync():
    ...
    try:
        ...
        result_override_dict = client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条通过企业微信应用发送的同步测试消息 (覆盖字典内容，指定用户)。"}
            },
            touser="USERID" # 可以在这里覆盖 touser
        )
...
async def main_dingtalk_async():
    ...
    try:
        ...
        result_default_at = await client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的异步测试消息 (使用默认 @ 人)。"
        )
        print(f"✅ 钉钉消息发送成功 (默认 @ 人): {result_default_at}")

        # 发送字符串消息，并覆盖 @ 人参数（指定手机号）
        result_override_at = await client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的异步测试消息 (覆盖 @ 人，指定手机号)。",
            at={"atMobiles": ["13900139000"], "isAtAll": False}
        )
        print(f"✅ 钉钉消息发送成功 (覆盖 @ 人): {result_override_at}")
        ...
        result_dict_override_at = await client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条来自 XQCSendMessage 的异步测试消息 (字典，覆盖 @ 人，指定手机号)。"},
                "at": {"atMobiles": ["13700137000"], "isAtAll": False}
            }
        )
        print(f"✅ 钉钉消息发送成功 (字典，覆盖 @ 人): {result_dict_override_at}")
...
async def main_wecom_webhook_async():
    ...
    try:
        ...
        result_override_str = await client.send(
            "【XQCSendMessage】这是一条来自 XQCSendMessage 的异步 Webhook 测试消息 (覆盖字符串内容，指定用户)。",
            touser="USERID" # 可以在这里覆盖 touser
        )
        print(f"✅ 企业微信 Webhook 消息发送成功 (覆盖字符串): {result_override_str}")
        ...
        result_override_dict = await client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条来自 XQCSendMessage 的异步 Webhook 测试消息 (覆盖字典内容，指定用户)。"}
            },
            touser="USERID" # 可以在这里覆盖 touser
        )
        print(f"✅ 企业微信 Webhook 消息发送成功 (覆盖字典): {result_override_dict}")
...
async def main_wecom_app_async():
    ...
    try:
        ...
        result_override_dict = await client.send(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条通过企业微信应用发送的异步测试消息 (覆盖字典内容，指定用户)。"}
            },
            touser="USERID" # 可以在这里覆盖 touser
        )
        print(f"✅ 企业微信应用消息发送成功 (覆盖字典): {result_override_dict}")
...
async def main_direct_dingtalk():
    ...
    try:
        ...
        result_dict_override_at = await send_dingtalk_async(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条通过 XQCSendMessage 直接发送的异步钉钉测试消息 (字典，指定手机号)。"},
                "at": {"atMobiles": ["13700137000"], "isAtAll": False}
            },
            webhook=WEBHOOK_URL,
            secret=SECRET
...
try:
    ...
    result_direct_dict_override = send_wecom_webhook(
        {
            "msgtype": "text",
            "text": {"content": "【XQCSendMessage】这是一条通过 XQCSendMessage 直接发送的同步 Webhook 测试消息 (字典，指定用户)。"}
        },
        webhook=WEBHOOK_URL,
        touser="USERID" # 可以在这里覆盖 touser
    )
    print(f"✅ 直接调用企业微信 Webhook 消息发送成功 (字典，指定用户): {result_direct_dict_override}")
...
async def main_direct_wecom_app_async():
    ...
    try:
        ...
        result_direct_dict_override = await send_wecom_app_async(
            {
                "msgtype": "text",
                "text": {"content": "【XQCSendMessage】这是一条通过企业微信应用直接发送的异步测试消息 (字典，指定用户)。"},
            },
            corpid=CORP_ID,
            corpsecret=CORP_SECRET,
            agentid=AGENT_ID,
