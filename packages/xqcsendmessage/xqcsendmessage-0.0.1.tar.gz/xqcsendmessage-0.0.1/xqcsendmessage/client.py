...
class BaseClient:
    ...
    def __init__(self, sender_type: SenderType, **config: Any):
        ...
        if sender_type == "email":
            ...
            # 钉钉支持的额外参数，如 at
            # 企业微信支持的额外参数，如 touser, toparty, totag, agentid, safe, enable_duplicate_check, duplicate_check_interval
            # 同时兼容旧的 message 参数
            for key in ["msgtype", "touser", "toparty", "totag", "safe", "enable_duplicate_check", "duplicate_check_interval", "at", "message"]:
                if key in config:
    ...
    def send(self, message: Optional[Union[str, Dict[str, Any]]] = None, **kwargs: Any) -> Dict[str, Any]:
        ...
        if self._initial_sender_type == "email":
            ...
            if isinstance(message, str):
                # 检查 merged_kwargs 中是否有 msgtype 和 text.content，如果有则覆盖
                text_content = merged_kwargs.pop("message", message) # 兼容旧的 message 参数
                if isinstance(text_content, dict) and "text" in text_content and "content" in text_content["text"]:
                    text_content = text_content["text"]["content"]
                elif isinstance(text_content, dict) and "content" in text_content:
                    text_content = text_content["content"]
                
                final_message_body = {
                    "msgtype": "text",
                    "text": {"content": text_content if text_content else message}
                }
            elif isinstance(message, dict):
            ...
            # 将 final_message_body 中的内容合并到 merged_kwargs 中，以便发送器统一处理
            # 这里的合并逻辑需要确保 message 本身的内容（如果是字典）不会被 merged_kwargs 中的同名键覆盖
            # 也就是说，final_message_body 中的内容是最终的消息结构，merged_kwargs 提供的是额外的参数
            # 最终传递给 sender.send 的是：消息体 (final_message_body) 和额外的 kwargs (merged_kwargs)
            # 底层 sender.send 方法会处理合并逻辑
            if "message" in merged_kwargs:
