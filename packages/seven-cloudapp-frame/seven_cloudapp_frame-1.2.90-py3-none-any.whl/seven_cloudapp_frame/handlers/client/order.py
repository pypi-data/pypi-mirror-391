...
class UpdateOrderAddressHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        try:
            ...
            result = prize_order_model.update_table('real_name=%s,telephone=%s,province=%s,city=%s,county=%s,street=%s,address=%s,sync_status=%s,sync_count=0,sync_result=%s',
                                                    "order_no=%s",
                                                    params=[real_name, telephone, province, city, county, street, address, sync_status, '', order_no])
            if result is False:
