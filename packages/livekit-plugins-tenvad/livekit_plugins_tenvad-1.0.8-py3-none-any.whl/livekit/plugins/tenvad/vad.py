...
class VAD(agents.vad.VAD):
    """
    TEN Voice Activity Detection (VAD) class.

    This class provides functionality to detect speech segments within audio data using the TEN VAD model.
    """  # noqa: E501
    ...
    def load(
        ...
        *,
        ...
        This method loads the TEN VAD model and prepares it for inference. When options are not provided,
        ...
        **Example:**
            ...
            def prewarm(proc: JobProcess):
                proc.userdata["vad"] = ten.VAD.load()
...
class VADStream(agents.vad.VADStream):
    ...
    async def _main_task(self) -> None:
        inference_data = np.empty(self._model.window_size_samples, dtype=np.float32)
        ...
        async for input_frame in self._input_ch:
            ...
            while True:
                ...
                inference_data = np.frombuffer(
                    inference_frame.data[: self._model.window_size_samples], dtype=np.int16
                )

                # run the inference
                p = await self._loop.run_in_executor(self._executor, self._model, inference_data)
