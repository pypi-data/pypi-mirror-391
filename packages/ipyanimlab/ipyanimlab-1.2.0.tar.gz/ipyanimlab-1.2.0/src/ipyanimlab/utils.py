...
def fast_negexp(x):
    ...
    Args:
        :x: (float or tensor): input value(s) to approximate the negative exponential for
...
def halflife_to_damping(halflife, eps=1e-5):
    ...
    Args:
        :halflife: (float or tensor): halflife controlling spring decay speed
...
def quat_abs(q):
    ...
    Args:
        :q: (tensor): quaternion(s) [..., 4] where q[...,0] is the real component
...
def quat_from_scaled_angle_axis(x, eps=1e-8):
    ...
    Args:
        :x: (tensor): scaled angle-axis vector(s) [..., 3]
        :eps: (float): small epsilon for numerical stability
...
def simple_spring_damper_exact(x, v, x_goal, halflife, dt):
    ...
    Args:
        :x: (float or tensor): current position
        :v: (float or tensor): current velocity
        :x_goal: (float or tensor): target position
        :halflife: (float): time for amplitude to decay by half
        :dt: (float): time step
...
def simple_spring_damper_exact_quat(x, v, x_goal, halflife, dt):
    ...
    Args:
        :x: (tensor): current orientation quaternion [..., 4]
        :v: (tensor): current angular velocity (scaled angle-axis form) [..., 3]
        :x_goal: (tensor): target orientation quaternion [..., 4]
        :halflife: (float): time for angular difference to halve
        :dt: (float): time step
...
def critical_velocity_spring(x, v, a, v_goal, halflife, dt):
    """Exact update for critically damped spring system with acceleration

    Args:
        :x: (float or tensor): current position
        :v: (float or tensor): current velocity
        :a: (float or tensor): current acceleration
        :v_goal: (float or tensor): target velocity
        :halflife: (float): time for velocity difference to halve
        :dt: (float): time step
...
def inertialize_transition_vec3(off_x, off_v, src_x, src_v, dst_x, dst_v):
    ...
    Args:
        :off_x: (tensor [3]): current positional offset
        :off_v: (tensor [3]): current velocity offset
        :src_x: (tensor [3]): source position before transition
        :src_v: (tensor [3]): source velocity before transition
        :dst_x: (tensor [3]): destination position after transition
        :dst_v: (tensor [3]): destination velocity after transition
...
def inertialize_update_vec3(in_x, in_v, off_x, off_v, halflife, dt):
    ...
    Args:
        :in_x: (tensor [3]): input position
        :in_v: (tensor [3]): input velocity
        :off_x: (tensor [3]): current offset position
        :off_v: (tensor [3]): current offset velocity
        :halflife: (float): time for offset to decay by half
        :dt: (float): time step
...
def inertialize_transition_quat(off_x, off_v, src_x, src_v, dst_x, dst_v):
    ...
    Args:
        :off_x: (tensor [4]): current orientation offset quaternion
        :off_v: (tensor [3]): current angular velocity offset
        :src_x: (tensor [4]): source orientation quaternion before transition
        :src_v: (tensor [3]): source angular velocity before transition
        :dst_x: (tensor [4]): destination orientation quaternion after transition
        :dst_v: (tensor [3]): destination angular velocity after transition
...
def inertialize_update_quat(in_x, in_v, off_x, off_v, halflife, dt):
    ...
    Args:
        :in_x: (tensor [4]): input orientation quaternion
        :in_v: (tensor [3]): input angular velocity
        :off_x: (tensor [4]): current offset quaternion
        :off_v: (tensor [3]): current angular velocity offset
        :halflife: (float): time for rotational offset to decay by half
        :dt: (float): time step
