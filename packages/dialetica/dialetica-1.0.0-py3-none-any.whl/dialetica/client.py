...
class Dialetica:
    ...
    class UsersClient:
        ...
        def create(self, user: UserCreate) -> UserResponse:
            ...
            response = self.client._make_request("POST", "/users", user.model_dump(mode = "json"))
            return UserResponse(**response)
        
        def bulk_create(self, users: List[UserCreate]) -> List[UserResponse]:
            """Create multiple users at once"""
            created_users = []
            for user in users:
                response = self.client._make_request("POST", "/users", user.model_dump(mode = "json"))
                created_users.append(UserResponse(**response))
            return created_users
        
        def get(self, user_id: str) -> Optional[UserResponse]:
            """Get a user by ID"""
            try:
                response = self.client._make_request("GET", f"/users/{user_id}")
                return UserResponse(**response)
            except Exception:
                return None
        ...
        def list(self) -> List[UserResponse]:
            """List all users"""
            response = self.client._make_request("GET", "/users")
            return [UserResponse(**user_data) for user_data in response]
        
        def update(self, user_id: str, user: UserCreate) -> Optional[UserResponse]:
            """Update a user"""
            try:
                response = self.client._make_request("PUT", f"/users/{user_id}", user.model_dump(mode = "json"))
                return UserResponse(**response)
            except Exception:
                return None
        ...
        def delete(self, user_id: str) -> bool:
            """Delete a user"""
            try:
                self.client._make_request("DELETE", f"/users/{user_id}")
                return True
            except Exception:
                return False
    ...
    class APIKeysClient:
        ...
        def create(self, key_name: str, expires_in_days: Optional[int] = None):
            """Create a new API key"""
            data = {
                "key_name": key_name,
                "expires_in_days": expires_in_days
            }
            response = self.client._make_request("POST", "/api-keys", data)
            return response
        
        def list(self):
            """List all API keys for the authenticated user"""
            response = self.client._make_request("GET", "/api-keys")
            return response
        
        def delete(self, api_key: str) -> bool:
            """Delete an API key"""
            try:
                self.client._make_request("DELETE", f"/api-keys/{api_key}")
                return True
            except Exception:
                return False
