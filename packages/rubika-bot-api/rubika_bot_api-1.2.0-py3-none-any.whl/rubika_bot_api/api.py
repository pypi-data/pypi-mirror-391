...
class Robot:
    ...
    async def _validate_token(self):
        ...
        if self._token_validated:
            return self._me
        # defensive: fail fast on empty token to avoid building URLs like /v3//getMe
        if not self.token or not str(self.token).strip():
            raise InvalidTokenError("Empty bot token provided. Set BOT_TOKEN in environment or pass a valid token.")
        try:
            if not self.session:
    ...
    async def _post(self, method: str, data: Dict[str, Any], timeout: int = 20) -> Dict[str, Any]:
        ...
        # Defensive: ensure token present before building URL
        if not self.token or not str(self.token).strip():
            raise InvalidTokenError("Empty bot token provided. Set BOT_TOKEN in environment or pass a valid token.")
        ...
        async with self._request_semaphore:
            ...
            # Build request URL carefully to avoid accidental double slashes
            token_part = self.token.strip()
            url = f"{API_URL.rstrip('/')}/{token_part}/{method.lstrip('/')}"
            retries = 3
            last_error = None
