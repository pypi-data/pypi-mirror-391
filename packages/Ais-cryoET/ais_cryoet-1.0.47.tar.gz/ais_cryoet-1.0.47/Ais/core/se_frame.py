...
class Segmentation:
    ...
    def add_box(self, pixel_coordinates):
        ...
        if cfg.settings["DEBUG_ANNOTATE_EASYMODE_FILAMENT"]: # TODO: remove this :)
            starting_slice = self.current_slice
            this_xyz = (pixel_coordinates[0], pixel_coordinates[1], self.current_slice)
            prev_xyz = Segmentation.__debug_easymode_annotate_filaments_previous_coordinate
            if self.n_boxes > 1:
                line = np.linspace(prev_xyz, this_xyz, num=int(np.linalg.norm(np.array(this_xyz)-np.array(prev_xyz)))*2)
                for _z in range(-2, 3):
                    for coord in line:
                        ...
                        for i in range(-2, 3):
                            for j in range(-2, 3):
                                if i**2 + j**2 <= 5:
                                    xi = min(max(x + i, 0), self.width - 1)
                                    yj = min(max(y + j, 0), self.height - 1)
                                    self.data[yj, xi] = 1
            Segmentation.__debug_easymode_annotate_filaments_previous_coordinate = this_xyz
            self.set_slice(starting_slice - 1)
            self.set_slice(starting_slice)
