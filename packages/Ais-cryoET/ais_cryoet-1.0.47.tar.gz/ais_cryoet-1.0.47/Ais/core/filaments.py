...
def tangent_to_euler_zyz(tangent_zyx):
    tangent_zyx = np.atleast_2d(tangent_zyx)
    tangent_zyx = tangent_zyx / np.linalg.norm(tangent_zyx, axis=1, keepdims=True)

    eulers = []
    for tangent in tangent_zyx:
        axis = np.cross([0, 0, 1], tangent)
        axis_norm = np.linalg.norm(axis)

        if axis_norm < 1e-10:  # Parallel or antiparallel
            if tangent[2] > 0:
                rot = R.identity()
            else:
                rot = R.from_euler('Y', 180, degrees=True)
        else:
            angle = np.arccos(np.clip(tangent[2], -1, 1))
            rot = R.from_rotvec(angle * axis / axis_norm)

        euler = rot.as_euler('ZYZ', degrees=True)
        eulers.append(euler)

    return np.atleast_2d(eulers)
...
class Filament:
    ...
    def sample_coordinates(self, spacing, offset=0):
        ...
        return np.hstack([coords, euler_angles])
...
def pick_filament(mrcpath, out_path, threshold, spacing_nm, size_nm, binning, margin, pixel_size=10.0, min_length=50.0):
    """
    ...
    df = pd.DataFrame(columns=['rlnCoordinateZ', 'rlnCoordinateY', 'rlnCoordinateX', 'rlnAngleRot', 'rlnAngleTilt', 'rlnAnglePsi', 'aisFilamentID'])
    for j, f in enumerate(filaments):
        c = f.sample_coordinates(spacing=spacing_nm / pixel_size, offset=spacing_nm / pixel_size / 2.0)
        for z, y, x, rot, tilt, psi in c:
            df.loc[len(df)] = [z, y, x, rot, tilt, psi, int(j)]
