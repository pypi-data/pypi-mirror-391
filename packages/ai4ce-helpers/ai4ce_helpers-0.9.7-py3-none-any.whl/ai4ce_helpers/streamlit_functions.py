...
from ._backend_calls import _backend_GET, get_running_backend_version
from .functions import move_parameter_placeholder, cleanup_parameters
...
def st_load_components(uid_list: list[str] = None) -> dict[str, dict]:
    ...
    with st.spinner("Loading components..."):
        ...
        for component_uid, component in st.session_state['db']['components'].items():
            st.session_state['db']['components'][component_uid] = cleanup_parameters(move_parameter_placeholder(component))
...
def st_show_backend_version():
    ...
    st.markdown(f"Backend Version: {version}")